...
const settings = {
  ...
  beforeAgentStart: {
    enabled: true,
    sources: [{ id: "two", name: "Two", kind: "text" as const, content: "BEFORE", reinjectOnCompaction: false }],
  },
};
...
test("hook bridge persists session-start context once per branch and composes transient prompts", () => {
  ...
  branch.push({
    type: "custom_message",
    message: { customType: SESSION_START_HOOK_CUSTOM_TYPE, details: { compactionEntryId: "compact-2" } },
  });
  handlers.get("session_compact")![0]!({ compactionEntry: { id: "compact-2" } }, context);
  assert.equal(messages.length, 3);
