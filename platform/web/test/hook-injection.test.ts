...
import {
  HookInjectionBridge,
  SESSION_START_HOOK_CUSTOM_TYPE,
} from "../src/server/pi/hook-injection.ts";
...
test("hook bridge persists session-start context once per branch and composes transient prompts", () => {
  ...
  handlers.get("session_start")![0]!({}, context);
  assert.equal(messages.length, 1);
  const result = handlers.get("before_agent_start")![0]!(
    { systemPrompt: "BASE" },
    context,
  );
  ...
  assert.deepEqual((messages[1] as any).details, {
    version: 1,
  ...
  branch.push({
    type: "custom_message",
    customType: SESSION_START_HOOK_CUSTOM_TYPE,
    details: { compactionEntryId: "compact-1" },
  });
  handlers.get("session_compact")![0]!(
    { compactionEntry: { id: "compact-1" } },
    context,
  ...
  branch.push({
    ...
    message: {
      customType: SESSION_START_HOOK_CUSTOM_TYPE,
      details: { compactionEntryId: "compact-2" },
    },
  });
  handlers.get("session_compact")![0]!(
    { compactionEntry: { id: "compact-2" } },
  ...
  let spawnHooks: any;
  busHandlers.get("pylon:spawn-hooks-request")!({
    version: 1,
  ...
  assert.equal(
    spawnHooks.sessionStart.customType,
    SESSION_START_HOOK_CUSTOM_TYPE,
  );
