...
test("StateQL password broker reuses endpoint consent, separates kinds and targets, and invalidates", async () => {
  ...
  const connectRequest = (character: string) => stateqlRequest("read", {
...
test("StateQL accepts explicit SQLite sources and rejects bare paths or wrong stored drivers", async () => {
  ...
  acceptedBridge.answer({ requestId: acceptedRequests[0].requestId, sessionGeneration: 3, method: "input", value: source });
  ...
  invalidBridge.answer({ requestId: invalidRequests[0].requestId, sessionGeneration: 3, method: "input", value: "./private.sqlite" });
  ...
  invalidBridge.answer({ requestId: invalidRequests[1].requestId, sessionGeneration: 3, method: "input", value: wrongValue });
...
test("StateQL rejects malformed or unsupported connection sources without retention", async () => {
  ...
  const invalidSources = ["sqlite:", "sqlite: ", "sqlite::memory:", "sqlite://host/app", "postgres:database", "postgres://", "https://localhost/app"];
...
test("StateQL credential bindings reject identity changes, expire, and clear on generation replacement", async () => {
  let now = 1_000;
  const requests: UiRequest[] = [];
  const bridge = new RemoteUiBridge((request) => requests.push(request), 1_000, () => {}, 100, () => now);
  const host = stateqlHost(bridge);
  const sentinel = DATABASE_URL;
  ...
  await assert.rejects(
    host.requestStateQLCredential(stateqlRequest("read", {
