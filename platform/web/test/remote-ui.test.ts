...
function stateqlHost(
  bridge: RemoteUiBridge,
  generation = 3,
): StateQLCredentialHost {
  return bridge.context("session-1", generation) as ReturnType<
    RemoteUiBridge["context"]
  > &
    StateQLCredentialHost;
}
...
test("provider auth prompts mark secrets without publishing their value", async () => {
  ...
  const pending = bridge.authPrompt(
    "session-1",
    3,
...
test("StateQL credentials use masked metadata-only prompts, reuse reads, and require write escalation", async () => {
  ...
  assert.equal(
    await host.requestStateQLCredential(stateqlRequest("read")),
    sentinel,
  );
  ...
  bridge.answer({
    requestId: writePrompt.requestId,
    sessionGeneration: 3,
    method: "input",
    value: sentinel,
  });
  ...
  assert.equal(
    JSON.stringify({ requests, snapshot: bridge.snapshot() }).includes(
      sentinel,
    ),
    false,
  );
...
test("StateQL password broker reuses endpoint consent, separates kinds and targets, and invalidates", async () => {
  ...
  const reference = (character: string) =>
    `PYLON_STATEQL_BROKERED_${character.repeat(48)}`;
  ...
  assert.match(
    String(prompt.payload.message),
    /connect to postgres@db\.example\.com:5432\/app with read-only access/,
  );
  ...
  assert.equal(
    await host.requestStateQLPassword(queryRequest, target),
    password,
  );
  assert.equal(requests.length, 1);
  ...
  const defaultTimeout = host.requestStateQLPassword(
    connectRequest("G"),
    defaultTarget,
  );
...
test("StateQL accepts explicit SQLite sources and rejects bare paths or wrong stored drivers", async () => {
  ...
  const acceptedBridge = new RemoteUiBridge((request) =>
    acceptedRequests.push(request),
  );
  const accepted =
    stateqlHost(acceptedBridge).requestStateQLCredential(sqliteRequest);
  assert.match(
    String(acceptedRequests[0].payload.message),
  ...
  assert.equal(
    JSON.stringify({
      acceptedRequests,
      snapshot: acceptedBridge.snapshot(),
    }).includes(source),
    false,
  );

  const invalidRequests: UiRequest[] = [];
  const invalidBridge = new RemoteUiBridge((request) =>
  ...
  assert.equal(
    JSON.stringify({
      invalidRequests,
      snapshot: invalidBridge.snapshot(),
    }).includes(wrongValue),
    false,
  );
});
...
test("StateQL credential prompts single-flight, cancel without binding, and retry cleanly", async () => {
  ...
  const joinedCancelled = host.requestStateQLCredential(
    stateqlRequest("read", { signal: joinedController.signal }),
  );
  ...
  bridge.answer({
    requestId: joinedPrompt.requestId,
    sessionGeneration: 3,
    method: "input",
    value: sentinel,
  });
  ...
  bridge.cancelAll();
  const controller = new AbortController();
  const cancelled = host.requestStateQLCredential(
    stateqlRequest("read", { signal: controller.signal }),
  );
  ...
  assert.throws(
    () =>
      bridge.answer({
        requestId: cancelledPrompt.requestId,
        sessionGeneration: 3,
        method: "input",
        value: sentinel,
      }),
...
test("StateQL credential bindings reject identity changes, expire, and clear on generation replacement", async () => {
  ...
  await assert.rejects(
    host.requestStateQLCredential(
      stateqlRequest("read", { actorId: "other-session" }),
    ),
    /actor does not match/,
  );
  await assert.rejects(
    host.requestStateQLCredential(
      stateqlRequest("read", { reference: "invalid-reference" }),
    ),
  ...
  assert.throws(
    () =>
      bridge.answer({
        requestId: stalePrompt.requestId,
        sessionGeneration: 3,
        method: "input",
        value: sentinel,
      }),
  ...
  assert.equal(
    await host.requestStateQLCredential(stateqlRequest()),
    undefined,
  );
  const replacement = bridge.context("session-2", 4) as ReturnType<
    RemoteUiBridge["context"]
  > &
    StateQLCredentialHost;
  ...
  bridge.answer({
    requestId: reboundPrompt.requestId,
    sessionGeneration: 4,
    method: "input",
    value: sentinel,
  });
  ...
  bridge.dispose();
  assert.equal(
    await replacement.requestStateQLCredential(replacementRequest),
    undefined,
  );
...
test("StateQL credential prompt publication failures clear single-flight state for retry", async () => {
  ...
  const host = stateqlHost(bridge);
  await assert.rejects(
    host.requestStateQLCredential(stateqlRequest()),
    /publish failed/,
  );
...
test("remote UI fails closed on abort, timeout, and generation cancellation", async () => {
  ...
  const controller = new AbortController();
  const confirmation = ui.confirm("Confirm", "Proceed?", {
...
test("remote UI returns one bounded answer for every questionnaire item", async () => {
  const requests: UiRequest[] = [];
  const bridge = new RemoteUiBridge((request) => requests.push(request));
  const ui = bridge.context("session-1", 8) as ReturnType<
    RemoteUiBridge["context"]
  > & {
    questionnaire(
