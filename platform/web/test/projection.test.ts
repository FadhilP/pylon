...
test("turn index keeps bounded prompt metadata without assistant bodies", () => {
  const turns = projectConversationTurnIndex([
    ...
    {
      role: "assistant",
      content: "large response that does not belong in the index",
    },
...
test("provider auth driver events publish live sign-in links", () => {
  ...
  assert.deepEqual(published, [
    { type: "provider.auth", payload: providerAuth },
  ]);
});
...
test("session status retains background completion until that session is selected", () => {
  ...
  assert.deepEqual(projection.unseenCompletionSessionIds(), [
    "background",
    "external",
  ]);
  assert.deepEqual(published, [
    ...
    {
      type: "session.status",
      payload: { sessionId: "external", state: "sleeping", completed: true },
    },
  ]);
...
test("correlated user starts reuse the optimistic pending message ID", () => {
  ...
  assert.equal(
    projection.snapshot().conversation.messages[0]?.id,
    "pending-command-1",
  );
  ...
  assert.equal(
    projection.snapshot().conversation.messages[0]?.entryId,
    "entry-1",
  );
});
...
test("projection maps SDK event names and retains active messages", () => {
  ...
  assert.deepEqual(result.conversation.compaction, {
    active: true,
    reason: "threshold",
  });
  ...
  replacement.conversation.messages = [
    { id: "history", role: "user", text: "Loaded history", streaming: false },
  ];
  projection.apply({
    type: "session.replaced",
...
test("history projection attaches persisted tool durations", () => {
  ...
  assert.equal(
    projected.messages.find((message) => message.role === "tool")?.tool
      ?.durationMs,
    1_250,
  );
});
...
test("compaction completion appends a durable disclosure without changing streaming state", () => {
  ...
  const projection = new RuntimeProjection(initial, (type, payload) =>
    published.push({ type, payload }),
  );
  ...
  assert.deepEqual(projection.snapshot().conversation.compaction, {
    active: false,
    reason: "threshold",
  });
  ...
  projection.apply(
    session({ type: "compaction_end", reason: "manual", aborted: true }),
  );
  assert.equal(projection.snapshot().conversation.messages.length, 1);
});
    ...
    projectMessages([
      {
        ...
        compaction: {
          ...
          display: {
            ...
            records: display.records.map((record) => ({
              ...record,
              ignored: true,
            })),
          },
  ...
  const malformed = projectMessages([
    {
      ...
      compaction: {
        ...
        display: {
          ...display,
          toolResults: [{ sourceEntryId: "result", text: "x".repeat(2_001) }],
        },
      },
    },
  ])[0];
...
test("next prompt keeps the expanded existing-session turn", () => {
  ...
  assert.equal(
    snapshot.conversation.messages.filter((message) => message.role === "tool")
      .length,
    120,
  );
...
test("history projection pairs bounded redacted tool inputs with results", () => {
      ...
      content: [
        {
          ...
          id: "call-1",
          name: "write",
          arguments: { path: "src/app.ts", password: "hidden" },
        },
      ],
    },
    {
  ...
  assert.doesNotMatch(
    ...
    /hidden file contents|notes\.txt/,
  );
    ...
    session({
      ...
      message: {
        ...
        diagnostics: [
          {
            type: "pi-continuity-compaction-interruption",
            details: { version: 1, requestId: "compact-1" },
          },
        ],
      },
    }),
...
test("history projection keeps stable global IDs and skips old non-delegate payload serialization", () => {
  ...
  const history = [
    {
      role: "assistant",
      content: [
        { type: "toolCall", id: "old", name: "read", arguments: oldArguments },
      ],
    },
    {
      role: "toolResult",
  ...
  const page = projectConversation(history, {
    start: 2,
    end: 12,
...
test("history pages do not mark tool calls running when their result is on a later page", () => {
  ...
  const tool = page.messages.find(
    (message) => message.tool?.id === "paged-call",
  );
  ...
  assert.equal(
    failed.messages.find((message) => message.tool?.id === "paged-call")?.tool
      ?.status,
    "failed",
  );
});
...
test("existing-session projection keeps the complete latest user turn", () => {
  ...
  const historyStart = Math.min(
    tailStart,
    latestVisibleUserIndex(history) ?? tailStart,
  );
  const projected = projectConversation(history, {
    start: historyStart,
    limitMessages: false,
...
test("projections reject invalid dates and negative worktree counts", () => {
  ...
  assert.deepEqual(
    projection.snapshot().conversation.messages[0]?.changedFiles,
    [{ path: "valid.ts", additions: 3, deletions: 1 }],
  );
});
...
test("history projection retains complete delegated activity with bounded event text", () => {
  ...
  assert.doesNotMatch(
    projected.delegatedRuns[0]?.request ?? "",
    /hidden|apiToken/,
  );
});
...
test("live delegated activity appends correlated deltas without replacing prior events", () => {
  ...
  const activity =
    projection.snapshot().conversation.delegatedRuns[0]?.activity ?? [];
...
test("pi-spawn executions expose stable child metadata and ignore list actions", () => {
  ...
  assert.deepEqual(
    projected.delegatedRuns.map(
      ({ id, action, threadId, request, response }) => ({
        id,
        action,
        threadId,
        request,
  ...
  assert.equal(
    agentColorId(projected.delegatedRuns[0]!),
    agentColorId(projected.delegatedRuns[1]!),
  );
  assert.deepEqual(projected.delegatedRuns[0]?.usage, {
    input: 3,
    output: 5,
    cacheRead: 1,
    cacheWrite: 0,
...
test("background pi-spawn controls update the original delegated run without duplicates", () => {
  ...
  assert.deepEqual(
    projection
      .snapshot()
      .conversation.delegatedRuns.map(
        ({ id, status, runId, threadId, response, usage }) => ({
          id,
          status,
          runId,
          threadId,
...
test("live spawn-session adoption gains its authorized child ID from updates", () => {
  ...
  projection.apply(
    session({
      ...
      partialResult: {
        content: [],
        details: {
          piSpawn: { version: 2, kind: "agent", id: "untrusted-session" },
          state: "running",
        },
      },
    }),
  ...
  assert.equal(
    projection.snapshot().conversation.delegatedRuns[0]?.threadId,
    undefined,
  );
    ...
    session({
      ...
      partialResult: {
        ...
        details: {
          ...
          state: "running",
          activity: [
            { kind: "call", tool: "read", text: '{"path":"README.md"}' },
          ],
        },
      },
    }),
  ...
  assert.equal(
    projection.snapshot().conversation.delegatedRuns[0]?.response,
    undefined,
  );
  ...
  const projected = projectConversation([
    {
      role: "assistant",
      content: [
        { type: "toolCall", id: "scout-1", name: "repo_scout", arguments: {} },
      ],
    },
    {
      role: "toolResult",
        ...
        activity: [
          {
            kind: "result",
            tool: "read",
            text: `password: x ${"x".repeat(1_988)}`,
          },
        ],
      },
    },
...
test("projection suppresses duration-only delegated heartbeats", () => {
          ...
          activity: [
            {
              kind: "call",
              tool: "read",
              text: '{"path":"a.ts","password":"hidden"}',
            },
          ],
        },
      },
  ...
  assert.equal(
    projection.snapshot().conversation.delegatedRuns[0]?.durationMs,
    1_000,
  );
  ...
  assert.equal(
    published.filter((event) => event.type === "delegate.update").length,
    2,
  );
  projection.apply(
    session({
...
test("Repo Scout live records settle with usage and bounded tool history", () => {
  ...
  projection.apply(
    session({
      ...
      partialResult: {
        content: [
          { type: "text", text: "Scout child activity:\nread src/app.ts" },
        ],
        details: {
          state: "running",
          activity: [
...
test("projection publishes live session names and agent timing metadata", () => {
  ...
  projection.apply(
    session({
      ...
      files: [
        { path: "src/app.ts", additions: 4, deletions: 2, binary: false },
      ],
    }),
  );
  projection.apply(
    session({ type: "session_info_changed", name: "Renamed by timeline" }),
  );
  ...
  assert.equal(
    snapshot.conversation.messages.at(-1)?.gitBranch,
  ...
  assert.deepEqual(
    published
      .filter((event) => event.type === "session.info")
      .map((event) => event.payload),
    [{ sessionId: "session", name: "Renamed by timeline" }],
  );
  assert.equal(
    (
      published.find((event) => event.type === "agent.start")?.payload as {
        startedAt: string;
      }
    ).startedAt,
    startedAt,
  ...
  assert.equal(
    (
      published.find((event) => event.type === "agent.end")?.payload as {
        durationMs: number;
      }
    ).durationMs,
    1_234,
...
test("agent completion restores an assistant lost during session selection", () => {
  ...
  assert.equal(
    completion.assistantMessage?.text,
    "Completed while switching sessions",
  );
});

test("authoritative refresh repairs a terminal assistant omitted from live events", () => {
  const published: string[] = [];
  const projection = new RuntimeProjection(runtime(), (type) =>
    published.push(type),
  );
  const fresh = runtime();
  ...
  assert.equal(
    projection.snapshot().conversation.messages[0]?.text,
...
test("authoritative refresh settles delegated runs missed during session switching", () => {
  ...
  assert.equal(
    projection.snapshot().conversation.delegatedRuns[0]?.status,
    "completed",
  );
  assert.equal(
    published.filter((event) => event.type === "delegate.update").length,
...
test("agent completion does not duplicate live or snapshotted assistants", () => {
  ...
  const live = new RuntimeProjection(runtime(), (type) =>
  ...
  assert.equal(
    live.snapshot().conversation.messages[0]?.entryId,
    "final-assistant",
  );
  assert.deepEqual(
  ...
  const snapshotted = new RuntimeProjection(loaded, (type) =>
    loadedEvents.push(type),
  );
  snapshotted.apply(
    session({
      type: "agent_end",
  ...
  assert.equal(
    malformed.snapshot().conversation.messages[0]?.workDurationMs,
    undefined,
  );
});
...
test("agent completion flushes pending text before its terminal event", () => {
  ...
  assert.equal(projection.snapshot().conversation.messages.length, 1);
  assert.deepEqual(published.slice(-3), [
    "message.update",
    "message.end",
    "agent.end",
  ]);
...
test("projection keeps one active timer across retry attempts", () => {
  ...
  assert.equal(
    projection.snapshot().conversation.messages.at(-1)?.workDurationMs,
    undefined,
  );
  ...
  projection.apply(
    session({
      type: "agent_start",
      turnId: "turn-1",
      workStartedAt: "2026-01-01T00:00:10.000Z",
      modelName: "GPT-5",
    }),
  );
  assert.equal(projection.snapshot().conversation.workStartedAt, originalStart);
  ...
  assert.equal(projection.snapshot().conversation.workStartedAt, undefined);
  assert.equal(
    (published.at(-1)?.payload as { willRetry?: boolean }).willRetry,
    false,
  );
});
...
test("projection persists terminal agent errors and clears them for retries and new runs", () => {
  ...
  assert.equal(
    projection.snapshot().conversation.agentError,
    "provider rejected request",
  );
  assert.equal(
    (
      [...published].reverse().find((event) => event.type === "agent.end")
...
test("terminal agent events settle residual tool and delegated activity", () => {
  ...
  stopped.apply(
    session({ type: "agent_end", turnId: "turn-stopped", stopped: true }),
  );
  ...
  errored.apply(
    session({ type: "agent_error", turnId: "turn-error", willRetry: true }),
  );
...
test("projection retains stopped tool-only run metadata without an assistant message", () => {
  ...
  assert.equal(
    projection.snapshot().conversation.messages[0]?.workDurationMs,
    undefined,
  );
...
test("projection retains bounded extension UI state and publishes mutation events", () => {
  ...
  assert.deepEqual(state.widgets, [
    { key: "plan", lines: ["one", "two"], placement: "aboveEditor" },
  ]);
