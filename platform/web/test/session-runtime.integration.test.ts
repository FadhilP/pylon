...
test("user completion resolves its entry ID after persistence", async () => {
  ...
  const firstEntryId = session.appendMessage({
    role: "user",
    content: "Prompt A",
    timestamp: Date.now(),
  });
  ...
  const secondEntryId = session.appendMessage({
    role: "user",
    content: "Prompt B",
    timestamp: Date.now(),
  });
  ...
  const merged = mergeHistoryMessages(
    ...
    [
      ...
      {
        id: "message-1",
        entryId: String(forwarded[0]?.entryId),
        role: "user",
        text: "Prompt B",
        streaming: false,
      },
  ...
  assert.equal(
    deferUserMessageEndEntryId(
      {
        type: "message_end",
        message: { role: "user", content: "Unpersisted prompt" },
      },
      () => true,
      () => secondEntryId,
    ...
    deferUserMessageEndEntryId(
      {
        ...
        message: { role: "user", content: "Compatibility event" },
      },
      () => true,
      latestUserEntryId,
      (payload) => forwarded.push(payload),
...
test("agent_settled recovers missed agent_end state and abort does not latch stopping", async () => {
  ...
  try {
    ...
    await driver.abort();
    assert.equal(
      (await driver.snapshot()).conversation.stopping ?? false,
      false,
    );
    await driver.abort();
    assert.equal(
...
test("Continuity compaction continuation preserves timing, suppresses interruption, and honors user abort", async () => {
  ...
  try {
    ...
    session._emit({ type: "agent_settled" });
    assert.equal(
      (await driver.snapshot()).conversation.workStartedAt,
      startedAt,
    );
    assert.ok(
      events.some(
        (event) =>
          ...
          event.payload?.type === "continuity_compaction_interruption",
      ),
    );
    assert.equal(
      events
    ...
    const terminalEventsBeforeAbort = events.filter(
      (event) =>
        event.type === "session.event" && event.payload?.type === "agent_end",
    ).length;
    ...
    assert.equal(
      abortTerminalEvents.length,
      1,
      "user abort owns the native terminal event",
    );
      ...
      pi.events.on("pi-continuity:plan-action", (request: any) => {
        ...
        request.respond(
          request.expectedRevision === 9
            ? Promise.reject(new Error("plan revision changed"))
            : Promise.resolve(),
        );
      });
    },
...
test("memory runtime bridge forwards scoped CAS mutations and maps stale errors", async () => {
  ...
  try {
    ...
    await driver.migrateContinuityMemory({
      expectedGeneration: handle.sessionGeneration,
    });
    assert.deepEqual(
      mutations
    ...
    assert.throws(
      () =>
        driver.migrateContinuityMemory({
          expectedGeneration: handle.sessionGeneration + 1,
        }),
      { name: "StaleGenerationError" },
    );
...
test("runtime publishes usage after a completed message is persisted", async () => {
  ...
  try {
    ...
    assert.equal(
      events.some(
        (event) =>
          event.type === "session.event" && event.payload?.type === "usage",
      ),
      false,
    );
...
test(
  "completed work metadata survives runtime restart",
  { timeout: 45_000 },
  async () => {
    ...
    try {
      ...
      const message = (await driver.snapshot()).conversation.messages.find(
        (item) => item.entryId === assistantEntryId,
      );
...
test(
  "completed tool duration survives runtime restart",
  { timeout: 45_000 },
  async () => {
...
test("runtime accepts only correlated background spawn progress", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "pylon-background-spawn-progress-"),
  );
  const cwd = join(root, "workspace");
  const agentDir = join(root, "agent");
  await Promise.all([mkdir(cwd), mkdir(agentDir)]);
  ...
  try {
    ...
    (driver as any).eventBus.emit(
      ...
      progress({ parentSessionId: "wrong-parent" }),
    );
    ...
    (driver as any).eventBus.emit(
      "pylon:spawn-progress",
      progress({ runId: "stale-run" }),
    );
    assert.equal(
      (await driver.snapshot()).conversation.delegatedRuns[0]?.runId,
      runId,
    );
    (driver as any).eventBus.emit(
      "pylon:spawn-progress",
...
test("StateQL rows bridge normalizes a bounded page", async () => {
    ...
    factory(pi) {
      pi.events.on("pylon:stateql-rows-request", (value: any) => {
        ...
        value.respond(
          Promise.resolve({
            result_id:
              value.handle === "wrong-result" ? "other-result" : value.handle,
            offset: value.offset,
            limit: value.limit,
            rows,
  ...
  try {
    ...
    await assert.rejects(
      driver.stateqlRows("wrong-result", 0, 10),
      /returned invalid rows/,
    );
    await assert.rejects(
      driver.stateqlRows("oversized", 0, 10),
      /returned oversized rows/,
    );
  } finally {
    await driver.dispose();
    await rm(root, { recursive: true, force: true });
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(
        JSON.stringify({ prompts, events, snapshot }).includes(sentinel),
        false,
      );
      assert.equal(
...
test(
  "public SDK binds RPC UI, aborts, replaces, discovers Pylon, and shuts down",
  { timeout: 60_000 },
  async () => {
    ...
    const probe: InlineExtension = {
      ...
      factory(pi) {
        ...
        pi.on("session_before_switch", async (_event, ctx) => {
          ...
          if (
            !(await ctx.ui.confirm("Cancel replacement?", "Test cancellation"))
          )
            return { cancel: true };
        });
        pi.registerCommand("phase0-probe", {
          handler: async (_args, ctx) => {
            observations.dialogs.push([
              "select",
              await ctx.ui.select("Choose", ["A", "B"]),
            ]);
            observations.dialogs.push([
              "confirm",
              await ctx.ui.confirm("Confirm", "Proceed?"),
            ]);
            observations.dialogs.push(["input", await ctx.ui.input("Input")]);
            observations.dialogs.push([
              "editor",
              await ctx.ui.editor("Editor", "before"),
            ]);
            ctx.abort();
          },
        });
    ...
    const unsubscribe = driver.subscribe((event) => {
      ...
      queueMicrotask(() => {
        ...
        if (method === "confirm") {
          ...
          void driver.answerUiRequest({ ...base, method, value: "B" });
        else
          void driver.answerUiRequest({
            ...base,
            method,
            value: method === "input" ? "typed" : "edited",
          });
      });
    });
    ...
    try {
      ...
      await driver.prompt({
        commandId: "notify-command",
        expectedGeneration: 1,
        message: "/phase0-notify",
      });
      assert.equal(
        (await driver.snapshot()).commandResult?.output,
        "Command result",
      );
      ...
      const commandNames =
        (await driver.snapshot()).sessionControls.commands?.map(
          (command) => command.name,
        ) ?? [];
      assert.ok(
        commandNames.includes("compact:2"),
        JSON.stringify(commandNames),
      );
      const compacting = driver.prompt({
      ...
      assert.equal(
        (await driver.snapshot()).commandResult?.command,
        "phase0-empty",
      );
      driver.dismissCommandResult("empty-command", 1);
      await driver.prompt({
        commandId: "compact-failure",
        expectedGeneration: 1,
        message: "/compact:2 fail",
      });
      ...
      const stopPending = driver.subscribe((event) => {
        if (
          event.type === "ui.event" &&
          (event.payload as UiRequest).payload.title === "Wait for cancellation"
        )
          resolvePending();
      });
      const waiting = driver.prompt({
      ...
      const cancelled = await driver.newSession({
        parentSessionId: foreignParent.getSessionId(),
      });
      ...
      const sameProject = await driver.newSession({
        parentSessionId: foreignParent.getSessionId(),
      });
      ...
      const packageFailure = await driver.newSession({
        parentSessionId: failureParent.getSessionId(),
      });
      ...
      assert.ok(
        failed.diagnostics.some((item) =>
          item.message.includes("failed to load"),
        ),
      );
      ...
      unsubscribe();
      await driver.dispose();
      const testSessions = (await SessionManager.listAll()).filter((session) =>
        session.cwd.startsWith(root),
      );
      await Promise.all(
        testSessions.map((session) => rm(session.path, { force: true })),
      );
      await rm(root, { recursive: true, force: true });
    }
...
test(
  "repository packages load, toggle, and save settings",
  { timeout: 45_000 },
  async () => {
    ...
    const driver = new SessionRuntime({
      extensionFactories: [settingsRefreshProbe],
    });
    ...
    try {
      ...
      assert.ok(
        initialPackages.packages.some(
          (item) => item.id === "pi-timeline" && item.active,
        ),
      );
      const allThinking = [
        "off",
      ...
      assert.deepEqual(
        initialPackages.packages.find((item) => item.id === "pi-spawn")
          ?.settings,
        {
          kind: "spawn",
      ...
      await assert.rejects(
        driver.setPackageEnabled({ packageId: "pylon-core", enabled: false }),
        /required/,
      );
      const disabled = await driver.setPackageEnabled({
        packageId: "pi-timeline",
        enabled: false,
      });
      assert.equal(disabled.sessionGeneration, 1);
      assert.equal(
        (await driver.snapshot()).operational.timeline.availability,
        "available",
      );
      assert.ok(
        (await driver.listPackages()).packages.some(
          (item) => item.id === "pi-timeline" && !item.enabled && item.active,
        ),
      );
      const sieve = (await driver.listPackages()).packages.find(
        (item) => item.id === "pi-sieve",
      )?.settings;
      assert.equal(sieve?.kind, "sieve");
      if (sieve?.kind !== "sieve")
        throw new Error("pi-sieve settings are unavailable");
      ...
      assert.deepEqual(
        (await driver.listPackages()).packages.find(
          (item) => item.id === "pi-sieve",
        )?.settings,
        { ...sieve, threshold },
      );
      ...
      for (const [packageId, settings] of [
        ...
        const updated = await driver.updatePackageSettings({
          packageId,
          settings,
        });
      ...
      assert.equal(
        unchangedRuntime.operational.sieve.threshold,
        sieve.threshold,
      );
      ...
      assert.deepEqual(
        (await driver.snapshot()).activeTools,
        initialActiveTools,
      );
      ...
      try {
        ...
        assert.equal(
          nextSnapshot.operational.timeline.availability,
          "unavailable",
        );
        const nextPackages = await next.listPackages();
        assert.ok(
          nextPackages.packages.some(
            (item) =>
              item.id === "pi-timeline" && !item.enabled && !item.active,
          ),
        );
        assert.deepEqual(
          nextPackages.packages.find((item) => item.id === "pi-spawn")
            ?.settings,
          {
            kind: "spawn",
            agentAvailability: "active",
...
test("session deletion only falls back when trash is unavailable", async () => {
  ...
  try {
    ...
    const unavailable = Object.assign(new Error("spawn trash ENOENT"), {
      code: "ENOENT",
    });
...
test(
  "driver deletes only inactive sessions and blocks concurrent lifecycle changes",
  { timeout: 30_000 },
  async () => {
    ...
    try {
      ...
      await assert.rejects(
        driver.switchSession({ sessionId: switchable.getSessionId() }),
...
test(
  "fork validates idle state without rejecting its own lifecycle mutation",
  { timeout: 30_000 },
  async () => {
...
test(
  "driver starts without a root manifest and still loads required core",
  { timeout: 20_000 },
  async () => {
...
test(
  "existing long sessions keep the latest user turn and every tool result",
  { timeout: 20_000 },
  async () => {
    ...
    try {
      ...
      assert.equal(
        snapshot.conversation.messages.at(-1)?.systemSource,
      ...
      const earlier = await driver.conversationHistory({
        cursor: snapshot.conversation.historyCursor!,
      });
      assert.deepEqual(
      ...
      const snapshotEntryIds = new Set(
        snapshot.conversation.messages.map((message) => message.entryId),
      );
      assert.equal(
        earlier.messages.some((message) =>
          snapshotEntryIds.has(message.entryId),
        ),
        false,
      );
    } finally {
...
test(
  "driver serves only versioned attachment content from the active branch",
  { timeout: 20_000 },
  async () => {
    ...
    const fileMessage = promptFilesMessage([
    ...
    try {
      ...
      const snapshot = await driver.snapshot();
      assert.deepEqual(
        snapshot.conversation.messages[0]?.attachments?.map(
          ({ sourceEntryId, kind, name }) => ({ sourceEntryId, kind, name }),
        ),
        [
      ...
      assert.deepEqual(
        await driver.conversationAttachment({
      ...
      const fileAttachment = await driver.conversationAttachment({
        sourceEntryId: fileEntryId,
        index: 0,
      });
      assert.equal(fileAttachment.kind, "file");
      assert.equal(
        fileAttachment.kind === "file" ? fileAttachment.text : undefined,
        "hello",
      );
      await assert.rejects(
        () =>
          driver.conversationAttachment({
            sourceEntryId: imageEntryId,
            index: 1,
          }),
        /unavailable/,
...
test(
  "driver pages the complete visible branch after compaction",
  { timeout: 20_000 },
  async () => {
    ...
    for (let index = 0; index < 155; index++) {
      ...
      if (index === 134)
        compactionEntryId = session.appendCompaction(
          "## Compact summary",
          messageIds[120]!,
          1_000,
          {
            ...
            records: [
              {
                sourceEntryId: "user-source",
                role: "user",
                text: "Original request",
              },
              {
                sourceEntryId: "failed-source",
                role: "tool",
                text: "exact failure",
                isError: true,
              },
              {
                sourceEntryId: "assistant-source",
                role: "assistant",
                text: "Continued work",
              },
              {
                sourceEntryId: "result-source",
                role: "tool",
                text: "exact result",
              },
              {
                sourceEntryId: "prior-summary",
                role: "summary",
                text: "Prior canonical summary",
              },
            ],
            history: {
              read: [{ path: "src/read.ts", sourceEntryId: "read-source" }],
              modified: [
                { path: "src/changed.ts", sourceEntryId: "changed-source" },
              ],
            },
            supplements: [],
          },
    ...
    try {
      ...
      const compaction = snapshot.conversation.messages.find(
        (message) => message.entryId === compactionEntryId,
      );
      ...
      assert.deepEqual(compaction?.compaction?.display, {
        ...
        failedTools: [
          { sourceEntryId: "failed-source", text: "exact failure" },
        ],
      ...
      const expectedContextAfter = buildSessionContext(
        session.getBranch(),
        compactionEntryId,
      ).messages.reduce((total, message) => total + estimateTokens(message), 0);
      assert.equal(
        compaction?.compaction?.contextAfterTokens,
      ...
      const firstPage = await driver.conversationHistory({
        cursor: snapshot.conversation.historyCursor!,
      });
