...
function loadTool(bridge: WorkspaceApplyTool) {
  ...
  return { registered, beforeAgentStart: () => handlers.get("before_agent_start")?.() };
}
...
test("workspace apply tool fails closed and schedules the approved revision once", async () => {
  ...
  bridge.setHandler(async request => {
    ...
    if (request.type === "inspect") {
      return { available: true, targetBranch: "main", changedCount: 2, revision: "revision-1", mode: "worktree" };
    }
  });
  ...
  assert.deepEqual(beforeAgentStart(), {
    message: { customType: "pylon-workspace-apply-result", display: false, content: "Applied to main." },
  });
