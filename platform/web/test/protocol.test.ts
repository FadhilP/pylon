...
test("command and memory validation allowlists bounded v28 commands and attachments", () => {
  ...
  assert.equal(validateCommand({ type: "updatePackageSettings", packageId: "pi-scout", settings: { kind: "scout", mode: "session", webSearch: true }, commandId: "settings", expectedGeneration: 1 }).ok, true);
  assert.equal(validateCommand({ type: "updatePackageSettings", packageId: "pi-scout", settings: { kind: "scout", mode: "session", webSearch: "yes" }, commandId: "settings", expectedGeneration: 1 }).ok, false);
  assert.equal(validateCommand({ type: "updatePackageSettings", packageId: "pi-advisor", settings: { kind: "advisor", mode: "session", webSearch: true }, commandId: "settings", expectedGeneration: 1 }).ok, false);
