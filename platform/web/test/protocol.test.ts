...
test("command validation allowlists bounded v27 commands and attachments", () => {
  ...
  assert.equal(validateCommand({ type: "updatePackageSettings", packageId: "pi-spawn", settings: { kind: "spawn", agentAvailability: "active", sessionAvailability: "deferred" }, commandId: "spawn-settings", expectedGeneration: 1 }).ok, true);
  assert.equal(validateCommand({ type: "updatePackageSettings", packageId: "pi-spawn", settings: { kind: "spawn", agentAvailability: "sometimes", sessionAvailability: "active" }, commandId: "spawn-settings-invalid", expectedGeneration: 1 }).ok, false);
