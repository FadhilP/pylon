...
test("Database is a package-gated peer panel, not an Inspector tab", async () => {
  const [app, database, inspector, styles, prototype] = await Promise.all([
    ...
    readFile(new URL("../prototypes/database-redesign/03-command-ledger.html", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(prototype, /<details class="result-preview">/);
  assert.match(prototype, /\.result-preview>summary/);
  assert.match(prototype, /\.panel-head \.sub,[^}]+\{font-size:12px\}/);
