...
test("prompt attachment bridge injects hidden file context exactly once", () => {
  ...
  assert.deepEqual({ name: detail.name, mimeType: detail.mimeType, size: detail.size }, { name: "notes.txt", mimeType: "text/plain", size: 5 });
  assert.equal(first.message.content.slice(detail.contentStart, detail.contentEnd), "hello");
