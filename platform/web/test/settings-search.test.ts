...
function buildInput(overrides: Partial<SearchInput> = {}): SearchInput {
  return { providers: [], models: [], packages: [], toolPolicies: [], ...overrides };
}
...
test("settings search builds navigable destinations for loaded package, skill, and hook settings", () => {
  const index = buildSettingsSearchIndex(
    buildInput({
      ...
      hookSettings: {
        sessionStart: {
          enabled: true,
          sources: [{ id: "team-rules", name: "Team conventions", kind: "text", content: "private-hook-content" }],
        },
  ...
  assert.deepEqual(searchSettings(index, "History Tools")[0]?.control, { kind: "package", packageId: "pi-history" });
