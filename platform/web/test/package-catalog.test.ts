...
test("package catalog discovers safe packages and persists an allowlist", async () => {
  ...
  try {
    ...
    assert.ok(
      first.extensionPaths.some((path) => path.endsWith("unmanaged.ts")),
    );

    const enabled = await catalog.setEnabled("pi-b", true);
    assert.deepEqual([...enabled.enabledIds].sort(), [
      "pi-a",
      "pi-b",
      "pylon-core",
    ]);
...
test("package catalog confines and delegates package-owned settings", async () => {
  ...
  try {
    ...
    const previous = await catalog.updateSettings("pi-settings", {
      kind: "helios",
      headed: false,
    });
    assert.deepEqual(previous, { kind: "helios", headed: true });
    assert.deepEqual(await catalog.readSettings("pi-settings"), {
      kind: "helios",
      headed: false,
    });
