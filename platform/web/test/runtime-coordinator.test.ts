...
test("session index pages projects, counts user messages, and searches unloaded sessions", async () => {
  ...
  try {
    ...
    const withoutDraft = await index.list(
      {},
      { ...options, activeId: draft.id, fallbacks: [draft], userCountFor: () => 0 },
    );
    assert.equal(
    ...
    await Promise.all(
      [first.getSessionFile(), second.getSessionFile()].map(path => (path ? rm(path, { force: true }) : undefined)),
    );
    await rm(root, { recursive: true, force: true });
  }
});
...
test("session index reuses its persisted cache and rebuilds corrupt or outdated caches", async () => {
  ...
  try {
    ...
    result = await warm.list({ query: "Added after initial cache" }, options);
    assert.equal(
      result.projects.flatMap(project => project.sessions).some(item => item.id === added!.getSessionId()),
      false,
    );
    ...
    assert.equal(
      result.projects.flatMap(project => project.sessions).filter(item => item.id === session.getSessionId()).length,
      1,
    );
...
test("switching a session index agent directory drops pending paths from the prior agent", async () => {
  ...
  try {
    ...
    const result = await index.list(
      {},
      { activeId: second.getSessionId(), generation: 2, stateFor: () => "sleeping" as const },
    );
...
test("running spawned sessions route through their parent until the child turn ends", async () => {
  ...
  try {
    ...
    assert.equal(
      completed.projects.flatMap(project => project.sessions).find(item => item.id === child.getSessionId())
        ?.runningUnderParentSessionId,
      undefined,
    );
    assert.equal(
      (await coordinator.switchSession({ sessionId: child.getSessionId() })).sessionId,
      child.getSessionId(),
    );
  } finally {
    await coordinator.dispose();
...
test("session selection retries a snapshot invalidated by a background completion", async () => {
  ...
  const slot = {
    ...
    eventRevision: 0,
    lastActivityAt: Date.now(),
    driver: { runtimeState: () => "idle", runtimeDetails: () => ({ workStartedAt: undefined }) },
    lastState: "running",
...
test("queue pump retries only a transient busy prompt rejection", async () => {
  ...
  const slot = {
    ...
    driver: {
      ...
      prompt: async (input: any) => {
        if (++attempts === 1)
          throw new Error("Agent is already processing. Specify streamingBehavior to queue the message.");
        internal.onSlotEvent(slot, {
          type: "session.event",
          payload: { type: "message_start", clientMessageId: input.commandId, message: { role: "user" } },
        });
...
test("accepted queued prompts remain snapshot-visible until their user message materializes", async () => {
  ...
  assert.deepEqual(
    internal.queueReadModel(slot).items.map((item: any) => [item.commandId, item.state]),
  ...
  const replacement = internal.translateSnapshot(
    { conversation: { queue: { steering: 0, followUp: 0 } }, sessionControls: {} },
    slot,
  );
  assert.deepEqual(
...
test("runtime replacement requeues an in-flight delivery and ignores its stale resolution", async () => {
  ...
  internal.onSlotEvent(slot, { type: "session.replaced", sessionId: slot.id, sessionGeneration: 5, runtime: {} });
  assert.deepEqual(
    internal.queueReadModel(slot).items.map((item: any) => [item.commandId, item.state]),
...
test("queued follow-up remains visible while a turn timer is active", async () => {
  ...
  coordinator.subscribe(event => {
    if (event.type === "queue.changed")
      published.push(event.queue.items?.map(item => `${item.preview}:${item.state}`) ?? []);
  });
  const slot = {
    id: "session",
    innerGeneration: 4,
...
test("session status publishes work timer changes even when runtime state is unchanged", () => {
  ...
  assert.deepEqual(
    ...
    ["2026-07-30T10:00:00.000Z", "2026-07-30T10:00:00.000Z", "2026-07-30T10:00:00.000Z", null],
  );
});
...
test("background attention status carries explicit sound cue intent", () => {
  ...
  const slot = {
    id: "background",
    eventRevision: 0,
    driver: { runtimeState: () => "attention", runtimeDetails: () => ({ workStartedAt: undefined }) },
    lastState: "running",
    lastActivityAt: Date.now(),
  };
  internal.slots.set(slot.id, slot);
...
test("settings persist while sessions run, but provider logout waits for idle", async () => {
  ...
  const background = { id: "background", driver: { canSleep: () => !running } };
  internal.generation = 1;
  internal.selectedId = selected.id;
  internal.slots.set(selected.id, selected);
  ...
  await coordinator.updateHookSettings({
    settings: { sessionStart: { enabled: false, sources: [] }, beforeAgentStart: { enabled: false, sources: [] } },
  });
...
test("runtime pool warm-switches without rebuilding and wakes sleeping sessions", { timeout: 45_000 }, async () => {
  ...
  try {
    ...
    assert.equal(
      (await driver.listSessions()).activeSessions.some(session => session.id === other.getSessionId()),
      false,
    );
    ...
    selectedSlot.driver.fileSuggestions = () =>
      new Promise(resolve => {
        releaseSuggestions = () =>
          resolve({ protocolVersion: PROTOCOL_VERSION, sessionGeneration: 1, available: true, paths: [] });
      });
    ...
    await assert.rejects(
      driver.setSessionActive({ sessionId: other.getSessionId(), active: false }),
    ...
    assert.equal(
      unpinned.projects.flatMap(project => project.sessions).find(session => session.id === other.getSessionId())
        ?.pinned,
      false,
    );
    ...
    assert.equal(
      (await driver.listSessions()).projects.some(project =>
        project.sessions.some(session => session.id === other.getSessionId()),
      ),
      false,
    );
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const secondSourcePrompt = (await driver.snapshot()).conversation.messages.find(
        message => message.role === "user" && message.entryId,
      );
