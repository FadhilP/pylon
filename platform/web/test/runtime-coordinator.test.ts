...
test("project and session policies defer effective changes until a running turn settles", async () => {
  ...
  const registry = new ProjectRegistry(
    join(agentDir, "pylon-web", "projects.json"),
    root,
  );
  ...
  const makeSlot = (id: string, initialState: "idle" | "running") => {
    ...
    let snapshot = {
      ...runtime(id),
      runtimePolicy: registry.runtimePolicy(projectId, id),
    };
    const applied: any[] = [];
    const slot: any = {
      id,
      ...
      driver: {
        ...
        runtimeDetails: () => ({
          ...
          cwd,
          userMessageCount: 1,
          workStartedAt: state === "running" ? "turn" : undefined,
        }),
...
test("session selection publishes before its workspace refresh completes", async () => {
  ...
  assert.equal(
    events.find((event) => event.type === "session.replaced")?.runtime
      .workspace,
    undefined,
  );
  assert.equal(
    events.some((event) => event.type === "workspace.revision"),
    false,
  );
  ...
  assert.equal(
    events.filter((event) => event.type === "workspace.revision").length,
    1,
  );
});
...
test("slot disposal waits for a background workspace refresh", async () => {
  ...
  const internal = coordinator as any;
  let releaseRefresh!: () => void;
  const workspaceRefresh = new Promise<void>((resolve) => {
    releaseRefresh = resolve;
  });
...
test("session index pages projects, counts user messages, and searches unloaded sessions", async () => {
  ...
  try {
    ...
    const options = {
      ...
      workStartedAtFor: (id: string) =>
        id === second.getSessionId() ? workStartedAt : undefined,
    };
    const page = await index.list(
      { projectId: projectIdForCwd(cwd), limit: 1 },
      options,
    );
    ...
    const withoutDraft = await index.list(
      {},
      {
        ...options,
        activeId: draft.id,
    ...
    assert.equal(
      withoutDraft.activeSessions.some((session) => session.id === draft.id),
      false,
    );
    ...
    assert.equal(
      withPrompt.activeSessions.some((session) => session.id === draft.id),
      true,
    );
  } finally {
    await Promise.all(
      [first.getSessionFile(), second.getSessionFile()].map((path) =>
...
test("session index refreshes one changed session without SDK-wide scans", async () => {
  ...
  try {
    const index = new SessionIndex();
    const options = {
      activeId: session.getSessionId(),
      generation: 1,
      stateFor: () => "idle" as const,
    };
    await index.list({}, options);
    persistSession(session, "New lifecycle text");
    index.invalidateSession(
...
test("session index reuses its persisted cache and rebuilds corrupt or outdated caches", async () => {
  ...
  try {
    ...
    const record = cache.records.find(
      (item: any) => item.session.id === session.getSessionId(),
    );
    assert.ok(record);
    ...
    warm.invalidateSession(
      session.getSessionId(),
      session.getSessionFile(),
      cwd,
    );
    result = await warm.list(
      { query: "Changed file replaces cached sentinel" },
      options,
    );
    assert.equal(result.projects[0]?.sessions[0]?.id, session.getSessionId());
    ...
    assert.equal(
      result.projects
        ...
        .some((item) => item.id === added!.getSessionId()),
      false,
    );
    ...
    assert.equal(
      result.projects
        .flatMap((project) => project.sessions)
        .filter((item) => item.id === session.getSessionId()).length,
      1,
    );
    ...
    result = await new SessionIndex(undefined, isolatedAgentDir).list(
      { query: "Persisted cache source" },
      options,
    );
    assert.equal(result.projects[0]?.sessions[0]?.id, session.getSessionId());
  } finally {
...
test("switching a session index agent directory drops pending paths from the prior agent", async () => {
  ...
  await Promise.all(
    [cwdA, cwdB, sessionsA, sessionsB].map((path) =>
...
test("targeted session refresh isolates cwd values and follows path changes", async () => {
  ...
  const cwdB = join(root, "workspace-b");
  await Promise.all(
    [custom, moved, cwdA, cwdB].map((path) => mkdir(path, { recursive: true })),
  );
  ...
  try {
    ...
    assert.equal(
      sessions.filter((session) => session.id === first.getSessionId()).length,
      1,
    );
    assert.equal(
      sessions.find((session) => session.id === second.getSessionId())
        ?.projectId,
      projectIdForCwd(cwdB),
    );
    ...
    sessions = result.projects.flatMap((project) => project.sessions);
    assert.equal(
      sessions.some((session) => session.id === first.getSessionId()),
      false,
    );
    assert.equal(
      sessions.some((session) => session.id === second.getSessionId()),
      true,
    );
  } finally {
    await rm(root, { recursive: true, force: true });
...
test("session switching restores live delegated-agent metadata from the background runtime", async () => {
  ...
  try {
    ...
    const background = (coordinator as any).slots.get(initial.sessionId).driver
      .runtime.session;
    background._emit({
    ...
    await coordinator.switchSession({ sessionId: initial.sessionId });
    const [completed] = (await coordinator.snapshot()).conversation
      .delegatedRuns;
    ...
    await Promise.all(
      sessions.map((session) => rm(session.path, { force: true })),
    );
    await rm(root, { recursive: true, force: true });
  }
});
...
test("running spawned sessions route through their parent until the child turn ends", async () => {
  ...
  try {
    const parent = await coordinator.start({
      cwd,
      agentDir,
      repositoryRoot: root,
    });
    ...
    const child = SessionManager.create(cwd, dirname(parentPath), {
    ...
    const marker = {
      ...
      path: child.getSessionFile(),
      cwd,
    };
    const session = parentSlot.driver.runtime.session;
    session._emit({
      type: "tool_execution_start",
      toolCallId: "spawn-session",
    ...
    await waitFor(() =>
      (coordinator as any).externalSpawnRuns.has(child.getSessionId()),
    );
    const running = await coordinator.listSessions();
    const summary = running.activeSessions.find(
      (item) => item.id === child.getSessionId(),
    );
    assert.equal(summary?.runningUnderParentSessionId, parent.sessionId);
    assert.equal(summary?.runtimeState, "running");
    await assert.rejects(
      coordinator.switchSession({ sessionId: child.getSessionId() }),
      /running under its parent session/,
    );
    ...
    assert.equal(
      completed.projects
        .flatMap((project) => project.sessions)
        .find((item) => item.id === child.getSessionId())
        ?.runningUnderParentSessionId,
      undefined,
    );
    assert.equal(
      (await coordinator.switchSession({ sessionId: child.getSessionId() }))
        .sessionId,
      child.getSessionId(),
    );
  } finally {
...
test("memory mutations forward scoped IDs and revisions, and reject changed generations", async () => {
  ...
  try {
    ...
    const updates: any[] = [],
      ...
      migrations: any[] = [],
      planActions: any[] = [];
    internal.slots.set("session", {
      id: "session",
      innerGeneration: 7,
      lastActivityAt: 0,
      unsubscribe: () => {},
      driver: {
        updateContinuityMemory: (input: any) => {
          updates.push(input);
          return Promise.resolve();
        },
        deleteContinuityMemory: (input: any) => {
          deletes.push(input);
          return Promise.resolve();
        },
        migrateContinuityMemory: (input: any) => {
        ...
        continuityPlanAction: (input: any) => {
          planActions.push(input);
          return Promise.resolve();
        },
        dispose: async () => {},
      },
    ...
    await coordinator.updateContinuityMemory({
      expectedGeneration: 1,
      scope: "user",
      id: userId,
      trigger: "responding",
    ...
    await coordinator.deleteContinuityMemory({
      expectedGeneration: 1,
      scope: "project",
      id: projectId,
      expectedRevision: 4,
    ...
    await coordinator.continuityPlanAction({
      expectedGeneration: 1,
      action: "approve",
      resetContext: true,
      expectedRevision: 2,
    ...
    assert.deepEqual(planActions[0], {
      expectedGeneration: 7,
      action: "approve",
      resetContext: true,
      expectedRevision: 2,
    });
    const pending = coordinator.migrateContinuityMemory({
      expectedGeneration: 1,
    });
    await Promise.resolve();
...
test("session selection retries a snapshot invalidated by a background completion", async () => {
  ...
  const fresh = runtime("background", [
    {
      ...
      role: "assistant",
      text: "Completed while switching",
      streaming: false,
    },
  ]);
  const slot = {
  ...
  const completion = events.find(
    (event) => event.type === "session.status" && event.completed === true,
  );
  assert.equal(snapshots, 2);
  assert.equal(
    events.some((event) => event.type === "session.event"),
    false,
  );
  assert.equal(
    replacement.runtime.conversation.messages[0]?.text,
    "Completed while switching",
  );
...
test("awake sessions recover once from an invalid runtime snapshot", async () => {
  ...
  internal.sessionIndex.resolve = async () => ({
    id: poisoned.id,
    cwd: "repo",
    path: "session.jsonl",
  });
  ...
  internal.snapshotFor = async (slot: unknown) =>
    slot === poisoned ? invalid : runtime(poisoned.id);
  ...
  assert.equal(
    events.filter((event) => event.type === "session.replaced").length,
    1,
  );
});
...
test("awake-session recovery stops after one invalid reconstruction", async () => {
  ...
  await assert.rejects(
    coordinator.switchSession({ sessionId: poisoned.id }),
  ...
  assert.equal(
    events.some((event) => event.type === "session.replaced"),
    false,
  );
});
...
test("awake-session recovery leaves running invalid slots untouched", async () => {
  ...
  internal.disposeSlot = async () => {
    disposed = true;
  };

  await assert.rejects(
    coordinator.switchSession({ sessionId: awake.id }),
    /operational\.verification\.checks/,
  );
...
test("queued prompts stay ordered and continue after queued control failures", async () => {
  ...
  const slot = {
    ...
    driver: {
      ...
      runtimeDetails: () => ({
        workStartedAt:
          state === "running" ? "2026-07-30T10:00:00.000Z" : undefined,
      }),
      prompt: async (input: any) => {
  ...
  assert.deepEqual(
    [first.commandId, second.commandId, third.commandId],
    ["command-first", "command-second", "command-third"],
  );

  assert.equal(
    (
      await coordinator.queuedPrompt({
        queueId: second.id,
        expectedGeneration: 1,
      })
    ).message,
  ...
  await coordinator.restoreQueuedPrompt({
    queueId: second.id,
    expectedGeneration: 1,
  });
  await coordinator.steerQueuedPrompt({
    queueId: third.id,
    commandId: "steer-third",
    expectedGeneration: 1,
  });
  ...
  assert.deepEqual(
    internal.queueReadModel(slot).items.map((item: any) => item.preview),
    ["fourth"],
  );
  ...
  const slot = {
    ...
    driver: {
      ...
      prompt: async (input: any) => {
        if (++attempts === 1)
          throw new Error(
            "Agent is already processing. Specify streamingBehavior to queue the message.",
          );
        internal.onSlotEvent(slot, {
          type: "session.event",
          payload: {
...
test("accepted queued prompts remain snapshot-visible until their user message materializes", async () => {
  ...
  assert.deepEqual(
    internal
      .queueReadModel(slot)
      .items.map((item: any) => [item.commandId, item.state]),
    [["command-visible", "delivering"]],
  );
  ...
  assert.deepEqual(
    replacement.conversation.queue.items.map((item: any) => item.commandId),
    ["command-visible"],
  );
  ...
  internal.onSlotEvent(slot, {
    ...
    payload: {
      type: "message_start",
      clientMessageId: "another-command",
      message: { role: "user" },
    },
  });
  assert.equal(slot.displayPendingPrompts.length, 1);
  ...
  internal.onSlotEvent(slot, {
    ...
    payload: {
      type: "message_start",
      clientMessageId: "command-visible",
      message: { role: "user" },
    },
  });
  assert.deepEqual(internal.queueReadModel(slot).items, []);
...
test("steer deliveries remain visible and failures restore only prompts that did not materialize", async () => {
  ...
  const slot = {
    ...
    driver: {
      ...
      steer: async (input: any) => {
        if (behavior === "materialize-reject")
          internal.onSlotEvent(slot, {
            type: "session.event",
            payload: {
              type: "message_start",
  ...
  assert.deepEqual(
    internal.queueReadModel(slot).items.map((item: any) => item.commandId),
    ["command-accepted", "command-later"],
  );
  ...
  internal.onSlotEvent(slot, {
    ...
    payload: {
      type: "message_start",
      clientMessageId: "command-accepted",
      message: { role: "user" },
    },
  });
  assert.deepEqual(internal.queueReadModel(slot).items, []);
...
test("runtime replacement requeues an in-flight delivery and ignores its stale resolution", async () => {
  ...
  await coordinator.queuePrompt({
    ...
    commandId: "command-replaced",
    expectedGeneration: 1,
  });
  const queued = internal.queueReadModel(slot).items[0];
  const steering = coordinator.steerQueuedPrompt({
    queueId: queued.id,
    expectedGeneration: 1,
  });
  ...
  if ((slot as any).queueFlushTimer)
    clearTimeout((slot as any).queueFlushTimer);
});
...
test("queued follow-up remains visible while a turn timer is active", async () => {
  ...
  coordinator.subscribe((event) => {
    if (event.type === "queue.changed")
      published.push(
        event.queue.items?.map((item) => `${item.preview}:${item.state}`) ?? [],
      );
  });
  const slot = {
    id: "session",
...
test("session status publishes work timer changes even when runtime state is unchanged", () => {
  ...
  assert.deepEqual(
    events.map((event) => event.workStartedAt),
    [
...
test("background attention status carries explicit sound cue intent", () => {
  ...
  const attention = events.find(
    (event) => event.type === "session.status" && event.state === "attention",
  );
...
test("settings persist while sessions run, but provider logout waits for idle", async () => {
  ...
  const selected = {
    ...
    driver: {
      canSleep: () => true,
      setPackageEnabled: async () => {
        updates.push("enabled");
      },
      updatePackageSettings: async () => {
        updates.push("package");
      },
      updateHookSettings: async () => {
        updates.push("hooks");
      },
  ...
  await coordinator.setPackageEnabled({
    packageId: "pi-sieve",
    enabled: false,
  });
  await coordinator.updatePackageSettings({
    packageId: "pi-sieve",
    settings: {
      kind: "sieve",
      activePruning: true,
  ...
  await coordinator.updateHookSettings({
    settings: {
  ...
  await assert.rejects(
    coordinator.logoutProvider("mock", 1),
...
test(
  "runtime pool warm-switches without rebuilding and wakes sleeping sessions",
  { timeout: 45_000 },
  async () => {
    ...
    driver.subscribe((event) => {
      if (event.type === "session.status")
        states.push([event.sessionId, event.state]);
    });
    ...
    try {
      ...
      assert.equal(
        (await driver.listPackages()).sessionGeneration,
        (await driver.snapshot()).sessionGeneration,
      );
      assert.equal(
        (await driver.listSessions()).activeSessions.some(
          (session) => session.id === other.getSessionId(),
        ),
        false,
      );
      const awakeSessions = await driver.listSessions({
        projectId: projectIdForCwd(cwd),
      });
      assert.equal(
        awakeSessions.projects[0]?.sessions.some(
          (session) => session.id === initial.sessionId,
        ) ?? false,
        false,
      );

      const selectedSlot = (driver as any).selected();
      const originalFileSuggestions = selectedSlot.driver.fileSuggestions.bind(
        selectedSlot.driver,
      );
      const originalRefreshWorkspace = (driver as any).refreshWorkspace.bind(
        driver,
      );
      let releaseRefresh!: () => void;
      ...
      selectedSlot.driver.fileSuggestions = () =>
        new Promise((resolve) => {
          releaseSuggestions = () =>
            resolve({
              protocolVersion: PROTOCOL_VERSION,
      ...
      await assert.rejects(
        staleSuggestions,
        /session changed while loading file suggestions/,
      );
      ...
      await entered;
      const switchedDuringList = await driver.switchSession({
        sessionId: other.getSessionId(),
      });
      releaseList();
      assert.equal(
        (await listingDuringSwitch).sessionGeneration,
        switchedDuringList.sessionGeneration,
      );
      ...
      await waitFor(() =>
        states.some(
          ([id, state]) => id === other.getSessionId() && state === "sleeping",
        ),
      );
      await driver.setSessionActive({
        sessionId: other.getSessionId(),
        active: true,
      });
      await driver.setSessionPinned({
        sessionId: other.getSessionId(),
        pinned: true,
      });
      assert.equal(starts, 3);
      await driver.renameSession({
        sessionId: other.getSessionId(),
      ...
      assert.equal(
        unpinned.projects
          .flatMap((project) => project.sessions)
          .find((session) => session.id === other.getSessionId())?.pinned,
        false,
      );
      await driver.setSessionActive({
        sessionId: other.getSessionId(),
      ...
      const selectedOther = await driver.switchSession({
        sessionId: other.getSessionId(),
      });
      assert.equal(starts, 4);
      const archived = await driver.archiveSession({
        sessionId: other.getSessionId(),
        expectedGeneration: selectedOther.sessionGeneration,
      });
      assert.equal(
        (await driver.listSessions()).projects.some((project) =>
      ...
      assert.equal(
        (await driver.listArchived()).sessions[0]?.id,
        other.getSessionId(),
      );
      await driver.restoreSession({
        sessionId: other.getSessionId(),
        expectedGeneration: archived.sessionGeneration,
      });
...
test(
  "pinned sessions persist, wake on restart, and never sleep",
  { timeout: 30_000 },
  async () => {
    ...
    try {
      ...
      const summary = sessions.activeSessions.find(
        (session) => session.id === pinned.getSessionId(),
      );
...
test(
  "General sessions use the built-in home scope instead of the selected project",
  { timeout: 20_000 },
  async () => {
    ...
    try {
      await driver.start({ cwd, agentDir, repositoryRoot: root });
      const created = await driver.newSession({
      ...
      assert.equal(
        registry.workspaceForSession(created.sessionId)?.projectId,
        GENERAL_PROJECT_ID,
      );
      assert.equal(
        registry.workspaceForSession(created.sessionId)?.mode,
        "local",
      );
      assert.ok(
        sessions.projects.some(
          (project) =>
            project.id === GENERAL_PROJECT_ID && project.cwd === generalCwd,
        ),
      );
      const snapshot = await driver.snapshot();
      assert.equal(
        snapshot.runtimePolicy.effective.toolOverrides?.code_search,
        "disabled",
      );
      assert.equal(
        snapshot.runtimePolicy.effective.toolOverrides?.bash,
        undefined,
      );
...
test(
  "new sessions apply the effective workspace policy before the first prompt",
  { timeout: 20_000 },
  async () => {
    ...
    try {
      ...
      assert.equal(
        ...
        snapshot.runtimePolicy.effective.workspace,
      );
      ...
      assert.equal(
        movedSnapshot.workspace?.mode,
        movedSnapshot.runtimePolicy.effective.workspace,
      );
      assert.equal(movedSnapshot.workspace?.mode, "worktree");
      assert.equal(
        registry.workspaceForSession(movedSnapshot.sessionId)?.mode,
        "worktree",
      );

      const isolated = await driver.newSession({
        expectedGeneration: movedSnapshot.sessionGeneration,
      });
      const isolatedSnapshot = await driver.snapshot();
      assert.equal(
        isolatedSnapshot.workspace?.mode,
        isolatedSnapshot.runtimePolicy.effective.workspace,
      );
      assert.equal(isolatedSnapshot.workspace?.mode, "worktree");
      assert.equal(
        registry.workspaceForSession(isolated.sessionId)?.mode,
        "worktree",
      );
    } finally {
      await driver.dispose();
...
test(
  "Local draft provisioning leaves the project branch and worktree list unchanged",
  { timeout: 20_000 },
  async () => {
    ...
    try {
      ...
      persistSession(
        (slot.driver as any).runtime.session.sessionManager,
      ...
      assert.equal(
        (await run("git", ["branch", "--show-current"], { cwd })).stdout.trim(),
        branchBefore,
      );
      assert.equal(
        (await run("git", ["worktree", "list", "--porcelain"], { cwd })).stdout,
        worktreesBefore,
      );
      assert.equal(
        (
          await run("git", ["branch", "--list", "pylon-session-*"], { cwd })
        ).stdout.trim(),
        "",
      );
    } finally {
...
test(
  "Local sessions can run concurrently in one project",
  { timeout: 20_000 },
  async () => {
...
test(
  "session changes apply from a worktree and Project folder without committing",
  { timeout: 45_000 },
  async () => {
    ...
    try {
      ...
      await (driver as any).moveSelectedFromLocal(
        slot,
        projectIdForCwd(cwd),
        "worktree",
      );
      await writeFile(
        join(slot.driver.runtimeDetails().cwd, "README.md"),
        "base\nisolated\n",
      );
      ...
      assert.equal(
        (await readFile(join(cwd, "local.txt"), "utf8")).trim(),
        "keep local",
      );
      assert.equal((await driver.snapshot()).workspace?.mode, "worktree");
      assert.equal(
        (await run("git", ["status", "--porcelain"], { cwd })).stdout.includes(
          "README.md",
        ),
        true,
      );
      ...
      assert.match(
        (await driver.workspaceDiff({ path: "README.md" })).text ?? "",
        /^\+project-folder$/m,
      );
      assert.equal(
        (await run("git", ["branch", "--show-current"], { cwd })).stdout.trim(),
      ...
      assert.equal(
        (await run("git", ["log", "-1", "--pretty=%s"], { cwd })).stdout.trim(),
        "Initial",
      );
    } finally {
      await driver.dispose();
...
test(
  "fork translates the coordinator generation to the selected runtime generation",
  { timeout: 30_000 },
  async () => {
    ...
    try {
      await driver.start({ cwd, agentDir, repositoryRoot: root });
      const selected = await driver.switchSession({
      ...
      const prompt = (await driver.snapshot()).conversation.messages.find(
      ...
      assert.equal(
        (await driver.snapshot()).sessionName,
        "Forked after switching",
      );
      const sessions = await driver.listSessions();
      ...
      assert.ok(
        Date.now() - Date.parse(sessions.activeSessions[0]!.modifiedAt) <
          60_000,
      );
      await assert.rejects(
        driver.fork({
...
test(
  "repeated Local forks retain Local workspace records and allow concurrent sessions",
  { timeout: 30_000 },
  async () => {
    ...
    try {
      ...
      const sourcePrompt = (await driver.snapshot()).conversation.messages.find(
        (message) => message.role === "user" && message.entryId,
      );
      ...
      assert.equal(
        registry.workspaceForSession(first.sessionId)?.mode,
        "local",
      );

      const selectedSource = await driver.switchSession({
        sessionId: sourceId,
      });
      const secondSourcePrompt = (
      ...
      assert.equal(
        registry.workspaceForSession(second.sessionId)?.mode,
        "local",
      );
...
test(
  "empty project registry uses parking runtime and add/remove keeps the workspace",
  { timeout: 30_000 },
  async () => {
    ...
    try {
      ...
      assert.equal(
        (await driver.addProject({ expectedGeneration: 1 })).cancelled,
        true,
      );
      ...
      const archived = await driver.archiveProject({
        projectId: projectIdForCwd(cwd),
        expectedGeneration: added.sessionGeneration,
      });
      assert.equal((await driver.snapshot()).projectAvailable, false);
      assert.equal(
        (await driver.listArchived()).projects[0]?.id,
        projectIdForCwd(cwd),
      );
      await driver.restoreProject({
        projectId: projectIdForCwd(cwd),
        expectedGeneration: archived.sessionGeneration,
      });
      assert.equal(
        (await driver.listSessions()).projects[0]?.id,
        projectIdForCwd(cwd),
      );

      await driver.removeProject({
        projectId: projectIdForCwd(cwd),
        expectedGeneration: archived.sessionGeneration,
      });
      assert.equal((await driver.snapshot()).projectAvailable, false);
...
test(
  "disposing the coordinator aborts and clears an open directory picker",
  { timeout: 20_000 },
  async () => {
    ...
    try {
      ...
      await assert.rejects(
        driver.addProject({ expectedGeneration: 1 }),
        /already open/,
      );
      await driver.dispose();
      await rejected;
