...
test("background completion status queues one cue without duplicating selected completion", () => {
  const background = appendWebAudioCue([], {
    ...
    sessionId: "selected",
    type: "session.status",
    payload: { sessionId: "background", state: "idle", completed: true },
  });
...
test("background attention status queues one cue without duplicating selected requests", () => {
  const background = appendWebAudioCue([], {
    ...
    sessionId: "selected",
    type: "session.status",
    payload: { sessionId: "background", state: "attention" },
  });
...
test("non-completion events stay silent", () => {
  ...
  assert.equal(appendWebAudioCue(cues, { eventId: "status", sessionId: "selected", type: "session.status", payload: { sessionId: "selected", state: "idle" } }), cues);
