...
export function formatHookSources(
  hook: HookReadModel,
  hookName: "sessionStart" | "beforeAgentStart" | "sessionCompact",
): string | undefined {
  if (!hook.enabled || !hook.sources.length) return undefined;
  const sources = hook.sources.map(
    (source) =>
...
function branchHasSessionStartMessage(branch: unknown[]): boolean {
  return branch.some((entry) => {
    ...
    return (
      ...
      (item.customType === SESSION_START_HOOK_CUSTOM_TYPE ||
        item.message?.customType === SESSION_START_HOOK_CUSTOM_TYPE)
    );
  });
}

function branchHasCompactionReinjection(
  branch: unknown[],
  ...
  return branch.some((entry) => {
    ...
    return (
      ...
      (item.customType === SESSION_START_HOOK_CUSTOM_TYPE ||
        item.message?.customType === SESSION_START_HOOK_CUSTOM_TYPE) &&
      (item.details?.compactionEntryId === compactionEntryId ||
        item.message?.details?.compactionEntryId === compactionEntryId)
    );
  });
}

function compactionContent(
  settings: HookSettingsReadModel,
): string | undefined {
  return formatHookSources(
    {
      ...settings.sessionStart,
      sources: settings.sessionStart.sources.filter(
        (source) => source.reinjectOnCompaction === true,
      ),
    },
    "sessionCompact",
  );
    ...
    factory: (pi) => {
      const unsubscribe = pi.events.on("pylon:spawn-hooks-request", (value) => {
        ...
        if (request?.version !== 1 || typeof request.provide !== "function")
          return;
        const sessionStart = formatHookSources(
          this.settings.sessionStart,
          "sessionStart",
        );
        const sessionCompact = compactionContent(this.settings);
        const beforeAgentStart = formatHookSources(
          this.settings.beforeAgentStart,
          "beforeAgentStart",
        );
        request.provide({
          ...(sessionStart
      ...
      pi.on("session_start", (_event, ctx) => {
        ...
        const content = formatHookSources(
          this.settings.sessionStart,
          "sessionStart",
        );
        if (
          !content ||
          branchHasSessionStartMessage(ctx.sessionManager.getBranch())
        )
          return;
      ...
      pi.on("session_compact", (event, ctx) => {
        ...
        if (
          !compactionEntryId ||
          !content ||
          branchHasCompactionReinjection(
