...
function parseFingerprint(value: unknown): Fingerprint | undefined {
  ...
  return finite(item.dev) && finite(item.ino) && finite(item.size) && item.size >= 0
    && finite(item.mtimeMs) && finite(item.ctimeMs)
...
function parseRecord(value: unknown): CacheRecord | undefined {
  ...
  if (!validText(session.path) || !session.path || !validText(session.id, 128) || !session.id
    || !validText(session.cwd) || !validText(session.created, 100) || Number.isNaN(Date.parse(session.created))
    || !validText(session.modified, 100) || Number.isNaN(Date.parse(session.modified))
    || !Number.isSafeInteger(session.messageCount) || Number(session.messageCount) < 0
    || typeof session.firstMessage !== "string" || typeof session.allMessagesText !== "string"
    ...
    || !Number.isSafeInteger(item.userMessageCount) || Number(item.userMessageCount) < 0) return;
...
async function sessionFiles(root: string): Promise<Array<{ path: string; fingerprint: Fingerprint }>> {
  ...
  const files = (await mapLimit(directories, async (directory) => {
  ...
  return (await mapLimit(files, async (path) => {
    try {
...
async function parseSession(path: string, before: Fingerprint, retries = 1): Promise<CacheRecord | undefined> {
  ...
  try {
    ...
    const consistentOwner = owner && owners.every((candidate) => candidate?.id === owner.id
    ...
    return {
      ...
      session: {
        ...
        modified: new Date(lastActivityTime && lastActivityTime > 0 ? lastActivityTime : Number.isNaN(headerTime) ? after.mtimeMs : headerTime).toISOString(),
        messageCount,
...
export class SessionSummaryCache {
  ...
  async refresh(sessionId: string, path: string): Promise<IndexedSession | undefined> {
    ...
    for (const [candidate, value] of this.records) {
      if (candidate === key || value.session.id === sessionId || record && value.session.id === record.session.id) this.records.delete(candidate);
    }
