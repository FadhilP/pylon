...
const PONYTAIL_SKILL = readFileSync(
  PONYTAIL_SKILL_PATHS.find((path) => existsSync(path)) ??
    PONYTAIL_SKILL_PATHS[0],
  "utf8",
).replaceAll("\r\n", "\n");
...
export function cloneHookSettings(
  settings: HookSettingsReadModel,
  ...
  for (const hook of [cloned.sessionStart, cloned.beforeAgentStart])
    for (const source of hook.sources)
      source.reinjectOnCompaction = source.reinjectOnCompaction === true;
  return cloned;
}
...
export function validPersistedHookSettings(
  value: unknown,
): value is HookSettingsConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const config = value as Record<string, unknown>;
  if (
    config.version !== 1 ||
    !exactKeys(config, ["version", "settings"]) ||
    !validHookSettings(config.settings)
  )
    return false;
...
export class HookSettingsStore {
  ...
  async read(): Promise<HookSettingsReadModel> {
    try {
      ...
      if (!validPersistedHookSettings(value))
        throw new Error("hook settings config is invalid");
      ...
      if (
        ...
        error.message === "hook settings config is invalid"
      )
        throw error;
      throw new Error("hook settings config is invalid");
    }
  ...
  async update(settings: HookSettingsReadModel): Promise<void> {
    if (!validHookSettings(settings))
      throw new Error("hook settings are invalid");
