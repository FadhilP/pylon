  ...
  readonly extension: InlineExtension = {
    ...
    factory: (pi) => {
      ...
      pi.on("session_start", () => {
        ...
        if (!coordinated)
          pi.setActiveTools(
            pi
              .getActiveTools()
              .filter((name) => name !== "apply_session_changes"),
          );
      });
      ...
      pi.registerTool({
        ...
        description:
          "Request applying this session's changes to the registered project's current branch only when the user explicitly asks. Requires confirmation and runs after the current turn settles. Make this the final workspace-mutating tool call of the turn and do not modify workspace files afterward.",
        promptSnippet:
          "Apply this session's changes to the original project branch after approval",
        promptGuidelines: [
        ...
        async execute(
          ...
          const info = (await bridge.handler?.({ type: "inspect" })) as
            WorkspaceApplyToolInfo | undefined;
          if (!info?.available || !info.revision || !info.targetBranch) {
            const reason =
              info?.reason ?? "The web workspace host is unavailable.";
            return {
              content: [
                {
                  type: "text" as const,
                  text: `Session changes were not scheduled: ${reason}`,
                },
              ],
            };
          }
          if (!ctx.hasUI) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Session changes were not scheduled because confirmation UI is unavailable.",
                },
              ],
            };
          }
          ...
          if (!approved) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "The user declined applying session changes.",
                },
              ],
            };
          }
