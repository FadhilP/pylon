...
export class ProjectRegistry {
  ...
  async load(seedDirectories: string[] | (() => Promise<string[]>) = []): Promise<void> {
    ...
    try {
      ...
      const persistedGlobal =
        value.globalPolicy && typeof value.globalPolicy === "object" && !Array.isArray(value.globalPolicy)
        ...
        this.globalPolicy = {
          ...
          guardRules:
            Number(value.version) >= 13
              ? mergeGuardRules(persistedGlobal.guardRules as GuardRuleOverrides)
              : { ...DEFAULT_GUARD_RULES },
          ...
          toolOverrides:
            Number(value.version) >= 12
              ? cloneToolOverrides(persistedGlobal.toolOverrides as ToolOverrideReadModel)
              : {},
        };
      }
      ...
      const projects = value.projects.flatMap(item => {
        ...
        if (
          ...
          (typeof record.setupCommand !== "string" || record.setupCommand.length > 2_000)
        )
          return [];
        ...
        return [
          {
            ...
            ...(record.timelineEnabled !== undefined && (!legacyPolicy || record.timelineEnabled !== true)
              ? { timelineEnabled: record.timelineEnabled }
              : {}),
            ...(record.guardEnabled !== undefined ? { guardEnabled: record.guardEnabled } : {}),
            ...(record.guardRules && Object.keys(record.guardRules as GuardRuleOverrides).length
            ...
            (!legacyPolicy || record.guardTimeoutSeconds !== DEFAULT_DIALOG_TIMEOUT_SECONDS)
              ? { guardTimeoutSeconds: record.guardTimeoutSeconds }
              : {}),
            ...(record.clarifyTimeoutSeconds !== undefined &&
            (!legacyPolicy || record.clarifyTimeoutSeconds !== DEFAULT_DIALOG_TIMEOUT_SECONDS)
  ...
  async reorderProject(projectId: string, beforeProjectId?: string): Promise<void> {
    ...
    const before = beforeProjectId ? visible.findIndex(candidate => candidate.id === beforeProjectId) : -1;
    ...
    let visibleIndex = 0;
    this.projects = this.projects.map(candidate => (candidate.archivedAt ? candidate : visible[visibleIndex++]!));
    await this.save();
  }
  ...
  async activateSession(sessionId: string): Promise<void> {
    this.assertLoaded();
    this.activeSessionOrder = [sessionId, ...this.activeSessionOrder.filter(id => id !== sessionId)].slice(
      0,
  ...
  runtimePolicy(projectId: string, sessionId: string): RuntimePolicyReadModel {
    ...
    const projectGuardTimeout =
      project.guardTimeoutSeconds === undefined ? this.globalPolicy.guardTimeoutSeconds : project.guardTimeoutSeconds;
  ...
  async updateRuntimePolicy(input: RuntimePolicyUpdate): Promise<RuntimePolicyReadModel> {
    ...
    if (
      (input.guardTimeoutSeconds !== "inherit" && !validDialogTimeout(input.guardTimeoutSeconds)) ||
    ...
    if (input.guardRules !== undefined && !validGuardRules(input.guardRules, input.scope === "global"))
  ...
  private parseSessionWorkspace(value: unknown): SessionWorkspaceRecord[] {
    ...
    if (
      optional.some(
        key =>
          record[key] !== undefined && (typeof record[key] !== "string" || !record[key] || record[key]!.length > 4_096),
      )
    )
      return [];
    return [
  ...
  private parseSessionPolicy(value: unknown): SessionPolicyRecord[] {
    ...
    if (
      ...
      (record.toolOverrides !== undefined && !validToolOverrides(record.toolOverrides))
    )
      return [];
    return [
      {
...
function validToolOverrides(value: unknown): value is ToolOverrideReadModel {
  return (
    ...
    Object.entries(value as Record<string, unknown>).every(
      ([tool, mode]) => validToolName(tool) && (mode === "active" || mode === "deferred" || mode === "disabled"),
    )
  );
}
...
function validDialogTimeout(value: unknown): value is DialogTimeoutSeconds {
  return value === null || (Number.isSafeInteger(value) && (value as number) >= 15 && (value as number) <= 86_400);
}
