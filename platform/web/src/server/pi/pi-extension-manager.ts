...
export function validPiPackageSource(source: string): boolean {
  return (
    source.length > 0 &&
    source.length <= 500 &&
    (NPM_SOURCE.test(source) || GIT_SOURCE.test(source))
  );
}
...
export function isPylonPackageSource(source: string): boolean {
  ...
  return (
    ...
    /^(?:git:)?(?:https?:\/\/|ssh:\/\/git@|git@)?github\.com[/:]fadhilp\/pylon(?:\.git)?(?:@|#|$)/.test(
      value,
    )
  );
}
...
function exactResourcePath(resource: ResolvedResource): string {
  ...
  if (!base)
    throw new Error(
      "extension cannot be configured because Pi did not report its base directory",
    );
  const value = relative(base, resource.path).replaceAll("\\", "/");
...
export class PiExtensionManager {
  ...
  projectTrusted(): boolean {
    return (
      !this.projectTrustRequired() || this.trustStore.get(this.cwd) === true
    );
  }
  ...
  async list(
    runtime: LoadExtensionsResult | undefined,
    generation: number,
  ): Promise<ExtensionListSnapshot> {
    ...
    const visibleResources = resolved.extensions.filter(
      (resource) => !this.excluded(resource),
    );
    const extensions: NativeExtensionReadModel[] = visibleResources.map(
      (resource) => {
        const scope =
    ...
    return {
      ...
      packages: this.packages
        ...
        .filter(
          ({ source }) =>
            validPiPackageSource(source) && !isPylonPackageSource(source),
        )
  ...
  async setEnabled(id: string, enabled: boolean): Promise<void> {
    ...
    if (resource.metadata.origin === "package") {
      ...
      const index = current.findIndex(
        (entry) => packageSource(entry) === resource.metadata.source,
      );
      if (index < 0) throw new Error("extension package is unavailable");
      const entry = current[index]!;
      current[index] = {
        ...(typeof entry === "string" ? { source: entry } : entry),
        extensions: withExactOverride(
          typeof entry === "string" ? undefined : entry.extensions,
          exact,
          enabled,
        ),
      };
  ...
  async install(source: string, scope: ExtensionScope): Promise<void> {
    ...
    await this.packages.installAndPersist(source, {
      local: scope === "project",
    });
    await this.settings.flush();
  }
  ...
  async remove(source: string, scope: ExtensionScope): Promise<void> {
    this.assertPackageMutation(source, scope);
    const removed = await this.packages.removeAndPersist(source, {
      local: scope === "project",
    });
    await this.settings.flush();
    if (!removed)
      throw new Error("extension package is not configured in that scope");
  }

  private excluded(resource: ResolvedResource): boolean {
    return (
      this.excludedPaths.has(resolve(resource.path)) ||
      isPylonPackageSource(resource.metadata.source)
    );
  }
  ...
  private assertPackageMutation(source: string, scope: ExtensionScope): void {
    ...
    if (!validPiPackageSource(source))
      throw new Error("unsupported extension package source");
  }

  private assertTrusted(scope: ExtensionScope): void {
    if (scope === "project" && !this.projectTrusted())
      throw new Error("project extensions require a trusted project");
  }
}
