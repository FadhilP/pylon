...
export function isPylonPackageSource(source: string): boolean {
  ...
  return /^npm:@fadhilp\/pylon(?:@|$)/.test(value)
    || /^(?:git:)?(?:https?:\/\/|ssh:\/\/git@|git@)?github\.com[/:]fadhilp\/pylon(?:\.git)?(?:@|#|$)/.test(value);
}

function opaqueId(scope: ExtensionScope, path: string): string {
  return createHash("sha256").update(`${scope}\0${resolve(path)}`).digest("hex").slice(0, 32);
}

function scopeSettings(settings: SettingsManager, scope: ExtensionScope): { extensions: string[]; packages: PackageSource[] } {
...
function exactResourcePath(resource: ResolvedResource): string {
  ...
  const value = relative(base, resource.path).replaceAll("\\", "/");
  if (!value || value.startsWith("../") || isAbsolute(value)) throw new Error("extension path is outside its configured scope");
  return value;
}
...
export class PiExtensionManager {
  ...
  constructor(readonly cwd: string, readonly agentDir: string, settings?: SettingsManager, excludedPaths: string[] = []) {
  ...
  async list(runtime: LoadExtensionsResult | undefined, generation: number): Promise<ExtensionListSnapshot> {
    ...
    return {
      ...
      packages: this.packages.listConfiguredPackages()
  ...
  async setEnabled(id: string, enabled: boolean): Promise<void> {
    const resolved = await this.packages.resolve(async () => "skip");
    const resource = resolved.extensions.filter((entry) => !this.excluded(entry)).find((entry) => {
