...
import type {
  ...
  DelegatedAgentActivityReadModel,
  DelegatedAgentKind,
  DelegatedAgentRunReadModel,
  DelegatedAgentUsageReadModel,
...
import {
  ...
  validCompletionSessionId,
} from "../../shared/session-completions.ts";
...
const delegatedAgentKinds = new Set<DelegatedAgentKind>([
  "advisor",
  "grunt",
  "repo_scout",
  "web_scout",
...
export function encodeHistoryCursor(index: number): string {
  return Buffer.from(`h:${Math.max(0, Math.floor(index))}`).toString(
    "base64url",
  );
}
...
export function decodeHistoryCursor(cursor: string): number | undefined {
  try {
    ...
    if (
      !/^h:\d+$/.test(decoded) ||
      encodeHistoryCursor(Number(decoded.slice(2))) !== cursor
    )
      return undefined;
...
export function encodeTurnIndexCursor(index: number): string {
  return Buffer.from(`t:${Math.max(0, Math.floor(index))}`).toString(
    "base64url",
  );
}
...
export function decodeTurnIndexCursor(cursor: string): number | undefined {
  try {
    ...
    if (
      !/^t:\d+$/.test(decoded) ||
      encodeTurnIndexCursor(Number(decoded.slice(2))) !== cursor
    )
      return undefined;
    const index = Number(decoded.slice(2));
    return Number.isSafeInteger(index) ? index : undefined;
...
function imageAttachments(
...
): MessageAttachmentReadModel[] | undefined {
  ...
  const attachments = content.flatMap((part, index) => {
    ...
    const imageIndex = content
      .slice(0, index)
      .filter((item) => object(item).type === "image").length;
    return [
      {
...
function promptFileAttachments(
  value: unknown,
  sourceEntryId?: unknown,
): MessageAttachmentReadModel[] | undefined {
  const raw = object(value);
  ...
  if (
    raw.customType !== PROMPT_FILES_CUSTOM_TYPE ||
    details.version !== 2 ||
...
function compactionDisplay(
  ...
  const source = (item: unknown) => {
    ...
    return (
      ...
      record.sourceEntryId.length > 0 &&
      record.sourceEntryId.length <= MAX_COMPACTION_DISPLAY_SOURCE_ID &&
      typeof record.text === "string" &&
      record.text.length > 0 &&
      record.text.length <= MAX_COMPACTION_DISPLAY_TEXT
    );
  };
  ...
  const historyRecord = (item: unknown) => {
    ...
    return (
      ...
      record.path.length <= MAX_COMPACTION_DISPLAY_PATH &&
  ...
  if (
    ...
    !Array.isArray(raw.failedTools) ||
    !Array.isArray(raw.toolResults) ||
    raw.records.length + raw.failedTools.length + raw.toolResults.length >
      MAX_COMPACTION_DISPLAY_RECORDS ||
    !raw.records.every(record) ||
    !raw.failedTools.every(source) ||
    !raw.toolResults.every(source) ||
    !history ||
  ...
  const sourceValue = (item: unknown) => {
    const record = item as Record<string, unknown>;
    return {
      sourceEntryId: record.sourceEntryId as string,
  ...
  return {
    ...
    history: {
      read: history.read.map(historyValue),
      modified: history.modified.map(historyValue),
    },
  };
}
...
function compactionMessage(value: unknown): MessageReadModel["compaction"] {
  const raw = object(value);
  if (
    !Number.isSafeInteger(raw.contextAfterTokens) ||
    Number(raw.contextAfterTokens) < 0
  )
    return undefined;
...
export function continuityCompactionInterruptionId(
  value: unknown,
): string | undefined {
  const raw = object(value);
  if (
  ...
  for (let index = raw.diagnostics.length - 1; index >= 0; index--) {
    ...
    if (
      diagnostic.type === "pi-continuity-compaction-interruption" &&
      details.version === 1 &&
      typeof details.requestId === "string" &&
      details.requestId.length > 0 &&
      details.requestId.length <= 128
    )
...
function latestMessages(messages: MessageReadModel[]): MessageReadModel[] {
  ...
  const insertion = tail.findIndex(
    (message) => messages.indexOf(message) > userIndex,
  );
...
export function browserValue(value: unknown, depth = 0): unknown {
  ...
  if (Array.isArray(value))
    return value.slice(0, 50).map((item) => browserValue(item, depth + 1));
  if (typeof value === "object")
    return Object.fromEntries(
      Object.entries(value as Record<string, unknown>)
        ...
        .map(([key, item]) => [
          key.slice(0, 100),
          browserValue(item, depth + 1),
        ]),
    );
  return undefined;
}

function spawnToolKind(
  value: unknown,
): "spawn_agent" | "spawn_session" | undefined {
  return value === "spawn_agent" || value === "spawn_session"
    ? value
    : undefined;
}
...
function delegatedAgentKind(
  value: unknown,
  input?: unknown,
): DelegatedAgentKind | undefined {
  ...
  if (
    !spawnExecutionActions.has(String(action)) ||
    (action === "adopt" && kind !== "spawn_session")
  )
    return undefined;
  return kind;
}
...
function delegatedUsage(
  value: unknown,
): DelegatedAgentUsageReadModel | undefined {
  const raw = object(value);
  ...
  if (
    [input, output, cacheRead, cacheWrite, cost].some(
      (item) => item === undefined,
    )
  )
    return undefined;
...
function delegatedActivity(
  value: unknown,
): DelegatedAgentActivityReadModel[] | undefined {
  if (!Array.isArray(value)) return undefined;
  ...
  for (const rawValue of value) {
    ...
    if (activityText) {
      try {
        activityText =
          browserJson(JSON.parse(activityText))?.slice(
            0,
            MAX_AGENT_ACTIVITY_TEXT,
          ) ?? activityText;
      } catch {
        /* Non-JSON tool input remains bounded plain text. */
      }
      activityText = activityText
        .replace(/\bBearer\s+\S+/gi, "Bearer <redacted>")
        .replace(
          /\b((?:api[_-]?key|token|secret|password|cookie)\s*[:=]\s*)(?:bearer\s+)?[^\s,;]+/gi,
          "$1<redacted>",
        )
...
function delegatedRequest(input: unknown): string | undefined {
  ...
  return (
    text(raw.request ?? raw.task ?? raw.prompt, MAX_PAYLOAD_TEXT) || undefined
  );
}
...
function spawnMetadata(
  ...
  const threadId =
    marker.version === 1 && marker.kind === expectedKind
      ? delegatedId(marker.id)
      : undefined;
  const runId =
    details.background === true ? delegatedId(details.runId) : undefined;
  return {
    ...(validAction
      ? { action: action as "create" | "continue" | "adopt" }
      : {}),
    ...(threadId ? { threadId } : {}),
    ...(runId ? { runId } : {}),
  };
...
function validSpawnControlTarget(
  ...
  if (
    !previous ||
    kind !== previous.kind ||
    delegatedId(details.runId) !== previous.runId
  )
    return false;
...
function delegatedEndStatus(
  ...
  const failed =
    ...
    (spawned &&
      ...
      !["completed", "running"].includes(details.status));
  if (failed) return "failed";
  if (!spawned || details.status !== "running") return "completed";
  const metadata = spawnMetadata(kind, undefined, details);
  const startedAt =
    typeof details.startedAt === "string" &&
    !Number.isNaN(Date.parse(details.startedAt));
  return details.background === true &&
    metadata.runId &&
    metadata.threadId &&
    startedAt
    ? "running"
...
function updateDelegatedRun(
  ...
  const activity =
    fullActivity && fullActivity.length >= previousActivity.length
      ? fullActivity
      : activityDelta?.length
        ? [...previousActivity, ...activityDelta]
        : previousActivity;
  const nextStatus =
  ...
  const partialResponse =
    status === "running" ? text(details.partialResponse) : "";
  const response =
    status === "running"
      ? partialResponse || previous?.response
      : messageText(raw);
  ...
  const startedAt =
    ...
    !Number.isNaN(Date.parse(details.startedAt))
      ...
      : undefined;
  const modelName =
    text(details.advisorModel ?? details.model, 200) || undefined;
...
function mergeDelegatedRun(
  ...
  const activity =
    live.activity.length >= transcript.activity.length
      ? live.activity
      : transcript.activity;
  const usage = terminal.usage ?? live.usage ?? transcript.usage;
  const sessionUsage =
    terminal.sessionUsage ?? live.sessionUsage ?? transcript.sessionUsage;
  const durationMs =
    terminal.durationMs ?? live.durationMs ?? transcript.durationMs;
  return {
    id: transcript.id,
    ...
    ...((live.agentName ?? transcript.agentName)
      ? { agentName: live.agentName ?? transcript.agentName }
      : {}),
    ...((live.startedAt ?? transcript.startedAt)
      ? { startedAt: live.startedAt ?? transcript.startedAt }
      : {}),
    ...((live.request ?? transcript.request)
      ? { request: live.request ?? transcript.request }
      : {}),
    ...((terminal.response ?? live.response ?? transcript.response)
      ? { response: terminal.response ?? live.response ?? transcript.response }
      : {}),
    ...((live.modelName ?? transcript.modelName)
      ? { modelName: live.modelName ?? transcript.modelName }
      : {}),
    ...((live.thinkingLevel ?? transcript.thinkingLevel)
      ? { thinkingLevel: live.thinkingLevel ?? transcript.thinkingLevel }
      : {}),
    ...((live.runId ?? transcript.runId)
      ? { runId: live.runId ?? transcript.runId }
      : {}),
    ...((live.threadId ?? transcript.threadId)
      ? { threadId: live.threadId ?? transcript.threadId }
      : {}),
    ...((live.action ?? transcript.action)
      ? { action: live.action ?? transcript.action }
      : {}),
    ...(durationMs === undefined ? {} : { durationMs }),
    ...(usage ? { usage: { ...usage } } : {}),
...
export function mergeDelegatedRuns(
  ...
  for (const run of live) {
    ...
    runs.set(
      run.id,
      persisted ? mergeDelegatedRun(persisted, run) : structuredClone(run),
    );
  }
...
export function browserJson(value: unknown): string | undefined {
  ...
  try {
    return JSON.stringify(browserValue(value), null, 2).slice(0, MAX_TEXT);
  } catch {
    return "[unserializable input]";
  }
}

export function latestVisibleUserIndex(
  messages: unknown[],
  end = messages.length,
): number | undefined {
  for (
    let index = Math.min(messages.length, Math.max(0, Math.floor(end))) - 1;
    index >= 0;
    index--
  ) {
...
/** Mutable bookkeeping carried across one pass over conversation history. */
interface ConversationScan {
  includeDelegated: boolean;
  toolCalls: Map<string, { name: string; rawInput?: unknown; turn: number }>;
  delegatedRuns: Map<string, DelegatedAgentRunReadModel>;
  spawnRunIds: Map<string, string>;
}
...
function collectToolStatuses(
  ...
  for (const message of messages) {
    ...
    if (raw.isError !== true && statuses.get(raw.toolCallId) === "failed")
      continue;
...
/** Records an assistant turn's tool calls, returning placeholders for calls with no result. */
function scanAssistantToolCalls(
  ...
  for (const [partIndex, part] of content.entries()) {
    ...
    if (scan.includeDelegated && kind) {
      ...
      trimMap(scan.delegatedRuns, MAX_DELEGATED_RUNS);
    }

    if (!context.projected || context.projectedToolResultIds.has(item.id))
      continue;
    const fallbackId = `history-${context.index}-tool-${partIndex}`;
    const durationMs = context.toolDurations?.get(item.id);
    unmatched.push({
      id: fallbackId,
      role: "tool",
      text: "",
      streaming: false,
      tool: {
        id: id(item.id, fallbackId),
        name,
        input: browserJson(item.arguments),
        status: context.completedToolStatuses.get(item.id) ?? "running",
        ...(durationMs === undefined ? {} : { durationMs }),
      },
    });
  }
...
/** Folds a delegated agent's tool result into its run. */
function recordDelegatedResult(
  scan: ConversationScan,
  raw: Record<string, unknown>,
  toolId: string,
  kind: DelegatedAgentKind,
  ...
  if (!scan.includeDelegated) return;
  const details = object(raw.details);
  const run = updateDelegatedRun(
    scan.delegatedRuns.get(toolId),
    kind,
    toolId,
    turn,
    rawInput,
  ...
  scan.delegatedRuns.set(toolId, run);
  if (run.runId) scan.spawnRunIds.set(run.runId, toolId);
  trimMap(scan.delegatedRuns, MAX_DELEGATED_RUNS);
}

/** Applies a status/cancel call onto the run it targets, never reviving a settled one. */
function recordSpawnControl(
  scan: ConversationScan,
  raw: Record<string, unknown>,
  controlKind: DelegatedAgentKind,
): void {
  if (!scan.includeDelegated) return;
  const details = object(raw.details);
  const canonicalId = scan.spawnRunIds.get(delegatedId(details.runId) ?? "");
  const previous = canonicalId
    ? scan.delegatedRuns.get(canonicalId)
    : undefined;
  ...
  if (previous.status !== "running" && status !== previous.status) return;
  scan.delegatedRuns.set(
    canonicalId,
    updateDelegatedRun(
      previous,
...
export function projectConversation(
  ...
  const start = Math.min(
    ...
    Math.max(
      0,
      Math.floor(options.start ?? Math.max(0, messages.length - MAX_MESSAGES)),
    ),
  );
  const end = Math.min(
    messages.length,
    Math.max(start, Math.floor(options.end ?? messages.length)),
  );
  const includeDelegated = options.includeDelegated !== false;
  const pinnedUserIndex = latestVisibleUserIndex(messages, end) ?? -1;
  const completedToolStatuses = collectToolStatuses(messages);
  const projectedToolResultIds = new Set(
    messages.slice(0, end).flatMap((message) => {
      const raw = object(message);
      return role(raw.role) === "tool" && typeof raw.toolCallId === "string"
        ? [raw.toolCallId]
        : [];
    }),
  );
  const scan: ConversationScan = {
    includeDelegated,
    toolCalls: new Map(),
    delegatedRuns: new Map(),
    spawnRunIds: new Map(),
  };
  const { toolCalls, delegatedRuns } = scan;
  ...
  for (let index = 0; index < end; index++) {
    ...
    if (fileCount) {
      if (latestProjectedUser) {
        ...
        if (attachments)
          latestProjectedUser.attachments = [
            ...(latestProjectedUser.attachments ?? []),
            ...attachments,
          ];
      }
    ...
    const unmatchedCalls =
      messageRole === "assistant" && Array.isArray(raw.content)
        ? scanAssistantToolCalls(scan, raw.content, {
            index,
            userTurn,
            projected: index >= start,
            projectedToolResultIds,
            completedToolStatuses,
            toolDurations: options.toolDurations,
          })
        : [];
    if (messageRole === "tool") {
      ...
      const control =
        controlKind &&
        spawnControlActions.has(String(spawnAction(call?.rawInput)));
      if (kind)
      ...
      projectedMessages.push({
        ...
        ...(typeof raw.entryId === "string"
          ? { entryId: id(raw.entryId, fallbackId) }
          : {}),
        role: "tool",
        text: messageText(raw),
        streaming: false,
        ...(createdAt(raw.timestamp ?? raw.createdAt)
          ? { createdAt: createdAt(raw.timestamp ?? raw.createdAt) }
          : {}),
        tool: {
          id: toolId,
          name,
    ...
    const result: MessageReadModel = {
      id: `history-${index}`,
      ...(typeof raw.entryId === "string"
        ? { entryId: id(raw.entryId, `history-${index}`) }
        : {}),
      role: messageRole,
      text: messageText(raw),
      streaming: false,
      ...(createdAt(raw.timestamp ?? raw.createdAt)
        ? { createdAt: createdAt(raw.timestamp ?? raw.createdAt) }
        : {}),
      ...(messageRole === "user" && raw.canUndo === true
        ? { canUndo: true }
        : {}),
      ...(images ? { attachmentCount: images } : {}),
      ...(attachments ? { attachments } : {}),
      ...(messageRole === "system" && typeof raw.customType === "string"
        ? { systemSource: text(raw.customType, 200) }
        : {}),
...
export function projectConversationTurnIndex(
  messages: unknown[],
  ...
  for (let index = 0; index < messages.length; index++) {
    ...
    turns.push({
      ...
      preview: (
        text ||
        (images
          ? `${images} attached image${images === 1 ? "" : "s"}`
          : "Empty prompt")
      ).slice(0, 120),
      ...(createdAt(raw.timestamp ?? raw.createdAt)
        ? { createdAt: createdAt(raw.timestamp ?? raw.createdAt) }
...
export class RuntimeProjection {
  ...
  private readonly delegatedRuns = new Map<
    string,
    DelegatedAgentRunReadModel
  >();
  ...
  constructor(
    ...
    for (const message of runtime.conversation.messages)
      this.messages.set(message.id, { ...message });
    for (const tool of runtime.conversation.tools)
  ...
  snapshot(): RuntimeSnapshot {
    return {
      ...
      conversation: {
        ...
        messages: [...this.messages.values()].map((message) => ({
          ...message,
        })),
        ...
        delegatedRuns: [...this.delegatedRuns.values()]
          .slice(-MAX_DELEGATED_RUNS)
          .map((run) => structuredClone(run)),
      },
      operational: cloneOperational(this.runtime.operational),
      ...(this.runtime.providerAuth
        ? { providerAuth: structuredClone(this.runtime.providerAuth) }
        : {}),
      extensionUi: {
        ...this.runtime.extensionUi,
        notifications: this.runtime.extensionUi.notifications.map((item) => ({
          ...item,
        })),
        statuses: this.runtime.extensionUi.statuses.map((item) => ({
          ...item,
        })),
        widgets: this.runtime.extensionUi.widgets.map((item) => ({
          ...item,
          lines: [...item.lines],
        })),
      },
  ...
  unseenCompletionSessionIds(): string[] {
    return [...this.unseenCompletions];
  }

  refresh(runtime: RuntimeSnapshot): void {
    const finalAssistant = [...runtime.conversation.messages]
      .reverse()
    ...
    this.runtime = {
      ...
      providerAuth: runtime.providerAuth
        ? structuredClone(runtime.providerAuth)
        : undefined,
      runtimePolicy: structuredClone(runtime.runtimePolicy),
      metrics: { ...runtime.metrics },
      discoverIndex: runtime.discoverIndex
        ? { ...runtime.discoverIndex }
        : undefined,
      conversation: {
        ...this.runtime.conversation,
  ...
  apply(event: DriverEvent): void {
    ...
    if (event.type === "session.status") {
      ...
      if (
        event.completed &&
        validCompletionSessionId(sessionId) &&
        ...
        while (this.unseenCompletions.size > MAX_UNSEEN_COMPLETIONS) {
          this.unseenCompletions.delete(
            this.unseenCompletions.values().next().value!,
          );
        }
      }
      this.publish("session.status", {
        sessionId,
        state: event.state,
        ...(Object.hasOwn(event, "workStartedAt")
          ? { workStartedAt: event.workStartedAt }
          : {}),
    ...
    if (event.type === "command.result") {
      this.runtime.commandResult = event.result
    ...
    const kind = String(raw.type ?? raw.event ?? raw.kind ?? "").replace(
      /-/g,
      "_",
    );
    if (kind === "message_start" || kind === "message_starting")
      return this.messageStart(raw);
    if (kind === "message_update" || kind === "message_delta")
      return this.messageUpdate(raw);
    if (kind === "message_end" || kind === "message_complete")
      return this.messageEnd(raw);
    if (kind === "continuity_compaction_interruption")
      return this.removeActiveMessage(raw);
    if (
      kind === "tool_execution_start" ||
      kind === "tool_start" ||
      kind === "tool_call_start"
    )
      return this.toolStart(raw);
    if (
    ...
    if (
      ...
      return this.retry(raw, kind);
    if (
      kind === "compaction" ||
      kind === "compaction_start" ||
      kind === "compaction_end"
    )
      return this.compaction(raw, kind);
    if (kind === "metrics" || kind === "session_stats" || kind === "usage")
      return this.metrics(object(raw.metrics ?? raw.stats ?? raw));
    if (kind === "worktree_summary") return this.worktreeSummary(raw);
    if (kind === "discover_index") return this.discoverIndex(raw);
    if (kind === "session_info_changed") return this.sessionInfoChanged(raw);
    if (kind === "session_controls_error" || kind === "runtime_error")
      return this.runtimeError(raw);
    if (kind === "prompt_undo") return this.promptUndo(raw);
    if (kind === "agent_start") return this.agentStart(raw);
    if (kind === "agent_end" || kind === "agent_error")
      return this.agentEnd(raw, kind);
  }

  /** Routes one extension UI request or surface update onto the read model. */
  private applyUiEvent(raw: Record<string, unknown>): void {
    const method = String(raw.method);
    const payload = object(raw.payload);
    if (
      raw.kind === "request" &&
      ["select", "confirm", "input", "editor", "questionnaire"].includes(method)
    ) {
      this.pendingUi = {
        ...
        ...(typeof raw.timeoutSeconds === "number"
          ? {
              timeoutSeconds: Math.min(
                86_400,
                Math.max(1, Math.round(raw.timeoutSeconds)),
              ),
            }
          : {}),
      ...
      this.runtime.extensionUi.notifications = [
        ...this.runtime.extensionUi.notifications,
        item,
      ].slice(-10);
      ...
      const item =
        typeof payload.text === "string"
          ? ({ key, text: text(payload.text, 500) } satisfies UiStatusReadModel)
          : undefined;
      this.runtime.extensionUi.statuses = replaceKey(
        this.runtime.extensionUi.statuses,
        key,
        item,
        25,
      );
      ...
      const placement =
        ...
        payload.placement === "belowEditor"
          ...
          : undefined;
      const item = lines
        ? ({ key, lines, placement } satisfies UiWidgetReadModel)
        : undefined;
      this.runtime.extensionUi.widgets = replaceKey(
        this.runtime.extensionUi.widgets,
        key,
        item,
        10,
      );
      ...
      this.publish("ui.editor-text", {
        text: this.runtime.extensionUi.editorText,
        revision: this.runtime.extensionUi.editorRevision,
      });
    }
  }

  /** Mirrors pi-discover's index state, ignoring unknown states. */
  private discoverIndex(raw: Record<string, unknown>): void {
    const value = object(raw.value);
    this.runtime.discoverIndex = ["idle", "indexing", "error"].includes(
      String(value.state),
    )
  ...
  private sessionInfoChanged(raw: Record<string, unknown>): void {
    this.runtime.sessionName = text(raw.name, 200) || undefined;
    this.publish("session.info", {
      sessionId: this.runtime.sessionId,
      name: this.runtime.sessionName,
    });
  }

  private runtimeError(raw: Record<string, unknown>): void {
    this.publish("runtime.error", {
      message:
        text(raw.message, 1_000) || "Could not apply the queued model change",
    });
  }

  /** Re-marks which user messages the runtime still allows undoing or forking. */
  private promptUndo(raw: Record<string, unknown>): void {
  ...
  private agentStart(raw: Record<string, unknown>): void {
    this.latestAssistantMessageId = undefined;
    if (raw.metrics) this.metrics(object(raw.metrics));
    const startedAt =
      this.runtime.conversation.workStartedAt ??
      (typeof raw.workStartedAt === "string" &&
      !Number.isNaN(Date.parse(raw.workStartedAt))
        ? raw.workStartedAt
        : new Date().toISOString());
    this.runtime.conversation.workStartedAt = startedAt;
    this.runtime.conversation.workModelName =
      text(raw.modelName, 200) || undefined;
    this.runtime.conversation.workThinkingLevel = thinkingLevel(
      raw.thinkingLevel,
    );
  ...
  /** Settles the finished turn: final assistant message, timing, and stop/error state. */
  private agentEnd(raw: Record<string, unknown>, kind: string): void {
    if (kind === "agent_end" && raw.metrics) this.metrics(object(raw.metrics));
    if (kind === "agent_end") {
      this.runtime.gitBranch =
        typeof raw.gitBranch === "string"
          ? text(raw.gitBranch, 200) || undefined
          : undefined;
    }
    const durationMs = Number.isSafeInteger(raw.workDurationMs)
      ? Math.min(
          7 * 24 * 60 * 60 * 1_000,
          Math.max(0, raw.workDurationMs as number),
        )
      : undefined;
    const willRetry = raw.willRetry === true && raw.stopped !== true;
    if (!willRetry && kind === "agent_end") this.flush();
    const assistant =
      !willRetry && kind === "agent_end"
    ...
    this.settleRunningWork(
      terminalActivityStatus(kind === "agent_error" ? "error" : "end", {
        stopped,
        willRetry,
      }),
    );
    this.runtime.conversation.stopping = false;
    const agentError = willRetry
      ? undefined
      : text(raw.errorMessage, 1_000) || undefined;
    this.runtime.conversation.agentError = agentError;
    this.runtime.conversation.stoppedRun =
      stopped && durationMs !== undefined
        ? {
            turnId,
    ...
    this.publish(`agent.${kind.slice(6)}`, {
      ...
      assistantMessage:
        !willRetry && kind === "agent_end" && assistant
  ...
  private settleRunningWork(status: "completed" | "failed"): void {
    ...
    this.delegatedRuns.clear();
    for (const run of settled.delegatedRuns)
      this.delegatedRuns.set(run.id, run);
  }

  private reconcileFinalAssistant(
    raw: Record<string, unknown>,
  ...
  private reconcileAssistant(canonical: MessageReadModel): MessageReadModel {
    ...
    if (!current && canonical.entryId) {
      current = [...this.messages.values()].find(
        (message) => message.entryId === canonical.entryId,
      );
    }
    if (!current && this.latestAssistantMessageId) {
      const live = this.messages.get(this.latestAssistantMessageId);
      if (live?.role === "assistant" && live.text === canonical.text)
        current = live;
    }
    ...
    if (missing)
      this.publish("message.start", { ...reconciled, streaming: true });
    if (missing || unfinished) {
      this.publish("message.end", {
        id: reconciled.id,
  ...
  private messageStart(raw: Record<string, unknown>): void {
    ...
    if (fileCount) {
      const user = [...this.messages.values()]
        .reverse()
        .find((item) => item.role === "user");
      if (user) {
        user.fileAttachmentCount = fileCount;
        const attachments = promptFileAttachments(
          message,
          raw.entryId ?? message.entryId,
        );
        if (attachments)
    ...
    const messageId =
      correlatedId ||
      id(
        raw.messageId ?? raw.id ?? message.id,
    ...
    const item: MessageReadModel = {
      ...
      streaming: true,
      createdAt:
        createdAt(message.timestamp ?? raw.timestamp) ??
        new Date().toISOString(),
      ...(attachmentCount(message)
        ? { attachmentCount: attachmentCount(message) }
        : {}),
      ...(imageAttachments(message, raw.entryId ?? message.entryId)
        ? {
      ...
      ...(typeof message.customType === "string"
        ? { systemSource: text(message.customType, 200) }
        : {}),
    };
    if (item.role === "tool") {
      ...
      item.tool = {
        id: toolId,
        name:
          text(message.toolName ?? raw.toolName, 200) ||
          activity?.name ||
          "Tool",
        input: activity?.input,
        status:
          message.isError === true || raw.isError === true
            ? "failed"
  ...
  private messageEnd(raw: Record<string, unknown>): void {
    ...
    const messageId =
      this.activeMessageId ??
      id(raw.messageId ?? raw.id ?? object(raw.message).id, "message");
    const current = this.messages.get(messageId);
    if (current) {
      ...
      if (typeof entryId === "string")
        current.entryId = id(entryId, current.id);
  ...
  private removeActiveMessage(raw: Record<string, unknown>): void {
    ...
    if (this.latestAssistantMessageId === messageId)
      this.latestAssistantMessageId = undefined;
    this.activeMessageId = undefined;
    this.runtime.conversation.streaming = false;
  ...
  private spawnProgress(raw: Record<string, unknown>): void {
    ...
    const run = projectDelegatedToolEvent(
      ...
      previous,
      event,
      this.runtime.metrics.userMessages,
    );
    if (!run) return;
    const appendsActivity =
      raw.phase === "update" &&
      Array.isArray(object(object(raw.result).details).activityDelta);
  ...
  private toolStart(raw: Record<string, unknown>): void {
    this.flush();
    const toolId = id(
      raw.toolCallId ?? raw.toolId ?? raw.id,
      `tool-${this.tools.size + 1}`,
    );
  ...
  private toolUpdate(raw: Record<string, unknown>): void {
    ...
    const next = projectDelegatedToolEvent(
      "update",
      toolId,
      old,
      raw,
    ...
    const appendsActivity = Array.isArray(
      object(object(raw.partialResult).details).activityDelta,
    );
  ...
  private toolEnd(raw: Record<string, unknown>): void {
    this.flush();
    const toolId = id(raw.toolCallId ?? raw.toolId ?? raw.id, "tool");
    const old = this.tools.get(toolId);
    ...
    const projectedDuration = boundedNumber(
      raw.durationMs,
      7 * 24 * 60 * 60 * 1_000,
    );
    const durationMs =
    ...
    const item: ToolActivityReadModel = {
      ...
      status:
        raw.isError === true || raw.failed === true || raw.error
          ? "failed"
          : "completed",
    ...
    const controlKind = spawnToolKind(name);
    if (!controlKind || !spawnControlActions.has(String(spawnAction(input))))
      return;
    ...
    const target = canonicalId
      ? this.delegatedRuns.get(canonicalId)
      : undefined;
    if (!canonicalId || !validSpawnControlTarget(target, controlKind, details))
      return;
    const status = delegatedEndStatus(raw, controlKind, details);
    if (target.status !== "running" && status !== target.status) return;
    const next = updateDelegatedRun(
      target,
      controlKind,
      canonicalId,
      target.turn,
  ...
  private setDelegatedRun(
    run: DelegatedAgentRunReadModel,
    previous?: DelegatedAgentRunReadModel,
    appendsActivity = false,
  ): void {
    this.delegatedRuns.set(run.id, run);
    trimMap(this.delegatedRuns, MAX_DELEGATED_RUNS);
    for (const [runId, id] of this.spawnRunIds)
      if (!this.delegatedRuns.has(id)) this.spawnRunIds.delete(runId);
    const publishAppend =
      previous &&
      (appendsActivity ||
        run.activity === previous.activity ||
        (run.activity.length >= previous.activity.length &&
          isDeepStrictEqual(
            run.activity.slice(0, previous.activity.length),
            previous.activity,
          )));
    this.publish(
      "delegate.update",
      structuredClone(
        publishAppend
          ? {
              ...run,
              activity: run.activity.slice(previous.activity.length),
              activityMode: "append",
              activityBase: previous.activity.length,
            }
          : run,
      ),
    );
  }
  private queue(raw: Record<string, unknown>): void {
    this.runtime.conversation.queue = {
      steering: Array.isArray(raw.steering)
        ? raw.steering.length
        : Number.isSafeInteger(raw.steering)
          ? Math.max(0, raw.steering as number)
          : 0,
      followUp: Array.isArray(raw.followUp)
        ? raw.followUp.length
        : Number.isSafeInteger(raw.followUp)
          ? Math.max(0, raw.followUp as number)
          : 0,
    };
    this.publish("queue.update", this.runtime.conversation.queue);
  }
  private retry(raw: Record<string, unknown>, kind: string): void {
    this.runtime.conversation.retry = {
      active:
        kind !== "retry_end" &&
        kind !== "auto_retry_end" &&
        raw.active !== false,
      ...
      maxAttempts:
        typeof raw.maxAttempts === "number" ? raw.maxAttempts : undefined,
      message:
        text(raw.message ?? raw.errorMessage ?? raw.finalError, 1_000) ||
        undefined,
    };
    this.publish("retry.update", this.runtime.conversation.retry);
  }
  private compaction(raw: Record<string, unknown>, kind: string): void {
    ...
    if (
      kind === "compaction_end" &&
      metadata &&
      completed.role === "system" &&
      typeof completed.id === "string" &&
      typeof completed.entryId === "string"
    ) {
      const existing = [...this.messages.values()].find(
        (message) => message.entryId === completed.entryId,
      );
      item = {
        ...
        ...(createdAt(completed.createdAt)
          ? { createdAt: createdAt(completed.createdAt) }
          : {}),
  ...
  private metrics(raw: Record<string, unknown>): void {
    ...
    if (Array.isArray(raw.toolUsage)) {
      ...
      current.toolUsage = raw.toolUsage.slice(0, 200).flatMap((item) => {
        ...
        const name =
          typeof usage.name === "string" ? usage.name.slice(0, 200) : "";
        if (
          !name ||
          names.has(name) ||
          !Number.isSafeInteger(usage.calls) ||
    ...
    if (typeof raw.provider === "string")
      current.provider = raw.provider.slice(0, 200);
    this.publish("metrics.update", this.runtime.metrics);
  }
  private worktreeSummary(raw: Record<string, unknown>): void {
    ...
    if (!message || message.role !== "assistant" || !Array.isArray(raw.files))
      return;
  ...
  flush(): void {
    if (this.updateTimer) clearTimeout(this.updateTimer);
    this.updateTimer = undefined;
    if (this.pendingUpdate) this.publish("message.update", this.pendingUpdate);
    this.pendingUpdate = undefined;
    this.updateBytes = 0;
  }
}
