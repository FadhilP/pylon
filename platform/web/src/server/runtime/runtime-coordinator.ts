...
import { PiExtensionManager } from "../packages/pi-extension-manager.ts";
import { createPylonModelRuntime } from "./runtime-factory.ts";
import { createOsStateQLCredentialVault, type StateQLCredentialVault } from "../database/stateql-credential-vault.ts";
import type {
  DeleteSessionInput,
  CheckoutBranchInput,
...
import type { UiRequest, UiResponse } from "./remote-ui-bridge.ts";
...
interface RuntimeSlot {
  ...
  workspaceRefreshRequest?: number;
  workspaceRefreshPublication?: { sessionId: string; generation: number };
...
export class RuntimeCoordinator implements PiDriver {
  ...
  async workspaceEntry(path: string, moveDestination?: string, includeGitIndex = true): Promise<WorkspaceEntry> {
    const slot = this.selected();
    const generation = this.generation;
    const entry = await readWorkspaceEntry(slot.driver.runtimeDetails().cwd, path, moveDestination, includeGitIndex);
  ...
  async workspaceGitIndex(path: string): Promise<WorkspaceGitIndex> {
    const slot = this.selected();
    const generation = this.generation;
    const text = await readGitIndexText(slot.driver.runtimeDetails().cwd, path);
    this.assertSelected(slot, generation, "reading the Git index");
  ...
  async mutateWorkspace(input: WorkspaceMutationInput): Promise<WorkspaceMutationResult | void> {
    return this.withLifecycle(async () => {
      ...
      if (slot.id !== input.sessionId) throw new Error("workspace belongs to a different session");
      try {
        return await mutateWorkspace(slot.driver.runtimeDetails().cwd, input.mutation);
      } finally {
  ...
  private refreshWorkspace(slot: RuntimeSlot, publish: boolean): Promise<void> {
    slot.workspaceRefreshRequest = (slot.workspaceRefreshRequest ?? 0) + 1;
