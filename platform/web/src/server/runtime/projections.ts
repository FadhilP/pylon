...
import { settleRunningActivities, terminalActivityStatus } from "../../shared/sessions/transcript.ts";
import { MAX_UNSEEN_COMPLETIONS, validCompletionSessionId } from "../../shared/sessions/session-completions.ts";
  ...
  const historyRecord = (item: unknown) => {
    ...
    return (
      ...
      (record.sourceEntryId === undefined ||
        (typeof record.sourceEntryId === "string" && record.sourceEntryId.length <= MAX_COMPACTION_DISPLAY_SOURCE_ID))
    );
  };
...
function delegatedActivity(value: unknown): DelegatedAgentActivityReadModel[] | undefined {
  ...
  for (const rawValue of value) {
    ...
    if (activityText) {
      try {
        activityText = browserJson(JSON.parse(activityText))?.slice(0, MAX_AGENT_ACTIVITY_TEXT) ?? activityText;
      } catch {
...
function updateDelegatedRun(
  ...
  const startedAt =
    typeof details.startedAt === "string" && !Number.isNaN(Date.parse(details.startedAt))
      ? details.startedAt
      : undefined;
...
function scanAssistantToolCalls(
  ...
  for (const [partIndex, part] of content.entries()) {
    ...
    scan.toolCalls.set(item.id, { name, rawInput: item.arguments, turn: context.userTurn });

    const kind = delegatedAgentKind(name, item.arguments);
    if (scan.includeDelegated && kind) {
      scan.delegatedRuns.set(
        item.id,
        updateDelegatedRun(undefined, kind, item.id, context.userTurn, item.arguments, undefined, "running"),
      );
      trimMap(scan.delegatedRuns, MAX_DELEGATED_RUNS);
    }

    if (!context.projected || context.projectedToolResultIds.has(item.id)) continue;
...
function recordSpawnControl(
  ...
  const previous = canonicalId ? scan.delegatedRuns.get(canonicalId) : undefined;
  if (!canonicalId || !validSpawnControlTarget(previous, controlKind, details)) return;
  const status = delegatedEndStatus(raw, controlKind, details);
  if (previous.status !== "running" && status !== previous.status) return;
  scan.delegatedRuns.set(
    canonicalId,
    updateDelegatedRun(previous, controlKind, canonicalId, previous.turn, undefined, raw, status),
  );
}
...
export function projectConversation(
  ...
  const start = Math.min(
    messages.length,
    Math.max(0, Math.floor(options.start ?? Math.max(0, messages.length - MAX_MESSAGES))),
  );
...
export class RuntimeProjection {
  ...
  refresh(runtime: RuntimeSnapshot): void {
      ...
      const next = previous
        ? {
            ...
            activity: structuredClone(
              run.activity.length > previous.activity.length ? run.activity : previous.activity,
            ),
          }
        : structuredClone(run);
    ...
    this.runtime = {
      ...
      conversation: {
        ...
        stoppedRun: runtime.conversation.stoppedRun ? { ...runtime.conversation.stoppedRun } : undefined,
  ...
  apply(event: DriverEvent): void {
    ...
    if (kind === "tool_execution_start" || kind === "tool_start" || kind === "tool_call_start")
      return this.toolStart(raw);
    if (kind === "tool_execution_end" || kind === "tool_end" || kind === "tool_call_end" || kind === "tool_result")
    ...
    if (kind === "compaction" || kind === "compaction_start" || kind === "compaction_end")
    ...
    if (kind === "session_controls_error" || kind === "runtime_error") return this.runtimeError(raw);
    if (kind === "prompt_undo") return this.promptUndo(raw);
    if (kind === "agent_start") return this.agentStart(raw);
    if (kind === "agent_end" || kind === "agent_error") return this.agentEnd(raw, kind);
  }
    ...
    if (raw.kind === "request" && ["select", "confirm", "input", "editor", "questionnaire"].includes(method)) {
      this.pendingUi = {
        ...
        ...(typeof raw.timeoutSeconds === "number"
          ? { timeoutSeconds: Math.min(86_400, Math.max(1, Math.round(raw.timeoutSeconds))) }
          : {}),
      ...
      const placement =
        payload.placement === "aboveEditor" || payload.placement === "belowEditor" ? payload.placement : undefined;
  ...
  private promptUndo(raw: Record<string, unknown>): void {
    ...
    for (const message of this.messages.values()) {
      ...
      items.push({ id: message.id, canUndo: message.canUndo, canForkWithTimeline: message.canForkWithTimeline });
    }
  ...
  private agentStart(raw: Record<string, unknown>): void {
    ...
    const startedAt =
      ...
      (typeof raw.workStartedAt === "string" && !Number.isNaN(Date.parse(raw.workStartedAt))
        ? raw.workStartedAt
  ...
  private agentEnd(raw: Record<string, unknown>, kind: string): void {
    ...
    this.settleRunningWork(terminalActivityStatus(kind === "agent_error" ? "error" : "end", { stopped, willRetry }));
    this.runtime.conversation.stopping = false;
  ...
  private messageStart(raw: Record<string, unknown>): void {
    ...
    const correlatedId = typeof raw.clientMessageId === "string" ? id(`pending-${raw.clientMessageId}`, "") : "";
    ...
    const item: MessageReadModel = {
      ...
      ...(imageAttachments(message, raw.entryId ?? message.entryId)
        ? { attachments: imageAttachments(message, raw.entryId ?? message.entryId) }
        : {}),
  ...
  private toolStart(raw: Record<string, unknown>): void {
    this.flush();
    const toolId = id(raw.toolCallId ?? raw.toolId ?? raw.id, `tool-${this.tools.size + 1}`);
  ...
  private retry(raw: Record<string, unknown>, kind: string): void {
    this.runtime.conversation.retry = {
      active: kind !== "retry_end" && kind !== "auto_retry_end" && raw.active !== false,
      ...
      message: text(raw.message ?? raw.errorMessage ?? raw.finalError, 1_000) || undefined,
    };
  ...
  private compaction(raw: Record<string, unknown>, kind: string): void {
    this.runtime.conversation.compaction = {
      ...
      reason:
        raw.reason === "manual" || raw.reason === "threshold" || raw.reason === "overflow" ? raw.reason : undefined,
    };
    ...
    this.publish(
      "compaction.update",
      item ? { ...this.runtime.conversation.compaction, completedMessage: item } : this.runtime.conversation.compaction,
    );
  }
    ...
    if (Array.isArray(raw.toolUsage)) {
      ...
      current.toolUsage = raw.toolUsage.slice(0, 200).flatMap(item => {
        ...
        if (
          ...
          Number(usage.tokens) !== Number(usage.inputTokens) + Number(usage.outputTokens)
        )
          return [];
        names.add(name);
        return [
