...
import { cloneHookSettings } from "../settings/hook-settings.ts";
  ...
  readonly extension: InlineExtension = {
    ...
    factory: pi => {
      const unsubscribe = pi.events.on("pylon:spawn-hooks-request", value => {
        ...
        request.provide({
          ...(sessionStart
            ? { sessionStart: { customType: SESSION_START_HOOK_CUSTOM_TYPE, content: sessionStart } }
            : {}),
          ...(sessionCompact
            ? { sessionCompact: { customType: SESSION_START_HOOK_CUSTOM_TYPE, content: sessionCompact } }
            : {}),
      ...
      pi.on("session_compact", (event, ctx) => {
        ...
        if (
          !compactionEntryId ||
          !content ||
          branchHasCompactionReinjection(ctx.sessionManager.getBranch(), compactionEntryId)
        )
          return;
