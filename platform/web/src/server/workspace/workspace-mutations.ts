...
async function windowsFileOperation(script: string, source: string, destination: string): Promise<void> {
  await run(
    "powershell.exe",
    ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command", `$ErrorActionPreference = 'Stop'; ${script}`],
    {
...
async function context(cwd: string): Promise<Context> {
  ...
  if (await exists(join(root, ".gitmodules"))) {
    ...
    const result = await run(
      "git",
      ["config", "--file", join(root, ".gitmodules"), "--get-regexp", "^submodule\\..*\\.path$"],
...
async function confined(ctx: Context, path: string, missingLeaf = false): Promise<string> {
  ...
  for (let index = 0; index < parts.length; index++) {
    ...
    if (resolve(await realpath(current)) !== resolve(current))
      throw new Error("Workspace path changed or escapes its folder.");
  }
  return current;
}
...
async function inspect(
  ...
  path: string,
  retainBytes = false,
): Promise<{ version: string; moveFingerprint: string; items: Item[] }> {
  const items: Item[] = [];
  let bytes = 0;
  ...
  const visit = async (path: string, depth: number): Promise<void> => {
    if (items.length >= MAX_ENTRIES || depth > 64 || Date.now() > deadline)
      throw new Error(
        "Operation exceeds the safe folder inspection limit (2,000 entries / 64 MiB). Use local filesystem tools.",
      );
    const absolute = await confined(ctx, path);
    const stat = await lstat(absolute);
    const hash = createHash("sha256").update(
      JSON.stringify([ctx.root, path, stat.dev, stat.ino, stat.mode, stat.uid, stat.gid]),
    );
    ...
    if (item.kind === "directory") {
      ...
      bytes += stat.size;
      if (bytes > MAX_BYTES)
        throw new Error("Operation exceeds the safe inspection limit (64 MiB). Use local filesystem tools.");
      const handle = await open(absolute, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
      try {
        const before = await handle.stat();
        if (!before.isFile() || before.ino !== stat.ino || before.dev !== stat.dev)
          throw new Error("File changed while opening it.");
  ...
  return {
    items,
    moveFingerprint: `v1:${fingerprint.digest("hex")}`,
    version: createHash("sha256")
      .update(items.map(item => `${item.version}:${item.ctimeMs}:${item.links}`).join("\0"))
      .digest("hex"),
  };
}
...
function editable(item: Item): { text: string; bom: boolean; eol: string } {
  ...
  if (process.platform !== "win32" && process.platform !== "linux")
    throw new Error("Metadata-preserving saves are supported on Windows and Linux only.");
  if (process.getuid && item.uid !== process.getuid())
    throw new Error("Files owned by another user are read-only here.");
  const raw = item.bytes.toString("utf8");
  if (!Buffer.from(raw, "utf8").equals(item.bytes) || raw.includes("\0"))
    throw new Error("Only valid UTF-8 text can be edited.");
...
export async function readWorkspaceEntry(
...
): Promise<Omit<WorkspaceEntry, "sessionId" | "sessionGeneration">> {
  ...
  const entry = {
    ...
    moveFingerprint: result.moveFingerprint,
    moveDestination,
  ...
  try {
    return {
      ...entry,
      text: editable(result.items[0]).text,
      ...(includeGitIndex ? { gitIndexText: await readGitIndexText(cwd, path) } : {}),
    };
  } catch (error) {
    return { ...entry, readOnlyReason: (error as Error).message };
  }
}
...
export async function mutateWorkspace(
  ...
  if (mutation.action === "createDirectory") {
    await mkdir(source);
    return;
  }
  ...
  if (original.version !== mutation.expectedVersion)
    throw new Error("File or folder changed on disk. Reopen it before trying again; your draft has not been saved.");
  const revalidate = async () => {
    if ((await inspect(ctx, mutation.path)).version !== original.version)
      throw new Error("File or folder changed during the operation.");
  };
  if (mutation.action === "save") {
    const format = editable(original.items[0]);
    if (
      mutation.text.includes("\r") ||
      mutation.text.includes("\0") ||
      Buffer.from(mutation.text).toString("utf8") !== mutation.text
    )
    ...
    try {
      ...
      if (process.platform === "win32") {
        ...
        await windowsFileOperation(
          "Get-Acl -LiteralPath $env:PYLON_FILE_SOURCE | Set-Acl -LiteralPath $env:PYLON_FILE_DESTINATION",
          source,
          temporary,
        );
      } else {
        // Explicit preservation attributes make GNU cp fail rather than silently dropping security metadata.
        await run("cp", ["--preserve=mode,ownership,xattr", "--", source, temporary], {
          timeout: 10_000,
          maxBuffer: 64 * 1024,
        });
      }
      ...
      if (process.platform === "win32") {
        ...
        await windowsFileOperation(
          "[IO.File]::Replace($env:PYLON_FILE_SOURCE, $env:PYLON_FILE_DESTINATION, [NullString]::Value)",
          temporary,
          source,
        );
      } else {
      ...
      await unlink(temporary).catch((error: any) => {
        if (error.code !== "ENOENT")
          throw new Error(
            `${failure ? (failure as Error).message : "Save completed."} Temporary-file cleanup failed: ${error.message}`,
          );
      });
    }
    ...
    try {
      ...
      throw new Error(
        `File was written, but its saved version could not be confirmed. Your draft is retained; inspect the working copy before retrying. ${(error as Error).message}`,
      );
    }
  }
  if (mutation.action === "copy") {
    ...
    try {
      ...
      throw new Error(
        `Copy stopped; ${mutation.destination} may be incomplete. Inspect it before retrying. ${(error as Error).message}`,
      );
    }
    return;
  }
  ...
  if (mutation.action === "move") {
    ...
    if (original.items[0].kind === "file") {
      ...
      try {
        ...
        if (linked.version !== original.items[0].version || linked.links !== original.items[0].links + 1)
          throw new Error("Source changed during move.");
        await unlink(source);
      } catch (error) {
        throw new Error(
          `Move stopped; both paths may exist. Inspect them before retrying. ${(error as Error).message}`,
        );
      }
    } else {
      if ((await lstat(dirname(destination))).dev !== original.items[0].dev)
        throw new Error("Cross-device folder moves are not supported.");
      if (process.platform === "win32") {
        // Directory.Move refuses an existing destination, unlike POSIX rename.
        await windowsFileOperation(
          "[IO.Directory]::Move($env:PYLON_FILE_SOURCE, $env:PYLON_FILE_DESTINATION)",
          source,
          destination,
        );
      } else if (process.platform === "linux") {
        await run("mv", ["--no-clobber", "--no-target-directory", "--", source, destination], {
          timeout: 10_000,
          maxBuffer: 64 * 1024,
        });
        if (await exists(source))
          throw new Error("Destination exists or folder move was not completed. Inspect both paths before retrying.");
      } else {
        throw new Error("No-overwrite folder moves are supported on Windows and Linux only.");
      }
    }
    ...
    try {
      ...
      for (const item of [...original.items].reverse()) {
        ...
        if (item.kind === "file") {
          ...
          if (current.dev !== item.dev || current.ino !== item.ino || current.mode !== item.mode)
            throw new Error("Folder changed during deletion.");
          await rmdir(absolute);
        }
      }
    } catch (error) {
      throw new Error(
        `Deletion stopped; some entries may already be deleted. Refresh before retrying. ${(error as Error).message}`,
      );
    }
  }
}
