...
export class FileHistoryReader {
  ...
  private async load(input: ReadInput, signal: AbortSignal): Promise<FileHistoryResult> {
    ...
    for (const { checkpoint, source } of checkpoints) {
      const unchanged =
        ...
        source.blob === previous.blob &&
        (source.state === "available" || source.state === "deleted");
      if (unchanged) {
        skipped++;
        previous = source;
        continue;
    ...
    const skippedSessionTail = skipped || undefined;
    let commits: GitStop[] = [];
    let hasMore = false;
    ...
    const result: FileHistoryResult = {
      path: requestedPath,
      revision: key,
      stops: gitStops.concat(stops),
      ...(skippedSessionTail ? { skippedSessionTail } : {}),
  ...
  private async changeCounts(
    ...
    signal: AbortSignal,
  ): Promise<FileHistoryContent["changes"] | undefined> {
    try {
      signal.throwIfAborted();
      if (
        !before ||
        !after ||
        !["available", "deleted"].includes(before.state) ||
        !["available", "deleted"].includes(after.state)
      )
        return undefined;
  ...
  private async content(
    ...
    let changeBefore: Source | undefined;
    if (commit) {
      ...
      if (index >= 0)
        changeBefore =
          index > 0 ? checkpoints[index - 1].source : unknownBaseline && !input.context?.seed ? undefined : seed;
    }
    ...
    const changes = await this.changeCounts(git, changeBefore, after, signal);
