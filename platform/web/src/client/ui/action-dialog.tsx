...
interface ActionDialogProps {
  ...
  inputHint?: (value: string) => ReactNode;
...
export function ActionDialog({
  ...
  const validationError = value ? validateInput?.(value) : undefined;
  const valid = (!inputLabel || allowEmpty || Boolean(value.trim())) && !validationError;
    ...
    <div className="edit-confirm-backdrop action-dialog-backdrop" onMouseDown={closeBackdrop}>
      <form
        ...
        <div>
            ...
            <label className="action-dialog-field">
              ...
              {multiline ? (
                ...
                <input
                  ...
                  maxLength={maxLength}
                  disabled={busy}
                  list={suggestions ? "action-dialog-suggestions" : undefined}
                  autoComplete={suggestions ? "off" : undefined}
                  aria-invalid={Boolean(validationError)}
                  aria-describedby={inputHint || validationError ? "action-dialog-input-help" : undefined}
                  onChange={event => setValue(event.target.value)}
                />
              )}
          ...
          {suggestions && <datalist id="action-dialog-suggestions">
            {suggestions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
          </datalist>}
          {(inputHint || validationError) && <div id="action-dialog-input-help" aria-live="polite">
            {inputHint?.(value)}
            {validationError && <p className="workspace-edit-error">{validationError}</p>}
          </div>}
        </div>
        <footer>
