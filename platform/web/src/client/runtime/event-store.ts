...
export class RuntimeEventStore {
  ...
  async abort(): Promise<void> {
    ...
    this.set({ ...this.snapshot, runtime: { ...runtime, conversation: { ...runtime.conversation, stopping: true } } });
    try {
      ...
      if (current?.sessionId === runtime.sessionId) {
        this.set({
          ...this.snapshot,
          runtime: { ...current, conversation: { ...current.conversation, stopping: false } },
        });
      }
      throw error;
    }
  ...
  async jumpToHistory(cursor: string): Promise<void> {
    ...
    this.setHistorySegments(historyKey(runtime.sessionId, runtime.sessionGeneration), [
      { messages: page.messages, earlierCursor: page.earlierCursor, laterCursor: page.laterCursor },
    ]);
    this.set({ ...this.snapshot, historyWindow: this.historyWindow(runtime) });
  }
  ...
  async startProviderLogin(provider: string, authType: ProviderAuthType): Promise<void> {
    const runtime = this.requireReadyRuntime();
    await this.sendCommand({
  ...
  private waitForRuntimePolicyRevision(sessionId: string, previousRevision: number): Promise<void> {
    const ready = () =>
      this.snapshot.runtime?.sessionId === sessionId && this.snapshot.runtime.runtimePolicy.revision > previousRevision;
  ...
  private apply(event: WebEvent): void {
    ...
    if (event.type === "session.status") {
      ...
      if (
        ...
        ["sleeping", "idle", "running", "attention"].includes(String(status.state))
      ) {
    ...
    const notification = event.type === "ui.notify" ? (event.payload as UiNotificationReadModel) : current.notification;
  ...
  switch (event.type) {
          ...
          ? (asRecord(payload).items as unknown[]).flatMap(item => {
              ...
              return typeof value.id === "string" && typeof value.canUndo === "boolean"
                ? [
                    [
                      value.id,
                      { canUndo: value.canUndo, canForkWithTimeline: value.canForkWithTimeline === true },
                    ] as const,
                  ]
                : [];
            })
    ...
    case "delegate.update": {
      ...
      const next = mergeDelegatedRun(previous, {
        ...run,
        activity: activityMode === "append" && previous ? [...previous.activity, ...run.activity] : run.activity,
      });
      return {
        ...runtime,
        conversation: { ...conversation, delegatedRuns: replaceDelegatedRun(conversation.delegatedRuns, next) },
      };
    }
    case "turn.changes": {
    ...
    case "session.controls":
      return { ...runtime, sessionControls: payload as SessionControlsReadModel };
    case "provider.auth":
      return { ...runtime, providerAuth: payload as ProviderAuthReadModel };
    case "runtime.policy":
      return { ...runtime, runtimePolicy: payload as RuntimeSnapshot["runtimePolicy"] };
    case "queue.update":
      return { ...runtime, conversation: { ...conversation, queue: payload as ConversationReadModel["queue"] } };
    case "workspace.revision":
      return { ...runtime, workspace: payload as RuntimeSnapshot["workspace"] };
    case "retry.update":
      return { ...runtime, conversation: { ...conversation, retry: payload as ConversationReadModel["retry"] } };
    case "compaction.update": {
      const update = payload as ConversationReadModel["compaction"] & { completedMessage?: MessageReadModel };
      const compaction = { active: update.active, ...(update.reason ? { reason: update.reason } : {}) };
      return {
        ...runtime,
        conversation: {
          ...conversation,
    ...
    case "agent.end": {
      ...
      const turnGitBranch =
        typeof info.turnGitBranch === "string" ? info.turnGitBranch.slice(0, 200) || undefined : undefined;
      ...
      const messages =
        willRetry || durationMs === undefined || !messageId
          ...
          : settledMessages.map(message =>
              message.id === messageId || Boolean(assistant?.entryId && message.entryId === assistant.entryId)
                ? {
                    ...message,
        ...
        conversation: {
          ...
          stoppedRun:
            info.stopped === true && durationMs !== undefined
              ...
              : undefined,
          agentError:
            willRetry || typeof info.message !== "string" ? undefined : info.message.slice(0, 1_000) || undefined,
        },
      };
    }
    case "agent.error": {
    ...
    case "command.result": {
      ...
      return {
        ...runtime,
        commandResult:
          typeof item.id === "string" ? (item as unknown as NonNullable<RuntimeSnapshot["commandResult"]>) : undefined,
      };
    }
    case "ui.notify":
      return {
        ...
        extensionUi: {
          ...runtime.extensionUi,
          notifications: [
            ...runtime.extensionUi.notifications.filter(item => item.id !== (payload as { id?: string }).id),
    ...
    case "ui.editor-text": {
      ...
      return {
        ...runtime,
        extensionUi: { ...runtime.extensionUi, editorText: item.text, editorRevision: item.revision as number },
      };
    }
    default:
      return runtime;
