...
import {
  agentColorTokens,
  assignAgentColorSlots,
  type AgentIdentity,
} from "../shared/agent-colors.ts";
...
function agentColorMap(slots: ReadonlyMap<string, number>): AgentColorMap {
  return new Map(
    [...slots].map(([identity, slot]) => [identity, colorForSlot(slot)]),
  );
}
...
export function useAgentColors(
  ...
  const registry = useRef<{ sessionId?: string; slots: Map<string, number> }>({
    slots: new Map(),
  });
  if (registry.current.sessionId !== sessionId)
    registry.current = { sessionId, slots: new Map() };
  registry.current.slots = assignAgentColorSlots(
    registry.current.slots,
    agents,
  );
