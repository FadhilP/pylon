...
export function DatabaseQueryEditor({
  ...
  return (
    <div className="database-query-input">
      <pre ref={backdrop} aria-hidden="true">
        <DatabaseSyntax text={text + "\n"} language={language} />
      </pre>
      <textarea
        aria-label={language === "json" ? "MongoDB command JSON" : "SQL query"}
        spellCheck={false}
        autoCapitalize="off"
        autoCorrect="off"
        wrap="off"
        value={text}
        maxLength={65536}
        placeholder={language === "json" ? '{ "operation": "find", "collection": "users", "filter": {} }' : "SELECT …"}
