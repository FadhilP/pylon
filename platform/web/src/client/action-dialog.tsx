...
export function ActionDialog({ title, description, confirmLabel, busyLabel, busy, danger = false, inputLabel, initialValue = "", multiline = false, maxLength = 200, allowEmpty = false, onCancel, onConfirm }: ActionDialogProps) {
  ...
  return <div className="edit-confirm-backdrop action-dialog-backdrop" onMouseDown={closeBackdrop}>
    <form ref={dialogRef} className="edit-confirm-dialog action-dialog" role={danger ? "alertdialog" : "dialog"} aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} onKeyDown={onKeyDown} onSubmit={submit}>
      <header>
      ...
      <div>
        ...
        {inputLabel && <label className="action-dialog-field">{inputLabel}
          {multiline
            ? <textarea data-action-input value={value} maxLength={maxLength} disabled={busy} onChange={(event) => setValue(event.target.value)} />
            : <input data-action-input value={value} maxLength={maxLength} disabled={busy} onChange={(event) => setValue(event.target.value)} />}
