...
import { aggregatePairedAgentTiming, pairAgentActivity, pairedAgentToolDuration, pairedAgentToolStatus } from "../shared/agent-activity";
    ...
    <aside id="agents-panel" className={`inspector agents-panel is-open${selected ? " has-selection" : ""}`} aria-labelledby="agents-title" style={selected ? agentColor(selected, colors) : undefined}>
      <header>
        <div>
          ...
          {!selected && <span className="agents-run-count">{ordered.length} {ordered.length === 1 ? "run" : "runs"}</span>}
        </div>
...
function AgentList({ runs, models, colors, onSelect }: { runs: DelegatedAgentRunReadModel[]; models: ModelOptionReadModel[]; colors: AgentColorMap; onSelect: (id: string) => void }) {
  ...
  return <div className="agents-list">
    <div className="agents-filterbar" role="group" aria-label="Filter agent runs">
      <AgentFilterButton active={filter === "all"} onClick={() => setFilter("all")}>All</AgentFilterButton>
      <AgentFilterButton active={filter === "running"} onClick={() => setFilter("running")}>Live</AgentFilterButton>
      <AgentFilterButton active={filter === "failed"} onClick={() => setFilter("failed")}>Errors</AgentFilterButton>
      <span>Newest first</span>
    </div>
    <div className="agents-table-summary">
      <span><strong>{counts.running}</strong> working · <strong>{counts.completed}</strong> completed · <strong>{counts.failed}</strong> failed</span>
      <span>${totalCost.toFixed(4)}</span>
    </div>
    <div className="agents-table-head" aria-hidden="true"><span>Agent / task</span><span>Status</span><span>Cost</span><span>Time</span></div>
    <div className="agents-table-body">
      {[...turns].map(([turn, items]) => <section key={turn}>
        <h2>{turn > 0 ? `Turn ${turn}` : "Earlier turn"}</h2>
        {items.map((run) => <AgentRunRow key={run.id} run={run} models={models} colors={colors} onSelect={onSelect} />)}
      </section>)}
      {!visibleRuns.length && <div className="agents-filter-empty">No {filter === "running" ? "working" : "failed"} runs.</div>}
    </div>
  </div>;
}

function AgentFilterButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: string }) {
  return <button type="button" className={active ? "is-active" : undefined} aria-pressed={active} onClick={onClick}>{children}</button>;
}

function AgentRunRow({ run, models, colors, onSelect }: { run: DelegatedAgentRunReadModel; models: ModelOptionReadModel[]; colors: AgentColorMap; onSelect: (id: string) => void }) {
  ...
  return <button className={`agent-table-row is-${run.status}`} type="button" style={agentColor(run, colors)} onClick={() => onSelect(run.id)}>
    <span className="agent-table-main">
      <strong><AgentIdentity run={run} /></strong>
      <small title={request}>{request}</small>
      <span className="agent-table-meta"><span>{run.modelName ? modelLabel(run.modelName, models) : "Model pending"}</span>{toolCount > 0 && <span>{toolCount} {toolCount === 1 ? "tool" : "tools"}</span>}</span>
    </span>
    <span className={`agent-table-state is-${run.status}`}><span aria-hidden="true" />{run.status === "running" ? "Live" : run.status === "completed" ? "Done" : "Failed"}</span>
...
function useAgentToolNow(runs: DelegatedAgentRunReadModel[]): number {
  ...
  const hasRunningTools = runs.some((run) => run.status === "running"
    && pairAgentActivity(run.activity).some((tool) => !tool.completed));
  useEffect(() => {
...
function AgentActivity({ run, childRuntime = false, now }: { run: DelegatedAgentRunReadModel; childRuntime?: boolean; now?: number }) {
  ...
  return <AnimatedDetails
    ...
    summary={<>
      ...
      {timing && <time className={`tool-group-duration is-${timing.status}`} dateTime={`PT${timing.durationMs / 1_000}S`} aria-label={`${timing.status} tool duration ${formatToolDuration(timing.durationMs)}`}>{formatToolDuration(timing.durationMs)}</time>}
    </>}
  ><div className="agent-tools">
    {tools.map((tool, index) => {
      const status = pairedAgentToolStatus(tool, runRunning);
      const durationMs = pairedAgentToolDuration(tool, runRunning, now);
      return <details className={`tool-disclosure is-${status}`} key={tool.id ?? `${tool.tool}-${index}`}>
        <summary>
          <IconTool size={15} />
          <span className="tool-summary-copy"><strong>{tool.tool}</strong>{tool.input && <code>{tool.input.replace(/\s+/g, " ").trim()}</code>}</span>
          <span className="tool-status">{durationMs === undefined
            ? status
            : <><span className="sr-only">{status}, </span><time dateTime={`PT${durationMs / 1_000}S`}>{formatToolDuration(durationMs)}</time></>}</span>
        </summary>
        <div className="tool-details">
          <section><small>Input</small><pre>{tool.input || "No input"}</pre></section>
          <section><small>Output</small><pre>{tool.output || (status === "running" ? "Waiting for output…" : "No output")}</pre></section>
        </div>
      </details>;
    })}
  </div></AnimatedDetails>;
