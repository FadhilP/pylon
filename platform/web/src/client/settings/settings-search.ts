import { KEY_COMMANDS } from "../../shared/settings/keyboard.ts";
import { GUARD_RISK_CATEGORIES, GUARD_RULE_DESCRIPTIONS, GUARD_RULE_LABELS } from "../../shared/settings/guard-policy.ts";
import type { ToolPolicyReadModel } from "../../shared/protocol/events.ts";
import type {
...
const STATIC_ENTRIES: SettingsSearchEntry[] = [
  entry({
    ...
    description: "Connect accounts and API keys used by Pi.",
    keywords: "login sign in credentials",
  }),
  entry({
    id: "models",
    tab: "models",
  ...
  entry({
    ...
    label: "Agent models",
    description: "Choose models and thinking levels used by agents and background tasks.",
  }),
  entry({
    id: "packages",
    tab: "packages",
    section: "Packages",
    label: "Package defaults",
    description: "Configure package defaults and global tool exposure.",
  }),
  entry({
    ...
    description: "Install an extension package globally or into a registered project.",
    keywords: "npm source",
  }),
  entry({
    id: "extension-trust",
    ...
    description: "Trust or revoke trust for project extensions.",
  }),
  entry({
    id: "extension-reload",
    tab: "extensions",
    ...
    description: "Apply changed extension settings to active sessions.",
  }),
  entry({
    id: "skills",
    tab: "skills",
    section: "Skills",
    label: "Loaded skills",
    description: "Inspect skills available to the selected Pi session.",
    keywords: "agent capabilities manual",
  }),
  entry({
    ...
    section: "Hooks",
    label: "session_start",
    description: "Runs once when a logical session begins.",
    keywords: "session start lifecycle",
    hookKey: "sessionStart",
  }),
  entry({
    id: "hook-before-agent-start",
    tab: "hooks",
    section: "Hooks",
    label: "before_agent_start",
    description: "Runs before every prompt sent to the agent.",
    keywords: "before agent start lifecycle prompt",
    hookKey: "beforeAgentStart",
  }),
  entry({
    id: "policy-timeline",
    tab: "policy",
    ...
    label: "Timeline",
    description: "Keep recoverable checkpoints for session activity.",
  }),
  entry({
    id: "policy-guard",
    tab: "policy",
    section: "Policy › Activity and safety",
    label: "Guard",
    description: "Ask before guarded commands and paths run.",
    keywords: "approval safety",
  }),
  ...
  entry({
    id: "policy-workspace",
    tab: "policy",
    section: "Policy › Workspace defaults",
    label: "Workspace",
    description: "Choose where sessions begin work.",
    keywords: "local project folder checkout worktree",
  }),
  entry({
    id: "notification-complete",
    tab: "notifications",
    section: "Notifications › Sound cues",
    label: "Turn complete",
    description: "Played after the assistant finishes a turn.",
    control: { kind: "sound", cue: "turn-complete" },
  }),
  entry({
    ...
    description: "Played when Pylon needs approval or clarification.",
    control: { kind: "sound", cue: "attention" },
  }),
  entry({
    id: "appearance-theme",
    tab: "appearance",
    ...
    case "timeline":
      return [
        model("Timeline titles", "Generate semantic checkpoint and session titles."),
        {
          ...
          description: "Customize session and checkpoint naming prompts.",
          inline: false,
        },
        ...
        { key: "title-timeout", label: "Title generation timeout", description: "Maximum time for each title call." },
        {
          ...
          description: "Maximum tokens generated for a Timeline title.",
        },
        {
          key: "changed-files",
          label: "Changed files in title prompt",
          description: "Maximum changed-file paths supplied to title generation.",
        },
        {
          key: "rollback",
          label: "Rollback files when editing prompts",
          description: "Restore the matching checkpoint when an earlier prompt is edited.",
        },
      ];
    case "advisor":
      return [
        ...
        {
          key: "prompt",
          label: "Advisor system prompt",
          description: "Customize the package-owned prompt.",
          inline: false,
        },
        {
          key: "max-calls",
          label: "Maximum consultations",
          description: "Maximum Advisor calls for each user prompt.",
        },
        {
          key: "timeout",
    ...
    case "scout":
      return [
        ...
        { key: "results", label: "Web search results", description: "Default URL candidates returned by Web Scout." },
        {
          ...
          description: "Enable optional Web Scout search.",
        },
      ];
    case "grunt":
      return [
        ...
        { key: "thinking", label: "Eligible thinking levels", description: "Thinking levels a grunt run may use." },
        {
          ...
          description: "Customize the selected worker-mode prompt.",
          inline: false,
        },
    ...
    case "continuity":
      return [
        { key: "memory", label: "Durable memory", description: "Keep project notes and papercuts across sessions." },
        {
          ...
          description: "Remaining context tokens that trigger compaction.",
        },
        {
          key: "retained",
          label: "Continuity retained tokens",
          description: "Recent raw history retained after compaction.",
        },
        {
          key: "review-timeout",
          label: "Compaction review timeout",
          description: "Maximum time for a compaction reviewer call.",
        },
        {
          key: "review-output",
          label: "Compaction reviewer maximum output",
          description: "Maximum tokens generated by the compaction reviewer.",
        },
        ...
        {
          key: "compaction-reviewer",
          label: "Compaction reviewer",
          description: "Model profile that checks compaction summaries.",
        },
        {
          key: "prompt",
          label: "Continuity reviewer instructions",
          description: "Customize memory review and migration prompts.",
          inline: false,
        },
      ];
    case "sieve":
      return [
        ...
        { key: "threshold", label: "Pruning threshold", description: "Minimum result size eligible for pruning." },
        {
          ...
          description: "Threshold multiple that begins rollover.",
          target: settings.projectionMode === "stable" ? undefined : "Projection mode",
          inline: settings.projectionMode === "stable",
        },
        {
          key: "target",
          label: "Target multiplier",
          description: "Threshold multiple that ends rollover.",
          target: settings.projectionMode === "stable" ? undefined : "Projection mode",
          inline: settings.projectionMode === "stable",
        },
      ];
      ...
      return [
        {
          ...
          description: "Let private agents use every visible model.",
        },
        ...
        { key: "thinking", label: "Private-agent thinking", description: "Thinking levels a private agent may use." },
        {
          ...
          description: "Default prompt for a new private agent.",
          inline: false,
        },
        { key: "timeout", label: "Spawn timeout", description: "Maximum child runtime." },
        {
          ...
          description: "Default transcript message count returned by recent.",
        },
        {
          key: "recent-message",
          label: "Recent thread message characters",
          description: "Maximum characters per returned transcript message.",
        },
        {
          key: "recent-total",
