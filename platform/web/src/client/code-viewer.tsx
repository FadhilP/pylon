...
import {
  diffRows, parseDiff, selectedText, sourceLines, validateDiffContents,
  type CodeLine, type ContextGap, type DiffContents, type DiffContentsLoader, type DiffFile, type ExpandedContext,
...
class ViewerErrorBoundary extends Component<{ children: ReactNode; fallback: ReactNode }, { failed: boolean }> {
  ...
  componentDidCatch(error: unknown) { console.error("Code viewer failed; using raw text fallback", error); }
  render() { return this.state.failed ? this.props.fallback : this.props.children; }
}

export default function CodeViewer(props: ViewerProps) {
  return (
    <ViewerErrorBoundary key={`${props.mode}:${props.path}:${props.revision}`} fallback={<RawText text={props.unifiedDiff ?? props.text} />}>
      <Viewer {...props} />
    </ViewerErrorBoundary>
  );
}
...
function RawText({ text }: { text: string }) {
  return <pre className="file-code"><code>{text}</code></pre>;
}

function Viewer({ mode, path, text, targetLine, loadDiffFiles, unifiedDiff, showFileHeaders, renderHeaderIcon, scrollToFile }: ViewerProps) {
  ...
  const plainLines = useMemo<CodeLine[]>(() => mode === "file"
    ? sourceLines(text).map((text, index) => ({ kind: "context", text, newLine: index + 1 })) : [], [mode, text]);
  ...
  useEffect(() => {
    for (const language of new Set(mode === "file" ? [sourceLanguage(path)] : files.flatMap(file => [sourceLanguage(file.path), sourceLanguage(file.oldPath)])))
  ...
  const rows = useMemo(() => {
    ...
    files.forEach((file, index) => {
      ...
      for (const [lineIndex, line] of lines.entries()) {
        ...
        const key = line.kind === "gap" ? `gap:${line.id}` : `${line.kind}:${line.oldLine}:${line.newLine}:${lineIndex}`;
  ...
  const highlighted = useMemo(() => {
    ...
    const fileTokens = files.map((file, fileIndex) => {
      ...
      if (full) {
        syntaxTokens(full.oldFile.contents, sourceLanguage(file.oldPath)).forEach((tokens, index) => oldTokens.set(index + 1, tokens));
        ...
        for (const hunk of file.hunks) {
          for (const side of ["oldLine", "newLine"] as const) {
            ...
            const tokens = syntaxTokens(lines.map(line => line.text).join("\n"), side === "oldLine" ? sourceLanguage(file.oldPath) : language);
            lines.forEach((line, index) => (side === "oldLine" ? oldTokens : newTokens).set(line[side]!, tokens[index]));
          }
        }
      }
  ...
  useLayoutEffect(() => {
    ...
    if (!element) return;
    const update = () => setViewport({ top: element.scrollTop, left: element.scrollLeft, height: element.clientHeight, width: element.clientWidth });
    update();
  ...
  const getContents = async (file: number) => {
    ...
    finally { requests.current.delete(file); }
  };
  ...
  const expand = async (file: number, gap: ContextGap, direction: "start" | "end" | "all") => {
    ...
    try {
      ...
      setExpanded(current => {
        ...
        const next = direction === "all" ? { start: Number.MAX_SAFE_INTEGER, end: 0 }
      ...
      if (mounted.current) setLoading(current => { const next = new Set(current); next.delete(file); return next; });
    }
  };

  const navigatedLine = useRef<number | undefined>(undefined);
  useEffect(() => {
    if (!targetLine) { navigatedLine.current = undefined; return; }
    ...
    if (index < 0) {
      ...
      const gap = rows.find(row => row.kind === "gap" && targetLine >= row.newStart &&
  ...
  return (
    <div className="code-viewer-shell">
      ...
      <div ref={root} className={`code-viewer${mode === "diff" ? " is-diff" : ""}`} tabIndex={0}
        style={{ "--viewer-gutter-width": `${metrics.gutter}ch`, "--viewer-scroll-left": `${viewport.left}px`, "--viewer-width": `${viewport.width}px` } as CSSProperties}
        role="region" aria-label={mode === "diff" ? "File changes" : path}
        onScroll={event => setViewport({ top: event.currentTarget.scrollTop, left: event.currentTarget.scrollLeft,
          height: event.currentTarget.clientHeight, width: event.currentTarget.clientWidth })}
        onPointerDown={event => { if ((event.target as HTMLElement).closest("code")) setSelection(undefined); }}
        onPointerMove={event => { if (drag.current) drag.current.y = event.clientY; }}
        onPointerUp={() => { drag.current = undefined; setDragging(false); }}
        onPointerCancel={() => { drag.current = undefined; setDragging(false); }}
        onKeyDown={event => {
          if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "a") {
            event.preventDefault(); setSelection({ start: 0, end: rows.length - 1 });
          }
          if (event.key === "Escape") setSelection(undefined);
          if (event.target !== event.currentTarget || !["ArrowUp", "ArrowDown", "Home", "End"].includes(event.key)) return;
          event.preventDefault();
          const direction = event.key === "ArrowUp" || event.key === "End" ? -1 : 1;
          let index = event.key === "Home" ? 0 : event.key === "End" ? rows.length - 1
            : selection ? selection.end + direction : rowAt(rows, viewport.top);
          while (index >= 0 && index < rows.length && ["header", "gap", "note"].includes(rows[index].kind)) index += direction;
        ...
        <div className="code-viewer-content" style={{ height: totalHeight, minWidth: `${metrics.widest + metrics.gutter + 2}ch` }}>
          {rows.slice(first, last).map((row, offset) => {
            ...
            if (row.kind === "header") {
              ...
              return (
                <button key={row.key} className="code-viewer-header" style={style} aria-expanded={!collapsed.has(row.file)}
                  onClick={() => { setSelection(undefined); setCollapsed(current => {
                    const next = new Set(current); if (next.has(row.file)) next.delete(row.file); else next.add(row.file); return next;
                  }); }}>
                  {collapsed.has(row.file) ? <IconChevronRight size={14} aria-hidden="true" /> : renderHeaderIcon?.(row.text)}
                  <span className="code-viewer-path">{row.text}</span>
                  <span className="code-viewer-counts">{file.hunks.length ? <><del>−{deletions}</del><ins>+{additions}</ins></> : fileDescription(file)}</span>
                </button>
              );
            }
            if (row.kind === "gap") return (
              <div key={row.key} className="code-viewer-gap" style={style}>
                {loadDiffFiles && <span className="code-viewer-expand">
                  {row.id > 0 && <button disabled={loading.has(row.file)} aria-label="Expand 15 lines below" onClick={() => void expand(row.file, row, "start")}><IconChevronDown size={14} aria-hidden="true" /></button>}
                  {row.id < files[row.file].hunks.length && <button disabled={loading.has(row.file)} aria-label="Expand 15 lines above" onClick={() => void expand(row.file, row, "end")}><IconChevronUp size={14} aria-hidden="true" /></button>}
                </span>}
                <span className="code-viewer-context-label">{loading.has(row.file) ? "Loading context…" : row.count === undefined ? "More unchanged context may be available" : `${row.count} unmodified lines`}</span>
                {loadDiffFiles && <button className="code-viewer-expand-all" disabled={loading.has(row.file)} aria-label="Expand all unchanged lines" onClick={() => void expand(row.file, row, "all")}>Expand all</button>}
              </div>
            );
            if (row.kind === "note") return <div key={row.key} className="code-viewer-note" style={style}>{row.text}</div>;
            const gutter = (side: "oldLine" | "newLine") => row[side] === undefined ? <span /> : (
              <button aria-label={`${side === "oldLine" ? "Old" : "New"} line ${row[side]}`} tabIndex={-1}
                onPointerDown={event => {
                  event.preventDefault();
            ...
            return (
              <div key={row.key} className={`code-viewer-line is-${row.kind}${index >= selectionStart && index <= selectionEnd ? " is-selected" : ""}`} style={style} data-row={index}
                role="group" aria-label={`${row.kind === "deletion" ? "Deleted line" : row.kind === "addition" ? "Added line" : "Line"} ${row.newLine ?? row.oldLine}`}>
                <span className="code-viewer-gutter" aria-hidden="true">{gutter(row.kind === "deletion" ? "oldLine" : "newLine")}</span>
                <code><HighlightedLine line={row} tokens={highlighted.get(row.key)} /></code>
              </div>
            );
          })}
        </div>
...
function fileDescription(file: DiffFile): string {
  ...
  const additions = file.hunks.reduce((count, hunk) => count + hunk.filter(line => line.kind === "addition").length, 0);
  const deletions = file.hunks.reduce((count, hunk) => count + hunk.filter(line => line.kind === "deletion").length, 0);
  if (!additions && !deletions) return patch.isCreate ? "Empty file added" : patch.isDelete ? "Empty file deleted" : "File metadata changed";
...
function HighlightedLine({ line, tokens }: { line: CodeLine; tokens?: SyntaxToken[] }) {
  ...
  return (tokens ?? [{ content: line.text, className: "" }]).map((token, index) => {
    ...
    return <span key={index} className={token.className || undefined}>{parts}</span>;
  });
}
