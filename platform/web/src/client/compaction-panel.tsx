...
import type { CompactionDisplayReadModel, CompactionDisplaySourceReadModel, MessageReadModel } from "../shared/protocol/events";
...
export function CompactionPanel({ message, onClose }: { message: MessageReadModel; onClose: () => void }) {
  ...
  const hasSourceDetails = Boolean(display && (display.records.length || display.failedTools.length || display.toolResults.length
    || display.history.modified.length || display.history.read.length));
  const hasMetadata = Boolean(validTimestamp || message.entryId);

  return <aside id="compaction-panel" className="inspector compaction-panel is-open" aria-labelledby="compaction-title">
    <header>
      <div><IconFileText size={18} /><strong id="compaction-title">Compaction details</strong></div>
      <button className="icon-button" type="button" onClick={onClose} aria-label="Close compaction details"><IconX size={17} /></button>
    </header>
    <div className="compaction-panel-body">
      <dl className="compaction-panel-facts">
        {compaction.contextBeforeTokens !== undefined && <div><dt>Context before</dt><dd>~{formatCompactNumber(compaction.contextBeforeTokens)} tokens</dd></div>}
      ...
      {display && hasSourceDetails && <section className="compaction-panel-section" aria-labelledby="compaction-sources-title">
      ...
      {hasMetadata && <details className="compaction-panel-metadata">
        <summary><strong>Metadata</strong><IconChevronDown size={14} aria-hidden="true" /></summary>
        <dl>
          {validTimestamp && <div><dt>Created</dt><dd><time dateTime={message.createdAt}>{validTimestamp.toLocaleString()}</time></dd></div>}
          {message.entryId && <div><dt>Entry</dt><dd><code>{message.entryId}</code></dd></div>}
        </dl>
      </details>}
    </div>
  </aside>;
