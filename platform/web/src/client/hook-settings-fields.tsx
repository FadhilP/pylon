...
export function HookSettingsFields({ settings, loading, disabled, onUpdate }: {
  ...
  if (!draft) return <div className="settings-empty"><strong>Hook settings unavailable</strong></div>;

  const hook = draft[hookKey];
  const source = hook.sources.find((item) => item.id === sourceId);
  const info = HOOKS.find((item) => item.key === hookKey)!;
  const totalBytes = HOOKS.reduce((total, item) => total + draft[item.key].sources.reduce((sum, entry) => sum + bytes(entry.content), 0), 0);

  const commit = async (next: HookSettingsReadModel) => {
    setDraft(next);
    setError("");
    try { await onUpdate(next); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to save hook settings"); }
  };
  ...
  const updateSource = (nextSource: HookSourceReadModel, save: boolean) => {
    const next = { ...draft, [hookKey]: { ...hook, sources: hook.sources.map((item) => item.id === nextSource.id ? nextSource : item) } };
    setDraft(next);
  ...
  const saveSource = () => {
    ...
    const next = { ...draft, [hookKey]: { ...hook, sources: hook.sources.map((item) => item.id === source.id ? normalized : item) } };
    void commit(next);
  };
  ...
  const addFiles = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (hook.sources.length + files.length > MAX_SOURCES) return setError(`Each hook supports up to ${MAX_SOURCES} sources.`);
  ...
  return <div className="hooks-editor">
    <aside className="hooks-list" aria-label="Lifecycle hooks">
      <div className="hooks-list-label"><span>Lifecycle hooks</span><span>{HOOKS.filter((item) => draft[item.key].enabled).length} enabled</span></div>
      {HOOKS.map((item) => <button key={item.key} type="button" className={`hook-choice${hookKey === item.key ? " is-selected" : ""}`} onClick={() => setHookKey(item.key)}>
    ...
    <div className="hook-detail">
      <header><div><strong>{info.name}</strong><span>{info.detail}</span></div><label className="package-switch"><span className="sr-only">Enable {info.name}</span><input type="checkbox" role="switch" checked={hook.enabled} disabled={disabled} onChange={(event) => void updateHook({ ...hook, enabled: event.target.checked })} /></label></header>
      <div className="hook-source-toolbar"><div><strong>Sources</strong><span>{hook.sources.length} source{hook.sources.length === 1 ? "" : "s"} · combined in list order</span></div><div>
        <input ref={fileRef} type="file" multiple accept=".md,.txt,text/markdown,text/plain" disabled={disabled} onChange={(event) => void addFiles(event)} />
        <button type="button" disabled={disabled} onClick={() => fileRef.current?.click()}><IconPlus size={13} /> Add files</button>
        <button type="button" disabled={disabled} onClick={addWritten}><IconPencil size={13} /> Write new</button>
      </div></div>
