...
export function isDatabaseResult(value: unknown): value is DatabaseResult {
  return (
    ...
    value.columns.every(column => databaseRecord(column) && text(column.name) && text(column.type)) &&
    typeof value.cached === "boolean" &&
    databaseRecord(value.storage) &&
    text(value.storage.mode) &&
    text(value.storage.expires_at) &&
    Number.isFinite(Date.parse(value.storage.expires_at))
  );
}
  ...
  switch (command) {
    ...
    case "table.plan.batch":
      return (
        ...
        Array.isArray(value.required_overrides) &&
        value.required_overrides.every(text)
      );
    case "exec":
    case "mongo.exec":
    case "redis.exec":
    ...
    case "apply":
      return text(value.operation_id) && text(value.status);
    case "profile.list":
      return (
        Array.isArray(value.profiles) &&
        value.profiles.every(
          profile => databaseRecord(profile) && text(profile.profile) && typeof profile.read_only === "boolean",
        )
      );
    ...
    case "transaction.status":
      return (
        ...
        text(value.owner_actor_id) &&
        text(value.state) &&
        integer(value.statements) &&
        integer(value.pending_writes) &&
        typeof value.age_ms === "number"
      );
    case "transaction.commit":
      return text(value.transaction_id) && text(value.state) && integer(value.statements_executed);
    case "transaction.rollback":
      return text(value.transaction_id) && text(value.state) && integer(value.discarded_statements);
    case "inspect":
      return (
        ["tables", "collections", "columns", "indexes", "constraints", "primary_key"].some(key => key in value) ||
        text(value.table) ||
...
export function isDatabaseCommandResult(
  value: unknown,
  command: StateQLCommandInput["command"],
): value is StateQLCommandResult {
  if (
    !databaseRecord(value) ||
  ...
  if (
    ...
    !databaseRecord(response) ||
    !text(response.command_id) ||
    !text(response.session_id) ||
    !databaseRecord(response.meta) ||
    typeof response.meta.duration_ms !== "number" ||
    !Number.isFinite(response.meta.duration_ms) ||
    response.meta.duration_ms < 0
  )
    return false;
  if (response.ok === false)
    return (
      databaseRecord(response.error) &&
      text(response.error.code) &&
      text(response.error.message) &&
      typeof response.error.executed === "boolean" &&
      typeof response.error.retryable === "boolean"
    );
  return (
    response.ok === true &&
    Array.isArray(response.warnings) &&
    response.warnings.every(warning => databaseRecord(warning) && text(warning.code) && text(warning.message)) &&
    validCommandData(command, response.data)
  );
}
...
export function readDatabaseDrafts(storage: Pick<Storage, "getItem">): DatabaseDraft[] {
  try {
    ...
    return values
      .filter((value): value is DatabaseDraft => {
        if (
          !databaseRecord(value) ||
          value.version !== 1 ||
          !text(value.scope) ||
      ...
      .map(value => ({
        ...
        active: value.active,
        height: Math.max(120, Math.min(600, value.height)),
...
export function saveDatabaseDraft(storage: Pick<Storage, "getItem" | "setItem">, draft: DatabaseDraft): void {
  ...
  const tabs = draft.tabs
    ...
    .map(({ id, title, text, driver }) => ({ id, title, text, driver, saved: true }));
  const safe: DatabaseDraft = {
    version: 1,
    scope: draft.scope,
    sessionId: draft.sessionId,
    tabs,
    active: tabs.some(tab => tab.id === draft.active) ? draft.active : "history",
    height: draft.height,
    updatedAt: Date.now(),
  };
  ...
  const retained = readDatabaseDrafts(storage)
    .filter(item => item.scope !== draft.scope)
...
export function clearDatabaseDrafts(
  ...
  storage.setItem(
    DATABASE_DRAFTS_KEY,
    JSON.stringify(
      readDatabaseDrafts(storage).filter(draft => (scope ? draft.scope !== scope : draft.sessionId !== sessionId)),
    ),
  );
}
