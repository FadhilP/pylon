        ...
        return (
          <details
            ...
            <summary>
              ...
              <span className="section-kicker">{labels[kind]}</span>
              <small>{group.objects.length || undefined}</small>
            </summary>
            {group.objects.map(object => {
              const qualifiedName = object.schema ? `${object.schema}.${object.name}` : object.name;
              const fullName = object.kind === "function" && object.identity ? object.identity : qualifiedName;
              const label = object.schema === "public" && fullName.startsWith("public.") ? fullName.slice(7) : fullName;
              return (
                <button
                  className="database-object"
                  type="button"
                  key={identity(object)}
                  title={object.identity ?? qualifiedName}
                  aria-label={object.identity ?? qualifiedName}
                  disabled={disabled}
                  onClick={() =>
                    onOpen({
                      kind: object.kind,
