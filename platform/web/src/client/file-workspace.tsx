import { IconChevronLeft, IconFiles, IconFolder, IconSearch, IconSettings, IconTerminal2, IconX } from "@tabler/icons-react";
import { useEffect, useMemo, useRef, useState, type CSSProperties, type MutableRefObject, type ReactNode, type RefObject } from "react";
...
import { closeFileTab, openFileTab, selectFileTab, setFileTabView, workspaceStateForSession, type FileWorkspaceState } from "../shared/file-workspace-state";
...
export function FileWorkspace({
  ...
  const [loadedContent, setLoadedContent] = useState<{ key: string; value: WorkspaceFileContent | WorkspaceFileDiff }>();
  ...
  useEffect(() => {
    ...
    void runtimeStore.workspaceInventory(
      false,
      controller.signal,
      (next, wasTruncated) => {
  ...
  const validCachedContent = cachedContent
  ...
  useEffect(() => {
    ...
    void request.then((value) => {
      ...
      if (!active
        ...
        || snapshot.runtime.sessionGeneration !== generation) return;
    ...
    return () => { active = false; };
  }, [live.connection, contentKey, runtime?.ready, runtime?.sessionId, runtime?.sessionGeneration, runtime?.workspace?.revision]);
  ...
  const setSelectedView = (view: FileView) => updateUi((current) => current.selectedPath ? setFileTabView(current, current.selectedPath, view) : current);
  const closeFile = (path: string) => updateUi((current) => closeFileTab(current, path));
  return <section className={`file-workspace-shell${showExplorer ? "" : " has-session-navigation"}`}>
    {showExplorer && <aside className={`file-workspace-explorer${navigationOpen ? " is-open" : ""}`} aria-label="Workspace explorer" aria-hidden={mobile && !navigationOpen} inert={mobile && !navigationOpen}>
      <header>
        <strong><IconFolder size={16} />Explorer</strong>
        <button type="button" onClick={onSessions}><IconChevronLeft size={14} />Sessions</button>
      </header>
      <label className="files-search"><IconSearch size={15} /><input value={currentUi.query} onChange={(event) => updateUi((current) => ({ ...current, query: event.target.value }))} placeholder="Filter files" /></label>
      <nav className="files-tabs" aria-label="Explorer view">
        <button className={currentUi.tab === "changes" ? "is-active" : ""} onClick={() => updateUi((current) => ({ ...current, tab: "changes" }))}>Changes <span>{runtime?.workspace?.changedCount ?? 0}</span></button>
        <button className={currentUi.tab === "files" ? "is-active" : ""} onClick={() => updateUi((current) => ({ ...current, tab: "files" }))}>Files</button>
      </nav>
      <div className="files-list">
        {inventoryLoading && !files.length && !inventoryProgress && <span className="files-empty">Indexing workspace…</span>}
        {inventoryLoading && !files.length && inventoryProgress && <span className="files-progress">Loading {inventoryProgress.loaded.toLocaleString()} of {inventoryProgress.total.toLocaleString()} files…</span>}
        {!inventoryLoading && !visibleFiles.length && <span className="files-empty">{currentUi.tab === "changes" ? "No session changes" : "No files found"}</span>}
        {currentUi.tab === "changes" || currentUi.query.trim()
          ? visibleFiles.map((file) => <FileRow key={file.path} file={file} fullPath={Boolean(currentUi.query.trim())} selectedPath={currentUi.selectedPath} onSelect={(path) => selectFile(path, currentUi.tab === "changes" ? "diff" : "current")} />)
          : <FileTree files={visibleFiles} selectedPath={currentUi.selectedPath} onSelect={(path) => selectFile(path, "current")} />}
    ...
    {showExplorer && mobile && navigationOpen && <button className="sidebar-scrim" aria-label="Close explorer" onClick={onCloseNavigation} />}
      ...
      >
        <div className="file-workspace-main">
          <div className="file-workspace-editor-tabs" role="tablist" aria-label="Open files">
            {currentUi.openPaths.map((path) => <div key={path} className={currentUi.selectedPath === path ? "is-active" : ""}>
              <button type="button" role="tab" aria-selected={currentUi.selectedPath === path} onClick={() => selectOpenFile(path)}>
                <FileTypeIcon path={path} size={13} /><span>{path.split("/").at(-1) ?? path}</span>
              </button>
              <button type="button" onClick={() => closeFile(path)} aria-label={`Close ${path}`}>×</button>
            </div>)}
          </div>
          <section className="file-workspace-editor" aria-label="Open file">
            {currentUi.selectedPath ? <>
              <div className="file-viewer-toolbar">
                ...
                <span>
                  <button className={currentUi.view === "diff" ? "is-active" : ""} onClick={() => setSelectedView("diff")}>Diff</button>
                  <button className={currentUi.view === "current" ? "is-active" : ""} onClick={() => setSelectedView("current")}>Working copy</button>
                  {runtime?.workspace?.mode === "worktree" && <button className={currentUi.view === "base" ? "is-active" : ""} onClick={() => setSelectedView("base")}>Baseline</button>}
                  <button className="icon-button" onClick={() => closeFile(currentUi.selectedPath!)} aria-label="Close file"><IconX size={14} /></button>
                </span>
              </div>
              <FileContent value={viewerLoading && !visibleContent ? undefined : visibleContent} view={currentUi.view} targetLine={currentUi.selectedLine} onError={onError} />
