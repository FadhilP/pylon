...
export function useSessionCatalog(
  ...
  const [androidTooling, setAndroidTooling] =
    useState<HeliosAndroidToolingResult>();
  const [androidToolingBusy, setAndroidToolingBusy] = useState<
    "" | "install" | "remove"
  >("");
  ...
  const hooksLoading = useSessionResource(live, {
    ...
    stale:
      /session changed while listing hook settings|hook settings are stale/i,
    fallback: "Unable to load hook settings",
    reportError,
  });

  // Android tooling is only inspected while the dialog that shows it is open.
  const heliosActive = packages.some(
    (item) => item.id === "pi-helios" && item.active,
  );
  useEffect(() => {
    if (
      !settingsOpen ||
