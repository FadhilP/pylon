...
export type CachedHistory = Pick<
  ConversationReadModel,
  "messages" | "historyCursor" | "historyRemaining"
>;
...
function historyRange(messages: MessageReadModel[]): {
  start?: number;
  end?: number;
} {
...
export function mergeHistoryMessages(
  previous: MessageReadModel[],
  fresh: MessageReadModel[],
): MessageReadModel[] {
  const order = new Map<string, number>();
  const messages = [...previous, ...fresh];
  messages.forEach((message, index) => {
    const key = message.entryId
  ...
  return [
    ...
    const leftKey = left.entryId
      ? `entry:${left.entryId}`
      : `message:${left.id}`;
    const rightKey = right.entryId
      ? `entry:${right.entryId}`
      : `message:${right.id}`;
...
export function mergeHistorySegments(
  segments: MessageReadModel[][],
): MessageReadModel[] {
...
export function restoreCachedHistory(
  ...
  const hasGap =
    ...
    cachedEnd !== undefined &&
    cachedEnd + 1 < freshStart;
  const useCachedPaging =
    !hasGap &&
    (cached.historyCursor !== undefined || hasCompleteHistory(merged));
  return {
    ...runtime,
    conversation: {
      ...runtime.conversation,
      messages: merged,
      historyCursor: useCachedPaging
        ? cached.historyCursor
        : runtime.conversation.historyCursor,
      historyRemaining: useCachedPaging
        ? cached.historyRemaining
