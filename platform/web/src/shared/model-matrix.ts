...
function supportedLevels(
  ...
  const supported = new Set(
    model.thinkingLevels?.length ? model.thinkingLevels : ["off"],
  );
  return THINKING_LEVEL_ORDER.filter((level) => supported.has(level));
}

export function matrixThinkingAxis(
...
export function matrixSelectionAtPoint(
  ...
  return {
    modelIndex,
    model,
    level: nearestModelThinkingLevel(model, axis, targetIndex),
  };
}
...
export function moveMatrixSelection(
  ...
  | undefined {
  if (!models.length || !axis.length) return undefined;
  const nextModelIndex = Math.max(
    0,
    Math.min(models.length - 1, modelIndex + modelOffset),
  );
  ...
  const nextLevelIndex = Math.max(
    0,
    Math.min(available.length - 1, currentIndex + levelOffset),
  );
  return {
    modelIndex: nextModelIndex,
    model,
    level: available[nextLevelIndex] ?? "off",
  };
}
