...
export function parseFileReference(href: string): FileReference | undefined {
  ...
  if (
    ...
    return;
  if (
    /^(?:https?|mailto|ftp|data|javascript|file):/i.test(value) ||
  ...
  if (
    schemeLike &&
    (location !== suffix || !/[/.]/.test(value) || value.includes(":"))
  )
    return;
  ...
  if (
    line !== undefined &&
    (!Number.isSafeInteger(line) || line < 1 || line > MAX_LOCATION)
  )
    return;
  if (
    column !== undefined &&
    (!Number.isSafeInteger(column) || column < 1 || column > MAX_LOCATION)
  )
    return;
  return {
    path: value,
