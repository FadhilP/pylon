import type { DialogTimeoutSeconds, RuntimePolicyReadModel } from "./protocol/snapshots.ts";
...
export function runtimePolicySources(policy: RuntimePolicyReadModel): Record<"verify" | "timeline" | "guard" | "workspace" | "guardTimeout" | "clarifyTimeout", RuntimePolicySource> {
  const source = (sessionValue: unknown, projectValue: unknown): RuntimePolicySource =>
    sessionValue !== undefined ? "This session" : projectValue !== undefined ? "Project" : "Global";
  return {
    verify: policy.session.verify ? "This session" : "Project",
    timeline: source(policy.session.timelineEnabled, policy.project.timelineEnabled),
    ...
    guardTimeout: source(policy.session.guardTimeoutSeconds, policy.project.guardTimeoutSeconds),
    clarifyTimeout: source(policy.session.clarifyTimeoutSeconds, policy.project.clarifyTimeoutSeconds),
  };
}
