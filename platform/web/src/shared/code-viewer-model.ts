...
export interface ChangeRange { start: number; end: number }
...
export function parseDiff(text: string): DiffFile[] {
  try {
    return parsePatch(text).filter(patch => patch.oldFileName && patch.newFileName).map(patch => {
...
function markChangedWords(lines: CodeLine[]): void {
  for (let index = 0; index < lines.length;) {
    if (lines[index].kind !== "deletion") { index++; continue; }
...
export function loadDiffContents({ revision, base, current }: {
  revision: string;
  ...
  if (base.state !== "available" || current.state !== "available" || base.text === undefined || current.text === undefined)
...
export function validateDiffContents(file: DiffFile, contents: DiffContents): void {
  ...
  for (const hunk of file.hunks) {
    for (const line of hunk) {
      if ((line.oldLine !== undefined && oldLines[line.oldLine - 1] !== line.text) ||
          (line.newLine !== undefined && newLines[line.newLine - 1] !== line.text))
        throw new Error("Workspace changed while loading diff context");
    }
  }
  if (applyPatch(contents.oldFile.contents, file.patch, { autoConvertLineEndings: false }) !== contents.newFile.contents)
    throw new Error("Workspace changed while loading diff context");
}
...
export function diffRows(file: DiffFile, expanded: ExpandedContext = {}, contents?: DiffContents): DiffRow[] {
  ...
  const appendGap = (id: number, count?: number) => {
    ...
    const appendLine = (offset: number) => rows.push({
      kind: "context", text: current![newEnd + offset - 1], oldLine: oldEnd + offset, newLine: newEnd + offset,
    });
    for (let offset = 0; offset < start; offset++) appendLine(offset);
    const remaining = count === undefined ? undefined : count - start - end;
    if (remaining !== 0) rows.push({ kind: "gap", id, oldStart: oldEnd + start, newStart: newEnd + start, count: remaining });
  ...
  const completeFile = file.patch.isCreate || file.patch.isDelete || file.patch.oldFileName === "/dev/null" || file.patch.newFileName === "/dev/null";
  if (file.hunks.length && !completeFile) appendGap(file.hunks.length, current ? Math.max(0, current.length - newEnd + 1) : undefined);
  return rows;
}
...
export function selectedText(rows: readonly { kind: string; text?: string }[], start: number, end: number): string {
  return rows.slice(Math.min(start, end), Math.max(start, end) + 1)
    .filter(row => row.kind === "context" || row.kind === "addition" || row.kind === "deletion").map(row => row.text).join("\n");
}
