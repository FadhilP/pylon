...
export function finalAssistant(value: unknown): MessageReadModel | undefined {
  ...
  if (item.role !== "assistant" || typeof item.id !== "string" || !item.id || typeof item.text !== "string")
    return undefined;
  const message = {
    ...item,
    id: item.id.slice(0, 128),
    role: "assistant",
    text: item.text.slice(0, 60 * 1024),
    streaming: false,
  } as MessageReadModel;
...
export function reconcileFinalAssistant(
  messages: MessageReadModel[],
  assistant: MessageReadModel | undefined,
): MessageReadModel[] {
  if (!assistant) return messages;
  const matches = (message: MessageReadModel) =>
    message.id === assistant.id || Boolean(assistant.entryId && message.entryId === assistant.entryId);
