...
export type GuardRuleOverrides = Partial<
  Record<GuardRiskCategory, GuardAction>
>;
...
export function validGuardRules(
  value: unknown,
  complete = false,
): value is GuardRuleOverrides | EffectiveGuardRules {
...
export function resolveGuardRule(
  ...
  if (session[category])
    return { value: session[category], source: "This session" };
  if (project[category]) return { value: project[category], source: "Project" };
  return {
    value: global[category] ?? DEFAULT_GUARD_RULES[category],
    source: "Global",
  };
}
...
export const GUARD_RULE_DESCRIPTIONS: Record<GuardRiskCategory, string> = {
  ...
  "command.destructive-git-clean":
    "Git clean commands that remove untracked files.",
