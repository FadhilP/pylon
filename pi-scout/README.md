...
Calls are unlimited within one original user prompt. Every call uses a fresh child session and receives a redacted, bounded handoff from current parent context and tool intent, including log-free Verify and Timeline checkpoint metadata. `retryReason` supplies gap context; follow-ups must include relevant prior findings, new constraints, or parent-side findings because child-session history is never reused. Before a follow-up, the main model performs one bounded parent-side gap pass with targeted `fd`, `rg`, or narrow `read` over existing anchors when that can cheaply resolve gaps or sharpen the request. It combines related unresolved gaps and search criteria into one coherent follow-up instead of calling Scout once per finding, file, or question. It skips this pass when the prior report already identifies a specific broader trace requiring Scout.
