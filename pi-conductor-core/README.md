...
Then `/reload`. `/conductor` shows registered package policies plus latest bounded Guard decision. `/conductor doctor` also checks local Pi/Node compatibility, required and optional executables, old locks, quarantined state, configured child-model availability, and package tool surfaces without network calls. Guard remains independent final safety authority; Conductor never approves or weakens it.
