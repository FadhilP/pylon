...
export function childArgs(
  ...
  const excluded = [
    ...SPAWN_TOOLS,
    ...(agentPolicy?.disableSpecialists ? SPECIALIST_TOOLS : []),
  ];
  ...
  if (agentPolicy?.systemPrompt)
    args.push("--system-prompt", agentPolicy.systemPrompt);
  if (agentPolicy?.tools !== undefined) {
    if (agentPolicy.tools.length)
      args.push("--tools", agentPolicy.tools.join(","));
...
function spawnRuntimePolicy(
  ...
  return serialized !== undefined && Buffer.byteLength(serialized) <= 16 * 1024
    ? serialized
    : invalid;
}
...
export function createTurnRunner(pi: ExtensionAPI, runChild: RunChild) {
  return async function executeTurn(request: TurnRequest) {
    ...
    let thinking =
      kind === "agent"
        ? (policy as AgentPolicy | undefined)?.thinking
        : undefined;
    ...
    const onUiRequest = async (
      uiRequest: SpawnUiRequest,
      uiSignal: AbortSignal,
    ): Promise<SpawnUiResponse> => {
      if (background || !ctx.hasUI || uiRequest.method === "editor")
        return uiRequest.method === "confirm"
          ? { confirmed: false }
          : { cancelled: true };
      report(`${name} ${agentName} is waiting for input…`, {
        state: "attention",
        durationMs: elapsed(),
      });
      ...
      const dialogOptions = {
        signal: uiSignal,
        ...(uiRequest.timeout !== undefined
          ? { timeout: uiRequest.timeout }
          : {}),
      };
    ...
    try {
        ...
        async () => {
          ...
          return runChild(childArgs(kind, path, policy), {
            ...
            env: {
              ...
              PI_SPAWN_DEPTH: String(
                Number(process.env.PI_SPAWN_DEPTH ?? 0) + 1,
              ),
              PI_SPAWN_GUARD_POLICY: spawnRuntimePolicy(pi, cwd, id),
            },
            onUiRequest,
            onUsage: (usage) =>
              report(`${name} usage updated`, { durationMs: elapsed(), usage }),
            onText: (text) =>
              report("", { durationMs: elapsed(), partialResponse: text }),
            onState: (state) => {
            ...
            onActivity: (item, all) => {
              activity = all;
              report(`${name} activity: ${item.tool}`, {
                durationMs: elapsed(),
                activityDelta: [item],
              });
            },
      ...
      return {
        content: [
          { type: "text" as const, text: runText(kind, id, agentName, run) },
        ],
        details: {
          ...resultDetails(kind, id, path, cwd),
          runId,
          ...
          ...((run.thinking ?? thinking)
            ? { thinking: run.thinking ?? thinking }
            : {}),
          ...
          ...(run.error
            ? { failureCode: "child_error", failureMessage: run.error }
            : {}),
        },
        usage: {
