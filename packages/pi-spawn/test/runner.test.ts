...
const rpc = (
  ...
  state: typeof defaultState | null,
...
async function fake(
  body: string,
  stats: typeof defaultStats | null = defaultStats,
  state: typeof defaultState | null = defaultState,
) {
...
test("runner preserves every correlated tool event in one invocation", async () => {
  ...
  assert.deepEqual(
    run.activity.slice(-2).map(({ id, kind }) => ({ id, kind })),
    [
...
test("usage observer failures do not control the spawned child", async () => {
  const child = await fake(
    `if(command.type==='prompt'){emit({type:'message_end',message:{role:'assistant',content:[{type:'text',text:'done'}],stopReason:'stop',usage:{input:1}}});emit({type:'agent_settled'});setInterval(()=>{},1000);}`,
  );
  const run = await runSpawn([], {
    cwd: child.dir,
    prompt: "x",
