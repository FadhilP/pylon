...
- `continue` — requires `id` and `prompt`; creation policy cannot change. Set both `background: true` and `queue: true` to queue the continuation behind parent-owned work already running on that thread.
...
`create` and `continue` remain synchronous by default and wait for the child to settle. Set `background: true` only for independent work that should continue while the parent takes more turns; the start result returns `id` and `runId` immediately. A background `continue` still reports `busy` on overlap unless `queue: true` is also set, which creates a FIFO queued run that can be inspected or cancelled before it starts. Use `status` later to inspect a queued/running run or collect its final response and usage, or `cancel` to remove or stop it. Background runs and queues are runtime-local, reject interactive child dialogs, and are cancelled on session shutdown.
...
- `continue` — requires `id` and `prompt`; reopens the session in its recorded project. Set both `background: true` and `queue: true` to queue the continuation behind parent-owned work already running on that session.
- `status` — requires `id` and `runId`; returns the current state or collects a completed background result.
- `cancel` — requires `id` and `runId`; stops and collects a background run.
- `list` — lists sessions available from the active parent branch across projects.

`create`, `adopt`, and `continue` remain synchronous by default and wait for the child to settle. Set `background: true` only for independent work that should continue while the parent takes more turns; the start result returns `id` and `runId` immediately. A background `continue` still reports `busy` on overlap unless `queue: true` is also set, which creates a FIFO queued run that can be inspected or cancelled before it starts. Use `status` later to inspect a queued/running run or collect its final response and usage, or `cancel` to remove or stop it. Background runs and queues are runtime-local, reject interactive child dialogs, and are cancelled on session shutdown.
...
Each foreground prompt starts a fresh Pi RPC subprocess, waits for the agent to settle, returns the response and usage to the parent, then exits. A background prompt starts the same subprocess but returns a run reference immediately; a queued continuation waits for earlier parent-owned work on that thread, then starts its own subprocess. `status` or `cancel` later returns the terminal response and usage exactly once. The persisted child session is reopened for later prompts. Adoption records ownership before its first prompt, so the claim remains resumable if that prompt fails, is cancelled, or times out. Foreground child select, confirm, and input dialogs are shown through the invoking parent UI and their answers are returned only to that child; unavailable, malformed, unsupported, and all background dialogs cancel rather than hang.

Calls to different threads may run concurrently. pi-spawn rejects overlapping writes to the same thread within the current Pi/Pylon process unless a background continuation explicitly opts into the parent runtime's FIFO queue. Work active outside that queue still reports `busy`. Separate Pi processes cannot share that in-memory lock or queue, so do not adopt, open, or prompt a spawned session while it is active in another Pi process. Parent and child still share the workspace: use disjoint paths or separate worktrees for concurrent writes, then review changes before integration.
...
- Background runs and queued continuations: runtime-local, with up to 20 recent terminal results retained until collected or evicted; reload, session replacement, or exit cancels active and queued work.
