...
Active-result pruning defaults on. `/sieve active enable` and `/sieve active disable` save that decision for future runtimes. In global `enabled` mode, eligible text-only age-0 results strictly over the configured threshold are capped to that threshold including recovery metadata. Plain successful output retains balanced head and tail text; errors retain tail text. Ranked `symbol_search` and `code_search` output instead retains complete highest-ranked result objects and valid JSON. `relationship_graph` is preserved at ages 0 and 1 because pi-discover already bounds it structurally. Eligible plain source tools are `bash`, `grep`, `find`, `ls`, `rg`, `fd`, and `heartbeat_status`, plus `memory` list results. Short Heartbeat start/cancel results, Continuity updates, Memory mutations, `read`, and `index_status` remain excluded from size- and age-based pruning.
...
Text-only `read` results follow a separate conservative stale-read policy independent of the size threshold. An earlier successful read is replaced by a compact marker only when a successful `edit` or `write` to the same normalized path occurs in a later assistant message and a still-later successful read supersedes it. A post-mutation whole-file read always qualifies. A partial read qualifies only when it covers the earlier read's full line range and every intervening edit preserves line counts; writes and line-shifting edits require a whole-file reread. Same-assistant-message calls, failed mutations, mixed image results, malformed ranges, and reads without a covering post-mutation snapshot are preserved. Shell mutations are not inferred.
