...
Only old `bash`, `grep`, `find`, `ls`, `rg`, and `fd` results are eligible; `read` results are never changed. Successful results must contain only text blocks. Their age is the number of user messages after the result: at ages 2–5 the configured threshold applies, and at age 6+ it is halved (minimum 1,000 characters). Output strictly over its age-adjusted threshold is replaced with a compact marker containing the tool name and source character count; equality is retained.
...
Everything from the second-latest user message onward is preserved. If there are fewer than two user messages, nothing is transformed. Tool results are never deleted, and all non-content message fields remain intact.
