...
function oldResultAtAge(age: number, text: string, extra: Record<string, unknown> = {}) {
  return sieveMessages(
    [
      user("before"),
      textResult("bash", text, extra),
      ...Array.from({ length: age }, (_, index) => user(`after-${index}`)),
    ],
    4_000,
  );
}
...
test("projects only frozen Continuity v3 pairs without rewriting raw history", () => {
  ...
  assert.deepEqual(
    ...
    "reload rebuilds the same boundary",
  );
  assert.equal(
    continuityProjectionBoundary([
      ...retained,
...
test("fails open for uncertain Continuity pairs", () => {
  ...
  const cases: Array<{ messages: any[]; ids: string[] }> = [
    ...
    },
    {
      messages: [
        call("duplicate", "bash"),
        textResult("bash", source, { toolCallId: "duplicate", isError: false }),
        textResult("bash", source, { toolCallId: "duplicate", isError: false }),
      ],
      ids: ["duplicate"],
    },
...
test("restores protected historical results after baseline projection", () => {
  ...
  const messages = [
    ...
    call("mixed", "bash"),
    {
      ...textResult("bash", source, { toolCallId: "mixed", isError: false }),
      content: [{ type: "text", text: source }, image],
    },
    call("unsupported", "edit"),
    textResult("edit", source, { toolCallId: "unsupported", isError: false }),
    call("newest", "bash"),
...
test("partially retains old output and treats all text blocks as one source", () => {
  ...
  const content = [
    { type: "text", text: source.slice(0, 4_000) },
    { type: "text", text: source.slice(4_000) },
  ];
...
test("preserves age 0 and stored session messages", () => {
  const original = Object.freeze(
    textResult("fd", "x".repeat(SIEVE_THRESHOLD + 1), {
...
test("keeps recalls visible at age 0 then prunes eligible recalled output with age", () => {
  ...
  for (const untouched of [
    ...
    textResult("sieve_recall", "x".repeat(50_000), {
      isError: true,
      details: { found: true, sourceToolName: "bash", sourceIsError: false },
    }),
    textResult("sieve_recall", "x".repeat(50_000), { details: null }),
    textResult("sieve_recall", "x".repeat(50_000), { details: [] }),
    textResult("sieve_recall", "x".repeat(50_000), {
      details: { found: "yes", sourceToolName: "bash", sourceIsError: false },
    }),
    textResult("sieve_recall", "x".repeat(50_000), {
      details: { found: true, sourceToolName: 1, sourceIsError: false },
    }),
    textResult("sieve_recall", "x".repeat(50_000), {
      details: { found: true, sourceToolName: "bash", sourceIsError: "no" },
    }),
    textResult("sieve_recall", "x".repeat(50_000), {
...
test("partially prunes recoverable active results and registers full-result recovery", () => {
  ...
  const success = {
    ...
    content: [
      { type: "text", text: successText.slice(0, 2_001) },
      { type: "text", text: successText.slice(2_001) },
    ],
  };
...
test("generic duplicate indexing preserves first-match and unsupported-value behavior", () => {
  const call = (id: string, argumentsValue: Record<string, unknown>) => ({
    role: "assistant",
    content: [{ type: "toolCall", id, name: "rg", arguments: argumentsValue }],
  });
  ...
  const indexed = sieveMessages(
    [
      user("search"),
      call("rg-1", { pattern: "x", path: "." }),
      result("rg-1", { truncated: false }),
      call("rg-2", { path: ".", pattern: "x" }),
      result("rg-2", { truncated: false }),
      call("rg-3", { pattern: "x", path: "." }),
      result("rg-3", { truncated: false }),
    ],
    1_000,
    { pruneActive: true },
  );
  ...
  const unsupported = sieveMessages(
    [
      user("search"),
      call("date-1", { pattern: "x" }),
      result("date-1", { date }),
      call("date-2", { pattern: "x" }),
      result("date-2", { date: new Date(0) }),
      call("plain-1", { pattern: "y" }),
      result("plain-1", { truncated: false }),
      call("plain-2", { pattern: "y" }),
      result("plain-2", { truncated: false }),
    ],
    1_000,
    { pruneActive: true },
  );
  ...
  for (const [left, right] of [
    ...
    const edge = sieveMessages(
      [
        user("search"),
        call("edge-1", left),
        result("edge-1", { truncated: false }),
        call("edge-2", right),
        result("edge-2", { truncated: false }),
      ],
      1_000,
      { pruneActive: true },
    );
...
test("regression replay keeps stable and restored legacy history fixed during one tool turn", () => {
  ...
  for (let index = 0; index < 7; index++) {
    ...
    assert.deepEqual(
      stable.messages.slice(0, historicalLength),
      previousStable,
...
test("stable epochs diagnose source changes and taint later duplicate IDs without rewriting the original", () => {
  ...
  assert.equal(
    epoch.entries.get("collision")?.projectedMessage &&
      JSON.stringify(epoch.entries.get("collision")!.projectedMessage),
    frozen,
  );
  ...
  assert.equal(
    sourceEpoch.entries.get("collision")?.projectedMessage &&
      JSON.stringify(sourceEpoch.entries.get("collision")!.projectedMessage),
    frozen,
  );
...
test("runtime modes, persisted settings, active recall, thresholds, and telemetry", async () => {
  ...
  assert.match(
    notification,
    new RegExp(
      `actual net saved ~${expectedNetTokens} tokens; projected observe transformations 1; projected observe gross omitted ~${expectedGrossTokens} tokens; projected observe net saved ~${expectedNetTokens} tokens`,
    ),
  );
  ...
  assert.match(
    notification,
    new RegExp(
      `Threshold: > ~${Math.ceil(SIEVE_THRESHOLD / 4)} tokens \\(${SIEVE_THRESHOLD} JS characters; estimated at 4 characters/token\\)`,
    ),
  );
  ...
  const v2Source = textResult("bash", "v".repeat(oversizedLength), {
    toolCallId: "standard-v2-runtime",
    isError: false,
  });
  ...
  assert.equal(
    publishedStates.at(-1).stability.standardComparisons,
    comparisonsBeforeReset,
    "observe interruptions reset consecutive comparison state",
  );
  ...
  for (let index = 0; index < 9; index++) {
    rolloverMessages.push(
      textResult("bash", String(index).repeat(SIEVE_THRESHOLD + 1_000), {
        toolCallId: `runtime-roll-${index}`,
        isError: false,
      }),
    );
  }
  ...
  const recalledAgain = await recallTool.execute(
    "recall-call-2",
    { toolCallId: "active-runtime" },
    undefined,
    undefined,
  ...
  branch.splice(
    ...
    { id: "continuity-newest-result", type: "message", message: continuityNewest },
    {
      id: "continuity-v3",
      type: "compaction",
      firstKeptEntryId: "continuity-old-call",
      details: { type: "pi-continuity-compaction", version: 3 },
    },
  );
