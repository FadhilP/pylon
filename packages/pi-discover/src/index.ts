...
function excerpt(
  ...
  if (source.length <= MAX_EXCERPT_CHARS)
    return { line: bestLine + 1, text: source };
  ...
  const start = Math.max(
    0,
    Math.min(found - 80, source.length - MAX_EXCERPT_CHARS),
  );
  const prefix = start ? "…" : "";
  const suffix = start + MAX_EXCERPT_CHARS < source.length ? "…" : "";
  const text = source.slice(
    start,
    start + MAX_EXCERPT_CHARS - prefix.length - suffix.length,
  );
...
const PROJECTED_PATH =
  "CASE WHEN wr.prefix='' THEN f.path ELSE wr.prefix||'/'||f.path END";
...
function workspaceScope(workspaceId: number, scope: string, language?: string) {
  ...
  if (scope) {
    clauses.push(
      `(${PROJECTED_PATH}=? OR substr(${PROJECTED_PATH},1,length(?)+1)=?||'/')`,
    );
...
export class WorkspaceIndex {
  ...
  private ensureWorkspace(identity: RepositoryIdentity): void {
    ...
    try {
      ...
      const repoId = Number(
        (
          db
            .prepare("SELECT id FROM repositories WHERE root_key=?")
            .get(identity.rootKey) as { id: number }
        ).id,
      );
      db.prepare(
        `
      ...
      this.workspaceId = Number(
        (
          db
            .prepare("SELECT id FROM workspaces WHERE root_key=?")
            .get(identity.rootKey) as { id: number }
        ).id,
      );
      db.prepare(
        "INSERT OR IGNORE INTO workspace_repositories(workspace_id,repo_id,prefix) VALUES (?,?,?)",
      ...
      for (const repository of repositories) {
        ...
        const id = Number(
          (
            db
              .prepare("SELECT id FROM repositories WHERE root_key=?")
              .get(repository.rootKey) as { id: number }
          ).id,
        );
        db.prepare(
          "INSERT OR IGNORE INTO repository_states(repo_id) VALUES (?)",
        ).run(id);
  ...
  private apply(
    ...
    const findFile = db.prepare(
      "SELECT id, hash, dirty FROM files WHERE repo_id = ? AND path = ?",
    );
    const insertFile = db.prepare(
      "INSERT INTO files(repo_id,path,language,content,hash,size,dirty) VALUES (?,?,?,?,?,?,?)",
    );
    const updateFile = db.prepare(
      "UPDATE files SET language=?,content=?,hash=?,size=?,dirty=? WHERE id=?",
    );
    ...
    const insertFts = db.prepare(
      "INSERT INTO code_fts(rowid,content) VALUES (?,?)",
    );
    db.exec("BEGIN IMMEDIATE");
    try {
      const currentGeneration = Number(
        (
          db
            .prepare("SELECT generation FROM repository_states WHERE repo_id=?")
            .get(repoId) as { generation: number }
        ).generation,
      );
      ...
      for (const file of files) {
        const current = findFile.get(repoId, file.path) as
          { id: number; hash: string; dirty: number } | undefined;
        if (current?.hash === file.hash && current.dirty === Number(file.dirty))
          continue;
        let fileId: number;
        if (current) {
          fileId = current.id;
      ...
      db.prepare(
        "UPDATE repositories SET root=?,head=?,branch=?,indexed_at=? WHERE id=?",
      ).run(identity.root, identity.head, identity.branch, Date.now(), repoId);
      db.prepare(
        "UPDATE repository_states SET generation=generation+1 WHERE repo_id=?",
      ).run(repoId);
      db.exec("COMMIT");
      return true;
    } catch (error) {
  ...
  private async refreshRepository(
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      const snapshot =
        attempt === 0
          ? repository
          : await this.scanner.snapshotAt(repository.root);
      const state = db
        .prepare(
          `
      ...
      const full =
        forceFull || !state.indexed_at || state.head !== snapshot.head;
      const inventory = full
      ...
      const candidates =
        ...
        new Set([
          ...
          ...(
            db
              .prepare("SELECT path FROM files WHERE repo_id=? AND dirty=1")
      ...
      if (inventory) {
        ...
        for (const { path } of existing)
          if (!inventory.has(path)) removals.push(path);
      }
    ...
    throw new Error(
      `pi-discover repository changed repeatedly while indexing: ${repository.root}`,
    );
  }
  ...
  private publishWorkspace(
    ...
    const membershipState = createHash("sha256")
      .update(
        JSON.stringify(
          repositories
            .map(({ prefix, rootKey }) => [prefix, rootKey])
    ...
    try {
      ...
      db.prepare("DELETE FROM workspace_repositories WHERE workspace_id=?").run(
        this.workspaceId,
      );
      const insert = db.prepare(
        "INSERT INTO workspace_repositories(workspace_id,repo_id,prefix) VALUES (?,?,?)",
      );
      for (const repository of repositories)
        insert.run(
          this.workspaceId,
          ids.get(repository.rootKey)!,
          repository.prefix,
        );
      db.exec("COMMIT");
    } catch (error) {
  ...
  private async refreshNow(forceFull = false): Promise<void> {
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const physical = new Map(
        repositories.map((repository) => [repository.rootKey, repository]),
      );
      ...
      const latestSnapshot = await this.refreshRepository(
        ids.get(snapshot.rootKey)!,
        rootRepository,
        forceFull,
      );
    ...
    throw new Error(
      `pi-discover workspace changed repeatedly while indexing: ${this.scanner.root}`,
    );
  }
  ...
  prune(): Promise<{
    ...
    const next = this.pending.then(async () => {
      const db = this.database();
      const workspaces = db
        .prepare("SELECT id,root FROM workspaces")
        .all() as Array<{ id: number; root: string }>;
      const repositories = db
        .prepare("SELECT id,root FROM repositories")
        .all() as Array<{ id: number; root: string }>;
      const missingWorkspaces = (
        await Promise.all(
          workspaces.map(async (row) =>
            (await directoryExists(row.root)) ? undefined : row.id,
          ),
        )
      ).filter((id): id is number => id !== undefined);
      const missingRepositories = (
        await Promise.all(
          repositories.map(async (row) =>
            (await directoryExists(row.root)) ? undefined : row.id,
          ),
        )
      ).filter((id): id is number => id !== undefined);
      ...
      try {
        ...
        for (const id of missingWorkspaces)
          removedWorkspaces += Number(removeWorkspace.run(id).changes);
        const countFiles = db.prepare(
          "SELECT count(*) AS count FROM files WHERE repo_id=?",
        );
        const removeFts = db.prepare(
          "DELETE FROM code_fts WHERE rowid IN (SELECT id FROM files WHERE repo_id=?)",
        );
        const removeRepository = db.prepare(
        ...
        db.exec("COMMIT");
        if (this.workspaceId && missingWorkspaces.includes(this.workspaceId))
  ...
  private scopedPath(cwd: string, input?: string): string {
    ...
    if (escapesRoot(this.scanner.root, absolute))
  ...
  async searchSymbols(
    ...
    if (!query)
      throw new Error("Symbol query must contain a non-whitespace token");
    const limit = options.limit ?? DEFAULT_SYMBOL_RESULTS;
    const target = limit + 1;
    const scope = this.scopedPath(cwd, options.path);
    const { clauses: commonClauses, args: commonArgs } = workspaceScope(
      this.workspaceId!,
      scope,
      options.language,
    );
    ...
    const stages = [
      ...
      {
        clause:
          "s.name LIKE ? ESCAPE '\\' COLLATE NOCASE AND s.name<>? COLLATE NOCASE",
        args: [`${escapeLike(query)}%`, query],
      },
      {
        clause:
          "instr(lower(s.name),lower(?))>0 AND s.name NOT LIKE ? ESCAPE '\\' COLLATE NOCASE",
    ...
    for (const stage of stages) {
      ...
      `,
        ...
        .all(...commonArgs, ...stage.args, target - rows.length) as Array<
    ...
    const selected = rows.slice(0, limit) as RankedRows<
      Record<string, unknown>
    >;
  ...
  async searchCode(
    ...
    if (!sourceQuery)
      throw new Error("Code query must contain a non-whitespace token");
    const limit = options.limit ?? DEFAULT_CODE_RESULTS;
    const scope = this.scopedPath(cwd, options.path);
    const query = ftsQuery(sourceQuery);
    const { clauses, args } = workspaceScope(
      this.workspaceId!,
      scope,
      options.language,
    );
...
export function registerIndexTools(
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Search indexed repository symbols by name, kind, language, or path",
    promptGuidelines: [
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      const displayed = results.map(({ name, kind, path, line }) => ({
        name,
        kind,
        path,
        line,
      }));
      const text = structuredResults(
        { heuristic: true },
        displayed,
        maxBytes,
      ...
      return {
        ...
        details: {
          ...
          returned: JSON.parse(text).returned ?? 0,
          heuristic: true,
        },
      };
    },
  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      const displayed = results.map(({ path, line, text }) => ({
        path,
        line,
        text,
      }));
  ...
  pi.registerTool({
    ...
    label: "Index status",
    description:
      "Report local pi-discover SQLite index status for the current repository.",
