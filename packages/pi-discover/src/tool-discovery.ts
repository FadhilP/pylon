...
export type ToolDiscoveryResult = { error?: string; selected?: string[]; blocked?: string[] };
...
export function keywordRankTools(tools: readonly ToolMetadata[], query: string, limit = 3): ToolMetadata[] {
  ...
  return tools
    .map(tool => {
      ...
      const structured = [...(tool.aliases ?? []), ...(tool.capabilities ?? [])].map(normalizedQuery).filter(Boolean);
      const rank = [
...
export function rankInactiveTools(
  ...
  return keywordRankTools(
    tools.filter(tool => tool.name !== "search_tools" && !active.has(tool.name)),
    query,
    limit,
  );
}
...
export function createToolDiscovery(pi: ExtensionAPI): ToolDiscovery {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object(
      {
        ...
        query: Type.Optional(
          Type.String({
            minLength: 1,
            maxLength: 200,
            description: "Keywords to match against inactive tool names, advertised usages, and descriptions",
          }),
        ),
        limit: Type.Optional(
          Type.Integer({ minimum: 1, maximum: 6, description: "Maximum matching tools to activate; default 3" }),
        ),
      },
    ...
    async execute(
      ...
      if (!capability)
        return reply("Pylon tool coordination is unavailable; no tools were activated.", {
      ...
      if ((params.action ?? "search") === "reset") {
        ...
        clearTurnState();
        selectedTools.clear();
        return reply("Pylon tool selection reset. Definitions update next model turn.", {
          action: "reset",
          coordinator: reset,
        });
      }
      ...
      if (!queryKey)
        return reply("Provide query keywords to search inactive tools.", { action: "search", matches: [] });
      metrics.searches++;
      ...
      if (!names.length) {
        ...
        return reply(`No eligible inactive tools matched.${cacheHit ? " Already searched this tool inventory." : ""}`, {
          action: "search",
          matches: [],
      ...
      return reply(`${summary}${blockedSummary} Callable definitions update next model turn.`, {
        action: "search",
        matches: names,
  ...
  return {
    ...
    guidanceFor(event: any) {
      if (!event.systemPromptOptions?.selectedTools?.includes("search_tools")) return undefined;
      const capability = discoveryCapability(pi);
