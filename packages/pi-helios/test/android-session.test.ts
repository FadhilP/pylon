...
function harness() {
  ...
  const emulator = {
    ...
    avd: "Pixel_Test",
    async stop() {
      log.push("emulator-stop");
    },
    async cleanupUncertainStart() {
      log.push("emulator-cleanup");
    },
  };
  const sdk = {
    async start() {
      log.push("emulator-start");
      return emulator as any;
    },
    async verifyAttached(serial: string) {
      log.push("verify-attach");
      return { serial, avd: "Pixel_External" };
    },
  };
  const server = {
  ...
  const client = {
    ...
    async createSession(capabilities: Record<string, unknown>) {
      log.push(`session:${capabilities["appium:udid"]}`);
      return (this.sessionId = "session-1");
    },
    async deleteSession() {
    ...
    async fillElement(_id: string, text: string) {
      log.push(`fill:${text}`);
    },
    async back() {
      log.push("back");
  ...
  const manager = new AndroidSessionManager(exec as any, {
    acquireToolingLease: async () => {
      log.push("tooling-acquire");
      return async () => {
        log.push("tooling-release");
      };
    },
...
test("cleanup attempts all owned resources and retains uncertain server cleanup", async () => {
  ...
  first.client.deleteSession = async () => {
    first.log.push("session-delete-failed");
  ...
  second.server.stop = async () => {
    second.log.push("server-stop-retry");
  };
...
test("Android actions recheck package, require full bounds, and invalidate refs after partial fill", async () => {
  ...
  third.client.fillElement = async () => {
...
test("Android find refs remain actionable beyond rendered snapshot limits", async () => {
  ...
  const nodes = Array.from(
    { length: 260 },
    (_, index) =>
      `<android.widget.TextView package="com.example.app" class="android.widget.TextView" text="${index === 259 ? "Target Late" : `Item ${index}`}" clickable="true" enabled="true" bounds="[0,0][100,100]"/>`,
  ).join("");
