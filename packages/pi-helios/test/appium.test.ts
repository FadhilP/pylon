...
test("Appium client speaks bounded W3C lifecycle and interaction endpoints", async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.url === "/session" && request.method === "POST")
      return response.end(JSON.stringify({ value: { sessionId: "s1", capabilities: {} } }));
  ...
  try {
    ...
    assert.ok(
      calls.some(call => call.url === "/session/s1/actions" && call.body.actions[0].parameters.pointerType === "touch"),
    );
...
test("Appium status requires an explicit ready true", async () => {
  const server = createServer((_request, response) =>
    response.end(JSON.stringify({ value: { ready: false, message: "starting" } })),
  );
  server.listen(0, "127.0.0.1");
  await once(server, "listening");
  const address = server.address();
