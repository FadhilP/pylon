...
test("Appium client speaks bounded W3C lifecycle and interaction endpoints", async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.url === "/session/s1/appium/device/current_package")
      return send("com.example.app");
    if (request.url === "/session/s1/source") return send("<hierarchy/>");
    if (request.url === "/session/s1/screenshot")
      return send(PNG.toString("base64"));
    if (request.url === "/session/s1/window/rect")
      return send({ width: 1080, height: 1920 });
    if (request.url === "/session/s1/element")
      return send({ "element-6066-11e4-a52e-4f735466cecf": "e1" });
    return send(null);
  });
  ...
  try {
    ...
    assert.ok(
      calls.some(
        (call) =>
          call.url === "/session/s1/actions" &&
          call.body.actions[0].parameters.pointerType === "touch",
      ),
    );
    assert.ok(
      calls.some(
        (call) =>
          call.url === "/session/s1/element/e1/value" &&
          call.body.text === "hello",
      ),
    );
...
test("Appium client rejects non-loopback and path-bearing endpoints", () => {
  ...
  assert.throws(
    () => new AppiumClient("http://127.0.0.1:4723/wd/hub"),
    /origin/,
  );
});
...
test("Appium status requires an explicit ready true", async () => {
  const server = createServer((_request, response) =>
    response.end(
      JSON.stringify({ value: { ready: false, message: "starting" } }),
    ),
  );
  server.listen(0, "127.0.0.1");
  await once(server, "listening");
  ...
  try {
    await assert.rejects(
      new AppiumClient(`http://127.0.0.1:${address.port}`).status(),
      /not ready/,
    );
  } finally {
    server.close();
...
test(
  "Windows Appium resolution invokes npm-cli.js through Node without npm.cmd",
  ...
  async () => {
    ...
    try {
      ...
      await writeFile(
        join(appiumDirectory, "package.json"),
        JSON.stringify({ bin: { appium: "build/main.js" } }),
      );
      ...
      const invocation = await resolveAppium(exec, undefined, {
        npm_execpath: npmCli,
        PATH: "",
      });
      ...
      assert.ok(
        calls.every(({ command }) => command.toLowerCase() !== "npm.cmd"),
      );
      ...
      await assert.rejects(
        resolveAppium(exec, undefined, { npm_execpath: join(root, "npm.cmd") }),
        /absolute npm-cli\.js/,
      );
      await assert.rejects(
        resolveAppium(
      ...
      await writeFile(
        join(appiumDirectory, "package.json"),
        JSON.stringify({ bin: { appium: "../escape.js" } }),
      );
      await assert.rejects(
        resolveAppium(exec, undefined, { npm_execpath: npmCli, PATH: "" }),
...
test("explicit APPIUM_PATH bypasses npm discovery", async () => {
  ...
  try {
    ...
    const calls: string[][] = [];
    const invocation = await resolveAppium(
      async (command, args) => {
...
test("managed Appium refuses to fall back while recovery is incomplete", async () => {
  ...
  try {
    await mkdir(join(root, "pi-helios", "android-tooling", "previous"), {
      recursive: true,
    });
