...
test("persistent client falls back only before dispatch", async () => {
  ...
  const before = await PlaywrightCli.create(exec, {
    ...
    clientFactory: async () => ({
      async run() {
        throw new PlaywrightClientError("unavailable", false, "not dispatched");
      },
      async dispose() {},
    }),
  });
  ...
  const after = await PlaywrightCli.create(exec, {
    ...
    clientFactory: async () => ({
      async run() {
        throw new PlaywrightClientError("unavailable", true, "uncertain");
      },
      async dispose() {},
    }),
  });
...
test("file-backed snapshots reject unsafe or invalid artifacts without leaking paths", async t => {
  ...
  const cli = await PlaywrightCli.create(async () => ({
    code: 0,
    stdout: JSON.stringify({ snapshot: payload }),
    stderr: "",
    killed: false,
  }));
...
test("malformed top-level snapshots do not fall back to valid nested snapshots", async () => {
  const cli = await PlaywrightCli.create(async () => ({
    ...
    stdout: JSON.stringify({
      snapshot: { file: "ignored.yml", extra: true },
      result: { snapshot: "- button [ref=e1]" },
    }),
    stderr: "",
    killed: false,
  }));
...
test("adapter safely identifies missing browser stderr", async () => {
  ...
  await assert.rejects(
    ...
    (error: any) => {
      ...
      assert.equal(
        error.message,
        "Playwright browser is not installed; run the matching `playwright-cli install-browser` setup command",
      );
  ...
  for (const stderr of [
    "",
    "Error: launch failed for token=secret-value",
...
test("adapter maps timeout and cancellation without leaking subprocess details", async () => {
  const timed = await PlaywrightCli.create(async () => ({
    code: 1,
    stdout: "{}",
  ...
  let invoked = false;
  const cancelled = await PlaywrightCli.create(async () => {
    invoked = true;
    return { code: 0, stdout: "{}", stderr: "", killed: false };
  });
  ...
  controller.abort();
  await assert.rejects(
    cancelled.run(SESSION, { kind: "reload" }, controller.signal),
    (error: any) => error.category === "cancelled",
  );
