...
async function populate(directory: string) {
  ...
  await writeFile(
    ...
    JSON.stringify({ version: MANAGED_APPIUM_VERSION, bin: { appium: "bin/appium.js" } }),
  );
...
test("managed Android tooling stages, verifies, repairs, and removes safely", async () => {
  ...
  try {
        ...
        return {
          stdout: JSON.stringify({
            uiautomator2: {
              ...
              installPath: join(options.cwd, "node_modules", "appium-uiautomator2-driver"),
            },
          }),
        };
      },
    ...
    await writeFile(
      join(current, "node_modules", "appium-uiautomator2-driver", "package.json"),
