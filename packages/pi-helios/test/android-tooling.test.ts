  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      dependencies: {
        ...
        "appium-uiautomator2-driver": MANAGED_UIAUTOMATOR2_VERSION,
      },
    }),
  );
  await writeFile(
...
async function populate(directory: string) {
  ...
  await writeFile(
    join(appium, "package.json"),
    JSON.stringify({
      version: MANAGED_APPIUM_VERSION,
      bin: { appium: "bin/appium.js" },
    }),
  );
  await writeFile(join(appium, "bin", "appium.js"), "// fixture");
  await writeFile(
    join(driver, "package.json"),
    JSON.stringify({ version: MANAGED_UIAUTOMATOR2_VERSION }),
  );
}
...
test("managed Android tooling stages, verifies, repairs, and removes safely", async () => {
  ...
  try {
    ...
    const manager = new AndroidToolingManager({
      ...
      run: async (_command, args, options) => {
        ...
        if (args.includes("--version"))
          return { stdout: `${MANAGED_APPIUM_VERSION}\n` };
        return {
          stdout: JSON.stringify({
            uiautomator2: {
              installed: true,
              pkgName: "appium-uiautomator2-driver",
              version: MANAGED_UIAUTOMATOR2_VERSION,
              installPath: join(
                options.cwd,
                "node_modules",
    ...
    assert.ok(
      calls[0].includes("--userconfig") && calls[0].includes("--globalconfig"),
    );
    assert.notEqual(
      calls[0][calls[0].indexOf("--userconfig") + 1],
      calls[0][calls[0].indexOf("--globalconfig") + 1],
    );
    ...
    const invocation = await resolveManagedAppiumAt(current, {
      PYLON_TEST_SECRET: "hidden",
      ANDROID_SDK_ROOT: join(root, "sdk"),
    });
    ...
    await writeFile(
      join(current, "node_modules", "appium", "package.json"),
      JSON.stringify({ version: "0.0.0", bin: { appium: "bin/appium.js" } }),
    );
    assert.equal(
      (await manager.status()).state,
      "invalid",
      "tampered Appium package version is rejected",
    );
    ...
    assert.equal(
      (await manager.status()).state,
      "invalid",
      "tampered driver package version is rejected",
    );
    await populate(current);
    ...
    assert.equal(
      (await manager.status()).state,
      "ready",
      "failed repair preserves current tooling",
    );
...
test("managed Android tooling reports an existing operation lock", async () => {
  ...
  try {
    ...
    const templateDirectory = await fixture(root);
    await mkdir(
      join(agentDir, "pi-helios", "android-tooling", "operation.lock"),
      { recursive: true },
    );
...
test("managed Android tooling locks cannot be stolen and usage leases block mutation", async () => {
  ...
  try {
    ...
    const first = new AndroidToolingManager({
      agentDir,
      templateDirectory,
      run: run as any,
    });
    const second = new AndroidToolingManager({
      agentDir,
      templateDirectory,
      run: run as any,
    });
    ...
    await assert.rejects(
      second.remove(0),
      /Close active Helios Android sessions/,
    );
    await release();
    ...
    assert.equal(
      (await second.install()).state,
      "ready",
      "dead foreign-process leases are reclaimed",
    );
