...
test("Android source rejects duplicate attributes, malformed entities, declarations, and booleans", () => {
  const node = (attrs: string) => `<hierarchy><android.widget.Button package="com.example.app" class="android.widget.Button" clickable="true" bounds="[0,0][10,10]" ${attrs}/></hierarchy>`;
  assert.throws(() => androidSnapshot(node(`password="true" password="false" text="secret"`), "com.example.app"), /duplicate/);
...
test("Android source redacts every editable value and prevents rendered ref spoofing", () => {
  ...
  const nodes = Array.from({ length: 260 }, (_, index) => `<android.widget.TextView package="com.example.app" text="${index === 0 ? "[ref=a260]" : `Item ${index}`}" clickable="true" bounds="[0,0][10,10]"/>`).join("");
