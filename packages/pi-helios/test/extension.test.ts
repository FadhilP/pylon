      ...
      events: {
        on(name: string, handler: Function) {
          ...
          return () =>
            eventHandlers.set(
              name,
              (eventHandlers.get(name) ?? []).filter(item => item !== handler),
            );
        },
...
function successfulLookup() {
  return {
    code: 0,
    stdout: JSON.stringify({ Handle: WINDOW.handle, ProcessId: WINDOW.processId, Title: WINDOW.title }),
    stderr: "",
    killed: false,
  };
}
...
test("Android tool rejects cross-action fields and unsupported screenshots before backend access", async () => {
  ...
  await assert.rejects(
    tools
      .get("helios_android")
      .execute("missing-package", { action: "start", avd: "Pixel_Test" }, undefined, undefined, ctx),
    /requires appPackage/,
  );
  await assert.rejects(
    tools
  ...
  await assert.rejects(
    tools
      .get("helios_android")
      .execute("invalid-package-serial", { action: "packages", serial: "emulator-5555" }, undefined, undefined, ctx),
    /even console port/,
  );
  await assert.rejects(
    tools
      .get("helios_android")
      .execute("invalid-find", { action: "find", text: "Next", avd: "Pixel_Test" }, undefined, undefined, ctx),
    /does not accept avd/,
  );
  await assert.rejects(
    tools
...
test("embedded panel passively mirrors an agent-owned browser without taking control", async () => {
  ...
  await tools
    .get("helios_browser")
    .execute("start", { action: "start", url: "https://example.com" }, undefined, undefined, ctx);
  ...
  await tools
    .get("helios_browser")
    .execute("navigate", { action: "navigate", url: "https://example.com/next" }, undefined, undefined, ctx);
...
test("Web Scout falls back to bounded redacted text for raw public documents", async () => {
  ...
  await webScoutBrowser(
    {
      exec: async (_command: string, args: string[]) => {
        ...
        if (action === "tab-list")
          return {
            code: 0,
            stdout: JSON.stringify({ result: "- 0: (current) [Changelog](https://example.com/CHANGELOG.md)" }),
            stderr: "",
            killed: false,
          };
        if (action === "goto")
          return {
            code: 0,
            stdout: JSON.stringify({ snapshot: { file: "../invalid-snapshot" } }),
            stderr: "",
            killed: false,
          };
        if (action === "eval")
  ...
  const result = await tools
    .get("scout_browser")
    .execute("raw", { action: "navigate", url: "https://example.com/CHANGELOG.md" });
...
test("Web Scout does not consume page quota when navigation and text fallback fail", async () => {
  ...
  await webScoutBrowser(
    {
      exec: async (_command: string, args: string[]) => {
        ...
        if (action === "goto")
          return { code: 0, stdout: JSON.stringify({ snapshot: "- heading Success" }), stderr: "", killed: false };
        if (action === "eval")
          return {
            code: 0,
            stdout: JSON.stringify({
              result: JSON.stringify({ contentType: "text/html", text: "not an allowed fallback", truncated: false }),
            }),
            stderr: "",
            killed: false,
          };
  ...
  const browser = tools.get("scout_browser");
  await assert.rejects(
    browser.execute("failed", { action: "navigate", url: "https://example.com/first" }),
  ...
  await assert.rejects(
    browser.execute("limited", { action: "navigate", url: "https://example.com/third" }),
    /page limit/,
  );
  assert.equal(navigations, 2);
  for (const handler of handlers.get("session_shutdown") ?? []) await handler();
...
test("Web Scout child cleans proxy after browser start failure", async () => {
  ...
  await assert.rejects(
    tools.get("scout_browser").execute("id", { action: "navigate", url: "https://example.com" }),
    /command failed/i,
  );
  for (const handler of handlers.get("session_shutdown") ?? []) await handler();
});
...
test("browser find returns targeted refs usable by later actions", async () => {
  ...
  const { tools, handlers } = runtime({
    exec: async (_command: string, args: string[]) => {
      ...
      if (command === "tab-list")
        return {
          code: 0,
          stdout: JSON.stringify({ result: "- 0: (current) [Shop](https://example.com/)" }),
          stderr: "",
          killed: false,
        };
      if (command === "find")
        return {
          code: 0,
          stdout: JSON.stringify({ result: 'Found 21 matches for "Add to cart":\n\n- button "Add to cart" [ref=e9]' }),
          stderr: "",
          killed: false,
        };
      if (command === "click")
  ...
  await assert.rejects(
    browser.execute("invalid", { action: "find", text: "cart", regex: "cart" }, undefined, undefined, ctx),
    /exactly one/,
  );
...
test("browser semantic plan resolves unique elements and hides intermediate find output", async () => {
  ...
  const { tools, handlers } = runtime({
    exec: async (_command: string, args: string[]) => {
      calls.push(args);
      const command =
        args.find(value => ["open", "find", "fill", "click", "tab-list", "close"].includes(value)) ?? "unknown";
      if (command === "tab-list")
        return {
          code: 0,
          stdout: JSON.stringify({ result: "- 0: (current) [Form](https://example.com/)" }),
          stderr: "",
          killed: false,
        };
      if (command === "find") {
  ...
  const { tools } = runtime({
    exec: async (_command: string, args: string[]) => {
      ...
      if (command === "find") {
        ...
        return {
          code: 0,
          stdout: JSON.stringify({ result: 'Found 1 match for "Email":\n\n- textbox "Email" [ref=e2]' }),
          stderr: "",
          killed: false,
        };
      }
...
test("browser continue pages cached output without another CLI command and replaces refs", async () => {
  ...
  const { tools, handlers } = runtime({
    exec: async (_command: string, args: string[]) => {
      const command =
        args.find(value => ["open", "snapshot", "click", "tab-list", "close"].includes(value)) ?? "unknown";
      commands.push(command);
      if (command === "tab-list")
        return {
          code: 0,
  ...
  await assert.rejects(
    browser.execute("stale-ref", { action: "click", target: "e1" }, undefined, undefined, ctx),
    /stale/,
  );
...
test("browser snapshot mode defaults compact and full restores container targets", async () => {
  ...
  await assert.rejects(
    browser.execute("removed", { action: "snapshot", target: "e1" }, undefined, undefined, ctx),
    /stale/,
  );
...
test("browser batch compacts intermediate snapshots and metadata but keeps final output and images", async () => {
  ...
  const { tools, handlers } = runtime({
    exec: async (_command: string, args: string[]) => {
      const command =
        args.find(value => ["open", "goto", "reload", "screenshot", "snapshot", "tab-list", "close"].includes(value)) ??
        "unknown";
      commands.push(command);
      if (command === "tab-list") {
...
test("browser batch keeps intermediate warnings while omitting truncated snapshot text", async () => {
  const raw = [
    ...
    ...Array.from({ length: 100 }, (_, index) => `- button Item ${index} [ref=f1e${index + 2}]`),
  ].join("\n");
  let tabLists = 0;
  const { tools } = runtime({
    exec: async (_command: string, args: string[]) => {
      ...
      if (command === "open")
        return { code: 0, stdout: JSON.stringify({ snapshot: "- button Submit [ref=e1]" }), stderr: "", killed: false };
...
test("browser batch rejects mixed input and stops after declined attachment", async () => {
  ...
  await assert.rejects(
    browser.execute("mixed", { action: "start", actions: [{ action: "start" }] }, undefined, undefined, context()),
    /only actions/,
  );
  ...
  const { tools, handlers } = runtime({
    exec: async (_command: string, args: string[]) => {
      ...
      if (cliCommand === "list")
        return {
          code: 0,
          stdout: JSON.stringify({ browsers: [{ name: session, status: "open" }] }),
          stderr: "",
          killed: false,
        };
      if (cliCommand === "tab-list")
