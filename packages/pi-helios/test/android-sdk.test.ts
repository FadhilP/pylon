...
test("Android SDK exposes only emulator attachment identities", async () => {
  ...
  assert.deepEqual(await sdk.verifyAttached("emulator-5554"), {
    serial: "emulator-5554",
    avd: "Pixel_Test",
  });
  await assert.rejects(
    sdk.verifyAttached("physical-1"),
    /requires an emulator serial/,
  );
  await assert.rejects(
    sdk.verifyAttached("emulator-5555"),
    /even console port/,
  );
  assert.ok(
    calls.some((args) => args.join(" ") === "-s emulator-5554 emu avd name"),
  );
});
...
test("installed package inventory verifies identity and uses one fixed bounded adb query", async () => {
  ...
  assert.ok(
    execCalls.some(
      (args) => args.join(" ") === "-s emulator-5554 emu avd name",
    ),
  );
});
...
test("installed package parsing and capture fail closed", async () => {
  assert.deepEqual(
    parseInstalledPackages("package:com.zed\r\npackage:android\r\n"),
    ["android", "com.zed"],
  );
  ...
  const maximum = Array.from(
    { length: 4_096 },
    (_, index) => `package:com.package${index}`,
  ).join("\n");
  assert.equal(parseInstalledPackages(maximum).length, 4_096);
  assert.throws(
    () => parseInstalledPackages(`${maximum}\npackage:com.package4096`),
    /exceeds 4096 packages/,
  );
  assert.deepEqual(
    parseInstalledPackages(
      Array.from({ length: 4_097 }, () => "package:com.same").join("\n"),
    ),
    ["com.same"],
  );
  const exec = async (_command: string, args: string[]) =>
  ...
  await assert.rejects(
    sdk.listInstalledPackages("emulator-5554"),
    /exceeds 128KB/,
  );
  ...
  await assert.rejects(
    abortSdk.listInstalledPackages("emulator-5554", activeController.signal),
    /cancelled/,
  );
  ...
  controller.abort();
  await assert.rejects(
    sdk.listInstalledPackages("emulator-5554", controller.signal),
    /cancelled/,
  );
});
