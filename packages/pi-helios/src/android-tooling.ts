...
async function runCommand(command: string, args: string[], options: RunOptions): Promise<{ stdout: string }> {
  ...
  child.stdout?.on("data", (data: Buffer) => { stdout = tail(stdout, data); });
  child.stderr?.on("data", (data: Buffer) => { stderr = tail(stderr, data); });
  ...
  if (child.exitCode !== 0) throw new Error(`Android tooling setup failed: ${(stderr || stdout).replace(/[\r\n]+/g, " ").trim().slice(-500) || "no diagnostic output"}`);
  return { stdout };
}
...
export class AndroidToolingManager {
  ...
  constructor(options: { agentDir?: string; templateDirectory?: string; run?: RunCommand; env?: NodeJS.ProcessEnv } = {}) {
  ...
  private async ensureRoot(): Promise<void> {
    ...
    if (relative(agent, root) !== join("pi-helios", "android-tooling")) throw new Error("Android tooling root resolves outside the Pylon agent directory");
  ...
  private async claimLock(lock: string): Promise<void> {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        try { await writeFile(join(lock, "owner.json"), `${JSON.stringify({ pid: process.pid })}\n`, { mode: 0o600 }); }
  ...
  private paths() {
    ...
    return { root, current, previous: join(root, "previous"), lock: join(root, "operation.lock"), usage: join(root, "usage") };
  }

  private async activeUsageLeases(): Promise<number> {
    const { usage } = this.paths();
    try { await mkdir(usage, { mode: 0o700 }); }
    catch (error) { if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error; }
    const directory = await requireOwnedDirectory(usage, "Android tooling usage directory");
    let active = 0;
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      const candidate = join(directory, entry.name);
      if (!entry.isDirectory() || entry.isSymbolicLink()) { active++; continue; }
      let stale = false;
      try {
      ...
      try { await rename(candidate, abandoned); }
      catch (error) {
  ...
  async acquireUsageLease(): Promise<() => Promise<void>> {
    ...
    if (await exists(lock)) throw new Error("Android tooling setup is running; wait before starting an Android session");
    try { await mkdir(usage, { mode: 0o700 }); }
    catch (error) { if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error; }
    ...
    try { await writeFile(join(lease, "owner.json"), `${JSON.stringify({ pid: process.pid })}\n`, { mode: 0o600 }); }
    catch (error) { await rm(lease, { recursive: true, force: true }).catch(() => {}); throw error; }
  ...
  async status(): Promise<AndroidToolingStatus> {
    ...
    if (this.mutating || await exists(lock)) return { state: "busy", appiumVersion: MANAGED_APPIUM_VERSION, driverVersion: MANAGED_UIAUTOMATOR2_VERSION };
    if (!await exists(current) && await exists(previous)) return { state: "invalid", appiumVersion: MANAGED_APPIUM_VERSION, driverVersion: MANAGED_UIAUTOMATOR2_VERSION, message: "Managed Android tooling recovery is incomplete. Use Repair to restore it." };
    try {
      ...
      return { state: "invalid", appiumVersion: MANAGED_APPIUM_VERSION, driverVersion: MANAGED_UIAUTOMATOR2_VERSION, message: "Managed Android tooling is invalid or incomplete. Use Repair to replace it." };
    }
  }
  ...
  private async acquire(): Promise<() => Promise<void>> {
    ...
    try {
      ...
      claimed = true;
      if (await this.activeUsageLeases()) throw new Error("Close active Helios Android sessions before changing managed tooling");
      return async () => {
  ...
  private async recover(): Promise<void> {
    ...
    for (const entry of await readdir(root, { withFileTypes: true })) {
      if (!entry.name.startsWith("stage-") && !entry.name.startsWith("removed-") && !entry.name.startsWith("operation.lock.stale-")) continue;
  ...
  async install(activeSessions = 0, signal?: AbortSignal): Promise<AndroidToolingStatus> {
    ...
    try {
      ...
      await this.recover();
      const template = JSON.parse(await readFile(join(this.templateDirectory, "package.json"), "utf8")) as { dependencies?: Record<string, string> };
      if (template.dependencies?.appium !== MANAGED_APPIUM_VERSION || template.dependencies?.["appium-uiautomator2-driver"] !== MANAGED_UIAUTOMATOR2_VERSION) throw new Error("Bundled Android tooling versions are invalid");
      await mkdir(stage, { mode: 0o700 });
      for (const file of ["package.json", "package-lock.json"]) {
        await copyFile(join(this.templateDirectory, file), join(stage, file));
        await chmod(join(stage, file), 0o600).catch(() => {});
      }
      const npm = process.platform === "win32" ? { command: process.execPath, args: [await windowsNpmCli(this.env)] } : { command: "npm", args: [] };
      ...
      await Promise.all([writeFile(npmUserConfig, "", { mode: 0o600 }), writeFile(npmGlobalConfig, "", { mode: 0o600 })]);
      await this.run(npm.command, [...npm.args, "ci", "--ignore-scripts", "--omit=dev", "--no-audit", "--no-fund", "--userconfig", npmUserConfig, "--globalconfig", npmGlobalConfig], { cwd: stage, env, timeout: INSTALL_TIMEOUT_MS, signal });
      ...
      const version = await this.run(invocation.command, [...invocation.args, "--version"], { cwd: stage, env, timeout: 30_000, signal });
      if (version.stdout.trim() !== MANAGED_APPIUM_VERSION) throw new Error("Managed Appium version verification failed");
      const drivers = await this.run(invocation.command, [...invocation.args, "driver", "list", "--installed", "--json"], { cwd: stage, env, timeout: 60_000, signal });
      let driver: Record<string, unknown> | undefined;
      try { driver = (JSON.parse(drivers.stdout) as Record<string, Record<string, unknown>>).uiautomator2; } catch {}
      if (!driver || driver.installed !== true || driver.pkgName !== "appium-uiautomator2-driver" || driver.version !== MANAGED_UIAUTOMATOR2_VERSION || typeof driver.installPath !== "string") throw new Error("Managed UiAutomator2 verification failed");
      const installPath = await realpath(driver.installPath);
      const fromStage = relative(await realpath(stage), installPath);
      if (!fromStage || fromStage.startsWith("..") || isAbsolute(fromStage)) throw new Error("Managed UiAutomator2 resolves outside the staged installation");
      ...
      try { await rename(stage, current); }
      catch (error) {
        if (await exists(previous) && !await exists(current)) {
          try { await rename(previous, current); }
          catch { throw new Error("Android tooling promotion failed and the previous installation could not be restored. Use Repair to recover it."); }
        }
        throw error;
      }
      ...
      try { if (await exists(stage)) await rm(stage, { recursive: true, force: true }).catch(() => {}); }
