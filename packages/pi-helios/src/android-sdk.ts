...
export class AndroidSdk {
  ...
  constructor(
    ...
    terminateProcess: typeof terminateProcessTree = terminateProcessTree,
  ) {
    this.paths = paths;
    this.exec = exec;
  ...
  private async runAdbBounded(
    ...
    const child = this.spawnProcess(this.paths.adb, args, {
      shell: false,
      windowsHide: true,
    ...
    if (outcome !== "closed" && child.pid) {
      try {
        await this.terminateProcess(child, "adb package listing", 500, 5_000);
      } catch (error) {
