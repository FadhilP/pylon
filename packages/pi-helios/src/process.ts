...
async function runTaskkill(pid: number, force: boolean): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    killer.once("close", (code) => code === 0 || code === 128 ? resolve() : reject(new Error(`taskkill failed with exit code ${code}`)));
  });
}
...
export async function terminateProcessTree(child: ChildProcess, label: string, gracefulMs = 2_000, forceMs = 5_000): Promise<void> {
  if (exited(child)) return;
  if (!child.pid) throw new Error(`${label} process has no PID`);
  if (process.platform === "win32") await runTaskkill(child.pid, false).catch(() => {});
