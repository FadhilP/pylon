...
export class BrowserSessionManager {
  ...
  async acquireInteractive(piSessionId: string, owner: string): Promise<InteractiveBrowserState> {
    ...
    return this.serialized(managed, async () => {
      if (managed.record.ownership !== "owned")
        throw new Error("Embedded control is available only for Helios-owned browsers");
  ...
  async operateInteractive(
    ...
    if (!actions.length || actions.length > 4)
      throw new Error("Interactive browser request must contain 1 to 4 actions");
    const managed = this.requireManaged(piSessionId, actions[0]);
    return this.serialized(managed, async () => {
      if (managed.record.ownership !== "owned" || managed.interactiveOwner !== owner)
    ...
    await Promise.all(
      sessions.map(async managed => {
        await this.serialized(managed, async () => {
          ...
          await managed.cli
            .dispose()
            .catch(() => summary.cleanupWarnings.push("Could not delete private browser directory"));
        });
      }),
    );
    return summary;
  ...
  private requireManaged(piSessionId: string, action: BrowserAction): Managed {
    ...
    if (managed.record.state !== "ready" || managed.closingRequested)
      throw new Error(`Browser session is ${managed.closingRequested ? "closing" : managed.record.state}`);
    if (
      action.kind === "open" ||
  ...
  private async performOperation(
    ...
    if (this.sessions.get(managed.record.piSessionId) !== managed || managed.record.state !== "ready")
      throw new Error(`Browser session is ${managed.record.state}`);
    if (
      action.kind === "open" ||
      action.kind.startsWith("attach-") ||
    ...
    try {
      ...
      if ((error instanceof HeliosCliError && error.uncertainOutcome) || this.invalidatesReferences(action))
        managed.references.clear();
      throw error;
    }
    this.updateReferences(managed, action, result.snapshot);
  ...
  private async envelope(
    ...
    if (action === "tab-list") tabResult = result;
      ...
      try {
        tabResult = await managed.cli.run(managed.record.cliSessionName, { kind: "tab-list" }, signal);
      } catch (error) {
