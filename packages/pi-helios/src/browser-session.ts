...
function sessionMissing(error: unknown): boolean {
  return (
    error instanceof HeliosCliError && error.category === "session-missing"
  );
}

function staleSessionError(): Error {
  return new Error(
    "Helios browser session is stale; close or detach, then start again",
  );
}
...
export function validateCdpEndpoint(value: string): string {
  ...
  if (
    ...
    throw new Error(
      "CDP endpoint must be a loopback HTTP origin without credentials, path, query, or fragment",
    );
  }
  return url.origin;
}
...
function resultTabs(
  ...
  if (
    ...
    !selected.some((tab) => tab.index === currentIndex)
  ) {
...
function listedBrowsers(
  ...
  for (const item of value.browsers) {
    ...
    if (typeof browser.name !== "string" || typeof browser.status !== "string")
      return undefined;
...
export class BrowserSessionManager {
  ...
  constructor(
    exec: Exec,
    createCli: CliFactory = (value) =>
      PlaywrightCli.create(value, { persistentClient: true }),
  ...
  async start(
    ...
    if (this.sessions.has(piSessionId))
      throw new Error(
        "Pi session already has an active Helios browser session",
      );
    ...
    try {
      ...
      const action = {
        kind: "open",
        url,
        profileDirectory: record.profileDirectory!,
        headed,
      } as const;
      ...
      if (!cleaned && error instanceof Error)
        error.message += "; browser cleanup is uncertain, retry close";
      throw error;
    }
  }
  ...
  async attachCdp(
    ...
    return this.attach(
      piSessionId,
      "cdp-attached",
      { kind: "attach-cdp", endpoint: validateCdpEndpoint(endpoint) },
      signal,
    );
  }
  ...
  private async attach(
    piSessionId: string,
    ownership: Exclude<BrowserOwnership, "owned">,
    ...
    try {
      ...
      if (!cleaned && error instanceof Error)
        error.message += "; browser cleanup is uncertain, retry detach";
      throw error;
    }
  }
  ...
  async acquireInteractive(
    piSessionId: string,
    owner: string,
  ): Promise<InteractiveBrowserState> {
    const managed = this.sessions.get(piSessionId);
    if (!managed) throw new Error("No active Helios browser session");
    return this.serialized(managed, async () => {
      if (managed.record.ownership !== "owned")
        throw new Error(
          "Embedded control is available only for Helios-owned browsers",
        );
      if (managed.record.state !== "ready" || managed.closingRequested)
        throw new Error(`Browser session is ${managed.record.state}`);
      if (managed.interactiveOwner && managed.interactiveOwner !== owner)
        throw new Error("Helios browser is controlled by another Pylon tab");
      if (!managed.interactiveOwner && !(await this.releaseHeldInput(managed)))
        throw new Error(
          "Helios could not reset prior browser input; close the browser session",
        );
  ...
  async releaseInteractive(
    ...
    if (!managed) return { active: false, controlled: false };
    return this.serialized(managed, async () => {
      if (managed.interactiveOwner !== owner)
        return this.state(piSessionId, owner);
      if (!(await this.releaseHeldInput(managed))) {
        this.renewInteractive(managed, owner);
        throw new Error(
          "Helios could not release browser input; retry release or close the browser",
        );
      }
  ...
  async observeFrame(
    piSessionId: string,
    signal?: AbortSignal,
  ): Promise<BrowserOperationResult | undefined> {
    const managed = this.requireManaged(piSessionId, { kind: "screenshot" });
    if (managed.record.ownership !== "owned")
      throw new Error(
        "Embedded mirroring is available only for Helios-owned browsers",
      );
    if (managed.interactiveOwner || managed.pendingOperations) return undefined;
    return this.serialized(managed, () =>
      this.performOperation(managed, { kind: "screenshot" }, signal),
    );
  }

  async operateInteractive(
    piSessionId: string,
    ...
    if (!actions.length || actions.length > 4)
      throw new Error(
        "Interactive browser request must contain 1 to 4 actions",
      );
    const managed = this.requireManaged(piSessionId, actions[0]);
    return this.serialized(managed, async () => {
      if (
        managed.record.ownership !== "owned" ||
        managed.interactiveOwner !== owner
      )
        throw new Error("Embedded browser control lease is not active");
      this.renewInteractive(managed, owner);
      try {
        const results: BrowserOperationResult[] = [];
        for (const action of actions)
          results.push(await this.performOperation(managed, action, signal));
        return results;
      } finally {
        if (managed.interactiveOwner === owner)
          this.renewInteractive(managed, owner);
  ...
  async close(
    ...
    if (requested === "close" && managed.record.ownership !== "owned")
      throw new Error("Attached browsers may only be detached");
    if (requested === "detach" && managed.record.ownership === "owned")
      throw new Error("Owned browsers must be closed");
    if (managed.interactiveOwner && managed.interactiveOwner !== owner)
      throw new Error("Helios browser is under direct user control in Pylon");
    ...
    return this.serialized(managed, async () => {
      ...
      await managed.cli
        .dispose()
        .catch(() =>
          cleanupWarnings.push("Could not delete private browser directory"),
        );
      return {
        action: requested,
        ownership: managed.record.ownership,
  ...
  async shutdown(): Promise<BrowserShutdownResult> {
    const summary: BrowserShutdownResult = {
      failures: [],
      cleanupWarnings: [],
    };
  ...
  private requireManaged(piSessionId: string, action: BrowserAction): Managed {
    ...
    if (!managed)
      throw new Error(
        "No active Helios browser session; use start or attach first",
      );
    if (managed.record.state !== "ready" || managed.closingRequested)
      throw new Error(
        `Browser session is ${managed.closingRequested ? "closing" : managed.record.state}`,
      );
    if (
      action.kind === "open" ||
      action.kind.startsWith("attach-") ||
      action.kind === "close" ||
      action.kind === "detach" ||
      action.kind === "list"
    ) {
  ...
  private async performOperation(
    ...
    if (
      this.sessions.get(managed.record.piSessionId) !== managed ||
    ...
    try {
      result = await managed.cli.run(
        managed.record.cliSessionName,
        action,
        signal,
      );
    } catch (error) {
      ...
      if (
        (error instanceof HeliosCliError && error.uncertainOutcome) ||
        this.invalidatesReferences(action)
      )
        managed.references.clear();
      throw error;
    }
  ...
  private async expireInteractive(
    managed: Managed,
    owner: string,
    epoch: number,
  ): Promise<void> {
    await this.serialized(managed, async () => {
      if (
        managed.interactiveOwner !== owner ||
        managed.interactiveEpoch !== epoch
      )
        return;
      if (!(await this.releaseHeldInput(managed))) {
        this.renewInteractive(managed, owner);
  ...
  private async releaseHeldInput(managed: Managed): Promise<boolean> {
    for (const button of [...managed.heldButtons]) {
      try {
        await managed.cli.run(managed.record.cliSessionName, {
          kind: "mouse-up",
          button,
        });
  ...
  private validateReference(managed: Managed, action: BrowserAction): void {
    ...
    if (!managed.references.has(action.target))
      throw new Error(
        `Element reference ${action.target} is stale or was not returned by latest snapshot`,
      );
  }
  ...
  private async runCleanup(
    ...
    action: "close" | "detach",
    signal?: AbortSignal,
  ): Promise<{ cleaned: boolean; error?: unknown }> {
    try {
      await managed.cli.run(
        managed.record.cliSessionName,
        { kind: action },
        signal,
      );
      ...
      try {
        const listed = await managed.cli.run(managed.record.cliSessionName, {
          kind: "list",
        });
        const browsers = listedBrowsers(listed.value);
        if (
          browsers &&
          !browsers.some((item) => item.name === managed.record.cliSessionName)
        )
          return { cleaned: true };
      } catch {}
  ...
  private async cleanupUncertainStart(
    managed: Managed,
    action: "close" | "detach",
  ): Promise<boolean> {
    const cleanup = await this.runCleanup(managed, action);
  ...
  private async envelope(
    ...
    if (action === "tab-list") tabResult = result;
      ...
      try {
        tabResult = await managed.cli.run(
          managed.record.cliSessionName,
          { kind: "tab-list" },
          signal,
        );
    ...
    const returnedTabs = includeTabs
      ? resultTabs(managed.tabs, managed.page?.index)
      : undefined;
    const tabsOmitted =
      includeTabs &&
      managed.tabs &&
