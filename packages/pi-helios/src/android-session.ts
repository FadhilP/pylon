...
import {
  androidSnapshot,
  sameAndroidElement,
  type AndroidElementRef,
  type AndroidSnapshot,
} from "./android-source.ts";
import {
  AppiumClient,
  AppiumServer,
  resolveAppium,
  type AppiumInvocation,
} from "./appium.ts";
...
export type AndroidState =
  "starting" | "ready" | "cleanup-required" | "closing" | "closed";
...
function validatePackage(value: string): string {
  if (
    ...
    throw new Error("Android appPackage must be a Java-style package name");
  return value;
}
...
function validateActivity(value: string | undefined): string | undefined {
  ...
  if (!value || value.length > 500 || /[\r\n\0]/.test(value))
    throw new Error("Android appActivity is invalid");
  return value;
}
...
function targetRef(value: string): string {
  if (!/^a[1-9]\d{0,5}$/.test(value))
    throw new Error(
      "Android target must be a current snapshot reference such as a1",
    );
  return value;
}
...
export class AndroidSessionManager {
  ...
  summary(): {
    ...
    return {
      ...
      cleanupRequired: records.filter(
        (item) => item.state === "cleanup-required",
      ).length,
    };
  }
  ...
  async start(
    ...
    if (!avd || avd.length > 200 || /[\r\n\0]/.test(avd))
      throw new Error("Android AVD name is invalid");
    const managed = this.reserve(piSessionId, "owned", packageName);
    return this.serialized(managed, async () => {
      try {
        managed.releaseToolingLease =
          await this.dependencies.acquireToolingLease();
        ...
        if (
          ...
          error.message += "; Android cleanup is uncertain, retry close";
        throw error;
      }
    });
  }
    ...
    return this.serialized(managed, async () => {
      try {
        ...
        if (
          ...
          error.message += "; Android cleanup is uncertain, retry detach";
        throw error;
      }
    });
  }
  ...
  private reserve(
    ...
    if (this.shuttingDown)
      throw new Error("Helios Android manager is shutting down");
    if (this.sessions.has(piSessionId))
      throw new Error(
        "Pi session already has an active Helios Android session",
      );
  ...
  private assertStartupActive(managed: Managed): void {
    if (managed.closingRequested || this.shuttingDown)
      throw new Error("Android startup interrupted by cleanup");
  }
  ...
  private async startAutomation(
    ...
    await managed.client.createSession(
      capabilities(managed.record.serial, managed.record.packageName, activity),
      signal,
    );
    this.assertStartupActive(managed);
    const snapshot = androidSnapshot(
      await this.checkedSource(managed, signal),
      managed.record.packageName,
    );
    this.assertStartupActive(managed);
    ...
    return this.result(
      managed,
      managed.record.ownership === "owned" ? "start" : "attach",
      snapshot,
      startedAt,
    );
  }
  ...
  async operate(
    ...
    return this.serialized(managed, async () => {
      if (managed.record.state !== "ready" || managed.closingRequested)
        throw new Error(
          `Android session is ${managed.closingRequested ? "closing" : managed.record.state}`,
        );
      const startedAt = Date.now();
      ...
      if (action.kind === "snapshot" || action.kind === "find") {
        const snapshot = androidSnapshot(
          await this.checkedSource(managed, signal),
          managed.record.packageName,
          action.kind === "find" ? { text: action.text } : undefined,
        );
        managed.references = snapshot.refs;
      ...
      if (action.kind === "screenshot") {
        ...
        return {
          ...this.result(managed, action.kind, undefined, startedAt),
          artifactPath,
        };
      }
      if (action.kind === "tap" || action.kind === "fill") {
        ...
        if (!target)
          throw new Error(
            `Android element reference ${ref} is stale or was not returned by latest snapshot`,
          );
        const current = androidSnapshot(
          await this.checkedSource(managed, signal),
          managed.record.packageName,
        );
        ...
        if (!verified) {
          ...
          throw new Error(`Android element reference ${ref} is stale`);
        }
        if (!verified.enabled)
          throw new Error(`Android element reference ${ref} is disabled`);
        const rect = await client.windowRect(signal);
        const x = Math.floor(
          (verified.bounds.left + verified.bounds.right) / 2,
        );
        const y = Math.floor(
          (verified.bounds.top + verified.bounds.bottom) / 2,
        );
        if (
          verified.bounds.left < 0 ||
          verified.bounds.top < 0 ||
          verified.bounds.right > rect.width ||
          verified.bounds.bottom > rect.height ||
          x < 0 ||
          y < 0 ||
          x >= rect.width ||
          y >= rect.height
          ...
          throw new Error(
            `Android element reference ${ref} is outside the current viewport`,
          );
        try {
          if (action.kind === "tap") {
            await this.assertCurrentPackage(managed, signal);
            ...
            if (!verified.editable)
              throw new Error(
                `Android element reference ${ref} is not editable`,
              );
            if (action.text.length > 10_000)
              throw new Error("Android fill text exceeds 10000 characters");
            const latest = androidSnapshot(
              await this.checkedSource(managed, signal),
              managed.record.packageName,
            );
            ...
            if (!latestTarget)
              throw new Error(
                `Android element reference ${ref} changed before fill`,
              );
            const elementId = await client.findByXpath(
              latestTarget.xpath,
              signal,
            );
            await this.assertCurrentPackage(managed, signal);
      ...
      try {
        if (action.kind === "back") {
          ...
          if (
            !Number.isInteger(action.distance) ||
            action.distance < 10 ||
            action.distance > 90
          )
            throw new Error(
              "Android swipe distance must be an integer percentage from 10 to 90",
            );
          const rect = await client.windowRect(signal);
          const center = {
  ...
  private async assertCurrentPackage(
    managed: Managed,
    signal?: AbortSignal,
  ): Promise<void> {
    const current = await managed.client!.currentPackage(signal);
    if (current !== managed.record.packageName)
      throw new Error(
        `Android UI left expected package ${managed.record.packageName}; current package: ${current}`,
      );
  }
  ...
  private async assertExpectedPackage(
    managed: Managed,
    signal?: AbortSignal,
  ): Promise<void> {
    androidSnapshot(
  ...
  async close(
    piSessionId: string,
    requested: "close" | "detach",
    _signal?: AbortSignal,
  ): Promise<AndroidOperationResult> {
    const managed = this.sessions.get(piSessionId);
    if (!managed) throw new Error("No active Helios Android session");
    if (requested === "close" && managed.record.ownership !== "owned")
      throw new Error("Attached Android emulators may only be detached");
    if (requested === "detach" && managed.record.ownership !== "attached")
      throw new Error("Owned Android emulators must be closed");
    if (managed.closingRequested) throw new Error("Android session is closing");
    managed.closingRequested = true;
    return this.serialized(managed, () =>
      this.finishCleanup(managed, requested),
    );
  }

  async shutdown(): Promise<AndroidShutdownResult> {
    this.shuttingDown = true;
    const summary: AndroidShutdownResult = {
      failures: [],
      cleanupWarnings: [],
    };
    await Promise.all(
      [...this.sessions.values()].map(async (managed) => {
        const action =
          managed.record.ownership === "owned" ? "close" : "detach";
        managed.closingRequested = true;
        try {
          const result = await this.serialized(managed, () =>
            this.finishCleanup(managed, action),
          );
  ...
  private async finishCleanup(
    ...
    try {
      await this.releaseToolingLease(managed);
    } catch (error) {
      managed.record.state = "cleanup-required";
      managed.closingRequested = false;
      throw error;
    ...
    if (this.sessions.get(managed.record.piSessionId) === managed)
      this.sessions.delete(managed.record.piSessionId);
    return {
      ...this.result(managed, action, undefined, startedAt),
  ...
  private async cleanupResources(
    managed: Managed,
  ): Promise<{ warnings: string[]; failure?: unknown }> {
    const warnings: string[] = [];
    await managed.client
      ?.deleteSession()
      .catch(() =>
        warnings.push("Could not delete Appium session before server shutdown"),
      );
  ...
  private requireManaged(piSessionId: string): Managed {
    ...
    if (!managed)
      throw new Error(
        "No active Helios Android session; use start or attach first",
      );
    return managed;
  }
  ...
  private async cleanupFailedStart(managed: Managed): Promise<void> {
    ...
    if (managed.record.ownership === "owned")
      await managed.emulator
        ?.cleanupUncertainStart()
        .catch((error) => warnings.push(error));
    if (warnings.length) {
      managed.record.state = "cleanup-required";
      return;
