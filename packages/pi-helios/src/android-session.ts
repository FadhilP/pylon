...
function validatePackage(value: string): string {
  if (value.length > 255 || !/^[A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*)+$/.test(value))
...
export class AndroidSessionManager {
  ...
  async start(
    ...
    return this.serialized(managed, async () => {
      try {
        ...
        if (managed.record.state === "cleanup-required" && error instanceof Error)
  ...
  async operate(piSessionId: string, action: AndroidAction, signal?: AbortSignal): Promise<AndroidOperationResult> {
    ...
    return this.serialized(managed, async () => {
      if (managed.record.state !== "ready" || managed.closingRequested)
        throw new Error(`Android session is ${managed.closingRequested ? "closing" : managed.record.state}`);
      const startedAt = Date.now();
      ...
      if (action.kind === "tap" || action.kind === "fill") {
        ...
        if (!target)
          throw new Error(`Android element reference ${ref} is stale or was not returned by latest snapshot`);
        ...
        if (
          ...
          throw new Error(`Android element reference ${ref} is outside the current viewport`);
        try {
          if (action.kind === "tap") {
            await this.assertCurrentPackage(managed, signal);
      ...
      try {
        if (action.kind === "back") {
          ...
          if (!Number.isInteger(action.distance) || action.distance < 10 || action.distance > 90)
            throw new Error("Android swipe distance must be an integer percentage from 10 to 90");
          const rect = await client.windowRect(signal);
          const center = { x: Math.floor(rect.width / 2), y: Math.floor(rect.height / 2) };
          const amount = Math.floor(
            ((action.direction === "up" || action.direction === "down" ? rect.height : rect.width) * action.distance) /
              200,
          );
          const from = { ...center },
            to = { ...center };
  ...
  private async assertCurrentPackage(managed: Managed, signal?: AbortSignal): Promise<void> {
    ...
    if (current !== managed.record.packageName)
      throw new Error(`Android UI left expected package ${managed.record.packageName}; current package: ${current}`);
  }
  ...
  private async cleanupResources(managed: Managed): Promise<{ warnings: string[]; failure?: unknown }> {
    ...
    await managed.client
      ?.deleteSession()
      .catch(() => warnings.push("Could not delete Appium session before server shutdown"));
