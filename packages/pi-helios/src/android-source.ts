...
function parseXml(source: string): XmlNode {
  ...
  while (offset < xml.length) {
    ...
    const siblings =
      parent?.children.filter(item => item.name === name).length ?? roots.filter(item => item.name === name).length;
    ...
    for (const key of [
      ...
      if (attrs[key] !== undefined && attrs[key] !== "true" && attrs[key] !== "false")
        throw new Error(`Android source contains invalid boolean attribute ${key}`);
    }
    const node: XmlNode = { name, attrs, children: [], parent, path: `${base}/${name}[${siblings + 1}]`, depth };
...
function boundLines(
  ...
  maxLines: number,
  maxBytes: number,
): { text: string; truncated: boolean; omittedLines: number; omittedBytes: number; keptLines: number } {
  const kept: string[] = [];
  let bytes = 0;
  let index = 0;
...
export function androidSnapshot(source: string, expectedPackage: string, query?: { text: string }): AndroidSnapshot {
  ...
  const foreign = nodes.find(
    node =>
      parseBounds(node.attrs.bounds) && relevant(node) && node.attrs.package && node.attrs.package !== expectedPackage,
  );
  ...
  const candidates = nodes.filter(node => {
    ...
    const searchable = [attrs.text, attrs["content-desc"], attrs["resource-id"], attrs.class, node.name]
      .filter(Boolean)
      .join(" ");
  ...
  const lines = candidates.map((node, index) => {
    ...
    const secret =
      attrs.password === "true" ||
      SECRET_HINT.test(`${attrs["resource-id"] ?? ""} ${attrs["content-desc"] ?? ""}`) ||
...
export function sameAndroidElement(current: AndroidSnapshot, target: AndroidElementRef): AndroidElementRef | undefined {
  return [...current.allRefs.values()].find(
    ref => ref.xpath === target.xpath && ref.fingerprint === target.fingerprint,
  );
}
