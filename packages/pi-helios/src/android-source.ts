...
const SECRET_HINT =
  /(?:pass(?:word|code)?|pin|otp|token|secret|credential|auth)/i;
const CREDENTIAL_VALUE =
  /(?:\b(?:authorization|api[_-]?key|token|password|secret|cookie)\s*[:=]\s*\S+|\b(?:sk-ant-|sk-proj-|sk-|ghp_|github_pat_|AIza|xox[baprs]-)[A-Za-z0-9._-]{12,}|\beyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+)/i;

export interface AndroidBounds {
  left: number;
  top: number;
  right: number;
  bottom: number;
...
function decodeEntity(value: string): string {
  if (/&(?!#x[0-9a-f]+;|#\d+;|amp;|lt;|gt;|quot;|apos;)/i.test(value))
    throw new Error("Android source contains an unsupported XML entity");
  return value.replace(
    ...
    (_match, entity: string) => {
      ...
      if (
        ...
        (point !== 0x9 && point !== 0xa && point !== 0xd && point < 0x20)
      )
        throw new Error("Android source contains an invalid XML entity");
...
function parseAttributes(value: string): Record<string, string> {
  ...
  while (offset < value.length) {
    ...
    if (!match)
      throw new Error("Android source contains malformed XML attributes");
    if (Object.keys(attrs).length >= MAX_ATTRIBUTES)
      throw new Error("Android source node has too many attributes");
    const decoded = decodeEntity(match[3] ?? match[4] ?? "");
    if (decoded.length > MAX_ATTRIBUTE_LENGTH)
      throw new Error("Android source attribute is oversized");
    if (Object.hasOwn(attrs, match[1]))
      throw new Error("Android source contains duplicate XML attributes");
...
function parseXml(source: string): XmlNode {
  if (Buffer.byteLength(source) > MAX_SOURCE_BYTES)
    throw new Error("Android source exceeds 1MB limit");
  let xml = source.trim();
  if (xml.startsWith("<?xml")) {
    const declaration = xml.match(/^<\?xml(?:\s+[^?]*)?\?>/u)?.[0];
    if (!declaration || declaration.length > 300)
      throw new Error("Android source has an invalid XML declaration");
    xml = xml.slice(declaration.length).trim();
  }
  if (xml.includes("<!") || xml.includes("<?"))
    throw new Error("Android source declarations and DTDs are not supported");
  ...
  while (offset < xml.length) {
    ...
    if (start < 0 || xml.slice(offset, start).trim())
      throw new Error("Android source contains unsupported XML text nodes");
    let end = start + 1;
    let quote = "";
    for (; end < xml.length; end++) {
      ...
      if (character === "<")
        throw new Error("Android source contains malformed XML tags");
      if (character === ">") break;
    }
    ...
    if (body.startsWith("/")) {
      ...
      if (!node || node.name !== name)
        throw new Error("Android source contains mismatched XML tags");
      continue;
    }
    ...
    if (!nameMatch)
      throw new Error("Android source contains an invalid XML tag");
    const parent = stack.at(-1);
    const depth = parent ? parent.depth + 1 : 0;
    if (depth > MAX_DEPTH)
      throw new Error("Android source exceeds maximum depth");
    if (++count > MAX_NODES)
      throw new Error("Android source exceeds maximum node count");
    const name = nameMatch[1];
    const siblings =
      parent?.children.filter((item) => item.name === name).length ??
      roots.filter((item) => item.name === name).length;
    ...
    for (const key of [
      ...
      if (
        ...
        throw new Error(
          `Android source contains invalid boolean attribute ${key}`,
        );
    }
    const node: XmlNode = {
      name,
  ...
  if (stack.length || roots.length !== 1)
    throw new Error("Android source contains malformed XML");
  return roots[0];
}
...
function relevant(node: XmlNode): boolean {
  ...
  return Boolean(
    ...
    /EditText|AutoCompleteTextView|Button|CheckBox|Switch$/i.test(
...
function foreignAncestor(node: XmlNode, expectedPackage: string): boolean {
  for (let parent = node.parent; parent; parent = parent.parent) {
    if (parent.attrs.package && parent.attrs.package !== expectedPackage)
      return true;
  }
  return false;
}
...
export function androidSnapshot(
  ...
  const packages = new Set(
    nodes.map((node) => node.attrs.package).filter(Boolean),
  );
  if (!packages.has(expectedPackage)) {
    const current = [...packages].slice(0, 3).join(", ") || "unknown";
    throw new Error(
      `Android UI left expected package ${expectedPackage}; current package: ${current}`,
    );
  }
  const matcher = queryMatcher(query?.text);
  const foreign = nodes.find(
    (node) =>
      parseBounds(node.attrs.bounds) &&
      relevant(node) &&
      node.attrs.package &&
      node.attrs.package !== expectedPackage,
  );
  if (foreign)
    throw new Error(
      `Android source contains unsupported UI from package ${foreign.attrs.package}`,
    );
  const candidates = nodes.filter((node) => {
    ...
    if (
      !bounds ||
      attrs.package !== expectedPackage ||
      foreignAncestor(node, expectedPackage)
    )
      return false;
    const searchable = [
      attrs.text,
  ...
  const lines = candidates.map((node, index) => {
    ...
    const secret =
      attrs.password === "true" ||
      SECRET_HINT.test(
        `${attrs["resource-id"] ?? ""} ${attrs["content-desc"] ?? ""}`,
      ) ||
      CREDENTIAL_VALUE.test(label);
    if (editableValue) {
      redactions++;
      if (!attrs["content-desc"]) label = "[value redacted]";
    }
    if (secret && label !== "[value redacted]") {
...
export function sameAndroidElement(
  current: AndroidSnapshot,
  target: AndroidElementRef,
): AndroidElementRef | undefined {
  return [...current.allRefs.values()].find(
    (ref) =>
      ref.xpath === target.xpath && ref.fingerprint === target.fingerprint,
  );
}
