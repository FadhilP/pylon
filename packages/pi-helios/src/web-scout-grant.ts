...
export async function consumeWebScoutGrant(
  ...
  try {
    ...
    throw new Error("Web Scout browser grant is invalid");
  }
  if (typeof envelope.path !== "string" || typeof envelope.token !== "string")
    throw new Error("Web Scout browser grant is invalid");
  ...
  try {
    const [rootPath, directoryPath, fileInfo] = await Promise.all([
      realpath(GRANT_ROOT),
      realpath(directory),
      lstat(path),
    ]);
  ...
  try {
    ...
    throw new Error("Web Scout browser grant is unavailable");
  }
  if (
    grant?.version !== 1 ||
