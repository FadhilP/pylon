...
async function embeddedBrowserRequest(manager: BrowserSessionManager, request: EmbeddedRequest): Promise<unknown> {
  ...
  switch (request.action) {
    ...
    case "reload":
      ...
      break;
    case "resize":
      actions = [{ kind: "resize", width: request.width!, height: request.height! }];
      break;
    case "pointer":
      actions = [
    ...
    case "wheel":
      ...
      break;
    case "key":
      actions = [{ kind: request.phase === "down" ? "key-down" : "key-up", key: request.key! }];
      break;
    case "tab-list":
...
const browserActionFields = {
  url: Type.Optional(
    Type.String({
      description: "HTTP(S), about:blank, or an explicit local file: URL ending in .html or .htm",
      maxLength: 4096,
    }),
  ),
  ...
  browser: Type.Optional(
    StringEnum(["chrome", "msedge"] as const, { description: "Browser for extension attachment; ignored by start" }),
  ),
...
const browserPlanStepSchema = Type.Object(
  {
    ...
    match: Type.String({
      minLength: 1,
      maxLength: 500,
      description: "Exact visible text or accessible name used to resolve one current element",
    }),
...
const browserSchema = Type.Object(
  {
    ...
    plan: Type.Optional(
      Type.Array(browserPlanStepSchema, {
        minItems: 1,
        maxItems: 5,
        description: "Bounded semantic steps; each resolves exactly one element and stops on ambiguity or page change",
      }),
    ),
  },
  { additionalProperties: false },
...
const androidSchema = Type.Object(
  {
    ...
    target: Type.Optional(
      Type.String({
        ...
        maxLength: 7,
        description: "Element reference from the latest Android snapshot, such as a1",
      }),
    ),
    ...
    distance: Type.Optional(
      Type.Integer({ minimum: 10, maximum: 90, description: "Swipe distance as percentage of the screen dimension" }),
    ),
  },
  { additionalProperties: false },
);
...
function androidAction(params: AndroidInput): AndroidAction {
  switch (params.action) {
    ...
    case "fill":
      ...
      return { kind: "fill", target: requireAndroidField(params, "target"), text: requireAndroidField(params, "text") };
    case "swipe":
      rejectAndroidExtra(params, ["direction", "distance"]);
      return { kind: "swipe", direction: requireAndroidField(params, "direction"), distance: params.distance ?? 60 };
    default:
      throw new Error(`${params.action} is not an active Android-session operation`);
  }
}
...
function browserAction(params: BrowserParams): BrowserAction {
  switch (params.action) {
    ...
    case "snapshot":
      ...
      return { kind: "snapshot", target: params.target, depth: params.depth, snapshotMode: params.snapshotMode };
    case "continue":
    ...
    case "screenshot":
      ...
      if (params.target && params.fullPage)
        throw new Error("Element screenshot and full-page screenshot cannot be combined");
    ...
    default:
      throw new Error(`${params.action} is a lifecycle action, not an active-session operation`);
  }
}
...
function describe(result: BrowserOperationResult): string {
  ...
  if (result.action === "find" && ((result.findMatches ?? 0) > 20 || result.snapshotTruncated))
...
export default function heliosExtension(
  ...
  const manager = new BrowserSessionManager(exec, value =>
    PlaywrightCli.create(value, { persistentClient: options.persistentClient ?? true }),
  );
  ...
  const disposeAndroidTooling = pi.events.on("pylon:helios-android-tooling-request", (value: unknown) => {
    ...
    if (
      request?.version !== 1 ||
      !(["status", "install", "remove"] as const).includes(request.action as "status") ||
      typeof request.claim !== "function" ||
      typeof request.respond !== "function" ||
      !request.claim()
    )
      ...
      (async () => {
        ...
        try {
          ...
          return {
            ...
            label: "Helios",
            lines: [
              error instanceof Error ? `CLI: ${error.message}` : "CLI: unavailable",
  ...
  pi.on("session_shutdown", async (_event, ctx) => {
    ...
    for (const warning of [...summary.cleanupWarnings, ...androidSummary.cleanupWarnings])
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params: AndroidInput, signal, onUpdate, ctx) {
      ...
      if (params.action === "packages") {
        ...
        if (!approved)
          return {
            content: [{ type: "text" as const, text: "User declined Android package inventory." }],
            details: { declined: true },
          };
        onUpdate?.({
          content: [{ type: "text" as const, text: `Listing installed packages on ${serial}...` }],
          details: {},
        });
        ...
        try {
          ...
          return {
            ...
            details: { serial: inventory.serial, avd: inventory.avd, packageCount: inventory.packages.length },
          };
        } finally {
          if (ctx.hasUI) ctx.ui.setStatus?.("pi-helios", undefined);
        }
      ...
      if (params.action === "status") {
        ...
        if (!record)
          return {
            content: [{ type: "text" as const, text: "No active Helios Android session." }],
      ...
      if (params.action === "start" || params.action === "attach") {
        ...
        if (params.action === "start") {
          ...
          if (!approved)
            return {
              content: [{ type: "text" as const, text: "User declined Android emulator start." }],
              details: { declined: true },
            };
        ...
        if (!approved)
          return {
            content: [{ type: "text" as const, text: "User declined Android emulator attachment." }],
            details: { declined: true },
          };
        if (androidTooling.isMutating()) throw new Error("Android tooling setup is in progress");
        onUpdate?.({
          content: [{ type: "text" as const, text: `Attaching to Android emulator ${serial}...` }],
          details: {},
        });
      ...
      if (params.action === "screenshot" && ctx.model && !ctx.model.input.includes("image"))
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: describeAndroid(result) }, ...image.content],
          details: { ...result, artifactPath: undefined },
        };
      } finally {
        await rm(result.artifactPath, { force: true }).catch(() => {
          if (ctx.hasUI) ctx.ui.notify("Helios could not delete temporary Android screenshot.", "warning");
        });
      }
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (!approved)
        return {
          content: [{ type: "text" as const, text: "User declined screenshot capture." }],
          details: { declined: true },
        };
      ...
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: `Captured Windows window: ${windowTarget.title}` },
            ...result.content,
          ],
    ...
    async execute(toolCallId, params: BrowserInput, signal, onUpdate, ctx) {
      ...
      const executeAction = async (params: BrowserParams) => {
        ...
        if (params.action === "attach") {
          ...
          if (mode === "cdp") {
            ...
            if (!approved)
              return {
                content: [{ type: "text" as const, text: "User declined browser attachment." }],
                details: { declined: true },
              };
        ...
        if (params.action === "tabs" && params.tabAction === "close" && manager.get(id)?.ownership !== "owned") {
          if (
            ...
            return {
              content: [{ type: "text" as const, text: "User declined tab close." }],
              details: { declined: true },
            };
          }
        }
      ...
      const executePlan = async (plan: BrowserPlanStep[]) => {
        ...
        for (const [index, step] of plan.entries()) {
          onUpdate?.({
            content: [{ type: "text" as const, text: `Resolving browser plan step ${index + 1}...` }],
            details: {},
          });
          ...
          if (step.action !== "click") {
            try {
              observed = await manager.operate(id, { kind: "tab-list" }, signal);
            } catch {
              observed = undefined;
            }
          }
          const after =
            observed && !observed.metadataStale && observed.metadataAvailable !== false
      ...
      if (params.plan !== undefined) {
        if (Object.entries(params).some(([key, value]) => key !== "plan" && value !== undefined))
      ...
      for (const [index, action] of params.actions.entries()) {
        ...
        if (final || details.declined) {
          for (const item of result.content) {
            content.push(
              item.type === "text" ? { ...item, text: `Step ${index + 1} (${action.action}):\n${item.text}` } : item,
            );
          }
        } else {
