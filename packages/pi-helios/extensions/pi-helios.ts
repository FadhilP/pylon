...
import {
  AndroidSdk,
  diagnoseAndroid,
  validateEmulatorSerial,
} from "../src/android-sdk.ts";
import {
  AndroidSessionManager,
  type AndroidAction,
  type AndroidOperationResult,
} from "../src/android-session.ts";
...
const captureSchema = Type.Object({
  target: StringEnum(["window"] as const, {
    description: "Capture one named Windows window",
  }),
  title: Type.String({
    description: "Required Windows window-title substring",
    maxLength: 500,
  }),
});
...
async function embeddedBrowserRequest(
  manager: BrowserSessionManager,
  request: EmbeddedRequest,
): Promise<unknown> {
  const { sessionId: id, owner, signal } = request;
  if (!id || id.length > 200 || !/^web:[A-Za-z0-9-]{1,128}$/.test(owner))
    throw new Error("Invalid embedded browser identity");
  ...
  if (request.action === "start") {
    ...
    if (request.width !== undefined && request.height !== undefined) {
      await manager.operateInteractive(
        id,
        [{ kind: "resize", width: request.width, height: request.height }],
        owner,
        signal,
      );
    }
  ...
  if (request.action === "frame" && !manager.state(id, owner).controlled) {
    ...
    try {
      const data = await manager.readArtifact(
        id,
        result.artifactPath,
        MAX_EMBEDDED_FRAME_BYTES,
      );
      return {
        ...embeddedState(manager, id, owner),
        frame: {
  ...
  switch (request.action) {
    case "frame":
      ...
      break;
    case "navigate":
      actions = [{ kind: "navigate", url: request.url! }];
      break;
    case "back":
    case "forward":
    case "reload":
      actions = [{ kind: request.action }];
      break;
    case "resize":
      actions = [
        { kind: "resize", width: request.width!, height: request.height! },
    ...
    case "pointer":
      actions = [
        ...
        ...(request.phase === "down"
          ? [{ kind: "mouse-down", button: request.button! } as const]
          : request.phase === "up"
            ? [{ kind: "mouse-up", button: request.button! } as const]
            : []),
      ];
      break;
    case "wheel":
    ...
    case "key":
      actions = [
        {
          kind: request.phase === "down" ? "key-down" : "key-up",
          key: request.key!,
        },
      ];
      break;
    case "tab-list":
      actions = [{ kind: "tab-list" }];
      break;
    case "tab-new":
      actions = [{ kind: "tab-new", url: request.url }];
      break;
    case "tab-select":
      actions = [{ kind: "tab-select", index: request.tabIndex! }];
      break;
    case "tab-close":
      actions = [{ kind: "tab-close", index: request.tabIndex! }];
      break;
    default:
      throw new Error("Unsupported embedded browser action");
  }
  ...
  try {
    ...
    return {
      ...embeddedState(manager, id, owner),
      frame: { mimeType: "image/png" as const, data: data.toString("base64") },
    };
  } finally {
...
const browserActionFields = {
  url: Type.Optional(
    Type.String({
      description:
        "HTTP(S), about:blank, or an explicit local file: URL ending in .html or .htm",
      maxLength: 4096,
    }),
  ),
  ...
  browser: Type.Optional(
    StringEnum(["chrome", "msedge"] as const, {
      description: "Browser for extension attachment; ignored by start",
    }),
  ),
  target: Type.Optional(
    Type.String({
      ...
      maxLength: 32,
      description:
        "Element reference from latest snapshot, such as e12 or f1e12",
    }),
  ),
  text: Type.Optional(
    Type.String({
      maxLength: 10000,
      description: "Exact text to find; keep narrow to avoid large match sets",
    }),
  ),
  regex: Type.Optional(
    Type.String({
      maxLength: 500,
      description:
        "Regular expression to find; keep specific to avoid large match sets",
    }),
  ),
  ...
  depth: Type.Optional(
    Type.Integer({
      ...
      maximum: 20,
      description:
        "Snapshot depth; prefer 4-6 first, then target a returned ref for more detail",
    }),
  ),
  snapshotMode: Type.Optional(
    StringEnum(["compact", "full"] as const, {
      description:
        "Snapshot structure: compact flattens anonymous generic wrappers (default); full preserves them",
    }),
  ),
  cursor: Type.Optional(
    Type.String({
      ...
      maxLength: 35,
      description:
        "One-use cursor returned by truncated snapshot, find, or action output",
    }),
  ),
  fullPage: Type.Optional(Type.Boolean()),
  tabAction: Type.Optional(
    StringEnum(["list", "select", "create", "close"] as const),
  ),
  tabIndex: Type.Optional(Type.Integer({ minimum: 0, maximum: 100 })),
};
const browserActionSchema = Type.Object(
  { action: StringEnum(BROWSER_ACTIONS), ...browserActionFields },
...
const browserPlanStepSchema = Type.Object(
  {
    ...
    match: Type.String({
      ...
      maxLength: 500,
      description:
        "Exact visible text or accessible name used to resolve one current element",
    }),
...
const browserSchema = Type.Object(
  {
    ...
    plan: Type.Optional(
      Type.Array(browserPlanStepSchema, {
        ...
        maxItems: 5,
        description:
          "Bounded semantic steps; each resolves exactly one element and stops on ambiguity or page change",
      }),
    ),
  },
  { additionalProperties: false },
...
const androidSchema = Type.Object(
  {
    ...
    serial: Type.Optional(
      Type.String({ pattern: "^emulator-[0-9]{4,5}$", maxLength: 20 }),
    ),
    ...
    target: Type.Optional(
      Type.String({
        pattern: "^a[1-9][0-9]{0,5}$",
        maxLength: 7,
        description:
          "Element reference from the latest Android snapshot, such as a1",
      }),
    ),
    text: Type.Optional(Type.String({ maxLength: 10000 })),
    direction: Type.Optional(
      StringEnum(["up", "down", "left", "right"] as const),
    ),
    distance: Type.Optional(
      Type.Integer({
        minimum: 10,
        maximum: 90,
        description: "Swipe distance as percentage of the screen dimension",
      }),
    ),
  },
  { additionalProperties: false },
...
function rejectAndroidExtra(
  params: AndroidInput,
  allowed: readonly (keyof AndroidInput)[],
): void {
  const accepted = new Set<keyof AndroidInput>(["action", ...allowed]);
  for (const [key, value] of Object.entries(params)) {
    if (value !== undefined && !accepted.has(key as keyof AndroidInput))
      throw new Error(`${params.action} does not accept ${key}`);
  }
}
...
function androidAction(params: AndroidInput): AndroidAction {
  switch (params.action) {
    ...
    case "tap":
      rejectAndroidExtra(params, ["target"]);
      return { kind: "tap", target: requireAndroidField(params, "target") };
    case "fill":
      rejectAndroidExtra(params, ["target", "text"]);
      return {
        kind: "fill",
        target: requireAndroidField(params, "target"),
        text: requireAndroidField(params, "text"),
      };
    case "swipe":
      rejectAndroidExtra(params, ["direction", "distance"]);
      return {
        kind: "swipe",
        direction: requireAndroidField(params, "direction"),
        distance: params.distance ?? 60,
      };
    default:
      throw new Error(
        `${params.action} is not an active Android-session operation`,
      );
  }
}

function requireAndroidField<K extends keyof AndroidInput>(
  params: AndroidInput,
  key: K,
): NonNullable<AndroidInput[K]> {
  const value = params[key];
  if (value === undefined || value === "")
    throw new Error(`${params.action} requires ${String(key)}`);
  return value as NonNullable<AndroidInput[K]>;
}

function describeAndroid(result: AndroidOperationResult): string {
  const lines = [
    `Android ${result.action} ${result.outcome} (${result.ownership}).`,
    `Emulator: ${result.serial} / ${result.avd}.`,
    `Package: ${result.packageName}.`,
  ];
  if (result.snapshot !== undefined)
    lines.push(`Snapshot:\n${result.snapshot || "(no matching elements)"}`);
  if (result.snapshotRedactions)
    lines.push(`Redactions: ${result.snapshotRedactions}.`);
  if (result.snapshotTruncated)
    lines.push(
      `Remaining: ${result.snapshotOmittedLines ?? 0} lines / ${result.snapshotOmittedBytes ?? 0} bytes.`,
    );
  if (result.findMatches !== undefined)
    lines.push(`Matches: ${result.findMatches}.`);
...
function sessionId(ctx: any): string {
  ...
  if (typeof id !== "string" || !id)
    throw new Error("Helios requires a stable Pi session identity");
  return id;
}

function requireField<K extends keyof BrowserParams>(
  params: BrowserParams,
  key: K,
): NonNullable<BrowserParams[K]> {
  const value = params[key];
  if (value === undefined || value === "")
    throw new Error(`${params.action} requires ${String(key)}`);
...
function rejectExtra(
  params: BrowserParams,
  allowed: readonly (keyof BrowserParams)[],
): void {
  const accepted = new Set<keyof BrowserParams>(["action", ...allowed]);
  for (const [key, value] of Object.entries(params)) {
    if (value !== undefined && !accepted.has(key as keyof BrowserParams))
      throw new Error(`${params.action} does not accept ${key}`);
  }
}
...
function browserAction(params: BrowserParams): BrowserAction {
  switch (params.action) {
    case "navigate":
      ...
      return { kind: "navigate", url: requireField(params, "url") };
    case "snapshot":
      rejectExtra(params, ["target", "depth", "snapshotMode"]);
      return {
        kind: "snapshot",
    ...
    case "continue":
      rejectExtra(params, ["cursor"]);
      return { kind: "continue", cursor: requireField(params, "cursor") };
    case "find": {
      rejectExtra(params, ["text", "regex"]);
      if (Boolean(params.text) === Boolean(params.regex))
        throw new Error("find requires exactly one of text or regex");
      return { kind: "find", text: params.text, regex: params.regex };
    }
    case "screenshot":
      rejectExtra(params, ["target", "fullPage"]);
      if (params.target && params.fullPage)
        throw new Error(
          "Element screenshot and full-page screenshot cannot be combined",
        );
      return {
        kind: "screenshot",
        target: params.target,
    ...
    case "fill":
      ...
      return {
        kind: "fill",
        target: requireField(params, "target"),
        text: requireField(params, "text"),
      };
    case "press":
      rejectExtra(params, ["key"]);
      return { kind: "press", key: requireField(params, "key") };
    case "select":
      rejectExtra(params, ["target", "value"]);
      return {
        kind: "select",
        target: requireField(params, "target"),
        value: requireField(params, "value"),
      };
    case "back":
    case "forward":
    case "reload":
    ...
    case "tabs": {
      ...
      if (action === "list") {
        rejectExtra(params, ["tabAction"]);
        return { kind: "tab-list" };
      }
      if (action === "create") return { kind: "tab-new", url: params.url };
      if (params.url !== undefined)
        throw new Error(`tabs ${action} does not accept url`);
      return {
        kind: action === "select" ? "tab-select" : "tab-close",
        index: requireField(params, "tabIndex"),
      };
    }
    default:
      throw new Error(
        `${params.action} is a lifecycle action, not an active-session operation`,
      );
  }
}
...
function planAction(step: BrowserPlanStep, target: string): BrowserAction {
  switch (step.action) {
    case "fill":
      if (step.text === undefined || step.value !== undefined)
        throw new Error("Plan fill requires text and does not accept value");
      return { kind: "fill", target, text: step.text };
    case "select":
      if (step.value === undefined || step.text !== undefined)
        throw new Error("Plan select requires value and does not accept text");
    ...
    case "uncheck":
      if (step.text !== undefined || step.value !== undefined)
        throw new Error(`Plan ${step.action} does not accept text or value`);
...
function describe(result: BrowserOperationResult): string {
  const ownership = OWNERSHIP_ACTIONS.has(result.action)
    ? ` (${result.ownership})`
    : "";
  const lines = [`Browser ${result.action} ${result.outcome}${ownership}.`];
  if (PAGE_CONTEXT_ACTIONS.has(result.action)) {
    if (result.metadataAvailable === false && !result.page)
      lines.push("Page metadata unavailable.");
    else if (result.metadataStale && PAGE_CHANGE_ACTIONS.has(result.action))
      lines.push("Page metadata may be stale.");
    if (result.page)
      lines.push(`Page: ${result.page.title} (${result.page.url})`);
  }
  if (result.tabs)
    lines.push(
  ...
  if (result.snapshotContinuation)
    lines.push(`Continuation: ${result.snapshotContinuation}`);
  if (
    result.action === "find" &&
    ((result.findMatches ?? 0) > 20 || result.snapshotTruncated)
  )
    lines.push("Refine find query or continue with returned cursor.");
  for (const warning of result.cleanupWarnings ?? [])
    lines.push(`Warning: ${warning}.`);
  return lines.join("\n");
}

function compactBrowserDetails(
  result: BrowserOperationResult,
...
function describeCompactBatchStep(
  index: number,
  action: string,
  result: BrowserOperationResult,
  pageChanged: boolean,
): string {
  const ownership = OWNERSHIP_ACTIONS.has(action)
    ? ` (${result.ownership})`
    : "";
  ...
  if (PAGE_CONTEXT_ACTIONS.has(action)) {
    if (result.metadataAvailable === false && !result.page)
      lines.push("Page metadata unavailable.");
    else if (result.metadataStale && PAGE_CHANGE_ACTIONS.has(action))
      lines.push("Page metadata may be stale.");
    else if (pageChanged && result.page)
      lines.push(`Page: ${result.page.title} (${result.page.url})`);
  }
  if (result.snapshotRedactions)
    lines.push(`Redactions: ${result.snapshotRedactions}.`);
  if (result.snapshotTruncated)
    lines.push(
      `Intermediate snapshot omitted and truncated; ${result.snapshotOmittedLines ?? 0} lines / ${result.snapshotOmittedBytes ?? 0} bytes remain. Request a new snapshot if needed.`,
    );
  for (const warning of result.cleanupWarnings ?? [])
    lines.push(`Warning: ${warning}.`);
  return lines.join("\n");
...
async function withBrowserStatus<T>(
  ctx: any,
  action: string,
  operation: () => Promise<T>,
): Promise<T> {
  ...
  try {
    ...
    if (ctx.hasUI) ctx.ui.setStatus?.("pi-helios", undefined);
  }
}

export default function heliosExtension(
  pi: ExtensionAPI,
  options: { configPath?: string; persistentClient?: boolean } = {},
) {
  ...
  const manager = new BrowserSessionManager(exec, (value) =>
    PlaywrightCli.create(value, {
  ...
  const disposeEmbeddedBrowser = pi.events.on(
    "pylon:helios-browser-request",
    (value: unknown) => {
      const request =
        value && typeof value === "object"
          ? (value as Partial<EmbeddedRequest>)
          : undefined;
      if (
        request?.version !== 1 ||
        ...
        return;
      request.respond(
        embeddedBrowserRequest(manager, request as EmbeddedRequest),
      );
    },
  );
  const disposeAndroidTooling = pi.events.on(
    "pylon:helios-android-tooling-request",
    (value: unknown) => {
      const request =
        value && typeof value === "object"
          ? (value as Partial<AndroidToolingRequest>)
          : undefined;
      if (
        request?.version !== 1 ||
        !(["status", "install", "remove"] as const).includes(
          request.action as "status",
        ) ||
        typeof request.claim !== "function" ||
        typeof request.respond !== "function" ||
        !request.claim()
      ...
      request.respond(
        (async () => {
          ...
          if (request.action === "install")
            return androidTooling.install(
              androidManager.summary().total,
              request.signal,
            );
          return androidTooling.remove(
            androidManager.summary().total,
            request.signal,
          );
        })(),
  ...
  const cachedHealthDiagnostic = () => {
    if (!healthDiagnostic) {
      ...
      pending.catch(() => {
        if (healthDiagnostic === pending) healthDiagnostic = undefined;
      });
    }
    return healthDiagnostic;
  };
  const webScoutExtensionPath = fileURLToPath(
    new URL("./web-scout-browser.ts", import.meta.url),
  );
  const disposeWebScoutCapability = pi.events.on(
    "pi-helios:web-scout-capability",
    (request: any) => {
      ...
      (async () => {
        ...
        try {
          ...
          return {
            ...
            lines: [
              `CLI: ${version}`,
              `Browser sessions: ${sessions.total} (${sessions.owned} owned, ${sessions.attached} attached, ${sessions.cleanupRequired} cleanup-required)`,
              `Android sessions: ${androidSessions.total} (${androidSessions.owned} owned, ${androidSessions.attached} attached, ${androidSessions.cleanupRequired} cleanup-required)`,
              "Web Scout browser broker: ready",
            ],
            warning:
              sessions.cleanupRequired > 0 ||
              androidSessions.cleanupRequired > 0,
          };
        } catch (error) {
          return {
            version: 1,
            ...
            lines: [
              error instanceof Error
                ? `CLI: ${error.message}`
                : "CLI: unavailable",
              `Browser sessions: ${sessions.total}`,
              `Android sessions: ${androidSessions.total} (${androidSessions.cleanupRequired} cleanup-required)`,
            ],
            warning: true,
          };
        }
  ...
  pi.on("session_start", async () => {
    ...
    pi.events.emit("pylon:tool-policy", {
      ...
      toolUsage: {
        helios_browser:
          "navigate and interact with browser pages, tabs, and screenshots",
        helios_capture:
          "capture a consented Windows window for visual debugging",
        helios_android:
          "list packages on, start, or attach to an Android emulator and navigate one app with constrained Appium actions",
      },
    });
  });
  pi.on("session_shutdown", async (_event, ctx) => {
    ...
    disposeHealth();
    const [summary, androidSummary] = await Promise.all([
      manager.shutdown(),
      androidManager.shutdown(),
    ]);
    if (!ctx.hasUI) {
      if (
        summary.failures.length ||
        androidSummary.failures.length ||
        summary.cleanupWarnings.length ||
        androidSummary.cleanupWarnings.length
      ) {
        throw new Error(
          `Helios shutdown cleanup uncertain: ${summary.failures.length} browser failure(s), ${androidSummary.failures.length} Android failure(s), ${summary.cleanupWarnings.length + androidSummary.cleanupWarnings.length} warning(s)`,
        );
      }
      return;
    }
    if (summary.failures.length)
      ctx.ui.notify(
        `Helios could not ${summary.failures.map((item) => item.action).join("/")} ${summary.failures.length} browser session(s). Browser cleanup remains uncertain.`,
        "error",
      );
    if (androidSummary.failures.length)
      ctx.ui.notify(
        `Helios could not clean up ${androidSummary.failures.length} Android session(s). Emulator cleanup remains uncertain.`,
        "error",
      );
    for (const warning of [
      ...summary.cleanupWarnings,
      ...androidSummary.cleanupWarnings,
    ])
      ctx.ui.notify(`Helios cleanup warning: ${warning}`, "warning");
  ...
  pi.registerCommand("helios-doctor", {
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        ctx.ui.notify(
          `Helios CLI ready: ${version}. CLI compatibility is verified.`,
          "info",
        );
      } catch (error) {
        ctx.ui.notify(
  ...
  pi.registerCommand("helios-android-doctor", {
    ...
    handler: async (_args, ctx) => {
      try {
        const [android, appium] = await Promise.all([
          diagnoseAndroid(exec),
          resolveAppium(exec),
        ]);
        ctx.ui.notify(
        ...
        ctx.ui.notify(
          error instanceof Error
            ? error.message
            : "Helios Android diagnostic failed",
          "error",
        );
      }
    },
  ...
  pi.registerCommand("helios-visibility", {
    description:
      "Toggle whether future Helios-owned browsers are shown or headless",
    handler: async (args, ctx) => {
      const action = args.trim().toLowerCase() || "toggle";
      if (
        !(["toggle", "show", "hide", "status"] as const).includes(
          action as "toggle",
        )
      ) {
        ctx.ui.notify(
          "Usage: /helios-visibility [show|hide|toggle|status]",
          "warning",
        );
        return;
      }
      ...
      if (action !== "status")
        await saveConfig({ version: 1, headed: ownedHeaded }, settingsPath);
      const active = manager.get(sessionId(ctx));
      const unchanged =
        active?.ownership === "owned" ? " Active owned session unchanged." : "";
      ctx.ui.notify(
        `Future Helios-owned browsers: ${ownedHeaded ? "shown" : "hidden (headless)"}.${unchanged}`,
        "info",
      );
    },
  });
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Inspect one consented Android emulator or navigate one app through constrained Appium actions",
    promptGuidelines: [
    ...
    async execute(toolCallId, params: AndroidInput, signal, onUpdate, ctx) {
      ...
      if (params.action === "avds") {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Android AVDs (${avds.length}): ${avds.join(", ") || "none"}.`,
            },
          ],
          details: { avds },
        };
      ...
      if (params.action === "packages") {
        ...
        if (!ctx.hasUI)
          throw new Error(
            "Helios Android package inventory requires interactive confirmation",
          );
        const approved = await ctx.ui.confirm(
          "List installed Android packages?",
          `Helios will query system and user-installed package IDs from ${serial}. Package IDs may reveal sensitive apps, and the complete list may enter model/session history.`,
        );
        if (!approved)
          return {
            content: [
              {
                type: "text" as const,
                text: "User declined Android package inventory.",
              },
            ],
        ...
        onUpdate?.({
          content: [
            {
              type: "text" as const,
              text: `Listing installed packages on ${serial}...`,
            },
          ],
          details: {},
        });
        if (ctx.hasUI)
          ctx.ui.setStatus?.(
            "pi-helios",
            `android: listing packages on ${serial}`,
          );
        try {
          const inventory = await (
            await AndroidSdk.create(exec)
          ).listInstalledPackages(serial, signal);
          const heading = `Installed Android packages on ${inventory.serial} / ${inventory.avd} (${inventory.packages.length})`;
          return {
            content: [
              {
                type: "text" as const,
                text: `${heading}:${inventory.packages.length ? `\n${inventory.packages.join("\n")}` : " none."}`,
              },
            ],
            details: {
      ...
      if (params.action === "status") {
        ...
        if (!record)
          return {
            content: [
              {
                type: "text" as const,
                text: "No active Helios Android session.",
              },
            ],
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Android session ${record.state} (${record.ownership}): ${record.serial} / ${record.avd}; package ${record.packageName}.`,
            },
          ],
          details: { active: true, ...record },
        };
      }
      if (params.action === "start" || params.action === "attach") {
        if (!ctx.hasUI)
          throw new Error(
            "Helios Android start and attachment require interactive confirmation",
          );
        ...
        if (params.action === "start") {
          ...
          approved = await ctx.ui.confirm(
            ...
            `Helios will launch and control AVD “${avd}” for package ${packageName}, start a private loopback Appium server, and stop this emulator when closed. UI source and screenshots may contain secrets and enter model/session history.`,
          );
          if (!approved)
            return {
              content: [
                {
                  type: "text" as const,
                  text: "User declined Android emulator start.",
                },
              ],
              details: { declined: true },
            };
          if (androidTooling.isMutating())
            throw new Error("Android tooling setup is in progress");
          onUpdate?.({
            content: [
              { type: "text" as const, text: `Starting Android AVD ${avd}...` },
            ],
            details: {},
          });
          if (ctx.hasUI)
            ctx.ui.setStatus?.("pi-helios", `android: starting ${avd}`);
          try {
            const result = await androidManager.start(
              id,
              avd,
              packageName,
              params.appActivity,
            ...
            return {
              content: [
                { type: "text" as const, text: describeAndroid(result) },
              ],
              details: result,
            };
          } finally {
        ...
        const serial = requireAndroidField(params, "serial");
        approved = await ctx.ui.confirm(
          "Attach to Android emulator?",
          `Helios will control existing emulator ${serial} for package ${packageName} through a private loopback Appium server. Existing app data and UI may become accessible to the selected model and retained in Pi history. Helios will detach without stopping the emulator.`,
        );
        if (!approved)
          return {
            content: [
              {
                type: "text" as const,
                text: "User declined Android emulator attachment.",
              },
            ],
            details: { declined: true },
          };
        ...
        onUpdate?.({
          content: [
            {
              type: "text" as const,
              text: `Attaching to Android emulator ${serial}...`,
            },
          ],
          details: {},
        });
        if (ctx.hasUI)
          ctx.ui.setStatus?.("pi-helios", `android: attaching ${serial}`);
        try {
          const result = await androidManager.attach(
            id,
            serial,
            packageName,
            params.appActivity,
          ...
          return {
            content: [{ type: "text" as const, text: describeAndroid(result) }],
            details: result,
          };
        } finally {
          if (ctx.hasUI) ctx.ui.setStatus?.("pi-helios", undefined);
      ...
      onUpdate?.({
        content: [
          {
            type: "text" as const,
            text: `Running Android ${params.action}...`,
          },
        ],
        details: {},
      });
      const result = await androidManager.operate(
        id,
        androidAction(params),
        signal,
      );
      ...
      try {
        ...
        return {
          ...
          details: { ...result, artifactPath: undefined },
        };
      } finally {
        await rm(result.artifactPath, { force: true }).catch(() => {
          if (ctx.hasUI)
            ctx.ui.notify(
              "Helios could not delete temporary Android screenshot.",
              "warning",
            );
        });
      }
  ...
  pi.registerTool({
    ...
    description:
      "Capture one named Windows window for visual debugging only when the user asks to inspect it. Every capture requires fresh visible confirmation. Never use for monitoring; it cannot capture the whole desktop, run in the background, or control input.",
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      if (!ctx.hasUI)
        throw new Error("Helios capture requires interactive confirmation");
      if (ctx.model && !ctx.model.input.includes("image"))
        throw new Error("Selected model does not support image input");
      const windowTarget = await findWindow(
      ...
      const approved = await ctx.ui.confirm(
        "Allow screenshot?",
        `Helios will capture ${source}. Screenshots may contain secrets. Image and selected window metadata will be sent to selected model provider and retained in Pi session history.`,
      );
      if (!approved)
        return {
          content: [
            {
              type: "text" as const,
              text: "User declined screenshot capture.",
            },
          ],
          details: { declined: true },
        };
      onUpdate?.({
        content: [{ type: "text" as const, text: `Capturing ${source}...` }],
        details: {},
      });
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Captured Windows window: ${windowTarget.title}`,
            },
            ...result.content,
          ],
          details: { target: "window", windowTitle: windowTarget.title },
        };
      } finally {
        await rm(directory, { recursive: true, force: true }).catch(() =>
          ctx.ui.notify(
            `Helios could not delete temporary capture directory: ${directory}`,
            "warning",
          ),
        );
      }
    ...
    async execute(toolCallId, params: BrowserInput, signal, onUpdate, ctx) {
      const executeAction = async (params: BrowserParams) => {
        ...
        if (params.action === "start") {
          ...
          return {
            ...
            details: compactBrowserDetails(result),
          };
        }
        if (params.action === "attach") {
          ...
          if (!ctx.hasUI)
            throw new Error(
              "Helios browser attachment requires interactive confirmation",
            );
          const mode = requireField(params, "attachMode");
          if (mode === "cdp") {
            if (params.browser !== undefined)
              throw new Error("CDP attachment does not accept browser");
            const endpoint = validateCdpEndpoint(
              requireField(params, "endpoint"),
            );
            const approved = await ctx.ui.confirm(
              "Attach to existing browser?",
              `Helios will connect to ${endpoint}. Existing tabs, logins, and page data exposed by this endpoint may become accessible to selected model provider and retained in Pi session history. Helios will detach without closing browser.`,
            );
            if (!approved)
              return {
                content: [
                  {
                    type: "text" as const,
                    text: "User declined browser attachment.",
                  },
                ],
                details: { declined: true },
              };
            const result = await withBrowserStatus(ctx, "attach", () =>
              manager.attachCdp(id, endpoint, signal),
            );
            return {
              content: [{ type: "text" as const, text: describe(result) }],
              details: compactBrowserDetails(result),
          ...
          if (params.endpoint !== undefined)
            throw new Error("Extension attachment does not accept endpoint");
          const browser = requireField(params, "browser");
          const approved = await ctx.ui.confirm(
            "Attach through browser extension?",
            `Helios will connect through enabled Playwright bridge in ${browser}. Tabs, logins, and page data allowed by extension may become accessible to selected model provider and retained in Pi session history. Helios will detach without closing browser.`,
          );
          if (!approved)
            return {
              content: [
          ...
          const result = await withBrowserStatus(ctx, "attach", () =>
            manager.attachExtension(id, browser, signal),
          );
          return {
            content: [{ type: "text" as const, text: describe(result) }],
            details: compactBrowserDetails(result),
        ...
        if (params.action === "close" || params.action === "detach") {
          ...
          const result = await withBrowserStatus(ctx, action, () =>
            manager.close(id, action, signal),
          );
          return {
            content: [{ type: "text" as const, text: describe(result) }],
            details: compactBrowserDetails(result),
        ...
        if (
          params.action === "tabs" &&
          params.tabAction === "close" &&
          manager.get(id)?.ownership !== "owned"
        ) {
          if (
            !ctx.hasUI ||
            !(await ctx.ui.confirm(
              "Close user browser tab?",
              `Helios will close tab ${params.tabIndex ?? "current"} in attached user browser.`,
            ))
          ) {
            return {
              content: [
                { type: "text" as const, text: "User declined tab close." },
              ],
              details: { declined: true },
            };
          }
        ...
        onUpdate?.({
          content: [
            {
              type: "text" as const,
              text: `Running browser ${params.action}...`,
            },
          ],
          details: {},
        });
        const result = await withBrowserStatus(ctx, params.action, () =>
          manager.operate(id, browserAction(params), signal),
        );
        if (!result.artifactPath)
          return {
            content: [{ type: "text" as const, text: describe(result) }],
        ...
        try {
          ...
          return {
            content: [
              { type: "text" as const, text: describe(result) },
              ...image.content,
            ],
            details,
          };
          ...
          await rm(result.artifactPath, { force: true }).catch(() => {
            if (ctx.hasUI)
              ctx.ui.notify(
                "Helios could not delete temporary browser screenshot.",
                "warning",
              );
          });
        }
      ...
      const executePlan = async (plan: BrowserPlanStep[]) => {
        ...
        const steps: Array<{
          ...
          match: string;
          target?: string;
          details?: BrowserOperationResult;
        }> = [];
        for (const [index, step] of plan.entries()) {
          onUpdate?.({
            content: [
              {
                type: "text" as const,
                text: `Resolving browser plan step ${index + 1}...`,
              },
            ],
            details: {},
          });
          const found = await withBrowserStatus(ctx, "plan-find", () =>
            manager.operate(id, { kind: "find", text: step.match }, signal),
          );
          ...
          if (!target) {
            ...
            return {
              content: [
                {
                  type: "text" as const,
                  text: `${prefix}Plan stopped at step ${index + 1} (${step.action}): ${reason}.\n${describe(found)}`,
                },
              ],
              details: {
                steps,
                ...
                stoppedAt: index + 1,
                reason,
                find: compactBrowserDetails(found),
              },
            };
          }
          ...
          try {
            result = await withBrowserStatus(ctx, `plan-${step.action}`, () =>
              manager.operate(id, planAction(step, target), signal),
            );
          } catch (error) {
            const message =
              error instanceof Error ? error.message : String(error);
            throw new Error(
              `Browser plan step ${index + 1} (${step.action}) failed: ${message}`,
              { cause: error },
            );
          }
          const details = compactBrowserDetails(result);
          ...
          summaries.push(
            `Step ${index + 1} (${step.action} ${target}): completed.`,
          );
          let observed: BrowserOperationResult | undefined = result;
          if (step.action !== "click") {
            try {
              observed = await manager.operate(
                id,
                { kind: "tab-list" },
                signal,
              );
          ...
          const after =
            observed &&
            !observed.metadataStale &&
            observed.metadataAvailable !== false
              ? pageKey(observed.page)
              : undefined;
          const changed =
          ...
          if (changed || uncertain || final) {
            ...
            return {
              ...
              details: {
                steps,
                completed: steps.length,
                ...(reason && !final ? { stoppedAt: index + 1, reason } : {}),
              },
            };
          }
        }
      ...
      if (params.plan !== undefined) {
        if (
          Object.entries(params).some(
            ([key, value]) => key !== "plan" && value !== undefined,
          )
        )
          throw new Error("Browser plan must contain only plan");
        return executePlan(params.plan);
      }
      if (params.actions === undefined) {
        if (params.action === undefined)
          throw new Error("helios_browser requires action, actions, or plan");
      ...
      for (const [index, action] of params.actions.entries()) {
        ...
        try {
          ...
          const message =
            error instanceof Error ? error.message : String(error);
          throw new Error(
            `Browser batch step ${index + 1} (${action.action}) failed: ${message}`,
            { cause: error },
          );
        }
        steps.push({ action: action.action, details: result.details });
        const details = result.details as BrowserOperationResult & {
          declined?: boolean;
        };
        ...
        if (final || details.declined) {
          for (const item of result.content) {
            content.push(
              item.type === "text"
                ? {
                    ...item,
                    text: `Step ${index + 1} (${action.action}):\n${item.text}`,
                  }
                : item,
            );
          }
          ...
          content.push({
            type: "text",
            text: describeCompactBatchStep(
              index + 1,
              action.action,
              details,
              pageKey !== undefined && pageKey !== previousPage,
            ),
          });
          for (const item of result.content)
