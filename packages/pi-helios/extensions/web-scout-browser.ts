...
const parameters = Type.Object(
  {
    ...
    url: Type.Optional(
      Type.String({
        maxLength: 2048,
        description: "Required only for navigate.",
      }),
    ),
    target: Type.Optional(
      Type.String({
        ...
        maxLength: 32,
        description:
          "Required only for follow; use a link reference from the latest snapshot chunk.",
      }),
    ),
    cursor: Type.Optional(
      Type.String({
        ...
        maxLength: 35,
        description:
          "Required only for continue; consume the latest continuation cursor immediately.",
      }),
    ),
  },
...
function describe(
  ...
  const lines = [
    `Pages: ${pages}/${maxPages}. Actions: ${actions}/${maxActions}.`,
  ];
  if (result.page)
  ...
  if (result.snapshotTruncated)
    lines.push(
      `Snapshot truncated; ${result.snapshotOmittedLines ?? 0} lines remain.`,
    );
  ...
  else if (result.metadataAvailable === false)
...
function snapshotArtifactFailure(error: unknown): boolean {
  return (
    ...
    /snapshot artifact/i.test(error.message)
  );
}

export default async function webScoutBrowserExtension(
  pi: ExtensionAPI,
  options: { persistentClient?: boolean } = {},
) {
  ...
  const consumeAction = () => {
    actions++;
    if (actions > grant.maxActions)
      throw new Error("Web Scout action limit reached");
  };
  const ensurePageAvailable = () => {
    if (pages >= grant.maxPages)
      throw new Error("Web Scout page limit reached");
  };
  ...
  const snapshot = async (signal?: AbortSignal) =>
    acceptSnapshot(
      await manager.operate(sessionId, { kind: "snapshot", depth: 6 }, signal),
    );
  const observePage = async (
    ...
    try {
      ...
      return changed.snapshot === undefined
        ? await snapshot(signal)
        : acceptSnapshot(changed);
    } catch (error) {
      if (!snapshotArtifactFailure(error)) throw error;
      try {
        return acceptSnapshot(
          await manager.operate(sessionId, { kind: "page-text" }, signal),
        );
      } catch (fallbackError) {
        if (
          fallbackError instanceof HeliosCliError &&
          fallbackError.category === "invalid-output"
        )
          throw error;
        throw fallbackError;
      }
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Read public web pages through isolated bounded browser navigation",
    promptGuidelines: [
    ...
    async execute(_id, params, signal) {
      ...
      if (input.action === "navigate") {
        ...
        if (input.target !== undefined || input.cursor !== undefined)
          throw new Error("navigate accepts only url");
        ensurePageAvailable();
      ...
      if (input.url !== undefined)
        throw new Error(`${input.action} does not accept url`);
      if (input.action === "continue") {
        if (!input.cursor) throw new Error("continue requires cursor");
        if (input.target !== undefined)
          throw new Error("continue does not accept target");
        return response(
          input.action,
          acceptSnapshot(
            await manager.operate(
              sessionId,
              { kind: "continue", cursor: input.cursor },
              signal,
            ),
          ),
        );
      ...
      if (input.cursor !== undefined)
        throw new Error(`${input.action} does not accept cursor`);
      if (input.action === "follow") {
        if (!input.target) throw new Error("follow requires target");
        if (!linkRefs.has(input.target))
          throw new Error(
            "follow target must be a link reference from latest snapshot",
          );
        ensurePageAvailable();
        const href = await manager.operate(
          sessionId,
          { kind: "link-url", target: input.target },
          signal,
        );
        if (!href.resolvedUrl)
          throw new Error("Link has no public navigation URL");
      ...
      if (input.action === "back") {
        if (input.target !== undefined)
          throw new Error("back does not accept target");
        ensurePageAvailable();
      ...
      if (input.action !== "snapshot")
        throw new Error("Unsupported Web Scout browser action");
      if (input.target !== undefined)
        throw new Error("snapshot does not accept target");
