...
const toolSchema = Type.Object({
  ...
  target: Type.Optional(Type.String({ description: "SQLite path or credential-free PostgreSQL/MySQL URL. In Pylon Web, a server URL with username but no password opens a masked password dialog.", maxLength: 4096 })),
  sql: Type.Optional(Type.String({ maxLength: 100_000 })),
  where: Type.Optional(Type.String({ maxLength: 20_000 })),
  handle: Type.Optional(Type.String({ maxLength: 200 })),
  name: Type.Optional(Type.String({ maxLength: 200 })),
  as: Type.Optional(Type.String({ maxLength: 200 })),
  kind: Type.Optional(StringEnum(["schema", "table", "columns", "indexes", "constraints"] as const)),
  table: Type.Optional(Type.String({ maxLength: 500 })),
  params: Type.Optional(Type.Any({ description: "Positional JSON array or named JSON object" })),
  cache: Type.Optional(StringEnum(["auto", "bypass", "require"] as const)),
  read_only: Type.Optional(Type.Boolean()),
  secret_env: Type.Optional(Type.String({ description: "Environment variable whose value is the complete PostgreSQL/MySQL URL or explicit sqlite:<path> source; use instead of target", pattern: "^[A-Za-z_][A-Za-z0-9_]*$", maxLength: 200 })),
  profile: Type.Optional(Type.String({ maxLength: 200 })),
  replay: Type.Optional(Type.Boolean()),
  idempotency_key: Type.Optional(Type.String({ maxLength: 500 })),
  allow_unbounded: Type.Optional(Type.Boolean()),
  allow_destructive: Type.Optional(Type.Boolean()),
  offset: Type.Optional(Type.Integer({ minimum: 0, maximum: 10_000 })),
  limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 100 })),
  isolation: Type.Optional(Type.String({ maxLength: 50 })),
  timeout_ms: Type.Optional(Type.Integer({ minimum: 1, maximum: 2_147_483_647 })),
