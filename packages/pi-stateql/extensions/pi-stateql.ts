...
function validateInput(
  ...
  if (input.params !== undefined) {
    ...
    let encoded: string;
    try {
      encoded = JSON.stringify(input.params);
    } catch {
      throw new Error("params must be JSON-serializable");
    }
    if (Buffer.byteLength(encoded, "utf8") > MAX_PARAMS_BYTES)
      throw new Error(`params cannot exceed ${formatSize(MAX_PARAMS_BYTES)}`);
  }
    ...
    async execute(_toolCallId, input: StateQLToolInput, signal, onUpdate, ctx) {
      ...
      try {
        return await exclusive(async () => {
          ...
          try {
            response = await active.stateql.executeCommand(executionCommand);
          } catch (error) {
