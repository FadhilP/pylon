...
import { StateQL, type BatchCommand, type CredentialRequest, type StateQLActorOptions, type StateQLSnapshot } from "@fadhilp/stateql";
...
class FakeStateQL {
  ...
  constructor(options: StateQLActorOptions) { this.options = options; }
...
function harness(real = false) {
  ...
  else stateqlExtension(pi as any, {
...
test("composes environment and active Pylon credential resolution without retaining the host", async () => {
  ...
  try {
    ...
    const result = await value.tools.get("stateql").execute(
      "credential",
...
test("Pylon requires insecure TLS approval before releasing the brokered password", async () => {
  ...
  let confirmation = "";
  const ui = {
    async confirm(title: string, message: string) { order.push("confirm"); confirmation = `${title} ${message}`; return true; },
    setStatus() {},
    async requestStateQLCredential() { throw new Error("full source prompt should not run"); },
    async requestStateQLPassword() { order.push("password"); return "private"; },
  };
  value.instances[0].executeImpl = async (command) => {
...
test("Pylon invalidates brokered PostgreSQL and MySQL passwords after authentication failure", async () => {
  const value = await start();
  const failures = ["password authentication failed for user", "Access denied for user 'app'@'host' (using password: YES)"];
  let invalidations = 0;
  const ui = {
    async confirm() { throw new Error("normal brokered connect should not confirm twice"); },
    setStatus() {},
    async requestStateQLCredential() { throw new Error("full source prompt should not run"); },
    async requestStateQLPassword() { return "wrong"; },
  ...
  for (const [callId, target] of [
    ...
    await assert.rejects(
      value.tools.get("stateql").execute(callId, { command: "connect", target, read_only: true }, undefined, undefined, context({ ui })),
...
test("unknown internal password references fail closed without environment or full-source fallback", async () => {
  ...
  const ui = {
    async confirm() { return true; },
    setStatus() {},
    async requestStateQLCredential() { credentialCalls++; return sentinel; },
    async requestStateQLPassword() { passwordCalls++; return "private"; },
  };
  value.instances[0].executeImpl = async () => {
  ...
  try {
    ...
    await value.tools.get("stateql").execute("unknown", { command: "query", sql: "SELECT 1" }, undefined, undefined, context({ ui }));
...
test("missing Pylon credential capability fails closed without prompting or fallback", async () => {
  ...
  try {
    ...
    value.instances[0].executeImpl = async () => {
      ...
      return { ok: true, command_id: "cmd_missing", session_id: "s_1", data: {}, warnings: [], meta: { duration_ms: 1 } };
    };
    await value.tools.get("stateql").execute(
      "missing",
      { command: "query", sql: "SELECT 1" },
...
test("executes reads without confirmation and returns compact details", async () => {
  const value = await start();
  let confirmations = 0;
  const ctx = context({ ui: { async confirm() { confirmations += 1; return true; }, setStatus() {} } });
  const result = await value.tools.get("stateql").execute(
    "call-1",
  ...
  assert.equal(confirmations, 0);
  assert.deepEqual(value.instances[0].commands, [{ command: "query", sql: "SELECT id FROM users ORDER BY id LIMIT 5", params: [] }]);
...
test("snapshot bridge is bounded, actor-scoped, and single-claim", async () => {
  ...
  const request = {
    ...
    historyLimit: 25,
    claim() { claims += 1; return claims === 1; },
    respond(result: Promise<StateQLSnapshot>) { response = result; },
  };
  handler(request);
  handler(request);
  assert.ok(response);
...
test("confirmed operations fail closed and declined commands do not execute", async () => {
  ...
  const tool = value.tools.get("stateql");
  let confirmation = "";
  const declined = await tool.execute("call", { command: "exec", sql: "DROP TABLE users", replay: true, allow_destructive: true }, undefined, undefined,
    context({ ui: { async confirm(_title: string, message: string) { confirmation = message; return false; }, setStatus() {} } }));
...
test("rejects irrelevant, ambiguous, and oversized inputs before StateQL", async () => {
  ...
  await assert.rejects(
    tool.execute("call", { command: "query", sql: "SELECT ?", params: ["x".repeat(33 * 1024)] }, undefined, undefined, context()),
...
test("failures redact credentials and successful output stays within its advertised cap", async () => {
  ...
  value.instances[0].executeImpl = async () => ({
    ...
    session_id: "s_1",
    error: { code: "CONNECTION_FAILED", message: "postgres://user:password@example.com/app password=hunter2", retryable: true, executed: false },
    meta: { duration_ms: 1 },
  });
  await assert.rejects(
...
test("cancellation aborts, replaces the session instance, and permits the next command", async () => {
  ...
  value.instances[0].executeImpl = async () => new Promise((resolve) => {
    value.instances[0].options.signal!.addEventListener("abort", () => resolve({
      ok: false,
      command_id: "cmd_2",
      session_id: "s_1",
  ...
  const running = value.tools.get("stateql").execute(
    "call",
    { command: "query", sql: "SELECT 1" },
    controller.signal,
    undefined,
  ...
  const next = await value.tools.get("stateql").execute("next", { command: "query", sql: "SELECT 2" }, undefined, undefined, context());
...
test("snapshot remains available while database work is running", async () => {
  ...
  let release!: () => void;
  value.instances[0].executeImpl = async () => new Promise((resolve) => {
    release = () => resolve({ ok: true, command_id: "cmd_queued", session_id: "s_1", data: {}, warnings: [], meta: { duration_ms: 1 } });
  });
...
test("real StateQL credential resolution resumes one connect call without leaking the supplied URL", { timeout: 15_000 }, async () => {
  ...
  const ctx = context({
    ui: {
      ...
      async requestStateQLCredential(request: CredentialRequest) {
        ...
        return new Promise<string>((resolve) => { resolveCredential = resolve; });
      },
    },
  });
...
test("real StateQL integration reuses a linked workspace handle", async () => {
  ...
  try {
    ...
    await value.handlers.get("session_start")![0]({}, ctx);
    const shown = await value.tools.get("stateql").execute("show", { command: "show", handle: "users" }, undefined, undefined, ctx);
...
test("real StateQL integration persists commands and exposes bounded SQL snapshot history", async () => {
  ...
  try {
    ...
    value.events.get("pylon:stateql-snapshot-request")![0]({
      ...
      historyLimit: 10,
      claim: () => true,
      respond(result: Promise<StateQLSnapshot>) { repeated = result; },
    });
    assert.ok(repeated);
