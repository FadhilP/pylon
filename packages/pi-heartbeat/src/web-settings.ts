...
import { configPath, heartbeatSettings, loadConfig, saveConfig } from "./config.ts";

export async function readSettings({ agentDir }: { agentDir: string }) {
  return effectivePackageSettingsReadModel(heartbeatSettings, await loadConfig(configPath(agentDir)));
}

export async function updateSettings(value: unknown, { agentDir }: { agentDir: string }): Promise<void> {
  const values = extractPackageSettingsUpdate(heartbeatSettings, value);
