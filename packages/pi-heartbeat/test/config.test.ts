...
test("heartbeat settings use supplied agentDir and expose defaults for missing legacy config", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "heartbeat-settings-"));
  try {
