...
test("usage observer failures do not control the worker", async () => {
  ...
  await writeFile(
    script,
    `console.log(JSON.stringify({type:'message_end',message:{role:'assistant',content:[{type:'text',text:'done'}],stopReason:'stop',usage:{input:1}}}))`,
  );
  const run = await runPi([], {
    cwd: dir,
    invocation: { command: process.execPath, args: [script] },
...
test("runner stops before another turn when budget is exhausted", async () => {
  ...
  await writeFile(
    script,
    `console.log(JSON.stringify({type:'message_end',message:{role:'assistant',content:[{type:'text',text:'continue'}],stopReason:'toolUse',usage:{cost:{total:0.5}}}}))`,
  );
  const run = await runPi([], {
    cwd: dir,
    maxTurns: 1,
