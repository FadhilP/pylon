...
test("worker context keeps long identifiers the worker needs but drops provider keys", () => {
  ...
  const context = buildWorkerContext(
    [
      { type: "message", message: { role: "user", content: `Fix the regression introduced in ${commit}` } },
      { type: "message", message: { role: "assistant", content: `Deploy key is ${key}` } },
    ],
    6000,
  );
  assert.ok(context.includes(commit), "a commit hash must reach the worker intact");
  assert.ok(!context.includes(key), "a provider key must never reach the worker");
});
...
test("worker context redacts before whole-record budget packing", () => {
  ...
  const budget = 60;
  const context = buildWorkerContext([{ type: "message", message: { role: "user", content: secret } }], budget);
  assert.equal(context, "User: [possible credential redacted]");
  assert.equal(
    buildWorkerContext([{ type: "message", message: { role: "user", content: secret } }], budget, 10, [secret]),
    "",
  );
});
