...
test("isolated worktree includes dirty and untracked parent state then applies only worker delta", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      isolated.workerRoot.replace(/\\/g, "/"),
    );
    assert.equal(
      await readFile(join(isolated.workerRoot, "tracked.txt"), "utf8"),
      "parent dirty\n",
    );
    assert.equal(
      await readFile(join(isolated.workerRoot, "untracked.txt"), "utf8"),
      "parent untracked\n",
    );

    await writeFile(
      join(isolated.workerRoot, "tracked.txt"),
      "worker result\n",
    );
    await writeFile(
      join(isolated.workerRoot, "untracked.txt"),
      "worker untracked result\n",
    );
    await mkdir(join(isolated.workerRoot, "src"));
    await writeFile(
      join(isolated.workerRoot, "src", "new.ts"),
      "export const value = 1;\n",
    );

    const worker = await collectWorkerPatch(exec, isolated);
    assert.deepEqual(worker.changedPaths, [
      "src/new.ts",
      "tracked.txt",
      "untracked.txt",
    ]);
...
test("baseline creation disables repository hooks", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      (await collectWorkerPatch(exec, isolated)).changedPaths,
      [],
    );
  } finally {
    await removeIsolatedWorktree(exec, isolated);
...
test("apply rechecks parent state immediately before integration", async () => {
  ...
  try {
    ...
    await assert.rejects(
      applyWorkerPatch(exec, isolated, worker.patch),
      /immediately before patch apply/,
    );
  } finally {
    await removeIsolatedWorktree(exec, isolated);
  }
...
test("cleanup reports failures without throwing and releases the transaction", async () => {
  ...
  const failingExec = async (command: string, args: string[]) =>
    args.includes("remove")
      ...
      : exec(command, args);
  assert.deepEqual(await removeIsolatedWorktree(failingExec, isolated), [
    "worktree cleanup: busy",
  ]);
