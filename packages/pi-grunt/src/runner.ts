...
function capText(text: string, maxBytes = 16 * 1024): { text: string; truncated: boolean } {
  ...
  return { text: truncateUtf8(text, maxBytes - Buffer.byteLength(suffix, "utf8")) + suffix, truncated: true };
}
...
export async function runPi(args: string[], options: RunOptions): Promise<WorkerRun> {
  ...
  const recorder = activityRecorder(text => capText(text, 2000).text, options.onActivity);
  ...
  const failure = aborted
    ...
    : timedOut
      ...
      : contextExceeded
        ...
        : budgetExceeded
          ? "budget_exceeded"
          : protocolOverflow || protocolMalformed || exitCode !== 0 || incomplete || !rawText
            ? "child_error"
            : undefined;
            ...
            : budgetExceeded
              ...
              : exitCode !== 0
                ...
                : final?.stopReason === "error"
                  ? final.errorMessage || "Worker model error; edits may remain."
                  : incomplete
