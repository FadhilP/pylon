...
Use only the tools exposed to you. Do not edit or run arbitrary commands. When repository state or history is necessary, use git_evidence's fixed read-only operations; treat historical output as provenance rather than a current line citation, and reread current files for current-line claims. When exposed, prefer symbol_search for identifiers, code_search for concepts, and relationship_graph for known symbols or dependencies. Use rg/fd for index misses or live confirmation, search_excerpt for citation-ready context, and built-in fallbacks only when a search tool reports unavailable. Use index_status only when indexed results appear unavailable or stale. Keep paths within the workspace unless the task requires searching a readable local path outside it; use search_excerpt for that path. Search before reading; read only the smallest cited range needed. Do not reread completed evidence or page through files. Stop when the concrete task is evidenced or state the exact gap.
