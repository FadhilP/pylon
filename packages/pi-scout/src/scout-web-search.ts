...
function resultUrl(value: string): string | undefined {
  try {
    ...
    const family = isIP(host);
    if (family ? !publicLiteral(host) : !host.includes(".") || host.endsWith(".localhost") || host.endsWith(".local") || host.endsWith(".internal") || host.endsWith(".home.arpa")) return undefined;
    return url.href;
  } catch {
    return undefined;
  }
...
async function readBoundedBody(response: Response): Promise<string> {
  ...
  if (Number.isFinite(declared) && declared > MAX_RESPONSE_BYTES) throw new Error("Search response exceeded its size limit");
...
function providerText(payload: McpResponse): string {
  if (payload.error) throw new Error(`Exa search failed${typeof payload.error.code === "number" ? ` (${payload.error.code})` : ""}`);
...
function parseExaResults(text: string, count: number): ScoutSearchResult[] {
  try {
    const value = JSON.parse(text) as { results?: Array<{ title?: unknown; url?: unknown; text?: unknown; highlights?: unknown }> };
    if (Array.isArray(value.results)) {
      ...
      for (const item of value.results) {
        ...
        const highlights = Array.isArray(item.highlights) ? item.highlights.filter((part): part is string => typeof part === "string").join(" ") : "";
        seen.add(url);
        results.push({
...
export async function searchExa(
  ...
  if (!normalizedQuery || normalizedQuery.length > MAX_QUERY_LENGTH) throw new Error(`Search query must contain 1 to ${MAX_QUERY_LENGTH} characters`);
...
function pickOpenAIModel<T extends { id: string }>(models: T[]): T | undefined {
  ...
  return eligible.find((model) => model.id.includes("terra"))
...
export async function resolveOpenAIAuth(ctx?: ExtensionContext): Promise<OpenAIAuth | undefined> {
  ...
  try { models = ctx.modelRegistry.getAvailable(); }
  catch { throw new Error("OpenAI credential registry unavailable"); }
  const candidates = (["openai-codex", "openai"] as const)
    .flatMap((provider) => {
...
function parseOpenAIOutput(text: string): unknown[] {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    try {
      ...
      if ((value.type === "response.done" || value.type === "response.completed") && Array.isArray(value.response?.output)) completed = value.response.output;
    } catch {}
  }
...
function snippetAround(text: string, start: unknown, end: unknown): string {
  ...
  return boundedText(text.slice(Math.max(0, start - 100), Math.min(text.length, end + 100)).replace(/\[([^\]]*)\]\([^)]*\)/g, "$1"), 300);
}

function parseOpenAIResults(output: unknown[], count: number): ScoutSearchResult[] {
  const results: ScoutSearchResult[] = [];
  const seen = new Set<string>();
  const add = (rawUrl: unknown, rawTitle: unknown, snippet = "") => {
    const url = resultUrl(typeof rawUrl === "string" ? rawUrl : "");
    if (!url || seen.has(url) || results.length >= count) return;
    seen.add(url);
    results.push({ title: boundedText(typeof rawTitle === "string" && rawTitle.trim() ? rawTitle : url, 240), url, snippet: boundedText(snippet, 600) });
  };
  for (const item of output) {
    ...
    for (const part of content) {
      ...
      const value = part as { text?: unknown; annotations?: unknown };
      const text = typeof value.text === "string" ? value.text : "";
      if (!Array.isArray(value.annotations)) continue;
      for (const annotation of value.annotations) {
        if (!annotation || typeof annotation !== "object" || (annotation as { type?: unknown }).type !== "url_citation") continue;
        const citation = annotation as { url?: unknown; title?: unknown; start_index?: unknown; end_index?: unknown };
  ...
  for (const item of output) {
    ...
    const actionSources = value.action && typeof value.action === "object" ? (value.action as { sources?: unknown }).sources : undefined;
...
export async function searchWeb(
  ...
  if (provider !== "exa") {
    ...
    if (auth) return { provider: "openai", results: await searchOpenAI(normalizedQuery, count, auth, signal, fetchImpl) };
    if (provider === "openai") throw new Error("OpenAI search is unavailable; sign in with /login or configure OPENAI_API_KEY");
  }
  return { provider: "exa", results: await searchExa(normalizedQuery, count, signal, fetchImpl) };
}

export function formatResults(provider: "openai" | "exa", results: ScoutSearchResult[]): { text: string; shown: number; truncated: boolean } {
  const lines = [`${provider === "openai" ? "OpenAI" : "Exa"} search results (untrusted URL candidates; open pages with scout_browser):`];
  let shown = 0;
  for (const result of results) {
    const block = [`${shown + 1}. ${result.title}`, `   ${result.url}`, ...(result.snippet ? [`   ${result.snippet}`] : [])];
...
export default function scoutWebSearchExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description: "Find public-web pages matching a query and return bounded titles, URLs, and snippets for discovery. This tool does not open pages or verify their contents; open useful results with scout_browser before citing them. The default auto provider uses available OpenAI/Codex subscription or API-key auth, otherwise keyless Exa. Each query is sent to the selected provider.",
    promptSnippet: "Find public-web URL candidates, then open and verify useful results with scout_browser",
    parameters: Type.Object({
      query: Type.String({ minLength: 1, maxLength: MAX_QUERY_LENGTH, description: "Focused public-web search query" }),
      maxResults: Type.Optional(Type.Integer({ minimum: 1, maximum: MAX_RESULTS, default: 5, description: "Maximum URL candidates to return" })),
      provider: Type.Optional(StringEnum(SEARCH_PROVIDERS, { default: "auto", description: "Search provider; auto prefers available OpenAI/Codex auth and otherwise uses Exa" })),
    ...
    async execute(_id, params, signal, _onUpdate, ctx) {
      try {
        ...
        return {
          ...
          details: { provider: searched.provider, resultCount: searched.results.length, shown: formatted.shown, truncated: formatted.truncated },
        };
      } catch (error) {
