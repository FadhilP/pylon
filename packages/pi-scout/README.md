...
Reports use exact `path:start-end` citations with excerpts up to eight lines, target about 8 KiB, and cap at 32 KiB without clipping report blocks. The child has read-only `read`, `search_excerpt`, `grep`, `find`, and `ls`; when pi-discover is present it also gets advertised `rg`, `fd`, and `relationship_graph`. It has no shell, mutation tools, unrelated extensions, skills, or context files. Tool output caps at 24 KiB; history keeps 100 events. Timeouts fail nonfatally; retry with a focused gap.
