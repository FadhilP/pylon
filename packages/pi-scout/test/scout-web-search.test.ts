...
test("Exa search rejects off-origin redirects, oversized bodies, and malformed responses", async () => {
  ...
  await assert.rejects(
    searchExa("query", 5, undefined, async () => new Response(oversized)),
    /size limit/,
  );
  await assert.rejects(
    searchExa("query", 5, undefined, async () => new Response("not rpc")),
    /invalid search response/,
  );
  await assert.rejects(
    searchExa("", 5, undefined, async () => new Response(mcpResponse())),
    /1 to 300/,
  );
});
    ...
    results: [
      {
        ...
        snippet: "See Docs for the current reference.",
      },
...
test("registry provider controls the OpenAI endpoint and arbitrary auth headers are not forwarded", async () => {
  ...
  await searchWeb(
    ...
    "openai",
    undefined,
    context([{ provider: "openai", id: "gpt-5.4" }], jwt()),
    async (input, init) => {
...
test("auto search uses Exa only when OpenAI auth is unavailable and explicit OpenAI fails closed", async () => {
  ...
  await assert.rejects(
    searchWeb("query", 1, "openai", undefined, missing, async () => {
      requests++;
      return new Response(mcpResponse());
    }),
    /sign in with \/login/,
