...
test("child invocation does not mistake an embedding web host for the Pi CLI", async () => {
  ...
  try {
    ...
    assert.deepEqual(getPiInvocation(["--mode", "rpc"]).args, [
      embedded.args[0],
      "--mode",
      "rpc",
    ]);
...
test("context and cache-read sizes remain independent", () => {
  assert.equal(
    contextTokensFromUsage({ totalTokens: 210_000, cacheRead: 80_000 }),
    130_000,
  );
  assert.equal(
    cacheReadTokensFromUsage({ totalTokens: 210_000, cacheRead: 80_000 }),
    80_000,
  );
  assert.equal(
    contextTokensFromUsage({
...
test("runner aborts oversized protocol buffers", async () => {
  ...
  await writeFile(
    script,
    `process.stdout.write('x'.repeat(1024*1024*5+1));setTimeout(()=>{},10000);`,
  );
  const run = await runPi([], {
    cwd: dir,
    prompt: "x",
