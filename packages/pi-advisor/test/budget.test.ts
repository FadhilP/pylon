...
test("advisor budget defaults to $0.50 and limits output tokens", () => {
  ...
  assert.deepEqual(advisorBudget(model({ input: 3, output: 100, cacheRead: 0, cacheWrite: 0 }), 100_000, 8_192), {
    maxTokens: 2_000,
...
test("advisor budget refuses input that already exceeds the limit", () => {
  assert.deepEqual(advisorBudget(model({ input: 6, output: 10, cacheRead: 0, cacheWrite: 0 }), 100_000, 8_192), {
...
test("advisor budget accepts genuinely free models and rejects invalid pricing", () => {
  assert.deepEqual(advisorBudget(model({ input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }), 100_000, 200), {
    maxTokens: 200,
    estimatedInputCostUsd: 0,
  });
  assert.deepEqual(advisorBudget(model({ input: Number.NaN, output: 1, cacheRead: 0, cacheWrite: 0 }), 1, 1), {
