...
test("parallel Advisor calls serialize and report running duration", async () => {
  ...
  process.env.PI_CODING_AGENT_DIR = await mkdtemp(
    join(tmpdir(), "pi-advisor-extension-"),
  );
  ...
  try {
    ...
    assert.deepEqual(results[0].details.duplicateTelemetry, {
      records: 0,
      chars: 0,
    });
...
test("advisor records bounded redacted failure diagnostics", async () => {
  const previousDir = process.env.PI_CODING_AGENT_DIR;
  process.env.PI_CODING_AGENT_DIR = await mkdtemp(
    join(tmpdir(), "pi-advisor-failures-"),
  );
  await saveConfig({ version: 1, advisorModel: "test/model" });
  const model = {
    provider: "test",
  ...
  try {
    ...
    assert.doesNotMatch(
      providerError.details.failureMessage,
      /[\u0000-\u001f\u007f-\u009f\u2028\u2029]/,
    );
    assert.doesNotMatch(
      providerError.details.failureMessage,
    ...
    (async () => {
      ...
      if (mode === "exhaust" || mode === "paid")
        return {
          ...
          stopReason: "error",
          errorMessage:
            mode === "exhaust" ? "WebSocket error" : "503 model at capacity",
          usage: {
            input: 1,
            output: 0,
            cacheRead: 0,
            cacheWrite: 0,
            cost: { total: mode === "paid" ? 0.1 : 0 },
          },
        };
      return {
...
test("advisor call renders the executor request instead of the user prompt", () => {
  ...
  assert.equal(
    collapsed,
    "Advisor · test/model · 1 input · 2 output · R3 · W4 · $0.5000 · 1.3s",
  );
  ...
  const failed = tool
    .renderResult(
      {
        content: [
          { type: "text", text: "Advisor failed nonfatally: timeout." },
        ],
        details: {
          advisorModel: "test/model",
          durationMs: 1_250,
  ...
  assert.match(
    tool.parameters.properties.evidence.description,
    /Usually 3–5 concise, non-overlapping decisive ranges/,
  );
  assert.match(
    tool.parameters.properties.evidence.description,
    /use up to 8 only when independently necessary/,
  );
  assert.match(
    tool.parameters.properties.evidence.description,
    /150–300 total lines/,
