...
test("project identity handles Git marker files, symlink aliases, and Windows casing", async () => {
  ...
  try {
    ...
    assert.equal(normalizeProjectIdentity("C:\\Work\\Repo", "win32"), "c:/work/repo");
    ...
    assert.equal(statePath(item.agent, aliasRoot), statePath(item.agent, item.repo));

    const plain = join(item.root, "plain", "nested");
    await mkdir(plain, { recursive: true });
    assert.equal(await projectRoot(plain), plain);
    assert.notEqual(statePath(item.agent, plain), statePath(item.agent, item.repo));
  ...
  try {
    await Promise.all(Array.from({ length: 20 }, (_, index) =>
      updateProjectState(item.agent, item.nested, (state) => {
        const captured = capturePapercut(state, `Friction ${index}`, {}, undefined, `00000000-0000-4000-8000-${String(index).padStart(12, "0")}`);
    ...
    assert.equal(new Set(loaded.state.records.map((record) => record.id)).size, 20);
...
test("malformed state is quarantined and stale locks are recovered", async () => {
  ...
  try {
    ...
    assert.ok((await readdir(dirname(path))).some((name) => name.startsWith(`${path.split(/[\\/]/).pop()}.corrupt-`)));
    ...
    assert.equal((await loadProjectState(item.agent, item.nested)).state.records.length, 1);
    ...
    await mkdir(lock);
    await writeFile(join(lock, "owner.json"), JSON.stringify({ version: 1, token: "dead", pid: 999_999_999, createdAt: "2020-01-01T00:00:00.000Z" }));
    const old = new Date(Date.now() - 60_000);
    await utimes(lock, old, old);
    await updateProjectState(item.agent, item.nested, (state) => ({ state, result: undefined }));
    await assert.rejects(readFile(join(lock, "owner.json")), /ENOENT/);

    await writeFile(path, "x".repeat(MAX_STATE_BYTES + 1));
    await assert.rejects(loadProjectState(item.agent, item.nested), /exceeds 2 MiB limit/);
  } finally { await rm(item.root, { recursive: true, force: true }); }
});
