...
test("capture is bounded, normalized, metadata-bearing, and conservatively deduplicated", () => {
  ...
  const bounded = capturePapercut(
    initial,
    "Another friction",
    { model: `x\u0000${"y".repeat(300)}` },
    at,
  );
  ...
  assert.deepEqual(repeated.record.lastSource, {
    sessionId: "s2",
    model: "other",
  });

  assert.throws(
    () => cleanText("token=super-secret-value", 500, "message"),
    /possible credential/,
  );
  assert.throws(
    () => cleanText("x".repeat(501), 500, "message"),
    /at most 500/,
  );
});
...
test("lifecycle updates support unique prefixes and atomic batches", () => {
  ...
  assert.equal(
    ...
    before,
    "failed batches must not mutate state",
  );
  assert.throws(
    () => updatePapercuts(reopened.state, "resolve", ["aaaa"]),
    /resolution is required/,
  );
  assert.throws(
    () => updatePapercuts(reopened.state, "reopen", ["aaaa"], "extra"),
    /note is not valid/,
  );
});
...
test("closed records do not suppress a fresh occurrence", () => {
  ...
  const dismissed = updatePapercuts(
    captured.state,
    "dismiss",
    ["cccc"],
...
test("web mutations edit safely and delete only the expected record revision", () => {
  ...
  assert.equal(
    state.records[0].message,
    "First friction",
    "mutation must not modify its input",
  );
...
test("record limits fail visibly without eviction", () => {
  ...
  const template = capturePapercut(
    emptyState("/repo", at),
    "Template",
    {},
    at,
  ...
  assert.throws(
    () => capturePapercut(full, "One too many", {}, at),
