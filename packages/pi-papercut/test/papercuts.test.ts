...
test("lifecycle updates support unique prefixes and atomic batches", () => {
  ...
  const resolved = updatePapercuts(state, "resolve", ["aaaa", "bbbb"], "Covered by regression tests.", "2026-01-02T00:00:00.000Z");
  assert.deepEqual(resolved.records.map((record) => record.status), ["resolved", "resolved"]);
  assert.equal(listPapercuts(resolved.state, "open").length, 0);
  assert.equal(listPapercuts(resolved.state, "resolved").length, 2);
  assert.equal(state.records.every((record) => record.status === "open"), true);
  ...
  assert.throws(() => updatePapercuts(reopened.state, "dismiss", ["aaaa", "missing"], "Not actionable"), /unknown papercut id/);
...
test("closed records do not suppress a fresh occurrence", () => {
  const captured = capturePapercut(emptyState("/repo", at), "A stale cache hid changes", {}, at, "cccccccc-3333-4333-8333-333333333333");
  const dismissed = updatePapercuts(captured.state, "dismiss", ["cccc"], "Expected platform behavior", at);
  const fresh = capturePapercut(dismissed.state, "A stale cache hid changes", {}, "2026-01-03T00:00:00.000Z", "dddddddd-4444-4444-8444-444444444444");
