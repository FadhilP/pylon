...
The dedicated `memory` tool supports `list` and one batched `propose` call of at most two additions, replacements, or removals. In Pylon it is deferred until a Memory Reviewer is configured, then becomes active by default so reusable instructions can be proposed when encountered; explicit Tool exposure overrides still apply. User proposals require an exact active-branch quote; project contracts require bounded repository line ranges. Proposals are never durable until strict preflight, a configured tool-free Memory Reviewer, post-review validation, settlement, and any required Verify result all succeed.

The sequential, read-only `continuity_recall` tool searches bounded historical evidence from the current session. Execution scope is the default; explicit lineage scope includes pre-handoff active ancestry, and explicit all scope includes validated sibling branches. Explicit `project_sessions` scope searches the validated active branches of other persisted Pi sessions in registered workspaces with the same project owner (Git-backed when available); the current session is excluded, malformed or legacy sessions are skipped, and results are labeled as untrusted historical evidence. Optional `since` and `before` ISO-8601 UTC timestamps inclusively filter project-session entries; they are rejected for current-session scopes, as are invalid or inverted ranges. File operations use their call-entry time, and stored-result expansion requires both the call and result entries to be in range. Text recall excludes thinking, tool arguments/results, and unrelated custom messages. `project_sessions` is not an exact historical session-ID lookup: when a user supplies a session ID, activate `search_sessions`, pass its `sessionId` field, and use the requested subject as `query` instead of searching the ID as recall query text. Explicit `tools` mode searches sanitized assistant tool names and arguments, reports linked-result status and entry IDs, and expands result text only from an exact in-scope result entry ID or project-session address. Recall collects only enough matches for the requested eight-result page plus one lookahead match; its output and tool details report whether more matches are available instead of scanning for an exact total. Current-session records retain compact entry/role/time source addresses, while project-session records add a session ID and composite `sessionId:entryId` address. File- and tool-result expansion requires an exact in-scope entry ID for current-session scopes or composite address for `project_sessions` and returns only bounded, credential-redacted evidence already stored by Pi; it never reads workspace files or creates session or memory entries.

## Deterministic Compaction and Optional Review

Continuity intercepts manual `/compact` and automatic compaction. It always builds the canonical result deterministically and renders it as compact Markdown: active Work uses the latest request plus authoritative goal, plan, todos, constraints, verification, and best-effort observed file activity; ordinary sessions use stable bounded transcript extraction. When the latest request is already retained verbatim at the cut, the canonical summary records that fact instead of duplicating it. By default it retains approximately 25,000 recent raw tokens; **Continuity retained tokens** in Pylon package settings can set this from 1,000–50,000 and overrides Pi's `compaction.keepRecentTokens` only for Continuity-owned compactions. New compactions use a 20,000-character canonical budget and reserve at most 4,000 characters for supplemental excerpts. Generic context and reviewer input omit linked read/discovery errors only when a later same-tool, same-scope call succeeds, and collapse older exact duplicates; orphaned, unknown, protected, mutation, verification, and unresolved failures remain eligible. V1 and V2 Continuity compaction metadata remains readable; new entries use V3 structured metadata.
...
Planner and executor profiles are optional. Resetting either profile uses the main model and thinking level selected when `/plan` starts. Explicit plan mode selects the configured planner. Each stored plan revision requires at least one todo; missing summaries are derived from its todos. The planner uses `planSummary` for the approach and structured `workingSet`, `assumptions`, and `acceptanceCriteria` fields for the executor handoff. Continuity—not planner prose—presents every plan in the same `Goal`, `Approach`, `Working Set`, `Assumptions / Gaps`, `Acceptance Criteria`, `Constraints`, and numbered `Steps` structure. Revisions can carry current todo IDs so progress survives wording changes; duplicate normalized todo text and unknown or repeated IDs are rejected. When that revision settles in TUI or Pylon Web mode, including after clarification or requested changes, Continuity offers an untimed context-reset approval, current-context approval, or written revision feedback dialog exactly once. Dismissing either approval or feedback input leaves the revision pending so it can be offered again after the next settled turn.

### Approval and Handoff

`/plan approve` keeps the visible session history but inserts a persisted execution boundary so the executor receives only the approved structured handoff and messages that follow it. Approval first persists a revision-bound token; run, boundary, and kickoff records carry that token so interrupted approval can reconcile forward without duplicating effects. The transition remains recoverable until the executor turn starts. A boundary is applied only when its supported version and run/timeline identity match the current work. When planning used Scout, the plan carries compact paths, symbols, line ranges, assumptions, and unresolved gaps rather than the raw report. The reset executor validates those anchors with narrow reads and calls Scout only for changed repository state, missing anchors, or unresolved gaps. `/plan approve-current` keeps existing model context. Both approval paths share the same executor selection, run metadata, persistence, and state transition; only context reset behavior differs. Print and JSON modes use slash commands; Pylon Web uses the untimed plan-review dialog. If a configured model is unavailable, Continuity stops instead of silently using another model.
...
Memory V6 is a durable notebook of human-readable future-facing rules, separate from Active Work and read-only `continuity_recall` history. Notes keep canonical “When X, do Y” prose, source authority and snapshots, explicit disposition, independently computed enforcement authority, revision, and timestamps. Reviewer-drafted activation contracts are derived data and cannot authorize enforcement.

State is atomically replaced under `${PI_CODING_AGENT_DIR:-~/.pi/agent}/pi-continuity/memory-v6/state.json` with mode `0600`; compiled indexes live in a rebuildable sidecar. Storage permits up to 1,000 notes per owner and 2 MiB total; writes fail visibly rather than evicting notes. Runtime activation is allow-none and event-driven: only eligible notes with valid bounded trigger contracts subscribe to typed lifecycle/tool events. The initial lane deterministically matches `write`/`edit` path and tool facts, then queues one advisory custom message without blocking or transforming the tool. A session ledger deduplicates delivery, persists through custom entries, and rearms when compaction removes visibility. Lexical matching remains available only for explicit list/search and bounded write-time duplicate candidates; prompt similarity never activates a rule.

### Reviewer-gated proposals

Configure a dedicated reviewer with `/continuity memoryReviewer provider/model[:thinking]` or Pylon settings. There is no fallback model. Preflight derives ownership, validates target revisions, resolves exact user quotes, reads regular non-sensitive repository ranges below the canonical project root, bounds evidence to three ranges/120 lines per proposal, hashes fresh excerpts, rejects secrets and strong duplicates, and sends only a bounded packet. The reviewer returns strict `accept`, `rewrite`, `merge`, `defer`, or `reject` decisions plus a complete canonical mutation for accepted outcomes. Rewrites are classified and cannot broaden scope, obligation, authority, or enforcement. Truncated/malformed output, ungrounded paths or commands, stale targets/evidence, branch changes, provider failures, or missing credentials write nothing.
...
Startup first performs a restartable, idempotent V5→V6 migration. It byte-preserves note prose and source references, writes a hardened V5 backup plus journal, and imports every legacy note as archival so none can become enforced automatically. V4 migration then keeps its existing reviewer, credential screening, evidence, backup, ownership, and atomic-activation safeguards; `/memory migrate-v4` provides an explicit retry. Prepared batches resume safely and final activation validates the complete V6 notebook in one state replacement. Unresolved or uncompileable notes remain archival. V4/V5 sources stay read-only and recoverable; `/memory backups` lists backups and `/memory rollback` restores the pre-migration V6 notebook only while its activation revision is current.
