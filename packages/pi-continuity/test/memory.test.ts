...
import {
  applyReview, archivalActivationDraft, directDelete, directEdit, discardExpiredReviews, emptyMemoryState, enforceMemoryLimits, exactDuplicate,
  isMemoryState, migrateV5MemoryState, normalizeMemoryState, normalizeProposalBatch, notesForOwners, parseReviewerOutput,
  renderNote, stageReview, type MemoryStateFile, type NotebookNote, type ReviewRecord,
} from "../src/memory.ts";

const verified = { status: "verified" as const, verifiedAt: "2025-01-02T00:00:00.000Z", sourceSnapshotId: "f".repeat(64) };
const note = (overrides: Partial<NotebookNote> = {}): NotebookNote => ({
  ...
  relatedPaths: ["src/config.ts"], disposition: "archival", enforcementAuthority: "context_only",
...
const reviewedAdd = (overrides = {}) => ({
  operation: "add" as const, noteId: randomUUID(), scope: "project" as const, owner: "owner", trigger: "changing settings",
  guidance: "Restart the runtime after configuration changes.", authority: "project_contract" as const,
  sourceRefs: [{ type: "repository" as const, path: "src/config.ts", excerptSha256: "b".repeat(64) }],
  disposition: "archival" as const, enforcementAuthority: "context_only" as const, activationDraft: archivalActivationDraft(),
  rawProposal: { trigger: "changing settings", guidance: "Restart the runtime after configuration changes." }, rewriteCharacter: "format_only" as const,
  ...overrides,
});
const review = (overrides: Partial<ReviewRecord> = {}): ReviewRecord => ({
  reviewId: randomUUID(), sessionId: "s", toolCallId: "c", projectOwner: "owner", reviewedAt: "2025-01-02T00:00:00.000Z",
  status: "approved_pending", verificationStatus: verified, generation: 1, taskGeneration: 1, operations: [], rejectionCounts: {}, ...overrides,
});
...
test("reviewer output is V2, strict, and conservatively archives missing or invalid activation", () => {
  ...
  assert.throws(() => parseReviewerOutput(JSON.stringify({ version: 2, decisions: [{ proposalIndex: 0, verdict: "reject", reasonCode: "task_local", extra: true }] }), 1), /invalid/);
  assert.throws(() => parseReviewerOutput(JSON.stringify({ version: 2, decisions: [{ proposalIndex: 1, verdict: "reject", reasonCode: "task_local" }] }), 1), /unknown/);
  const sorted = parseReviewerOutput(JSON.stringify({ version: 2, decisions: [{ proposalIndex: 1, verdict: "reject", reasonCode: "task_local" }, { proposalIndex: 0, verdict: "defer", reasonCode: "insufficient_context" }] }), 2);
  assert.deepEqual(sorted.decisions.map((decision) => decision.proposalIndex), [0, 1]);
});

test("every documented reviewer decision branch parses", () => {
  const targetId = randomUUID(), draft = archivalActivationDraft();
  const decisions = [
    { proposalIndex: 0, verdict: "reject", reasonCode: "not_durable" },
    { proposalIndex: 0, verdict: "defer", reasonCode: "insufficient_context" },
    { proposalIndex: 0, verdict: "accept", operation: "replace", scope: "project", targetId, expectedRevision: 1, trigger: "changing settings", guidance: "Keep the existing rule.", authority: "project_contract", activationDraft: draft, reasonCode: "durable_rule" },
    { proposalIndex: 0, verdict: "rewrite", operation: "add", scope: "user", trigger: "when replying", guidance: "Keep replies concise.", authority: "user_instruction", activationDraft: draft, rewriteCharacter: "format_only", reasonCode: "normalized_rule" },
    { proposalIndex: 0, verdict: "merge", operation: "replace", scope: "user", targetId, expectedRevision: 1, trigger: "when replying", guidance: "Keep replies concise.", authority: "user_instruction", activationDraft: draft, rewriteCharacter: "format_only", reasonCode: "existing_rule" },
    { proposalIndex: 0, verdict: "accept", operation: "remove", scope: "project", targetId, expectedRevision: 1, reasonCode: "revoked_rule" },
  ];
  for (const decision of decisions) assert.equal(parseReviewerOutput(JSON.stringify({ version: 2, decisions: [decision] }), 1).decisions.length, 1);
});
...
test("invalid decision diagnostics are bounded and do not echo model fields", () => {
  ...
  assert.throws(() => parseReviewerOutput(JSON.stringify({ version: 2, decisions }), 20), (error: Error) => {
...
test("review staging and settlement are atomic and idempotent", () => {
  ...
  const staged = stageReview(state(), pending), committed = applyReview(staged, pending, "2025-01-03T00:00:00.000Z");
...
test("pending review capacity rejects new staging without evicting approved operations", () => {
  const reviews = Array.from({ length: 200 }, (_, index): ReviewRecord => review({ toolCallId: `call-${index}`, reviewedAt: new Date(1_700_000_000_000 + index).toISOString() }));
...
test("V5 migration preserves prose and provenance while archiving pending reviews", () => {
  ...
  const legacyReview = {
    ...
    operations: [{ operation: "add", noteId: randomUUID(), scope: "project", owner: "owner", trigger: "legacy trigger", guidance: "legacy guidance", authority: "project_contract", sourceRefs: legacyNote.sourceRefs }],
  };
  const legacy = { schemaVersion: 5, revision: 4, notes: [legacyNote], reviews: [legacyReview], updatedAt: "2025-01-01T00:00:00.000Z" };
  const now = "2025-02-01T00:00:00.000Z", migrated = migrateV5MemoryState(legacy, now);
  if (!migrated) assert.fail("valid V5 state should migrate");
  assert.equal(migrated.schemaVersion, 6); assert.equal(migrated.revision, 5);
  assert.equal(migrated.notes[0]?.trigger, legacyNote.trigger); assert.equal(migrated.notes[0]?.guidance, legacyNote.guidance);
  assert.deepEqual(migrated.notes[0]?.sourceRefs, legacyNote.sourceRefs);
  assert.equal(migrated.notes[0]?.disposition, "archival"); assert.equal(migrated.notes[0]?.enforcementAuthority, "context_only");
  assert.equal(migrated.reviews[0]?.status, "discarded"); assert.equal(migrated.reviews[0]?.operations[0]?.operation, "add");
