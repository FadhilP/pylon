...
const sources = [
  { sourceEntryId: "user-1", role: "user" as const, content: "Keep compatibility with old sessions.", sourceHash: hash("Keep compatibility with old sessions.") },
  { sourceEntryId: "assistant-1", role: "assistant" as const, content: "Decision: use exact source quotes.", sourceHash: hash("Decision: use exact source quotes.") },
  { sourceEntryId: "tool-1", role: "tool" as const, content: "provider unavailable", sourceHash: hash("provider unavailable"), isError: true },
];
...
test("review packet redacts credentials before model access and remains bounded", () => {
  ...
  const source = { sourceEntryId: "secret", role: "user" as const, content: `Use ${secret}`, sourceHash: hash(`Use ${secret}`) };
...
test("review completion reports bounded telemetry and propagates parent abort", async () => {
  ...
  await assert.rejects(callCompactionReviewer({
    model: { provider: "test", id: "reviewer" }, auth: { apiKey: "safe" }, profile: { model: "test/reviewer" },
    packet: packet(), sessionId: "session", signal: abort.signal,
