...
import {
  buildRuleIndex, candidateRules, compileActivationDraft, evaluateCompiledRule, evaluateTrigger, validateActivationDraft,
  type ActivationDraft, type EventFrame,
} from "../src/memory-activation.ts";

const frame = (kind: EventFrame["kind"], facts: EventFrame["facts"] = {}): EventFrame => ({ kind, sequence: 1, sessionId: "session", taskGeneration: 1, contextEpoch: 1, repository: "repo", facts });
const grounded = (): ActivationDraft => ({
  classification: "grounded",
  subscriptions: ["after_tool_result"],
  predicate: { all: [{ fact: "tool.name", op: "eq", value: "npm" }, { fact: "tool.exitCode", op: "gte", value: 1 }] },
  delivery: "warn",
...
test("activation compiles grounded rules, indexes deterministically, and evaluates", () => {
  ...
  assert.deepEqual(candidateRules(index, frame("after_tool_result")).map((rule) => rule.memoryId), ["a", "b"]);
  assert.equal(evaluateCompiledRule(one, frame("after_tool_result", { "tool.name": "npm", "tool.exitCode": 2 })), true);
  assert.equal(evaluateCompiledRule(one, frame("after_tool_result", { "tool.name": "npm", "tool.exitCode": 0 })), false);
...
test("activation supports bounded command predicates with hard negatives", () => {
  ...
  assert.equal(evaluateCompiledRule(rule, frame("before_tool_call", { "tool.name": "bash", "tool.command": "dart format ." })), true);
  assert.equal(evaluateCompiledRule(rule, frame("before_tool_call", { "tool.name": "bash", "tool.command": "echo dart format" })), false);
});
...
test("activation rejects hard-negative matches and preserves missing-fact tri-state", () => {
  ...
  assert.equal(evaluateTrigger({ not: { fact: "tool.name", op: "eq", value: "npm" } }, frame("after_tool_result")), "unknown");
  assert.throws(() => validateActivationDraft({ ...grounded(), examples: { positive: [{ event: "after_tool_result", facts: { "tool.name": "npm", "tool.exitCode": Number.NaN } }], hardNegative: grounded().examples.hardNegative } }), /invalid activation draft/);
});
...
test("activation semantic rules abstain after a deterministic match", () => {
  ...
  assert.equal(evaluateCompiledRule(rule, frame("after_tool_result", { "tool.name": "npm", "tool.exitCode": 1 })), "unknown");
  assert.equal(evaluateCompiledRule(rule, frame("after_tool_result", { "tool.name": "npm", "tool.exitCode": 0 })), false);
});

test("activation handles archival drafts without compilation", () => {
  const archival: ActivationDraft = {
    classification: "archival", subscriptions: [], delivery: "inject_once",
    lifecycle: { activateUntil: "explicit_revocation", rearmOn: [] }, examples: { positive: [], hardNegative: [] },
  };
...
test("activation validates exact keys, bounds, depth, and operators", () => {
  ...
  const oversized = grounded(); oversized.subscriptions = Array.from({ length: 17 }, () => "after_tool_result");
  assert.throws(() => validateActivationDraft(oversized), /invalid activation draft/);
  const deep = grounded(); let expression: any = { fact: "tool.name", op: "eq", value: "npm" };
  ...
  const operator = grounded(); operator.predicate = { fact: "tool.name", op: "gte", value: "npm" } as any;
  assert.throws(() => validateActivationDraft(operator), /invalid activation draft/);
});

test("activation uses native path glob matching", () => {
  assert.equal(evaluateTrigger({ fact: "file.path", op: "matchesGlob", value: "src/**/*.ts" }, frame("before_tool_call", { "file.path": "src/lib/file.ts" })), true);
  assert.equal(evaluateTrigger({ fact: "file.path", op: "matchesGlob", value: "[" }, frame("before_tool_call", { "file.path": "src/file.ts" })), false);
});
