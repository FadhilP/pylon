...
test("concurrent JSON updates do not lose writes", async () => {
  ...
  try {
    ...
    await Promise.all(Array.from({ length: 20 }, (_, value) => updateJson<number[]>(path, [], (items) => [...items, value], Array.isArray)));
    const items = await readJson<number[]>(path, [], Array.isArray);
    assert.equal(items.length, 20); assert.equal(new Set(items).size, 20);
...
test("dead stale lock is removed only after owner fencing is checked", async () => {
  ...
  try {
    const path = join(root, "state.json"), lock = `${path}.lock`;
    await mkdir(lock); await writeFile(join(lock, "owner.json"), JSON.stringify({ version: 1, token: "dead-owner", pid: 999_999_999, createdAt: "2000-01-01T00:00:00Z" }));
