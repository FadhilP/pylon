...
test("owner reassociation revises moved notes and backs current-owner collisions out of the result", () => {
  ...
  assert.equal(
    result.notes.filter((item) => item.owner === "current").length,
    3,
  );
  assert.equal(
  ...
  assert.equal(
    result.notes.find((item) => item.id === oldMove.id)?.revision,
    2,
  );
});
...
test("moved owner detection requires stale missing homes and two commits from exactly one owner", async () => {
  ...
  try {
    ...
    await exec("git", ["tag", "-a", "same-one", first, "-m", "same one"], {
      cwd: oldPath,
    });
    await exec("git", ["tag", "-a", "same-two", first, "-m", "same two"], {
      cwd: oldPath,
    });
    const tagOne = String(
        (await exec("git", ["rev-parse", "same-one"], { cwd: oldPath })).stdout,
      ).trim(),
      tagTwo = String(
        (await exec("git", ["rev-parse", "same-two"], { cwd: oldPath })).stdout,
      ).trim();
