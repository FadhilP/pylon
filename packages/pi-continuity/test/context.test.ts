...
const note = (trigger: string, guidance: string, relatedPaths?: string[]): NotebookNote => ({
  id: randomUUID(),
  scope: "project",
  owner: "o",
  trigger,
...
test("retrieval matches trigger, guidance, paths, and identifiers", () => {
  const notes = [
    note("changing package settings", "Restart the runtime after updates.", ["src/config.ts"]),
...
test("retrieval corpus keeps deterministic aliases, matches, and precision threshold", () => {
  ...
  for (const [query, expected] of cases) assert.equal(shortlistResolvedNotes(notes, query)[0]?.id, expected, query);
});
...
test("generic agent vocabulary does not retrieve unrelated rules", () => {
  ...
  assert.equal(
    shortlistResolvedNotes([rule], "Could relevant memory affect steps the model will take for the user?").length,
    0,
  );

  const applicable = note("delegating agent tasks", "Use the high-effort worker profile.");
  assert.equal(
    shortlistResolvedNotes([applicable], "delegate this agent task with the high-effort profile")[0]?.id,
    applicable.id,
  );
});
...
test("multi-query ties remain deterministic regardless of note order", () => {
  ...
  assert.deepEqual(
    shortlistResolvedQueries([first, second], ["package configuration"], 2).map(item => item.id),
    expected,
  );
  assert.deepEqual(
