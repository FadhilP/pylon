...
test("retrieval matches trigger, guidance, paths, and identifiers", () => {
  ...
  assert.equal(
    shortlistNotes(notes, "package configuration runtime")[0]?.id,
    notes[0]?.id,
  );
  assert.equal(
    shortlistNotes(notes, "dropdown category filters")[0]?.id,
    notes[1]?.id,
  );
  assert.equal(
    shortlistResolvedNotes(notes, "src/config.ts")[0]?.id,
    notes[0]?.id,
  );
...
test("promptQuery uses active work only for content-free continuation", () => {
  const work: any = {
    goal: "Implement package configuration",
    currentTodoId: "todo_1",
    todos: [{ id: "todo_1", text: "Update runtime settings" }],
  };
  assert.equal(
    promptQuery("continue", work),
    "Implement package configuration Update runtime settings",
  );
...
test("retrieval queries score prompt and planned intent independently", () => {
  ...
  assert.equal(
    shortlistResolvedQueries(
      [rule],
      retrievalQueries("Add account history", work),
    )[0]?.id,
    rule.id,
  );
  assert.equal(
    shortlistResolvedQueries(
      [note("database packages", "Keep both stable.")],
      ["database", "packages"],
    ).length,
    0,
...
test("generic agent vocabulary does not retrieve unrelated rules", () => {
  const rule = note(
    "work involving StateQL",
    "Treat StateQL as a project owned by the user.",
  );
  assert.equal(
    shortlistResolvedNotes(
      [rule],
      "Could relevant memory affect steps the model will take for the user?",
    ).length,
    0,
  );

  const applicable = note(
    "delegating agent tasks",
    "Use the high-effort worker profile.",
  );
  assert.equal(
    shortlistResolvedNotes(
      [applicable],
      "delegate this agent task with the high-effort profile",
    )[0]?.id,
    applicable.id,
  );
});
...
test("multi-query ties remain deterministic regardless of note order", () => {
  ...
  assert.deepEqual(
    shortlistResolvedQueries([first, second], ["package configuration"], 2).map(
      (item) => item.id,
    ),
    expected,
  );
  assert.deepEqual(
    shortlistResolvedQueries([second, first], ["package configuration"], 2).map(
      (item) => item.id,
    ),
    expected,
  );
...
test("memory injection reports only complete newly rendered notes", () => {
  ...
  const ninth = note(
    "database migration schema fallback",
    "Use the final fallback rule.",
  );
...
test("context injects at most two complete notebook rules within budget", () => {
  const notes = [
    note("changing package settings", "Restart the runtime after updates."),
    note("testing package settings", "Run the package tests."),
    note("publishing releases", "Tag only verified commits."),
  ];
  const context = buildContext(
    undefined,
    notes,
    "package settings runtime tests",
    150,
  );
  assert.match(context, /Continuity state/);
  assert.ok((context.match(/^Memory:/gm) ?? []).length <= 2);
  assert.equal(
    shortlistResolvedNotes(notes, "package settings runtime tests").length,
    2,
  );
  assert.doesNotMatch(context, /sourceReviewId|excerptSha256|confidence/);
  for (const line of context
    .split("\n")
    .filter((value) => value.startsWith("Memory:")))
    assert.match(line, /\.$/);
});
...
test("context backfills shorter ranked candidates when stronger rules exceed the budget", () => {
  const notes = [
    note(
      "package runtime configuration settings",
      `Oversized ${"guidance ".repeat(50)}`,
    ),
  ...
  const context = buildContext(
    undefined,
    notes,
    "package runtime configuration settings tests",
    100,
  );
...
test("parent candidates backfill oversized child rules and rendered rules stay single-line", () => {
  const children = Array.from({ length: 8 }, (_, index) =>
    note(
      `package runtime configuration settings child${index}`,
      `Oversized ${"guidance ".repeat(50)}`,
    ),
  );
