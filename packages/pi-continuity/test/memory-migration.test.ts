...
const fact = (key: string, scope: "user" | "project" = "project", owner = "owner") => ({
  key,
  text: `When ${key}, preserve the documented boundary.`,
  source: "README",
  scope,
  owner,
  evidencePaths: [],
...
const fakeReviewer = (failAt = -1) => {
  ...
  return (async (_model: any, prompt: any) => {
    ...
    const decisions = packet.map((item: any) => ({
      index: item.index,
      verdict: "rewrite",
      trigger: `using ${item.legacyKey}`,
      guidance: `For ${item.legacyKey}, use action-${item.legacyKey} and boundary-${item.legacyKey}.`,
...
async function writeV4(root: string, facts: any[], candidates: any[] = []) {
  const directory = join(root, "memory-v4");
...
test("migration availability hides absent, activated, and rolled-back V4 sources", async () => {
  ...
  try {
    ...
    await mkdir(join(root, "memory-v6"), { recursive: true });
    await writeFile(join(root, "memory-v6", "migration.json"), JSON.stringify(journal));
    assert.equal(await hasPendingV4Migration(root), false);
    await writeFile(join(root, "memory-v6", "migration.json"), JSON.stringify({ ...journal, status: "rolled_back" }));
    assert.equal(await hasPendingV4Migration(root), false);
    await writeFile(join(root, "memory-v6", "migration.json"), JSON.stringify({ ...journal, status: "failed" }));
    assert.equal(await hasPendingV4Migration(root), true);
  } finally {
...
test("pending migration backs up V4 and records reviewer unavailability", async () => {
  ...
  try {
    ...
    const journal = JSON.parse(await readFile(join(root, "memory-v6", "migration.json"), "utf8")) as MigrationJournal;
    assert.equal(journal.status, "pending");
    assert.match(journal.failureReason ?? "", /reviewer unavailable/);
    assert.ok((await readdir(join(root, "memory-v6", "backups"))).some(name => name.startsWith("memory-v4-")));
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("migration reviews all owners and activates once after every batch", async () => {
  ...
  try {
    ...
    await writeV4(root, [
      fact("global", "user", "legacy-user"),
      { ...fact("one", "project", ownerA), captureCommit: "a".repeat(40) },
      fact("two", "project", "owner-b"),
    ]);
    let commits = 0,
      committed: NotebookNote[] = [];
    const result = await migrateV4({
      ...
      profile: { model: "p/m" },
      sessionId: "s",
      completeReview: fakeReviewer(),
      commitAll: async notes => {
        commits++;
        committed = notes;
        return 7;
      },
    ...
    assert.equal(
      committed.find(note => note.owner === ownerA)?.sourceRefs.find(ref => ref.type === "migration")?.captureCommit,
      "a".repeat(40),
    );
    const journal = JSON.parse(await readFile(join(root, "memory-v6", "migration.json"), "utf8")) as MigrationJournal;
    assert.equal(journal.status, "activated");
    assert.equal(journal.activatedStateRevision, 7);
    const repeated = await migrateV4({
      root,
      ownerRoots: new Map(),
      model: {},
      auth: { apiKey: "safe" },
...
test("reviewer failure never activates a partial owner set and restart uses prepared progress", async () => {
  ...
  try {
    ...
    const failed = JSON.parse(await readFile(join(root, "memory-v6", "migration.json"), "utf8")) as MigrationJournal;
    assert.equal(failed.status, "failed");
    const resumed = await migrateV4({
      root,
      ownerRoots: new Map(),
      model: {},
      auth: { apiKey: "safe" },
...
test("malformed V4 is backed up and journaled without activation", async () => {
  ...
  try {
    const directory = join(root, "memory-v4");
    await mkdir(directory, { recursive: true });
    await writeFile(join(directory, "memory.json"), "{bad json");
    await assert.rejects(
      migrateV4({
        root,
        ownerRoots: new Map(),
    ...
    const backups = await readdir(join(root, "memory-v6", "backups"));
    assert.ok(backups.some(name => name.startsWith("memory-v4-")));
    const journal = JSON.parse(await readFile(join(root, "memory-v6", "migration.json"), "utf8")) as MigrationJournal;
...
test("credential-like legacy records are rejected individually without blocking safe migration", async () => {
  ...
  try {
    await writeV4(root, [
      fact("safe", "user", "default"),
      { ...fact("unsafe", "project", "owner"), text: "When configuring access, password=actual-secret-value" },
    ]);
    ...
    assert.equal(committed[0]?.scope, "user");
      ...
      [
        {
          ...
          id: "c",
          action: "add",
          evidencePaths: [{ path: "../secret", sha256: "a".repeat(64) }],
        },
      ],
    );
    let committed: NotebookNote[] = [];
