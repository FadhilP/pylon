...
const fakeReviewer = (failAt = -1) => {
  ...
  return (async (_model: any, prompt: any) => {
    ...
    const packet = JSON.parse(
      text.match(/<migration-data>(.*)<\/migration-data>/s)[1],
    );
    ...
    return {
      stopReason: "stop",
      content: [
        { type: "text", text: JSON.stringify({ version: 1, decisions }) },
      ],
    };
  }) as any;
};
async function writeV4(root: string, facts: any[], candidates: any[] = []) {
  ...
  await writeFile(
    ...
    JSON.stringify({ schemaVersion: 4, facts }),
  );
  await writeFile(
    join(directory, "candidates.json"),
    JSON.stringify({ schemaVersion: 4, candidates }),
  );
}
...
test("migration availability hides absent, activated, and rolled-back V4 sources", async () => {
  ...
  try {
    ...
    await writeFile(
      join(root, "memory-v6", "migration.json"),
      JSON.stringify(journal),
    );
    assert.equal(await hasPendingV4Migration(root), false);
    await writeFile(
      join(root, "memory-v6", "migration.json"),
      JSON.stringify({ ...journal, status: "rolled_back" }),
    );
    assert.equal(await hasPendingV4Migration(root), false);
    await writeFile(
      join(root, "memory-v6", "migration.json"),
      JSON.stringify({ ...journal, status: "failed" }),
    );
    assert.equal(await hasPendingV4Migration(root), true);
  } finally {
...
test("pending migration backs up V4 and records reviewer unavailability", async () => {
  ...
  try {
    await writeV4(root, [fact("contract")]);
    assert.equal(
      await recordPendingV4Migration(root, "reviewer unavailable"),
      true,
    );
    ...
    assert.ok(
      (await readdir(join(root, "memory-v6", "backups"))).some((name) =>
...
test("migration reviews all owners and activates once after every batch", async () => {
  ...
  try {
    ...
    assert.equal(
      committed.find((note) => note.scope === "user")?.owner,
      "default",
    );
    assert.equal(
      committed
        .find((note) => note.owner === ownerA)
        ?.sourceRefs.find((ref) => ref.type === "migration")?.captureCommit,
      "a".repeat(40),
    );
    const journal = JSON.parse(
      await readFile(join(root, "memory-v6", "migration.json"), "utf8"),
...
test("credential-like legacy records are rejected individually without blocking safe migration", async () => {
  ...
  try {
    await writeV4(root, [
      ...
      {
        ...fact("unsafe", "project", "owner"),
        text: "When configuring access, password=actual-secret-value",
      },
    ]);
