...
test("malformed V4 is backed up and journaled without activation", async () => {
  ...
  try {
    ...
    const backups = await readdir(join(root, "memory-v6", "backups")); assert.ok(backups.some((name) => name.startsWith("memory-v4-")));
    const journal = JSON.parse(await readFile(join(root, "memory-v6", "migration.json"), "utf8")) as MigrationJournal; assert.equal(journal.status, "failed");
  } finally { await rm(root, { recursive: true, force: true }); }
});
