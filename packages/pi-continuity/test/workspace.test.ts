...
test("workspace registration prunes only old missing inert entries", async () => {
  ...
  await writeFile(join(root, "workspaces.json"), JSON.stringify([
    ...
    { id: "owned", canonicalPath: join(root, "missing-owned"), projectOwner: "owner", createdAt: old, lastSeenAt: old },
  ]));

  await registerWorkspace(root, current);
  const workspaces = JSON.parse(await readFile(join(root, "workspaces.json"), "utf8"));
  assert.equal(workspaces.some((item: any) => item.id === "inert"), false);
  assert.equal(workspaces.some((item: any) => item.id === "owned"), true);
});
