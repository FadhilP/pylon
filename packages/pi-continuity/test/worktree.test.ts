...
test("Continuity worktree identity exactly matches Verify's canonical 16-hex algorithm", async () => {
  ...
  try {
    ...
    const expected = createHash("sha256").update(`${String(head.stdout).trim()}\n${String(status.stdout)}`).digest("hex").slice(0, 16);
...
test("unknown Git state is explicit and sensitive evidence names fail closed", async () => {
  ...
  try {
    ...
    await mkdir(join(root, "nested")); await writeFile(join(root, "nested", "secrets.json"), '{"token":"definitely-secret-value"}\n');
