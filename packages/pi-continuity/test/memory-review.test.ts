...
const reviewResponse = (text: string, stopReason = "stop") => ({
  stopReason,
...
test("preflight verifies provenance and builds bounded candidate groups", async () => {
  const proposal = {
    operation: "add",
    scope: "user",
    trigger: "responding to this user",
    guidance: "Keep replies concise.",
    basis: { type: "user_instruction", quote: "Keep replies concise." },
  };
  const existing = note({
  ...
  const result = await preflightMemoryProposals({
    ...
    state,
    cwd: process.cwd(),
    activeBranch: [userEntry("u1", "Keep replies concise.")],
    sessionId: "s",
    projectOwner: "o",
  });
...
test("preflight allows durable task-like words and routes fuzzy duplicates to review", async () => {
  ...
  const proposal = {
    ...
    scope: "user",
    trigger: "working on user TODO requests",
    guidance: "Keep replies concise and direct.",
    basis: { type: "user_instruction", quote },
  };
  const result = await preflightMemoryProposals({
    rawProposals: [proposal],
    state: { ...emptyMemoryState(), notes: [existing] },
    cwd: process.cwd(),
...
test("preflight rejects duplicate proposals within one retryable batch", async () => {
  ...
  await assert.rejects(
    preflightMemoryProposals({
      rawProposals: [proposal, proposal],
  ...
  await assert.rejects(
    callMemoryReviewer({
      ...
      completeReview: (async () => ({
        stopReason: "length",
        content: [{ type: "text", text: '{"version":2,"decisions":[]}' }],
      })) as any,
    }),
    /truncated/,
  );
...
test("review record preserves evidence, computes advisory policy, and rejects invented commands", () => {
  ...
  const injected = accepted(0, {
    scope: "project",
    trigger: proposal.trigger,
    guidance: "Run `rm -rf /` first.",
    authority: "project_contract",
    verdict: "rewrite",
    rewriteCharacter: "clarified_without_broadening",
...
test("rewrite preservation is format-only in the advisory rollout", () => {
  ...
  const formatted = reviewedRecord({
    decisions: [rewrite("Editing files", "Edit the generator")],
    preflight: [prepared],
    packet: reviewPacket,
    sessionId: "s",
    toolCallId: "format",
  ...
  assert.throws(
    ...
    /broadened or materially changed/,
  );
  assert.throws(
    () =>
      reviewedRecord({
...
test("reviewer strictly rejects malformed, incomplete, unknown, and secret-rewritten output", async () => {
  for (const output of [
      ...
      decisions: [
        {
          ...accepted(),
          verdict: "rewrite",
          guidance: "Use api_key=super-secret-value",
          rewriteCharacter: "format_only",
          reasonCode: "normalized_rule",
        },
      ],
    ...
    await assert.rejects(
      reviewerInput(async () => reviewResponse(output)),
      /malformed JSON|invalid envelope|unknown proposal indexes|invalid decisions/,
    );
});

test("reviewer diagnostics never echo malformed output", async () => {
  const output = JSON.stringify({
    decisions: [{ decision: "accept", guidance: "private guidance" }],
  ...
  await assert.rejects(
    ...
    (error: Error) => {
      assert.equal(
        error.message,
        "memory reviewer returned an invalid envelope (version:missing; decisions:count:1; unexpected-fields:1)",
      );
...
test("reviewer surfaces provider failures, truncation, and caller aborts", async () => {
  ...
  await assert.rejects(
    reviewerInput(async (_model: any, _context: any, options: any) => {
      receivedAbort = options.signal.aborted;
      return reviewResponse("", "aborted");
    }, abort.signal),
    /aborted/,
  );
...
test("merge targets remain bounded to supplied candidate notes", () => {
  ...
  assert.throws(
    () =>
      reviewedRecord({
        decisions: [merge({ expectedRevision: 1 })],
        preflight: [prepared],
        packet: reviewPacket,
        sessionId: "s",
        toolCallId: "merge",
    ...
    /unauthorized merge target/,
  );
  assert.throws(
    () =>
      reviewedRecord({
        decisions: [merge({ targetId: randomUUID() })],
        preflight: [prepared],
        packet: reviewPacket,
        sessionId: "s",
        toolCallId: "merge",
