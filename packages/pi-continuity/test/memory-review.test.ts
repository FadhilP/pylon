...
import { callMemoryReviewer, MEMORY_REVIEWER_OUTPUT_CONTRACT, MEMORY_REVIEWER_PROMPT, preflightMemoryProposals, resolveExactUserQuote, reviewedRecord, type PreflightProposal, type ReviewPacket } from "../src/memory-review.ts";
...
test("reviewer prompt supplies the exact output contract for every decision branch", () => {
  assert.ok(MEMORY_REVIEWER_PROMPT.endsWith(MEMORY_REVIEWER_OUTPUT_CONTRACT));
  for (const token of [
    '"version":1', '"proposalIndex"', '"verdict":"reject"', '"verdict":"accept"', '"verdict":"rewrite"', '"verdict":"merge"',
    '"operation":"remove"', '"authority":"user_instruction"|"project_contract"', "not_durable", "durable_rule", "normalized_rule", "existing_rule", "revoked_rule", "contradicted_rule",
  ]) assert.ok(MEMORY_REVIEWER_OUTPUT_CONTRACT.includes(token), token);
});
...
test("preflight accepts grounded user adds", async () => {
  ...
  const result = await preflightMemoryProposals({ rawProposals: [proposal], state: emptyMemoryState(), cwd: process.cwd(), activeBranch: [userEntry("u1", "Keep replies concise.")], sessionId: "s", projectOwner: "o" });
...
test("review record keeps per-proposal evidence batches and rejects ungrounded commands", () => {
  ...
  const record = reviewedRecord({ decisions, preflight: prepared, packet, sessionId: "s", toolCallId: "call", generation: 1, taskGeneration: 1, worktreeIdentity: "a".repeat(16) });
  ...
  assert.throws(() => reviewedRecord({ decisions: injected, preflight: prepared, packet, sessionId: "s", toolCallId: "other", generation: 1, taskGeneration: 1 }), /ungrounded/);
});

test("reviewer removals still require an explicit user revocation", () => {
  const quote = "Keep this rule.";
  const id = randomUUID(), proposal: MemoryProposal = { operation: "remove", scope: "user", targetId: id, expectedRevision: 1, reason: "revoked", basis: { type: "user_instruction", quote } };
  const prepared = [{ proposal, owner: "default", quote: { quote, sessionId: "s", entryId: "u", quoteSha256: sha256(quote), entrySha256: sha256(quote) }, sourceRefs: [{ type: "user_message" as const, sessionId: "s", entryId: "u", quoteSha256: sha256(quote) }], requiresVerification: false }];
  ...
  assert.throws(() => reviewedRecord({ decisions: [decision], preflight: prepared, packet, sessionId: "s", toolCallId: "c", generation: 1, taskGeneration: 1 }), /explicit user revocation/);
});
...
test("Terra-style schema drift produces bounded structural diagnostics without echoing output", async () => {
  const output = JSON.stringify({ decisions: [{ decision: "accept", operation: "add", scope: "user", trigger: "private trigger", guidance: "private guidance", reason: "durable" }], "api_key=super-secret-value": true });
  await assert.rejects(reviewerInput(async () => reviewResponse(output)), (error: Error) => {
    ...
    assert.doesNotMatch(error.message, /private|api_key|super-secret/);
    return true;
  });
});
...
test("exact quotes and merge targets remain unambiguous and authorized", () => {
  ...
  assert.throws(() => reviewedRecord({ decisions: [merge({ expectedRevision: 1 })], preflight: [prepared], packet, sessionId: "s", toolCallId: "merge", generation: 1, taskGeneration: 1 }), /unauthorized merge target/);
  assert.throws(() => reviewedRecord({ decisions: [merge({ targetId: randomUUID() })], preflight: [prepared], packet, sessionId: "s", toolCallId: "merge", generation: 1, taskGeneration: 1 }), /unauthorized merge target/);
  assert.throws(() => reviewedRecord({ decisions: [merge()], preflight: [prepared], packet: { ...packet, existingNotes: [{ ...target, owner: "other-owner" }] }, sessionId: "s", toolCallId: "merge", generation: 1, taskGeneration: 1 }), /unauthorized merge target/);
});
