...
export type CompactionHistoryRecord = { path: string; sourceEntryId?: string };
...
export type CompactionVerification = { state: string; scope?: string; runId?: string; worktreeId?: string };

type Preparation = { firstKeptEntryId: string; tokensBefore: number; settings: { keepRecentTokens: number } };
...
const entryId = (value: unknown) => filledString(value, MAX_SOURCE_ENTRY_ID_CHARS);
const genericRoles = oneOf("user", "assistant", "tool", "summary");
const supplementRoles = oneOf("user", "assistant", "tool");
const supplementCategories = oneOf("constraint", "decision", "error", "outcome", "context");

const validHistoryRecord = (value: any) =>
  Boolean(value) &&
  filledString(value.path, MAX_HISTORY_PATH_CHARS) &&
  optional(value.sourceEntryId, id => boundedString(id, MAX_SOURCE_ENTRY_ID_CHARS));
...
const validLegacyBase = (value: any) =>
  ...
  optional(value.handoffEntryId, id => boundedString(id, MAX_SOURCE_ENTRY_ID_CHARS)) &&
  optional(value.currentTaskEntryId, id => boundedString(id, MAX_SOURCE_ENTRY_ID_CHARS));
...
function activeDetails(
  value: unknown,
): value is LegacyCompactionDetails | StructuredCompactionDetails | ActiveWorkCompactionDetails {
  return (
...
function fenced(value: string) {
  const longest = (character: "`" | "~") =>
    Math.max(0, ...(value.match(character === "`" ? /`+/g : /~+/g) ?? []).map(run => run.length));
...
function toolErrorExclusions(entries: SessionEntry[], sourceStart: number) {
  ...
  const outcomes = entries.slice(sourceStart).flatMap((entry, offset) => {
    if (entry.type !== "message" || entry.message.role !== "toolResult" || typeof entry.id !== "string" || !entry.id)
      return [];
...
function sourceText(
  entry: SessionEntry,
): { role: CompactionReviewRole; content: string; isError?: boolean } | undefined {
  ...
  if (role === "toolResult") {
    const content = safe(textContent((entry.message as any).content), 4_000);
    return content
      ? { role: "tool", content, ...((entry.message as any).isError ? { isError: true } : {}) }
      : undefined;
  }
}
...
function buildActiveDraft({ branchEntries, preparation, work, verification }: BuildInput): BuiltDraft | undefined {
  ...
  const summary = buildActiveSummary(
    recordsFor(work, currentRequest, selected.firstKeptIndex === currentTaskIndex, history, verification),
    metadata,
  );
  if (!summary) return;
...
function renderGenericSummary(
  ...
  const activity = [
    ...
    ...history.read.map(
      record => `- Read/search: ${record.path}${record.sourceEntryId ? ` (entry ${record.sourceEntryId})` : ""}`,
    ),
  ];
  return [
...
function fitGenericRecords(records: GenericCompactionRecord[], history: CompactionHistory, sourceEntryCount: number) {
  ...
  while (
    selected.length &&
    renderGenericSummary(selected, history, sourceEntryCount).length > MAX_CANONICAL_SUMMARY_CHARS
  )
    selected.shift();
  return renderGenericSummary(selected, history, sourceEntryCount).length <= MAX_CANONICAL_SUMMARY_CHARS
    ? selected
    : undefined;
}
...
export function finalizeContinuityCompaction(
  ...
  for (let index = supplements.length - 1; index >= 0; index--) {
    ...
    if (candidate.length > MAX_SUPPLEMENTS || supplementSection(candidate).length > MAX_SUPPLEMENT_SUMMARY_CHARS)
      continue;
    seen.add(key);
