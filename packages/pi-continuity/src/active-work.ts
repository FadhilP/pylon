...
const normalizedTodoText = (text: string) =>
  text.trim().replace(/\s+/g, " ").toLowerCase();
const nonEmpty = (value: unknown) =>
  typeof value === "string" && value.length > 0;
const isModelRef = (value: any) =>
...
const validTodos = (todos: any) => {
  ...
  return (
    ...
    new Set(todos.map((todo: Todo) => normalizedTodoText(todo.text))).size ===
      todos.length
  );
};
...
export function isWork(value: any): value is Work {
  ...
  if (typeof value.goal !== "string" || typeof value.approved !== "boolean")
    return false;
  ...
  if (
    ...
    return false;
  if (!optional(value.handoff, (handoff) => validHandoff(handoff)))
    return false;
  if (!validTodos(value.todos)) return false;
  if (
    !optional(value.currentTodoId, (id) =>
      value.todos.some((todo: Todo) => todo.id === id),
    )
  )
    return false;
  if (!optional(value.latestFailure, (text) => typeof text === "string"))
    return false;
  if (!optional(value.nextAction, (text) => typeof text === "string"))
    return false;
  if (
    !optional(
      value.issue,
  ...
  if (!optional(value.planRevision, (revision) => integer(revision, 1)))
    return false;
  if (
    !optional(
      ...
      (offered) =>
        integer(offered, 1) &&
        (value.planRevision === undefined || offered <= value.planRevision),
    )
  )
    return false;
  if (!optional(value.approval, (approval) => validApproval(approval, value)))
    return false;
  if (
    !optional(value.revisionFeedback, (feedback) =>
      validRevisionFeedback(feedback, value.planRevision),
    )
  )
    return false;
  if (!optional(value.baseModel, isModelRef)) return false;
  if (!optional(value.baseThinking, (thinking) => typeof thinking === "string"))
    return false;
  if (!optional(value.completedAt, (at) => typeof at === "string"))
    return false;
  return (
...
export function setPlan(
  ...
  if (inputs.length > 12)
    throw Error("Plan cannot contain more than 12 todos.");
  ...
  if (
    ...
    new Set(normalized).size !== items.length
  )
  ...
  const oldByText = new Map(
    work.todos.map((todo) => [normalizedTodoText(todo.text), todo]),
  );
  ...
  work.todos = items.map((item, index) => {
    const prior = item.id
      ? oldById.get(item.id)
      : oldByText.get(normalized[index]!);
