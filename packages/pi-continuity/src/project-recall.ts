...
export type ProjectRecallSession = {
  sessionId: string;
  modifiedAt: string;
  entries: SessionEntry[];
};
export type ProjectRecallLoadResult = {
...
const canonical = (path: string) => {
  ...
  try { value = realpathSync.native(value); } catch { /* Historical paths may no longer exist. */ }
...
const defaultSource: ProjectSessionSource = {
  ...
  async read(path, remainingBytes) {
    ...
    if (
      ...
      info.size > MAX_PROJECT_SESSION_FILE_BYTES || info.size > remainingBytes
    ) return;
...
export function parseProjectSession(
  ...
  if (
    header?.type !== "session" || (header.version !== 2 && header.version !== 3) ||
    !safeId(header.id) || header.id !== expected.sessionId || typeof header.cwd !== "string" ||
  ...
  for (const entry of entries) {
    if (
      !entry || typeof entry !== "object" || !ENTRY_TYPES.has(entry.type) ||
      !safeId(entry.id) || header.id.length + entry.id.length + 1 > 200 || byId.has(entry.id) ||
...
export async function loadProjectRecallSessions(
  ...
  const owned = input.workspaces
    .filter((item) => item.projectOwner === input.projectOwner ||
      (!item.projectOwner && item.id === input.projectOwner))
    .sort((left, right) =>
  ...
  const cwdByCanonical = new Map(owned.slice(0, MAX_PROJECT_RECALL_WORKSPACES)
  ...
  let metadataSkipped = 0, metadataOverflow = false;
  for (const { cwd, sessionDir } of directories) {
    ...
    for (const session of listed.slice(0, MAX_PROJECT_RECALL_CANDIDATES_PER_WORKSPACE)) {
      const path = canonical(session.path), modified = session.modified.getTime();
      if (session.id === input.currentSessionId || path === currentFile) continue;
      if (
        !withinSessionDir(sessionDir, session.path) || !Number.isFinite(modified) ||
        !session.cwd || !cwdSet.has(canonical(session.cwd))
      ) { metadataSkipped++; continue; }
      unique.set(path, session);
    }
  }
  const discovered = [...unique.values()].sort((left, right) =>
  ...
  for (const info of candidates) {
    ...
    if (!loaded) { fileSkipped++; continue; }
    bytes += loaded.bytes;
    const entries = parseProjectSession(loaded.content, { sessionId: info.id, cwd: info.cwd });
    if (!entries) { fileSkipped++; continue; }
    sessions.push({ sessionId: info.id, modifiedAt: info.modified.toISOString(), entries });
  }
  return {
    sessions,
    skipped: metadataSkipped + fileSkipped,
    truncated: workspaceOverflow || metadataOverflow || discovered.length > candidates.length ||
      candidates.length > sessions.length + fileSkipped || bytes >= MAX_PROJECT_RECALL_BYTES,
  };
}
