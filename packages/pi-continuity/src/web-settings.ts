...
async function readPiSettings(path: string): Promise<Record<string, any>> {
  try {
    ...
    if (!object(value) || value.compaction !== undefined && !object(value.compaction)) throw new Error("invalid Pi settings");
    return value;
  } catch (error: any) {
...
async function updateReserveState(agentDir: string, next: ReserveState, expected: ReserveState): Promise<void> {
  ...
  await withFileLock(path, async () => {
    ...
    if (current.value !== expected.value || current.explicit !== expected.explicit || current.compactionPresent !== expected.compactionPresent)
      throw new Error("Pi compaction settings changed; reload and try again");
    if (current.value === next.value && current.explicit === next.explicit && current.compactionPresent === next.compactionPresent) return;
...
export async function updateSettings(value: any, { agentDir }: { agentDir: string }): Promise<void> {
  if (value?.kind !== "continuity" || typeof value.memoryEnabled !== "boolean" || !validReserveTokens(value.reserveTokens) || !validKeepRecentTokens(value.keepRecentTokens))
