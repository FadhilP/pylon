...
export type MemoryAuditEvent =
  ...
  | {
      ...
      owner: string;
      at: string;
      previousSourceRefs: MemorySourceRef[];
      previousSourceReviewId?: string;
    }
  | {
      type: "owner_reassociation";
      migrationId: string;
      oldOwner: string;
      owner: string;
      at: string;
      movedNoteIds: string[];
      suppressedNoteIds: string[];
      fromRevision: number;
    };
...
const enforcementAuthorities = new Set<EnforcementAuthority>([
  "context_only",
  "warning",
  "validation",
  "blocking_guard",
...
function verificationStatus(value: any): value is Extract<VerificationStatus, { status: "verified" }> {
  return (
    ...
    hash.test(value.sourceSnapshotId)
  );
}
function rawProposal(value: any): value is RawProposal {
  return (
    exactKeys(value, ["trigger", "guidance"]) &&
...
function sourceRef(value: any): value is MemorySourceRef {
  ...
  if (value.type === "user_message")
    return (
      exactKeys(value, ["type", "sessionId", "entryId", "quoteSha256"]) &&
      text(value.sessionId, 200) &&
      text(value.entryId, 200) &&
      hash.test(value.quoteSha256) &&
      safe(value.sessionId, value.entryId)
    );
  if (value.type === "repository")
    return (
      exactKeys(value, ["type", "path", "excerptSha256", "captureCommit"]) &&
      safeRelativePath(value.path) &&
      hash.test(value.excerptSha256) &&
  ...
  if (value.type === "migration")
    return (
      exactKeys(value, ["type", "legacyKey", "captureCommit"]) &&
      text(value.legacyKey, 200) &&
      safe(value.legacyKey) &&
      (value.captureCommit === undefined || commit.test(value.captureCommit))
    );
  return false;
}
...
export function isNotebookNote(value: any): value is NotebookNote {
  return (
    ...
    (value.relatedPaths === undefined ||
      (Array.isArray(value.relatedPaths) &&
        value.relatedPaths.length <= 5 &&
...
function reviewedOperation(value: any): value is ReviewedOperation {
  ...
  if (value.operation === "remove")
    return (
      exactKeys(value, ["operation", "targetId", "expectedRevision"]) &&
...
export function isReviewRecord(value: any): value is ReviewRecord {
  return (
    ...
    (value.evidenceBatches === undefined ||
      (Array.isArray(value.evidenceBatches) &&
        value.evidenceBatches.length <= 2 &&
        value.evidenceBatches.every(
          (batch: any) =>
            Array.isArray(batch) &&
            batch.length <= 3 &&
            batch.every(
              (range: any) =>
                exactKeys(range, ["path", "start", "end", "excerptSha256"]) &&
                safeRelativePath(range.path) &&
                integer(range.start, 1) &&
                integer(range.end, range.start) &&
                hash.test(range.excerptSha256),
            ),
        ))) &&
    (value.quoteRefs === undefined ||
      (Array.isArray(value.quoteRefs) &&
        value.quoteRefs.length <= 2 &&
        value.quoteRefs.every(
          (ref: any) =>
            exactKeys(ref, ["entryId", "entrySha256", "quoteSha256"]) &&
            text(ref.entryId, 200) &&
            hash.test(ref.entrySha256) &&
            hash.test(ref.quoteSha256),
        )))
  );
}

function auditEvent(value: any): value is MemoryAuditEvent {
  if (value?.type === "owner_reassociation")
    return (
      exactKeys(value, [
        "type",
        "migrationId",
...
export const emptyMemoryState = (): MemoryStateFile => ({
  schemaVersion: 6,
  revision: 0,
  notes: [],
  reviews: [],
...
export function normalizeMemoryState(value: any): MemoryStateFile | undefined {
  if (
    ...
    !value.notes.every(isNotebookNote) ||
    !Array.isArray(value.reviews) ||
    value.reviews.length > MEMORY_MAX_REVIEWS ||
  ...
  for (const review of value.reviews)
    if (reviewIds.has(review.reviewId)) return;
    else reviewIds.add(review.reviewId);
  try {
    enforceMemoryLimits(value);
  } catch {
    return;
  }
  return value;
...
export function migrateV5MemoryState(value: any, now = new Date().toISOString()): MemoryStateFile | undefined {
  ...
  for (const legacy of value.reviews) {
    if (
      ...
      !Array.isArray(legacy.operations) ||
      !reviewStatuses.has(legacy.status)
    )
      return;
  ...
  const migrated = {
    ...
    reviews,
    updatedAt: now,
    ...(value.audits ? { audits: value.audits } : {}),
  };
...
export function normalizeProposalBatch(value: unknown): MemoryProposal[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > 2)
    throw Error("memory propose requires one or two proposals");
...
export function parseReviewerOutput(raw: string, proposalCount: number): ReviewerOutput {
  ...
  try {
    ...
    throw Error("memory reviewer returned malformed JSON");
  }
  if (
...
export function exactDuplicate(
  ...
  return notes.find(
    note =>
      ...
      note.scope === scope &&
      note.owner === owner &&
      semanticIdentity(note.trigger, note.guidance) === identity,
  );
}
const duplicateWords = (value: string) =>
  new Set(
    (
...
export function enforceMemoryLimits(state: MemoryStateFile): void {
  ...
  if (Buffer.byteLength(serializedJson(state), "utf8") > MEMORY_MAX_FILE_BYTES)
    throw Error("memory notebook exceeds 2 MiB safety ceiling");
}
...
export function applyReview(
  ...
  if (
    ...
    stored.toolCallId !== review.toolCallId
  )
    throw Error("memory review is not pending in this state");
  ...
  for (const operation of review.operations) {
    if (operation.operation === "add") {
      ...
      notes.push({
        ...
        rawProposal: operation.rawProposal,
        rewriteCharacter: operation.rewriteCharacter,
        revision: 1,
        createdAt: now,
        updatedAt: now,
    ...
    if (!(
      ...
      (target.scope === "project" && target.owner === review.projectOwner)
    ))
      throw Error("memory review target crosses owner boundary");
...
export function directEdit(
  ...
  trigger = normalizeRuleText(trigger);
  guidance = normalizeRuleText(guidance);
  if (
    !text(trigger, 240) ||
    !text(guidance, 800) ||
...
export function discardExpiredReviews(
  state: MemoryStateFile,
  now = new Date(),
  ...
  const reviews = state.reviews.map(review => {
    ...
    return {
      ...
      discardReason: "abandoned after pending-review retention period",
...
/**
 * Re-checks a reviewed record against the latest state before it is staged.
 * The reviewer worked from a snapshot, so a concurrent write may have made its
 * operations duplicates or stale; either way the whole review is abandoned.
 */
...
export function stageReview(state: MemoryStateFile, review: ReviewRecord): MemoryStateFile {
  ...
  const next = {
    ...
    reviews: boundedReviews([...state.reviews, review]),
