...
export type MemoryOperationClass =
  "user_instruction_add" | "project_contract_write" | "merge_replace" | "reviewer_remove" | "v4_migration";
...
const exactKeys = (value: any, allowed: readonly string[]) =>
  value && typeof value === "object" && !Array.isArray(value) && Object.keys(value).every(key => allowed.includes(key));
...
export function parseMemoryEvaluationCorpus(value: unknown): MemoryEvaluationCase[] {
  ...
  return value.map((item: any) => {
    if (
      !exactKeys(item, ["id", "operationClass", "expected", "actual", "useful", "zeroCounts"]) ||
      ...
      !REQUIRED_ZERO_COUNTS.every(key => typeof item.zeroCounts[key] === "boolean")
    )
...
export function scoreMemoryEvaluation(corpus: MemoryEvaluationCase[]): MemoryEvaluationReport {
  ...
  for (const operation of operations) {
    ...
    const precision = approvals.length
      ? approvals.filter(item => item.expected === "approve").length / approvals.length
      : 0;
    const noOpAgreement = expectedRejects.length
      ? expectedRejects.filter(item => item.actual === "reject").length / expectedRejects.length
      : 0;
    ...
    const zeroCounts = Object.fromEntries(
      REQUIRED_ZERO_COUNTS.map(key => [key, cases.filter(item => item.zeroCounts[key]).length]),
    ...
    results[operation] = {
      ...candidate,
      passed: meetsEvaluationThreshold(candidate) && usefulnessAgreement[operation] === 1,
    };
  }
