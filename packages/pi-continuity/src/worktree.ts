...
async function git(cwd: string, args: string[], env?: Record<string, string>) {
  const result = await exec("git", args, { cwd, env: env ? { ...process.env, ...env } : process.env, maxBuffer: 64 * 1024 * 1024, timeout: 120_000, windowsHide: true });
...
const sensitivePart = (part: string) => {
  ...
  return lower === ".git" || lower === ".env" || lower.startsWith(".env.") || lower === ".npmrc" || lower === ".pypirc"
    || /^(?:credentials|secrets?|auth|tokens?)(?:\..+)?$/.test(lower) || /^id_(?:rsa|dsa|ecdsa|ed25519)$/.test(lower)
    || /\.(?:pem|key|p12|pfx|keystore|jks)$/.test(lower);
};
async function readSafeFile(cwd: string, raw: string) {
  if (typeof raw !== "string" || !raw || raw.length > 240 || raw.includes("\0") || isAbsolute(raw) || win32.isAbsolute(raw)) throw Error("invalid or sensitive evidence path");
  const parts = raw.split(/[\\/]+/);
  if (parts.some((part) => !part || part === "." || part === ".." || sensitivePart(part))) throw Error("invalid or sensitive evidence path");
  const root = await projectRoot(cwd), normalized = parts.join("/");
  let cursor = root;
  ...
  try {
    ...
    if (before.size !== after.size || before.mtimeMs !== after.mtimeMs || before.ino !== after.ino) throw Error("evidence file changed while reading");
    for (const component of components) {
      const current = await lstat(component.path).catch(() => undefined);
      if (!current || current.isSymbolicLink() || current.dev !== component.dev || current.ino !== component.ino) throw Error("evidence path changed while reading");
    }
    let content: string;
    try { content = new TextDecoder("utf-8", { fatal: true }).decode(bytes).replace(/\r\n/g, "\n"); }
    catch { throw Error("evidence file must be UTF-8 text"); }
...
export async function captureEvidenceRanges(cwd: string, ranges: EvidenceRange[]): Promise<CapturedEvidenceRange[]> {
  if (!Array.isArray(ranges) || !ranges.length || ranges.length > 3) throw Error("one to three evidence ranges are required");
  let totalLines = 0, totalCharacters = 0;
  const context = await projectContext(cwd, "unknown"), output: CapturedEvidenceRange[] = [];
  for (const range of ranges) {
    if (!Number.isSafeInteger(range.start) || !Number.isSafeInteger(range.end) || range.start < 1 || range.end < range.start) throw Error("invalid evidence line range");
    totalLines += range.end - range.start + 1;
    if (totalLines > 120) throw Error("evidence exceeds 120 lines per proposal");
    const file = await readSafeFile(cwd, range.path), lines = file.content.split("\n");
    ...
    output.push({ path: file.normalized, start: range.start, end: range.end, excerpt, excerptSha256: createHash("sha256").update(excerpt).digest("hex"), ...(context.captureCommit ? { captureCommit: context.captureCommit } : {}) });
  }
  return output;
}
...
export async function currentChangedPaths(cwd: string): Promise<Set<string> | undefined> {
  try {
    ...
    const result = await exec("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], { cwd: root, maxBuffer: 64 * 1024 * 1024, timeout: 120_000, windowsHide: true });
    const records = String(result.stdout).split("\0").filter(Boolean), paths = new Set<string>();
    for (let index = 0; index < records.length; index++) {
      const record = records[index]!, status = record.slice(0, 2), path = record.slice(3);
      if (status.includes("R") || status.includes("C")) { paths.add(path.replace(/\\/g, "/")); index++; }
...
export async function worktreeFingerprint(cwd: string): Promise<string | undefined> {
  try {
    ...
    const [head, status] = await Promise.all([
      ...
      exec("git", ["status", "--porcelain=v1", "--untracked-files=all"], { cwd: root, maxBuffer: 64 * 1024 * 1024, timeout: 15_000, windowsHide: true }),
    ]);
    return createHash("sha256").update(`${String(head.stdout).trim()}\n${String(status.stdout)}`).digest("hex").slice(0, 16);
  } catch { return undefined; }
}
