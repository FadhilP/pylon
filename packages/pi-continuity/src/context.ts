...
const words = (value: string) =>
  new Set(
    (value.toLowerCase().match(/[a-z0-9_-]{3,}/g) || []).map(normalizeWord).filter(word => !ignoredWords.has(word)),
  );
const identifiers = (value: string) =>
  new Set(value.toLowerCase().match(/[a-z0-9_-]+(?:[./\\][a-z0-9_.-]+)+|[a-z][a-z0-9]*_[a-z0-9_]+/g) || []);
...
export function shortlistNotes(notes: NotebookNote[], latest = "", work?: Work, limit = 2) {
  ...
  return notes
    .map((note): ScoredNote | undefined => {
      ...
      for (const query of queries) {
        ...
        if (
          strength &&
          (!best || strength > best.strength || (strength === best.strength && distinct.size > best.matches))
        )
...
export function buildContext(
  ...
  budget = 450,
  parent: NotebookNote[] = [],
  options: { resolvedQuery?: boolean; resolvedQueries?: string[]; candidateLimit?: number } = {},
) {
  ...
  const selectedParent = shortlistResolvedQueries(parent, queries, candidateLimit).filter(
    ...
    if (work.mode === "planning")
      ...
      lines.push(
        ...
        ...remaining
          ...
          .slice(0, 3)
          .map(todo => `Todo ${todo.id} [${todo.status}]: ${todo.text.slice(0, 160)}`),
  ...
  const header = content[0]!,
    memory = fitWholeLines(memoryLines, Math.min(600, max - header.length - 1), 2);
