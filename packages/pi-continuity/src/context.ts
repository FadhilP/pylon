...
const words = (value: string) => new Set((value.toLowerCase().match(/[a-z0-9_-]{3,}/g) || []).map(normalizeWord).filter((word) => !ignoredWords.has(word)));
const identifiers = (value: string) => new Set(value.toLowerCase().match(/[a-z0-9_-]+(?:[./\\][a-z0-9_.-]+)+|[a-z][a-z0-9]*_[a-z0-9_]+/g) || []);
  ...
  if (work) {
    ...
    if (work.mode === "planning") lines.push(
      ...
      lines.push(
        ...
        ...remaining.filter((todo) => todo.id !== current?.id).slice(0, 3).map((todo) => `Todo ${todo.id} [${todo.status}]: ${todo.text.slice(0, 160)}`),
