...
const boundedReason = (value: unknown) =>
  sanitizeAndClip(value instanceof Error ? value.message : String(value), 240)
    .replace(/\s+/g, " ")
    .trim();
...
export const isMigrationJournal = (value: any): value is MigrationJournal =>
  exactKeys(value, [
    ...
    "completedRecordIds",
    "reviewerBatchIds",
    "preparedV5OutputPath",
    "activatedStateRevision",
    "preMigrationBackup",
    "failureReason",
    "retryCount",
    "diagnostics",
...
const parseLegacyRecord = (
  ...
  try {
    assertSafe(
      ...
      value.text,
      value.source,
      ...(value.evidencePaths ?? []).map((entry: LegacyEvidence) => entry.path),
    );
  } catch {
    return;
  }
...
async function rawSource(path: string, backupDirectory: string, label: string) {
  try {
    ...
    const raw = await readFile(path),
      hash = sha(raw),
      backup = join(backupDirectory, `${label}-${hash}.json`);
...
function parseSource(
  ...
  try {
    ...
    throw Error(`malformed V4 ${field} source; raw backup preserved`);
  }
...
function parseDecisions(raw: string, count: number): MigrationDecision[] {
  ...
  try {
    ...
    throw Error("migration reviewer returned malformed JSON");
  }
  if (
    !value ||
    value.version !== 1 ||
  ...
  return value.decisions.map((decision: any) => {
    if (
      ...
      decision.index < 0 ||
      decision.index >= count ||
      indexes.has(decision.index) ||
      !["accept", "rewrite", "reject"].includes(decision.verdict) ||
      !reasons.has(decision.reasonCode)
    )
    ...
    if (decision.verdict === "reject") {
      ...
      decision.guidance.trim().length > 800 ||
      decision.trigger.trim().length + decision.guidance.trim().length > 1_000
    )
      throw Error("migration reviewer returned an invalid note");
    return decision;
  });
}
async function reviewBatch(input: {
  ...
  const message: Message = {
    ...
    content: [
      {
        type: "text",
        text: `Untrusted migration records:\n<migration-data>${JSON.stringify(packet)}</migration-data>`,
      },
    ],
    timestamp: Date.now(),
  };
...
async function migrateV4Unlocked(
  ...
  let journal = await readJson<MigrationJournal>(
    journalPath,
    {
      ...
      completedRecordIds: [],
      reviewerBatchIds: [],
      preparedV5OutputPath: preparedPath,
      retryCount: 0,
      diagnostics: [],
    },
    isMigrationJournal,
  ...
  for (const [candidate, values] of [
    [false, memoryRecords],
    [true, candidateRecords],
  ] as const)
    values.forEach((value, index) => {
      const recordId = `${candidate ? "candidate" : "fact"}:${index}`,
        parsed = parseLegacyRecord(value, recordId, candidate);
  ...
  for (const record of records) {
    ...
    if (
      ...
      diagnostics.push({
        ...
        reason: "pending candidate source cannot be resolved",
      });
      prepared.completedRecordIds.push(record.recordId);
      continue;
    }
    if (record.evidencePaths.length) {
      ...
      if (!cwd) {
        diagnostics.push({
          ...
          owner: record.owner,
          legacyKey: record.key,
          reason: "project root unavailable for evidence validation",
        });
        prepared.completedRecordIds.push(record.recordId);
        continue;
      }
      try {
        ...
        diagnostics.push({
          ...
          owner: record.owner,
          legacyKey: record.key,
          reason: "legacy evidence is unsafe, stale, or unavailable",
        });
        prepared.completedRecordIds.push(record.recordId);
        continue;
      }
    ...
    for (let offset = 0; offset < eligible.length; offset += 2) {
      ...
      for (const decision of decisions) {
        ...
        const now = new Date().toISOString(),
          trigger = normalizeRuleText(decision.trigger!),
        ...
        if (
          ...
          prepared.diagnostics.push({
            ...
            owner: note.owner,
            legacyKey: item.key,
            reason: "duplicate imported rule",
          });
          continue;
        }
        prepared.notes.push(note);
      }
      prepared.diagnostics = prepared.diagnostics.slice(-5_000);
      await writeMigrationJson(preparedPath, prepared);
      journal.completedRecordIds = [...new Set(prepared.completedRecordIds)];
      journal.diagnostics = prepared.diagnostics;
      await writeMigrationJson(journalPath, journal);
    }
    const probe: MemoryStateFile = {
      schemaVersion: 6,
      revision: 0,
