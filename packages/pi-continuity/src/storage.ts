...
const recoverableDataError = (error: any) => error instanceof SyntaxError || error?.code === "PI_CONTINUITY_INVALID_DATA";
...
async function readLockOwner(lock: string): Promise<LockOwner | undefined> {
  try {
    ...
    return value?.version === 1 && typeof value.token === "string" && Number.isSafeInteger(value.pid) && typeof value.createdAt === "string" ? value : undefined;
...
export async function withFileLock<T>(path: string, task: () => Promise<T>): Promise<T> {
  const lock = `${path}.lock`, token = randomUUID();
  await mkdir(dirname(path), { recursive: true, mode: 0o700 });
  for (let attempt = 0; ; attempt++) {
    try {
      await mkdir(lock, { mode: 0o700 });
      await writeFile(ownerFile(lock), JSON.stringify({ version: 1, token, pid: process.pid, createdAt: new Date().toISOString() } satisfies LockOwner), { mode: 0o600, flag: "wx" });
      break;
    } catch (error: any) {
      if (error?.code !== "EEXIST" || attempt >= LOCK_WAIT_ATTEMPTS) throw Error(`Unable to lock continuity state: ${path}`);
...
export async function writeJsonAtomic(path: string, value: any) { await writeBytesAtomic(path, serializedJson(value)); }
export async function writeJson(path: string, value: any) { await withFileLock(path, () => writeJsonAtomic(path, value)); }
export async function withStateLock<T>(directory: string, task: () => Promise<T>): Promise<T> { return withFileLock(join(directory, "state"), task); }
export async function updateJson<T>(path: string, fallback: T, update: (value: T) => T, valid: (value: any) => boolean = () => true): Promise<T> {
