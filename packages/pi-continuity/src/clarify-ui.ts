...
const label = (option: Option) => (option.description ? `${option.label} — ${option.description}` : option.label);
...
export async function askQuestionnaire(
  ...
  if (mode === "rpc" && remote.questionnaire) {
    const answers = await remote.questionnaire(
      questions.map(item => ({ question: item.question, options: item.options.map(label) })),
      options,
    );
  ...
  for (const item of questions) {
    ...
    const answer =
      choice === "Write a different answer…" ? (await ui.input("Custom answer", undefined, options))?.trim() : choice;
