...
const npmArgs = (name: string) =>
  process.platform === "win32" ? ["/d", "/s", "/c", "npm", "run", name] : ["run", name];
...
test("detects declared Composer and Deno manifest checks", async () => {
  ...
  await writeFile(
    join(root, "deno.json"),
    JSON.stringify({ tasks: { check: "deno check main.ts", test: "deno test", serve: "deno run main.ts" } }),
  );
...
test("uses Maven and Gradle wrappers and rejects Gradle markers without JVM plugins", async () => {
  ...
  assert.deepEqual(
    (await detectChecks(root)).checks.map(({ id, command, args }) => ({ id, command, args })),
    [
      {
        id: "maven:test",
        command: command ?? "./mvnw",
        args: process.platform === "win32" ? ["/d", "/s", "/c", "mvnw.cmd", "test"] : ["test"],
      },
      {
        id: "gradle:test",
        command: command ?? "./gradlew",
        args: process.platform === "win32" ? ["/d", "/s", "/c", "gradlew.bat", "test"] : ["test"],
      },
    ],
  );
