...
test("detects configured Python, Rust, Go, and Make checks", async () => {
  ...
  assert.deepEqual(
    (await detectChecks(root)).checks.map((check) => check.label),
  ...
  await writeFile(
    ...
    JSON.stringify({
      tasks: {
        ...
        test: "deno test",
        serve: "deno run main.ts",
      },
    }),
  );
  const checks = (await detectChecks(root)).checks;
  assert.deepEqual(
    checks.map(({ id, command, args }) => ({ id, command, args })),
    [
      {
        id: "composer:verify",
        command: "composer",
        args: ["run-script", "verify"],
      },
      {
        id: "composer:analyze",
        command: "composer",
        args: ["run-script", "analyze"],
      },
...
test("uses Maven and Gradle wrappers and rejects Gradle markers without JVM plugins", async () => {
  ...
  await writeFile(
    join(root, process.platform === "win32" ? "mvnw.cmd" : "mvnw"),
    "",
  );
  await writeFile(
    join(root, process.platform === "win32" ? "gradlew.bat" : "gradlew"),
    "",
  );
  const command =
    process.platform === "win32"
      ...
      : undefined;
  assert.deepEqual(
    (await detectChecks(root)).checks.map(({ id, command, args }) => ({
      id,
      command,
      args,
    })),
    [
      ...
      {
        id: "gradle:test",
        command: command ?? "./gradlew",
        args:
          process.platform === "win32"
            ? ["/d", "/s", "/c", "gradlew.bat", "test"]
            : ["test"],
      },
    ],
  );
  ...
  await writeFile(
    join(markerOnly, "settings.gradle"),
    "pluginManagement { plugins { id('java') } }\n",
  );
  await writeFile(
    join(markerOnly, "build.gradle"),
    "// java is mentioned but no plugin is declared\n",
  );
  ...
  const cases = [
    {
      ...
      file: "mix.exs",
      contents: "",
      expected: ["elixir:test", "mix", ["test"]],
    },
    {
      name: "nix",
      file: "flake.nix",
      contents: "{}",
      expected: ["nix:flake-check", "nix", ["flake", "check"]],
    },
    {
      name: "zig",
      file: "build.zig",
      contents: 'const step = b.step("test", "Run tests");',
      expected: ["zig:test", "zig", ["build", "test"]],
    },
  ] as const;
...
test("detects only .NET solutions containing confirmed test projects", async () => {
  ...
  await writeFile(
    join(root, "z.sln"),
    'Project("id") = "App.Tests", "tests\\App.Tests.csproj", "id"',
  );
...
test("reports six-check cap for immediate child packages", async () => {
  ...
  for (const name of ["a", "b", "c", "d"]) {
    ...
    await writeFile(
      ...
      JSON.stringify({ scripts: { check: "true", test: "true" } }),
    );
  }
  ...
  assert.deepEqual(
    detected.omitted.map((check) => check.id),
    ["d/npm:check", "d/npm:test"],
  );
});
