...
test("verify publishes bounded result metadata and session entry", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ scripts: { test: "node ok.js" } }),
  );
  ...
  const pi: any = {
    ...
    events: {
      emit: (channel: string, value: any) => events.push({ channel, value }),
    },
    ...
    exec: async (command: string, args: string[]) => {
      ...
      if (command === "git" && args[0] === "ls-files")
        return { code: 0, stdout: "file.ts\0", stderr: "" };
  ...
  const result = await tools
    .get("verify")
    .execute("call", { scope: "changed" }, undefined, undefined, {
      cwd,
      hasUI: false,
    });
  ...
  const published = events.find(
    (event) => event.channel === "pi-verify:result",
  )?.value;
  ...
  assert.match(
    ...
    /^Verification passed\. 1\/1 checks: npm:test · \d+\.\ds\. Hygiene passed\.$/,
  );
  assert.doesNotMatch(
    result.content[0].text,
    /Changed paths|PASS|npm run test|\nok\n?/,
  );
...
test("Verify never terminates early, even when the assistant emitted prior prose", async () => {
  ...
  const run = async (
    ...
    const caseCwd =
      kind === "no_checks"
        ? await mkdtemp(join(tmpdir(), "pi-verify-no-checks-"))
        : cwd;
    let heads = 0;
    let tool: any;
      ...
      exec: async (command: string, args: string[]) => {
        if (command === "git" && args[0] === "rev-parse") {
          ...
          return kind === "error"
            ...
            : {
                code: 0,
                stdout: `${kind === "stale" && heads > 1 ? "def" : "abc"}\n`,
                stderr: "",
              };
        }
        if (command === "git" && args[0] === "status")
          return {
            code: 0,
            stdout:
              kind === "clean" ? "" : kind === "failed" ? " M file.ts\n" : "",
            stderr: "",
          };
        if (command === "git" && args[0] === "diff")
          return {
            code: kind === "failed" ? 1 : 0,
            stdout: kind === "failed" ? "file.ts: bad whitespace\n" : "",
            stderr: "",
          };
    ...
    const result = await tool.execute(
      id,
      {
        scope: kind === "clean" ? "changed" : "project",
        ...(kind === "invalid" ? { checks: ["missing"] } : {}),
      },
      kind === "cancelled" ? { aborted: true } : undefined,
      undefined,
      {
        cwd: caseCwd,
        hasUI: false,
  ...
  for (const kind of [
    ...
    assert.equal(
      result.terminate,
      undefined,
      `${kind} must retain one evidence-aware follow-up`,
    );
    ...
    if (["failed", "cancelled", "stale", "error", "invalid"].includes(kind))
      assert.match(
        block.reason,
        /Write one caveated evidence-aware text-only final response and stop/i,
      );
    else
      assert.equal(
        block,
        undefined,
        `${kind} must not latch the non-passing guard`,
      );
  }

  const { handlers, tool, caseCwd } = await run("invalid");
  assert.match(
    handlers.get("tool_call")!({ toolName: "verify" }).reason,
    /already ended this agent run with error/i,
  );
  handlers.get("input")!({ source: "extension" });
  assert.match(
    handlers.get("tool_call")!({ toolName: "bash" }).reason,
  ...
  assert.match(
    handlers.get("tool_call")!({ toolName: "read" }).reason,
    /already ended this agent run/i,
  );
...
test("tool-only passing Verify remains nonterminal and reports capped check IDs compactly", async () => {
  ...
  extension({
    ...
    exec: async (command: string, args: string[]) => {
      ...
      return {
        code: 0,
        stdout: "verbose successful command output",
        stderr: "",
      };
    },
  } as any);
    ...
    undefined,
    {
      ...
      sessionManager: {
        getLeafEntry: () => ({
          ...
          message: {
            role: "assistant",
            content: [{ type: "toolCall", id: "tool-only", name: "verify" }],
          },
        }),
      },
    },
  ...
  assert.match(
    result.content[0].text,
    /6\/6 checks: npm:verify, npm:check, npm:typecheck, npm:lint, npm:test, make:verify/,
  );
  assert.match(
    result.content[0].text,
    /Skipped by six-check cap \(3\): make:check, make:test, make:lint/,
  );
  assert.doesNotMatch(
    result.content[0].text,
    /PASS|verbose successful command output|Changed paths/,
  );
});
...
test("verify reports live elapsed runtime while a check runs", async () => {
  ...
  await writeFile(
    join(cwd, "package.json"),
    JSON.stringify({ scripts: { test: "node slow.js" } }),
  );
  const tools = new Map<string, any>();
  ...
  const running = tools.get("verify").execute(
    ...
    },
    {
      cwd,
      hasUI: true,
      ui: { setStatus: (_id: string, status: string) => statuses.push(status) },
    },
  );
    ...
    await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        watchdog = setTimeout(
          () =>
            reject(
              new Error("Verify heartbeat was not reported within 10 seconds"),
            ),
          10_000,
        );
      }),
  ...
  const runtimeUpdate = updates.find(
    (update) => update.details.durationMs >= 1_000,
  );
  ...
  assert.match(
    tools
      .get("verify")
      .renderResult(runtimeUpdate, { expanded: false }, theme)
      .render(80)
      .join("\n"),
...
test("verify keeps failed check diagnostics", async () => {
  ...
  await writeFile(
    join(cwd, "package.json"),
    JSON.stringify({ scripts: { test: "node fail.js" } }),
  );
  const tools = new Map<string, any>();
  const pi: any = {
    registerTool: (tool: any) => tools.set(tool.name, tool),
  ...
  const result = await tools
    .get("verify")
    .execute("call", { scope: "project" }, undefined, undefined, {
      cwd,
      hasUI: false,
    });
  assert.equal(result.details.state, "failed");
...
test("verify selects a stable child-package check ID", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      scripts: { check: "node check.js", test: "node test.js" },
    }),
  );
  const tools = new Map<string, any>();
  ...
  const result = await tools
    ...
    .execute(
      "call",
      { scope: "project", checks: ["package-a/npm:test"] },
      undefined,
      undefined,
      { cwd, hasUI: false },
    );
...
test("runtime policy overrides model-supplied Verify checks", async () => {
  ...
  extension({
    ...
    on: (name: string, handler: (value: any, ctx?: any) => any) =>
