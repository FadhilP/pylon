...
test("Verify never terminates early, even when the assistant emitted prior prose", async () => {
  ...
  const run = async (
    ...
    const result = await tool.execute(
      id,
      { scope: kind === "clean" ? "changed" : "project", ...(kind === "invalid" ? { checks: ["missing"] } : {}) },
...
test("tool-only passing Verify remains nonterminal and reports capped check IDs compactly", async () => {
  ...
  const result = await tool.execute("tool-only", { scope: "project" }, undefined, undefined, {
    ...
    sessionManager: {
      getLeafEntry: () => ({
        type: "message",
        message: { role: "assistant", content: [{ type: "toolCall", id: "tool-only", name: "verify" }] },
      }),
    },
  });
  assert.equal(result.terminate, undefined);
...
test("verify runs checks from independent child-package directories concurrently", async () => {
  ...
  for (let attempt = 0; attempt < 100 && peak < 2; attempt++) await new Promise(resolve => setTimeout(resolve, 5));
  assert.equal(peak, 2);
  release();
  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        watchdog = setTimeout(() => reject(new Error("Verify heartbeat was not reported within 10 seconds")), 10_000);
      }),
    ]);
  } catch (error) {
  ...
  const pi: any = {
    ...
    exec: async (command: string, args: string[]) => {
      ...
      if (command === "git" && args[0] === "diff" && args.includes("--check"))
        return { code: 1, stdout: "file.ts:1: trailing whitespace.\n", stderr: "" };
      if (command === "git" && args[0] === "diff") return { code: 0, stdout: "", stderr: "" };
      checks++;
      return { code: 0, stdout: "ok\n", stderr: "" };
    },
...
test("verify selects a stable child-package check ID", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ scripts: { check: "node check.js", test: "node test.js" } }),
  );
  const tools = new Map<string, any>();
  ...
  const result = await tools
    .get("verify")
    .execute("call", { scope: "project", checks: ["package-a/npm:test"] }, undefined, undefined, { cwd, hasUI: false });
  assert.equal(result.details.state, "passed");
...
test("runtime policy overrides model-supplied Verify checks", async () => {
  ...
  await writeFile(
    join(cwd, "package.json"),
    JSON.stringify({ scripts: { test: "node test.js", lint: "node lint.js" } }),
  );
  let tool: any;
  ...
  sessionHandlers.get("session_start")!(
    {},
    { cwd, sessionManager: { getSessionId: () => "session", getEntries: () => [] } },
  );
  ...
  await tool.execute("policy", { scope: "project", checks: ["npm:test"] }, undefined, undefined, { cwd, hasUI: false });
