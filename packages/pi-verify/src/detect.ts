...
async function isDotnetTestProject(path: string) {
  ...
  return (
    project !== undefined &&
    (/<IsTestProject>\s*true\s*<\/IsTestProject>/i.test(project) ||
      /Microsoft\.NET\.Test\.Sdk/i.test(project))
  );
}
...
function hasPubDependency(source: string, name: string) {
  ...
  for (const line of source.split(/\r?\n/)) {
    ...
    if (line.trim() && indent <= dependenciesIndent)
      dependenciesIndent = undefined;
...
async function detectDotnet(cwd: string, files: string[], add: AddCheck) {
  ...
  for (const name of files.filter((file) => /\.slnx?$/i.test(file))) {
    ...
    const projects = [...solution.matchAll(/["']([^"']+\.csproj)["']/gi)].map(
      (match) => match[1].replaceAll("\\", "/"),
    );
    if (
      (
        await Promise.all(
          projects.map((project) => isDotnetTestProject(join(cwd, project))),
        )
      ).some(Boolean)
    ) {
      add(`dotnet:test:${name}`, `dotnet test ${name}`, "dotnet", [
        "test",
        name,
      ]);
      testSolution = true;
...
async function detectCabal(cwd: string, files: string[], add: AddCheck) {
  ...
  for (const name of files.filter((file) => file.endsWith(".cabal"))) {
    ...
    if (cabal !== undefined && /^\s*test-suite\s+/im.test(cabal)) {
      add(`haskell:cabal-test:${name}`, "cabal test all", "cabal", [
        "test",
        "all",
      ]);
      return;
...
async function checksAt(cwd: string, prefix = ""): Promise<Check[]> {
    ...
    try {
      ...
      for (const name of composerScripts)
        if (declaredScript(scripts[name]))
          add(`composer:${name}`, `composer ${name}`, "composer", [
            "run-script",
            name,
          ]);
    } catch {}
  ...
  if (denoText !== undefined) {
    try {
      ...
      for (const name of npmScripts)
        if (typeof tasks[name] === "string")
          add(`deno:${name}`, `deno ${name}`, "deno", ["task", name]);
    } catch {}
  }
  ...
  if (pyproject !== undefined) {
    ...
    if (/^\s*\[tool\.mypy(?:\.|\])/m.test(pyproject))
      add("python:mypy", "mypy", "python", ["-m", "mypy", "."]);
    if (/^\s*\[tool\.pytest(?:\.|\])/m.test(pyproject))
      add("python:pytest", "pytest", "python", ["-m", "pytest"]);
    if (/^\s*\[tool\.tox(?:\.|\])/m.test(pyproject))
      add("python:tox", "tox", "tox", []);
  }
  if (await exists(join(cwd, "tox.ini"))) add("python:tox", "tox", "tox", []);
  if (await exists(join(cwd, "noxfile.py")))
    add("python:nox", "nox", "nox", []);

  if (await exists(join(cwd, "Cargo.toml")))
    add("rust:test", "cargo test", "cargo", ["test"]);
  if (await exists(join(cwd, "go.mod")))
    add("go:test", "go test", "go", ["test", "./..."]);

  if (await exists(join(cwd, "pom.xml"))) {
    const check = await wrappedCommand(cwd, "mvnw", "mvnw.cmd", "mvn", [
      "test",
    ]);
  ...
  const gradleText = (
    await Promise.all(
      ["build.gradle", "build.gradle.kts"].map((file) => text(join(cwd, file))),
    )
  )
    .filter((value): value is string => value !== undefined)
    .join("\n");
  if (gradleText && gradlePlugin.test(gradleText)) {
    const check = await wrappedCommand(
      cwd,
      "gradlew",
      "gradlew.bat",
      "gradle",
  ...
  if (makefile !== undefined)
    for (const target of namedTargets)
      if (targetDeclared(makefile, target))
        add(`make:${target}`, `make ${target}`, "make", [target]);
  ...
  await addStandard("swift:test", "swift test", "Package.swift", "swift", [
    "test",
  ]);
  await addStandard("scala:test", "sbt test", "build.sbt", "sbt", ["test"]);
  await addStandard("ocaml:test", "dune runtest", "dune-project", "dune", [
    "runtest",
  ]);
  await addStandard("clojure:test", "lein test", "project.clj", "lein", [
    "test",
  ]);
  await addStandard("gleam:test", "gleam test", "gleam.toml", "gleam", [
    "test",
  ]);
  await addStandard("crystal:spec", "crystal spec", "shard.yml", "crystal", [
    "spec",
  ]);
  await addStandard("nix:flake-check", "nix flake check", "flake.nix", "nix", [
    "flake",
    "check",
  ]);
  await addStandard("haskell:stack-test", "stack test", "stack.yaml", "stack", [
    "test",
  ]);
  await addStandard("erlang:eunit", "rebar3 eunit", "rebar.config", "rebar3", [
    "eunit",
  ]);

  const pubspec = await text(join(cwd, "pubspec.yaml"));
  if (pubspec !== undefined) {
    if (hasPubDependency(pubspec, "flutter_test"))
      add("flutter:test", "flutter test", "flutter", ["test"]);
    else if (hasPubDependency(pubspec, "test"))
      add("dart:test", "dart test", "dart", ["test"]);
  }
  ...
  const justfile =
    (await text(join(cwd, "Justfile"))) ?? (await text(join(cwd, "justfile")));
  if (justfile !== undefined)
    for (const target of namedTargets)
      if (targetDeclared(justfile, target))
        add(`just:${target}`, `just ${target}`, "just", [target]);
...
export async function detectChecks(cwd: string): Promise<Detection> {
    ...
    for (const entry of entries
      .filter(
        (item) =>
          ...
          !ignoredChildren.has(item.name),
      )
      .sort((a, b) => a.name.localeCompare(b.name)))
      available.push(
        ...(await checksAt(join(cwd, entry.name), `${entry.name}/`)),
      );
  }
  ...
  return {
    checks: available.slice(0, LIMIT),
    available,
    omitted: available.slice(LIMIT),
  };
}
