...
test("executableAvailable distinguishes a spawned executable from ENOENT", async () => {
  ...
  assert.equal(await executableAvailable(`pylon-missing-${randomUUID()}`), false);
});

test("executableAvailable preserves cancellation", async () => {
  const controller = new AbortController();
  controller.abort(new Error("cancelled"));
  await assert.rejects(executableAvailable(process.execPath, controller.signal), /cancelled/);
});
