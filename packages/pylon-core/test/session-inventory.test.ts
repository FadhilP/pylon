...
test("session inventory reads headers without parsing transcripts and preserves duplicate paths", async () => {
  ...
  try {
    const first = await sessionFile(agentDir, "one", "first", `${JSON.stringify({ type: "session", id: "duplicate", cwd: "/one", timestamp: "2026-01-01T00:00:00.000Z" })}\nnot json\n${"x".repeat(128 * 1024)}`);
    const second = await sessionFile(agentDir, "two", "second", `${JSON.stringify({ type: "session", id: "duplicate", cwd: "/two", timestamp: "2026-01-01T00:00:00.000Z" })}\n`);
