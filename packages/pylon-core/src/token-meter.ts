...
function recordContext(
  ...
  const current = meter.byContext.get(key) ?? {
    records: 0,
    characters: 0,
...
function recordPackage(
  ...
  for (const [section, value] of Object.entries(context))
    recordContext(meter, packageName, section, value);
...
function recordDerivedTelemetry(
  ...
  if (result.toolName === "sieve_recall" && details.found === true) {
    ...
    meter.quality.sieveRecalledChars += result.content.reduce(
      (sum, part) =>
  ...
  const turns = Array.isArray(details.turns)
    ? details.turns.length
    : finite(details.turns) || 1;
  const status = String(details.status ?? details.failureCode ?? "completed");
  const context = Object.fromEntries(
    modelTool.context.map((key) => [key, result.input?.[key]]),
  );
...
export function parseTelemetryEvent(
  ...
  if (
    event.version !== 1 ||
    event.package !== "pi-timeline" ||
    event.kind !== "model_call"
  )
    return;
  if (
    typeof event.eventId !== "string" ||
    !/^[a-zA-Z0-9:_-]{1,128}$/.test(event.eventId)
  )
    return;
  if (
  ...
  if (
    !event.usage ||
    Object.keys(event.usage).sort().join(",") !==
      "cacheRead,cacheWrite,cost,input,output,turns"
  )
    return;
  ...
  if (
    ...
    event.usage.cost < 0 ||
    event.usage.cost > 1_000_000
  )
    return;
  if (
    !event.context ||
    Object.keys(event.context).sort().join(",") !== "request,result"
  )
    return;
  ...
  for (const [key, item] of Object.entries(
    ...
    if (
      !item ||
      Object.keys(item).sort().join(",") !== "characters,hash" ||
      !Number.isSafeInteger(item.characters) ||
      item.characters < 0 ||
      item.characters > 10_000_000 ||
...
export function recordTelemetryEvent(
  ...
  for (const [section, item] of Object.entries(event.context)) {
    ...
    const context = meter.byContext.get(key) ?? {
      records: 0,
      characters: 0,
      hashes: new Set<string>(),
    };
...
export function recordVerificationOutcome(meter: TokenMeter, value: any): void {
  if (
    ...
    !value.runId ||
    meter.seenVerificationIds.has(value.runId) ||
    typeof value.state !== "string" ||
    !/^[a-z-]{2,32}$/.test(value.state)
  )
    return;
  meter.seenVerificationIds.add(value.runId);
  meter.quality.verification.set(
    value.state,
    (meter.quality.verification.get(value.state) ?? 0) + 1,
  );
}
...
export function meterFromBranch(entries: readonly any[]): TokenMeter {
  ...
  for (const entry of entries) {
    if (entry?.type === "custom" && entry.customType === "pylon-telemetry")
      recordTelemetryEvent(meter, entry.data);
    if (entry?.type === "custom" && entry.customType === "pi-verify-result")
      recordVerificationOutcome(meter, entry.data);
...
export function formatTokenMeter(meter: TokenMeter): string {
  const rows = [...meter.byTool.entries()].sort(
    (a, b) =>
      b[1].argumentChars +
        b[1].resultChars -
        a[1].argumentChars -
        a[1].resultChars || a[0].localeCompare(b[0]),
  );
  let totalCalls = 0,
    totalInput = 0,
    totalOutput = 0,
  ...
  return [
    ...
    ...(lines.length
      ? lines
      : ["No completed tool calls in current session branch."]),
