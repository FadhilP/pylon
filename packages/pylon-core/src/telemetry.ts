...
function memoryEntry(kind: "review" | "migration", value: any) {
  if (
    ...
    !value.verdicts.every(
      (item: unknown) => typeof item === "string" && item.length <= 32,
    ) ||
...
function compactionReviewEntry(value: any) {
  ...
  if (value.outcome === "failed")
    return { version: 1, outcome: "failed", model: value.model };
  const complete =
    count(value.durationMs, Number.MAX_SAFE_INTEGER) &&
...
export function createTelemetry(pi: ExtensionAPI) {
  ...
  const disposers = [
    ...
    pi.events.on("pi-verify:result", (value: unknown) =>
      recordVerificationOutcome(meter, value),
    ),
    ...
    pi.events.on("pi-continuity:compaction-review-telemetry", (value: any) => {
      ...
      if (entry)
        appendQuietly(pi, "pi-continuity-compaction-review-telemetry", entry);
    }),
  ];
