...
export const pylonCoreSettings = definePackageSettings({
  ...
  fields: [
    { version: 1, key: "lineEditEnabled", label: "Revision-guarded numbered edits", type: "boolean", defaultValue: true, apply: "next-session" },
    { version: 1, key: "lineEditPriceRatio", label: "Line-edit price ratio", type: "number", defaultValue: 3, min: 1, max: 100, step: 0.1, apply: "next-session" },
    { version: 1, key: "delegateMaxAttempts", label: "Delegate maximum attempts", type: "integer", defaultValue: 3, min: 1, max: 10, apply: "next-operation" },
    { version: 1, key: "delegateRetryBaseMs", label: "Delegate retry base delay", type: "integer", defaultValue: 1_000, min: 100, max: 30_000, step: 100, unit: "ms", apply: "next-operation" },
  ],
} as const);

export type PylonCoreConfig = { version: 1; lineEditEnabled: boolean; lineEditPriceRatio?: number; delegateMaxAttempts?: number; delegateRetryBaseMs?: number };
export type EffectivePylonCoreConfig = { version: 1; lineEditEnabled: boolean; lineEditPriceRatio: number; delegateMaxAttempts: number; delegateRetryBaseMs: number };
...
export async function loadConfig(path = configPath()): Promise<PylonCoreConfig> {
  ...
  try { serialized = await readFile(path, "utf8"); } catch (error: any) { if (error?.code === "ENOENT") return defaultConfig(); throw error; }
  try {
    ...
    try { await rename(path, `${path}.corrupt-${randomUUID()}`); } catch (quarantineError: any) {
      throw new Error(`Could not quarantine invalid pylon-core config: ${quarantineError?.message ?? String(quarantineError)}`, { cause: error });
    }
