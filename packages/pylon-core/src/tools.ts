...
const validOwner = (value: unknown): value is string =>
  typeof value === "string" &&
  (value === "pylon-core" || /^pi-[a-z0-9-]+$/.test(value));
...
function parseToolUsage(
  ...
  if (!allowedTools)
    return {
      error: `${field} requires ${field === "toolUsage" ? "enabledTools" : "deferredTools"}`,
    };
  const entries = Object.entries(value);
  if (entries.length > 32)
    return { error: `${field} must contain at most 32 tools` };
  const allowed = new Set(allowedTools);
  const usage: Array<[string, string]> = [];
  for (const [tool, phrase] of entries) {
    if (!allowed.has(tool))
      return {
        error: `${field} keys must be ${field === "toolUsage" ? "enabled" : "deferred"} tools`,
      };
    if (
...
export function parseToolMessage(
  value: unknown,
): { message: ToolMessage } | { error: string } {
  ...
  if (
    input.deferredTools &&
    !input.deferredTools.every((tool) => enabledTools.includes(tool))
  )
  ...
  if (
    input.acknowledge !== undefined &&
    typeof input.acknowledge !== "function"
  )
  ...
  return {
    message: {
      ...
      ...(input.acknowledge
        ? { acknowledge: input.acknowledge as () => void }
        : {}),
    },
  };
}
