import { classifyCompatibility, type Compatibility, type GitState } from "./compatibility.ts";
...
const refName = (ref: string | null) => (ref === null ? "detached HEAD" : shortRef(ref));
...
export const describeCompatibility = (
  ...
  if (!result.allowed)
    return result.reason === "repository-mismatch"
      ? { label: "[blocked:repository]", detail: "Checkpoint belongs to a different repository." }
      : { label: "[blocked:HEAD]", detail: "Checkpoint HEAD commit differs from current HEAD." };
  ...
  if (result.refState === "target-detached") return { label: "[checkpoint:detached]", detail: divergedDetail };
  if (result.refState === "current-detached") return { label: "[current:detached]", detail: divergedDetail };
  ...
  return target.headRef === null
    ? { label: "[detached]", detail: "Checkpoint and current state use detached HEAD at the same commit." }
    : {
...
export const compatibilityLabel = (target: Snapshot, current: GitState, result?: Compatibility) =>
