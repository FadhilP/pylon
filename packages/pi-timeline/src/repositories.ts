...
async function childPaths(repository: Repository) {
  ...
  for (const path of nul(
    await git(repository.root, ["ls-files", "--others", "--exclude-standard", "--directory", "-z"]),
  )) {
...
export async function discoverRepositories(cwd: string): Promise<Repository[]> {
  ...
  for (let index = 0; index < queue.length; index++) {
    ...
    for (const path of await childPaths(repository)) {
      ...
      if (outside(workspace, childRoot)) throw Error(`Nested repository escapes workspace: ${path}`);
      ...
      if (physicalRoots.size >= 100) throw Error("pi-timeline nested repository limit exceeded");
