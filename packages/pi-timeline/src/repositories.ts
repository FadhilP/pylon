...
async function childPaths(repository: Repository) {
  ...
  for (const entry of nul(
    await git(repository.root, ["ls-files", "--stage", "-z"]),
  )) {
  ...
  for (const path of nul(
    ...
    while (
      ...
      !outside(repository.root, candidate)
    ) {
      const physical = await realpath(candidate),
        topLevel = await realpath(
          await git(physical, ["rev-parse", "--show-toplevel"]),
        );
...
export async function discoverRepositories(cwd: string): Promise<Repository[]> {
  ...
  const repositories: Repository[] = [
    { root: workspace, prefix: "", commonDir: await identify(workspace) },
  ];
  const queue = [
    { repository: repositories[0], ancestors: new Set([canonical(workspace)]) },
  ];
  const physicalRoots = new Set([canonical(workspace)]),
    prefixes = new Set([""]);
  ...
  for (let index = 0; index < queue.length; index++) {
    ...
    for (const path of await childPaths(repository)) {
      ...
      const topLevel = await realpath(
          await git(childRoot, ["rev-parse", "--show-toplevel"]),
        ),
        rootKey = canonical(childRoot);
      if (canonical(topLevel) !== rootKey) continue;
      if (ancestors.has(rootKey))
        throw Error(`Nested repository cycle: ${path}`);
      if (physicalRoots.has(rootKey))
        throw Error(`Duplicate nested repository: ${path}`);
      ...
      const child = {
        root: childRoot,
        prefix,
        commonDir: await identify(childRoot),
      };
      repositories.push(child);
      queue.push({
        repository: child,
        ancestors: new Set([...ancestors, rootKey]),
      });
    }
  }
  return repositories;
